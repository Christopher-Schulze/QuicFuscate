...
- Cargo workspace with the root `quicfuscate` package and eight backend leaf crates: `qf-common`, `qf-control-plane`, `qf-dns`, `qf-error`, `qf-instrumentation`, `qf-pki`, `qf-privilege`, and `qf-reality`
- The remaining product runtime stays under `src/`; key modules are `core.rs`, `transport/`, `stealth/`, `fec/`, `brain.rs`, `crypto/`, `qftls.rs`, `compress.rs`, and `optimize/`. The DNS implementation is owned by `crates/qf-dns/src/lib.rs` and the REALITY implementation by `crates/qf-reality/src/lib.rs`; both are re-exported from the root.
