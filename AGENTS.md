...
- **qf-cpu**: CPU feature detection, cache hierarchy and prefetch policy, SIMD dispatch contracts, FEC dispatch helpers, compression byte classification, ASCII classification, histograms, pattern search, runtime substring search, iterator reductions, sorting, argsort, transport congestion aggregation, bitmap/ECN helpers, and packet-number decoding. It depends on `qf-common`, `qf-telemetry`, and `qf-transport-types`, plus `libc` and `log`; root `optimize`, `simd`, and `accelerate` namespaces preserve compatibility paths while test/rust-tests-gated Base64 helpers remain root-owned.
