...
- H3: `src/transport/h3_parts/connection.rs` emits transactional client/server unidirectional control-stream prologues with SETTINGS, rejects local field-section, QPACK-table, and blocked-stream values above the 16 MiB ceiling before QPACK construction or wire emission, and parses full QUIC-varint stream prefixes, frame types, frame lengths, and fixed control payloads. Receive wiring is peer STREAM bytes -> fragmented prefix retention -> endpoint-owned classification -> per-stream frame-placement and message-sequence validation -> staged H3 events or raw WebTransport data. Unknown streams discard every chunk; control and distinct QPACK encoder/decoder streams are unique and critical; clients reject server-initiated bidirectional streams in the current profile. Request and push streams retain awaiting-headers, body, and trailers state, including interim-response handling. Server Push wiring is peer `MAX_PUSH_ID` plus latest request stream -> monotonic push ID -> request-stream `PUSH_PROMISE` -> independent server-unidirectional push stream with type, push ID, response `HEADERS`, and cover `DATA`. TODO-691/TODO-692 own this control/state closure, TODO-882 owns RFC 9204 instruction synchronization, and TODO-471 owns complete WebTransport settings and bidirectional negotiation. The shared transport writable queue skips drained entries so H3 responses are not starved behind the control stream. TODO-692 commit `21ca2e8`, Clippy Matrix run `31508779282`, and Main CI build-test job `93836990319` close the backend H3 gate with `1,642` root library tests and zero failures; the separate fuzz and protected Tauri lockfile failures remain outside this wiring result.
