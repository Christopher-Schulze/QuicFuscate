...
- Windows Wintun and kill-switch ownership: `src/interface.rs` selects the built-in backend only with `tun-windows` -> `src/interface/wintun.rs` securely loads the upstream DLL, creates one adapter/session, captures its LUID and session-owned read event, configures addresses and active MTU, and serializes packet operations against one shutdown event and one exactly-once teardown -> `src/implementations/client/killswitch/windows.rs` resolves the live alias to its LUID and transactionally replaces fixed persistent WFP provider/sublayer/filter identities across IPv4/IPv6 outbound transport layers, which also classify third-party transports and raw packets while preserving the exact UDP tuple -> ignored native tests cover data-plane lifecycle and WFP packet outcomes -> `scripts/utils/provision-wintun.ps1` pins archive/DLL hashes plus Authenticode -> CI and Tauri MSI paths provision the untracked DLL beside their executable. Run `30504148233`, attempt 2, job `90752348969` proves the pushed Wintun baseline; successor WFP, authenticated QuicFuscate traffic, process-exit, and signed MSI evidence remain open under TODO-528.
