...
- Exact ARM64 source `88a12ae` narrowed the remaining failure independently of the later file decomposition. Its three-child contract was failed/passed/passed. The failed clean opt-in trial lost 10 DATAGRAM and 2 control carriers before a later ACK, despite exact host-veth delivery and zero UDP kernel drops. A successful deliberate black-hole run lost 11 fresh STREAM carriers and zero retransmissions, then recovered. `QF_E2E_FEC_MODE=auto|off` now writes a single explicit FEC policy into both phase endpoints and the stability manifest so the same exact binary can separate active-FEC wire admission from QUIC receive/ACK progression.
