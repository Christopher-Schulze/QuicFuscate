...
- Client profile storage wiring (TODO-658, TODO-662): `src/implementations/client/profile.rs` remains a standalone exported API with no current runtime, CLI, desktop, or admin owner. `Profile::from_qkey()` derives 32 lowercase hexadecimal characters from 128 bits of fallible OS CSPRNG output; `add()` and `load()` reject empty or duplicate IDs before state replacement, and non-empty legacy IDs remain unchanged. Hosted macOS job `93669407470` passes all current profile tests `14/14` at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f`, including rapid creation, entropy failure, duplicate rejection, and legacy preservation; implementation commit `251dcf31c79e61c26d02628fe5ec584cdadc762b` is included and TODO-658 is closed. Atomic persistence remains the separately owned TODO-662 boundary.
