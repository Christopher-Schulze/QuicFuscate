...
- **Optimize/Engine/Admin**: engine config reload (TODO-645), io_uring executor ownership (TODO-646), io_uring unsafe/lifetime and receive-slot proof (TODO-687), io_uring partial-send disposition (TODO-798), admin HTTP connection limit (TODO-647), config write validation (TODO-648), memory-pool unsafe bounds (TODO-587), metrics export allocations (TODO-587, TODO-615), and TUN interface unaligned/fcntl safety (TODO-654, TODO-655).
...
- **Tracker gate:** `bash scripts/tests/audits/verify-audit-completeness.sh` currently fails before corpus validation with `unexpected tracker section 'Blocked'`; an independent filename/register reconciliation finds 708 tracker headings, 369 current detail files, and 340 archived detail files with no filename-ID orphan or duplicate. TODO-799 owns alignment of the validator with the canonical task lifecycle, TODO-800 owns the stale macOS runtime-reload fixtures exposed by the same CI run, and TODO-801 owns the missing opt-in io_uring kernel evidence.
