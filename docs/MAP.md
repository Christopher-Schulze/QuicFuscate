...
- **Server parity:** `src/implementations/server/parts/dns_signals.rs` uses the shared plain-DNS result contract and returns SERVFAIL instead of synthesizing NXDOMAIN or dropping a parseable failure response. TODO-611/TODO-668 retain rate limiting, TODO-669 retains timeout and allocation bounds, and TODO-770 retains complete wire-question preservation and admission hardening.
- **Targeted proof:** `CARGO_BUILD_JOBS=2 cargo test --locked --lib dns::tests -- --nocapture` passed 22/22, and `CARGO_BUILD_JOBS=2 cargo test --locked --lib test_server_dns_upstream_failure_returns_servfail -- --nocapture` passed 1/1. `cargo fmt --all -- --check` and `git diff --check` passed. Strict Clippy and the remote full gate remain pending before task closure.
