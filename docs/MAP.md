...
4. FEC encode/decode path: `src/core.rs` raw handshake/Zero gate -> safe block-boundary mode transition -> `src/fec/` (`AdaptiveFec`) -> `InterleavedEncoder` lane distribution -> `src/fec/wire.rs` versioned MTU-bounded envelope with protected QUIC length -> packet loss -> receiver-owned epoch/window decoder -> `InterleavedDecoder` lane routing -> rank-checked systematic recovery -> exact inner-length validation and removal for systematic or recovered sources -> `src/transport/connection.rs` authenticated QUIC receive -> transport observer hooks
