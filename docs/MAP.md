...
- Brain observer wiring: transport receive callbacks update packet count, reorder state, and size bins through lock-free atomic accumulators and sample inter-arrival bins every eighth packet -> `StealthBrain::apply_policy` drains those accumulators under its consolidated mutation lock. Transport control producers route through one bounded queue admission helper that coalesces the latest `MAX_DATA` and per-stream `MAX_STREAM_DATA` update and preserves terminal close frames under saturation. `Connection::close()` is first-close-wins under TODO-606; terminal close priority remains open under TODO-697, and local close error-kind semantics under TODO-772.
...
- `ServerHostResources::start()` (`src/implementations/server/mod.rs`, Linux runtime path):
  ...
  - `RoutingManager::setup()` assigns and verifies exact TUN interface addresses/prefixes and link-up state before the selected Linux firewall backend is admitted.
  - Non-Linux server TUN startup is rejected by `open_server_tun()` before creating host state; internal macOS/Windows rule generators are not shipped server capabilities.
- `main.rs`:
...
- TUN runtime ownership: `TunInterface::reader_loop_with_shutdown()` owns the cooperative atomic stop and Unix `poll(2)` wait; client and standalone server readers own their `JoinHandle`, drop their bounded receiver before joining, wake the native Wintun event where required, and release the TUN descriptor after the thread exits. `Notify` wakes both select loops for queued TUN frames, while adaptive housekeeping uses a 5 ms active floor and 250 ms idle ceiling.
...
- **Unsafe FEC/io_uring/auxiliary**: FEC GF tables and decoders (TODO-686), io_uring/io_driver (TODO-687), audit/limits (TODO-688), cpu_dispatch/telemetry/cache/lib.rs/tests (TODO-689).
...
- **macOS pf:** the client kill-switch checks the main ruleset and fails with `NotSupported` when its anchor is not referenced, but does not roll back a just-loaded anchor after that later failure. The internal server routing helper loads `com.quicfuscate.vpn` without equivalent active-ruleset proof; the supported server TUN runtime remains Linux-only and TODO-607 owns broader macOS teardown.
...
- **io_uring:** `UringBatchSender` has no cross-call pending queue: scratch vectors are preallocated and submissions are chunked to SQ capacity. TODO-646 Finding 1 is stale. Synchronous submit, wait, and CQE draining still run from client/server packet paths and remain open as an executor-ownership issue.
...
- **io_uring:** A failed SQ submission can leave accepted entries unresolved while sender scratch storage is reused. The opt-in `SendMsgZc` path returns after primary completions while notification completions still govern borrowed payload lifetime. Receive completion handling remaps an invalid index to the last slot and does not re-arm a zero-length receive despite the comment. TODO-687 owns ownership, notification lifetime, index validation, and slot rearming; synchronous submit/wait remains TODO-646.
...
- **Terminal close and queued sends:** Congestion-bypass control flushing can block a later CONNECTION_CLOSE behind an earlier ack-eliciting frame; TODO-606 now suppresses duplicate local close frames. FEC and DATAGRAM queues pop items before every serialization/seal stage has committed, so a later failure can lose local payload ownership. Server stop can report `Stopped` while a startup-timeout runtime thread remains live. TODO-697, TODO-698, and TODO-699 own priority, transactional send ownership, and server thread lifecycle.
...
- **Cargo tests:** 71 integration-test targets are declared and all 71 source paths exist. The static runner inventory contains 70 declared target names; `scripts/tests/suites/test-desktop-webadmin-rust-integration.sh` additionally invokes the missing `it-masque-runtime-integration`, whose only matching source is archived. TODO-774 owns the stale runner edge.
...
- **Examples:** `examples/tun_factory_example.rs` is Cargo- and crate-gated to `tun-tests`, while its `main()` contains unreachable `tun-windows`/`tun-ios` branches. TODO-775 owns the target contract; the production platform owners remain TODO-443 and the related Windows/TUN tasks.
- **Scope gate:** The current register has 701 tracker headings, 362 current detail files, and 374 archived detail files. Git enumerates 898 tracked and 26,372 ignored paths with no non-ignored untracked files. The fail-closed validator currently stops at tracked `archive/stealth/doh.rs`; TODO-773 owns the missing archive classifier and manifest boundary. TODO-795 owns the Unix admin CLI response-contract gap, TODO-796 owns the E2E migration finalization evidence gap, and TODO-797 owns the persisted logging-mode state gap. This prevents a whole-project completeness claim until the archive disposition is encoded and the validator passes.
