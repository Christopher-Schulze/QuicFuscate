...
- The multi-client harness offers `QF_E2E_EXTERNAL_EGRESS_CAPTURE=1` as an external-only diagnostic. It captures client 1 underlay UDP only on host veth `qf523h1`, only during each three-trial throughput phase, and emits bounded count/gap summaries. Exact ARM64 source `19ed8c8` rebuilt binary `c54d2d5e1c600790fcc0c2d437fdb5f3942e8337dadd2138896f0bf958ac6a2e` and passed the full gate: 8.048 to 10.300 Mbit/s, 27.97% gain, 2-second black-hole detection, 13,041,664 bytes in 21.152 seconds, zero queue events, and clean teardown. The default/opt-in capture summaries recorded 26,293/32,765 packets, 330,130/341,556-us maximum gaps, and 3/10 gaps at or above 100 ms. It does not change the product binary, runtime, payload handling, credentials, or normal gate.
