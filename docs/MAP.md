...
- Compression wiring: `src/compress.rs` writes safe-path zstd compression and decompression directly into caller-owned `MemoryPool` blocks via bulk buffer APIs; H3 centralizes MIME allow/deny evaluation with deny precedence, and `0x5A` / `0x5D` frame headers remain unchanged.
...
- Client IO ownership: inbound flushes reuse caller-owned 65,535-byte buffers, Linux outbound dispatch reuses one batch-reference vector per loop, and the client TUN reader transfers pool-backed `interface::TunPacket` blocks through its bounded channel so backpressure preserves ownership without a per-packet `Vec` allocation. Admin HTTP auth/session state uses `parking_lot`; its login limiter is a 10,000-key LRU with all-attempt accounting, and session replay uses a `HashSet` plus bounded FIFO eviction.
- Linux outbound dispatch wiring (TODO-578): `OutboundDispatch::IoUringBatch` remains owned by the io_uring sender; the shared `try_sendmmsg_batch()` match rejects accidental io_uring fall-through explicitly instead of silently returning zero sends, while `SendmmsgBatch` remains bounded by the payload count.
...
- ActiveProbeDetector: GFW_TLS_Probe detection; legacy DPI_QUIC_Scan response selector retained without a matching pattern; benign-ignored
...
- macOS: PF policy is available only when the main ruleset exposes the QuicFuscate anchor. Client cleanup touches only `com.quicfuscate.killswitch` and never disables shared PF; server routing no longer loads or cleans a PF anchor before native ownership proof exists. TODO-548 owns managed installation and privileged proof.
...
- **Crypto data plane**: constant-time tag comparison (TODO-626), key/IV length validation (TODO-627), AEGIS mutex/`unwrap` path (TODO-628, resolved by TODO-582), AEAD header-protection sample validation (TODO-629), GHASH test override removal (TODO-630), round-key zeroization (TODO-631), nonce/IV uniqueness (TODO-632), and QUIC KDF input validation (TODO-633).
...
- **Optimize/Engine/Admin**: engine config reload (TODO-645), io_uring executor ownership (TODO-646), io_uring partial-send disposition (TODO-798), admin HTTP connection limit (TODO-647), config write validation (TODO-648), memory-pool unsafe bounds (TODO-587), metrics export allocations (TODO-587, TODO-615), and TUN interface unaligned/fcntl safety (TODO-654, TODO-655). TODO-687 closed the io_uring unsafe/lifetime and receive-slot audit boundary; TODO-801 closed the additional Linux kernel evidence boundary.
- **Client/Server/DNS/PKI**: DNS backup path (TODO-649), DNS intercept spawn outcome (TODO-650), PKI timestamp fallback (TODO-656), CLI probe parse errors (TODO-657), profile ID collision (TODO-658), retry token length validation (TODO-659), blacklist sync ownership (TODO-660), admin per-operation timeout (TODO-661), profile save atomicity (TODO-662), client TUN dual-stack (TODO-663), Windows TUN backend confusion (TODO-664), admin replay store pruning (TODO-665), generic DNS helper admission (TODO-668), DNS allocation/timeout (TODO-669), DNS query admission and wire semantics (TODO-770), env var race/validation (TODO-670), umask file permissions (TODO-671), log rotation hooks (TODO-672), CLI unbounded strings (TODO-673), duplicate logging init (TODO-674), and audit blocking flush (TODO-675). Server TUN DNS flood control is resolved by TODO-611. DNS client runtime wiring is resolved by TODO-771; its Linux harness and native platform proof boundaries are documented in the task detail.
...
- **SIMD safety hardening**: TODO-593 now covers debug dimension checks for x86 GF(256) matrix kernels, capacity checks for BMI2 varint encoding, scalar delegation for the AVX2 header validator, zero-padded partial-shard encoding across scalar/NEON/GFNI backends, and fail-loud Windows SHA-256 fallback stubs.
- **SIMD Reed-Solomon compatibility path**: TODO-594 corrects the standalone x86 AVX2/GFNI Reed-Solomon encode/decode implementations, including cross-shard accumulation, canonical GF(256) coefficients, full matrix inversion, dynamic LUT multiplication, safe vector tails, and runtime shard metadata checks. Rosetta-executed x86 tests pass; production recovery remains wired through `src/fec/`, while broader FEC/SIMD/GF16 audits remain TODO-686, TODO-679, and TODO-715.
- **Unsafe memory and SIMD**: `UnsafeMemoryPool` bounds/affinity (TODO-678), full SIMD surface lacking safety docs/bounds (TODO-679), optimize brain/transport/stealth raw loops (TODO-680).
- **Unsafe crypto/transport/interface/privilege**: crypto primitives (TODO-681), transport syscalls (TODO-682), interface/Wintun/WFP (TODO-683), privilege/mlock/secrets (TODO-684).
- **Unsafe FEC/io_uring/auxiliary**: FEC GF tables and decoders (TODO-686), io_uring/io_driver lifetime ownership (TODO-687), io_uring unordered partial-send disposition (TODO-798), audit/limits (TODO-688), cpu_dispatch/telemetry/cache/lib.rs/tests (TODO-689).
...
- **Server data plane:** the shared client fan-out queue is uncapped and its drain materializes the complete backlog into a second container (TODO-612); TODO-613 closes the explicit PTB-for-all repair for oversized IPv4 uplink packets before either TUN write. The focused DF=0/DF=1 PTB regression and the native Linux PTB gate pass; TODO-806 owns the separate TTL-expiry failure observed later in the same broad harness.
...
- **Retry admission:** `RetryToken` can encode a valid bounded IPv6/20-byte-CID/64-byte-credential input to 169 bytes while `MAX_RETRY_TOKEN_LEN` remains 160; issuance therefore rejects that input. No duplicate length constant was found in the DDOS policy probe. Tracked in TODO-618; post-encoding validation work remains TODO-659.
- **Client runtime lifecycle:** `ClientRuntime::connect()` stores a `ClientConnection` before fallible runtime, UDP, TUN, and I/O setup. A later error leaves the runtime in `Running` with owned partial state, and a reconnect replaces the connection without the explicit close path. This is an unclosed lifecycle contract, not a proven heap leak. Tracked in TODO-619.
- **Client backend configuration:** the legacy `ClientBackend` setup path still hardcodes `10.8.0.2/24` and a derived `10.8.0.1` gateway instead of applying `tun_ip`, `tun_netmask`, and `tun_subnet_prefix`; TODO-620 is the sole owner after TODO-604 was closed as its duplicate.
- **Linux resolver ownership:** the legacy `/etc/resolv.conf` path cannot remove a file that did not exist before the session. Process-local backup state also cannot recover a previous crashed session and can overwrite a stale fixed backup; TODO-623 owns the absent-original contract, while TODO-649 owns backup path and symlink/target semantics.
- **macOS pf:** the client kill-switch checks the main ruleset and fails with `NotSupported` when its anchor is not referenced, but does not roll back a just-loaded anchor after that later failure. The supported server TUN runtime remains Linux-only; the server routing manager now rejects macOS before host mutation and retains only pure rule generation. TODO-624 owns the remaining client activation rollback.
- **Client FEC surface:** `pipeline.rs` remains outside the production module graph. The retained `FecCodec` wrapper constructs every received packet with ID zero and strips encode-side identifiers, so wiring it later without a framing contract would be incorrect. Tracked in TODO-625; TODO-602 is its closed duplicate.
...
- **Engine reload:** The generic `QuicFuscateEngine` exposes validated in-memory `update_config` but no file-backed reload owner. The standalone server has a separate validated file reload path with explicit next-connection-only scope. TODO-645 remains open for one supported reload and propagation contract.
- **io_uring:** `UringBatchSender` has no cross-call pending queue: scratch vectors are preallocated and submissions are chunked to SQ capacity. TODO-646 Finding 1 is stale. Synchronous submit, wait, and CQE draining still run from client/server packet paths and remain open as an executor-ownership issue; TODO-687 covers pointer/lifetime safety and TODO-798 covers exact partial-send disposition.
...
- **Linux DNS backup:** Restore copies the backup before removing it, so a failed copy preserves the backup. TODO-649 Finding 2 is closed as stale. The fixed backup path and resolver symlink/target contract remain open; TODO-623 owns absent-original and crash ownership.
- **TUN fcntl:** Linux and macOS check `F_GETFL` and `F_SETFL`, close only on the error path, and return before device publication. TODO-655 is closed as stale. The independent BMI2 unaligned-read finding remains TODO-654.
- **Probe parsing:** `qf-privilege-probe` and several E2E flags already return parse errors. TODO-657 is narrowed to silent defaults for `--timeout-ms` and `--hold-ms` in `qf-e2e-client` and `qf-e2e-desktop`.
- **Retry token:** Per-field bounds already exist. TODO-659 now owns the aggregate length calculation before body allocation and HMAC; the current late check rejects oversized tokens rather than emitting them.
- **Blacklist sync:** The feed synchronizer already enforces HTTPS, request timeout, body, and entry bounds, and the normal housekeeping owner records the dispatch time before spawning. TODO-660 remains for task ownership, shutdown cancellation/completion, and race-safe in-flight state.
- **Replay protection:** The admin session store uses bounded `VecDeque` plus `HashSet` and evicts one fingerprint at a time. TODO-665 Finding 1 is stale. The remaining issue is an undocumented replay-window tradeoff when FIFO eviction precedes session expiry.
...
- **Crypto corrections:** TODO-631's blanket round-key zeroization claim is stale because the AES-NI schedule exists only on x86_64 and is zeroized in its target-specific `Drop`; key and IV zeroization remains cross-target. TODO-642's zero-key fallback claim is stale because TLS cover entropy failure returns a typed crypto error before derivation. TODO-627, TODO-629, TODO-632, and TODO-633 retain the independent constructor, header-protection, nonce-lifecycle, and KDF input contracts.
- **AMX:** `src/optimize/parts/cpu_dispatch.rs` derives AMX planner flags from external `cpuid` output even when the binary is not compiled with `target_feature="amx-tile"`. `src/fec/parts/decoders.rs` can therefore select the AMX plan while the cfg-gated AMX call is absent and the next vector remains zero-initialized. `src/simd/amx.rs` also accepts unchecked dimensions and releases tile state only on the normal return path. TODO-676 owns detection, dimensions, and cleanup.
- **Unsafe memory pool:** `src/optimize/unsafe.rs` calls a field named `tls_cache` through shared `UnsafeCell` state without actual thread-local storage, permits fallback allocations to desynchronize capacity/available counters, and performs block-size-independent prefetch pointer arithmetic. `UnsafeCompressor` exposes a shared mutable zstd context through `Sync`. The historical `copy_to_block` inventory is absent from the current source. TODO-678 owns these boundaries.
- **SIMD:** The old AVX-512/GFNI Reed-Solomon delegation claim is stale for the active decoder. A real critical boundary remains in `find_pattern_sse42_short`, which loads 16 needle bytes when only a shorter needle is admitted. GF(256) matrix and BMI2 varint helpers still rely on debug-only dimension/output checks. TODO-679 owns the full safety proof.
- **Optimize and UDP:** `decode_packet_number` accepts packet-number lengths outside the QUIC 1..=4 contract before decode. Linux `sendmmsg`/`recvmmsg` negative-return handling exists, but receive counts and each `msg_len` are not explicitly bounded before slicing in all active batch paths. TODO-680 and TODO-682 own these checks.
- **Interface and platform:** P4a/P4b profile support can route through the BMI2 IP parser without an equivalent BMI2 runtime predicate. macOS interface-name extraction trusts an oversized kernel-reported length, and partial `writev` with zero progress can spin. Wintun and WFP cleanup mark ownership closed even when `CloseHandle`, `FreeLibrary`, or `FwpmEngineClose0` fails, preventing retry. TODO-683 owns these narrower findings; TODO-654 remains the unaligned BMI2 read.
- **Privilege and FFI:** `src/privilege/drop.rs` checks lookup status and non-null results but does not prove the returned pointer identity and builds `CStr` from libc name pointers without a null-pointer check. Memory locking returns errors, while the process-level caller intentionally logs and continues, so the best-effort security policy remains an explicit contract. TODO-684 owns this boundary. Windows path FFI also needs explicit embedded-NUL rejection; Unix path TOCTOU remains TODO-728 under TODO-688.
- **FEC unsafe inventory:** Active GF(256)/GF(16) wrappers clamp slice lengths before private vector calls; the old public raw-slice and SSSE3 claims are stale. The P4 GF16 threshold map can still select a VBMI2 threshold while the actual policy falls back to scalar when VBMI2 is unavailable. AMX remains TODO-676. TODO-686 owns the dispatch/performance contract.
- **io_uring:** The current sender copies payloads into owned slots before publishing pointers and quarantines after submit/protocol errors; SendMsgZc tracks `CQE_F_MORE` and waits for every announced notification, including errored primaries. Receive completion metadata is range-checked, zero-length and error receives are re-armed, and ring destruction precedes pool-block return. Client eventfd reads require exactly eight bytes. TODO-687 closed the unsafe/lifetime boundary and TODO-801 closed the additional Linux kernel evidence boundary; synchronous submit/wait remains TODO-646, and unordered partial-send retry disposition is TODO-798.
- **Auxiliary unsafe surface:** Windows NUMA FFI status checks and the process-global auto-tuner's explicit shutdown path are present. The non-iOS AArch64 prefetch fallback uses `read_volatile`, which can fault for an invalid offset instead of issuing a non-faulting hint. Test environment mutation is serialized only by a caller-held mutex. Telemetry, crate-root, and transport/config matches in the original unsafe register are source-text false positives. TODO-689 owns the remaining contract.
...
- **Feature propagation:** 60 declared test targets rely on crate-level feature cfgs without Cargo `required-features`. `run_cargo` injects `rust-tests` for test commands, but target-specific features and CI default/SIMD lanes remain implicit or incomplete. XDP's `internal_af_xdp_experimental` lane is not passed by the transport suite. TODO-734 owns this split contract.
...
- **Server parity:** `src/implementations/server/parts/dns_signals.rs` uses the shared plain-DNS result contract and returns SERVFAIL instead of synthesizing NXDOMAIN or dropping a parseable failure response. TODO-611 now owns the bounded server-TUN admission; TODO-668 retains generic helper admission, TODO-669 retains timeout and allocation bounds, and TODO-770 retains complete wire-question preservation and admission hardening.
...
- **Scope boundary:** The optional response cache was evaluated but not introduced because a correct cache requires explicit TTL handling plus transaction-ID and original-question projection. Generic `process_dns_query()` admission remains TODO-668; detached task outcome ownership remains TODO-650; timeout/allocation and complete wire semantics remain TODO-669 and TODO-770.
...
- **Separate owners:** Synchronous submit/wait/CQE draining remains TODO-646. The contiguous-prefix API can retry a later packet after an unordered earlier error; TODO-798 owns exact per-slot disposition and duplicate prevention.
...
- **Other CI failures:** The same completed CI run also fails the Windows core check at `src/privilege/drop.rs:676` because `CurrentIds` is unavailable (existing TODO-593/TODO-683 boundary), and strict macOS Clippy at `src/stealth/tls_cover.rs:393` plus `src/implementations/client/dns_runtime.rs:279,289` (existing TODO-709/TODO-752/TODO-787 lint boundary). These failures are recorded as existing owners and were not changed in this audit.
