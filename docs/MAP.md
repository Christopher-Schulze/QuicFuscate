...
- Root test and production paths now share one qf-common `EnvSnapshot` type. The deterministic pair constructor is available only through `rust-tests` feature wiring for test targets, avoiding a duplicate root implementation or default production API widening.
- Isolated qf-reality checking and `23/23` tests pass. Workspace default/all-target and `rust-tests` checks plus strict `rust-tests` Clippy pass; the target guard cleaned 13.9 GiB once during the feature matrix. The post-commit seam report, full workspace tests, release gate, Graphify status, and protected-path manifest are reconciled after the code commit.
