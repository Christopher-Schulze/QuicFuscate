...
- Windows: Kill-switch activation fails with `NotSupported`; stale exact legacy-rule cleanup remains. The existing dedicated TUN adapter is reference-owned and is never disabled or deleted during cleanup, but its pre-connection address and DNS state are not yet restored exactly. Native Windows client lifecycle readiness therefore remains open under TODO-542; TODO-528 owns the WFP/WinTUN replacement because broad `netsh` block rules cannot be safely overridden by endpoint/TUN allow rules.
