...
- **Scope gate:** The current register has 701 tracker headings, 362 current detail files, and 374 archived detail files. Git enumerates 899 tracked, 55,098 ignored, and 0 non-ignored untracked paths, for 55,997 accounted paths. TODO-773 now classifies the exactly three tracked `archive/` files as `historical-archive` evidence; the validator reports `historical-archive:3` and passes its register/detail/archive/scope checks. TODO-795 owns the Unix admin CLI response-contract gap, TODO-796 owns the E2E migration finalization evidence gap, and TODO-797 owns the persisted logging-mode state gap. The broader audit remains open for those separately owned target, runtime, native, evidence, and feature-contract boundaries.
