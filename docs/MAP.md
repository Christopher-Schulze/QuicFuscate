...
- `cargo fmt --all -- --check`, Bash syntax, `git diff --check`, and runtime guardrail 4j pass. Rust tests, Windows compilation, injected Win32 failure execution, and privileged residue proof were not run: local free space is 1.1 GiB, the host is macOS ARM64, and no Windows administrator/DLL environment is available. TODO-848 retains the remaining negative/native proof.
