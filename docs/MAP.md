...
- `scripts/tests/tun-e2e-fec-netns.sh`: exact-artifact 1,000-packet 0/5/10/25% ping gates and bounded JSON-verified iperf receiver sub-runs at 0% and 10% loss. Harness-source `83bf649` passed on Omega with ping loss 0/1/2/12% and receiver throughput 1.047610/1.047812 Mbit/s; quantitative FEC benefit and telemetry remain under TODO-557.
- `scripts/tests/tun-e2e-fec-burst-netns.sh`: exact-artifact correlated burst-loss gate with three isolated samples per profile, median and worst-case limits. Harness-source `09ea90a` passed on Omega with mild 1/1/2% (median 1%, maximum 2%) and heavy 3/1/2% (median 2%, maximum 3%). Comparative FEC benefit remains under TODO-557.
- `scripts/tests/tun-e2e-fec-transition-netns.sh`: exact-artifact clean -> moderate/severe loss -> recovered policy gate exports six client/server telemetry snapshots after a 150-ping impaired phase; Off remains Zero with no repairs or switches in every phase. Auto remains Zero while clean, commits non-Zero protection under loss, emits repairs by run end after any required source-window fill, returns to Zero within the 10-second-settle plus 250-ping recovery bound, and reserves Fountain outside the moderate profile. Explicit artifact directories also retain both runtime logs, the binary SHA-256, and panic/decryption counts. Comparative and statistical transition acceptance remains under TODO-557.
- Harness-source `39dd360` passed the Omega Auto/moderate contract with clean/impaired/recovered loss 0/21/0%, no panic/decrypt failures, and a retained exact-artifact manifest; Off and severe-profile reruns remain part of the broader TODO-557 matrix.
- The same exact artifact passed Off/moderate at 0/20/0% and Auto/severe at 0/42/0%, with retained manifests and clean teardown.
