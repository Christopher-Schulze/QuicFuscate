...
- `.github/workflows/ci.yml` executes the privileged release-binary proof independently for explicit nftables and iptables backends, requires a machine-readable backend marker, and uploads each proof log. Local Bash syntax, formatting, and diff hygiene pass; hosted Linux execution remains the closure gate. The local Linux cross target still lacks its C sysroot, so no local privileged result is inferred.
