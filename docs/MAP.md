...
- `scripts/tests/tun-e2e-fec-transition-netns.sh`: exact-artifact clean -> moderate/severe loss -> recovered policy gate exports client/server telemetry; Off remains Zero with no repairs or switches in every phase. Auto remains Zero while clean, commits non-Zero protection under loss, emits repairs by run end after any required source-window fill, and reserves Fountain outside the moderate profile. Comparative and statistical transition acceptance remains under TODO-557.
