...
- `scripts/tests/tun-e2e-fec-netns.sh`: exact-artifact uniform-loss gate whose `UNIFORM_PING_SCENARIOS` and `UNIFORM_IPERF_SCENARIOS` are the single source for both printed and executed cases. Harness-source `a8c41aa` passed on Omega with ping loss 0/1/2/27% and receiver throughput 1.047304/1.047557 Mbit/s; quantitative FEC benefit and telemetry remain under TODO-557.
- `scripts/tests/tun-e2e-fec-burst-netns.sh`: exact-artifact correlated burst-loss gate whose `BURST_SCENARIOS` owns profiles, repetitions bounds, and output. Harness-source `a8c41aa` passed on Omega with mild 2/1/0% (median 1%, maximum 2%) and heavy 3/1/3% (median 3%, maximum 3%). Comparative FEC benefit remains under TODO-557.
- `scripts/tests/tun-e2e-fec-transition-netns.sh`: exact-artifact clean -> moderate/severe loss -> recovered policy gate whose `TRANSITION_SCENARIOS` owns each profile's netem loss, all phase sample counts and bounds, recovery settle, and Fountain policy. It exports six client/server telemetry snapshots; Off remains Zero with no repairs or switches in every phase, while Auto adapts under loss and returns to Zero in the declared recovery phase. Explicit artifact directories retain the declared scenario, both runtime logs, the binary SHA-256, and panic/decryption counts. Comparative and statistical transition acceptance remains under TODO-557.
- Harness-source `d714df3` passed the complete Omega transition matrix against binary SHA-256 `322060ac…da8d32b`: Auto/moderate 0/12/0% within 5/35/10%, Off/moderate 0/18/0% with immutable Zero policy, and Auto/severe 0/42/0% within 5/60/10%. Each run printed its selected `TRANSITION_SCENARIOS` row, passed telemetry assertions, and had clean teardown.
- `scripts/tests/tun-e2e-fec-netem-adversity.sh`: exact-artifact 25-scenario ping/liveness matrix whose six declared contracts own every executed qdisc input, phase timing, and loss bound. Each completed scenario reads both telemetry endpoints for active-FEC, observed/lost packets, repairs, and mode switches. An optional new `QF_E2E_ARTIFACT_DIR` preserves only those snapshots with a binary-hash and contract manifest on success and on a later failure; throughput, overhead, mode-stability, and recovery-latency claims remain under TODO-557.
- Harness-source `04eea5c` passed all 25 Omega liveness cases with clean/loss/recovered 0/18/0% and clean teardown; it does not claim comparative FEC benefit or quantitative performance.
