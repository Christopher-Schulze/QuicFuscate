...
- The multi-client harness now offers `QF_E2E_EXTERNAL_EGRESS_CAPTURE=1` as an external-only diagnostic. It captures client 1 underlay UDP only on host veth `qf523h1`, only during each three-trial throughput phase, and emits bounded count/gap summaries. It does not change the product binary, runtime, payload handling, credentials, or normal gate. Native evidence is pending.
