...
26. FEC operator-policy and observability path: `src/main.rs` / `src/implementations/server/mod.rs` engine `FecMode` -> `src/fec/mod.rs` `FecConfig::apply_engine_mode()` -> independent `FecControlPolicy::{Off, Auto}` -> committed `AdaptiveFec` codec state -> `src/core.rs` actual wire send and `src/fec/wire.rs` accepted receive/recovery reports -> connection-local `FecTelemetrySnapshot` plus explicit process aggregates in `src/optimize/telemetry.rs` -> read-only server metric projections in `src/implementations/server/metrics.rs`.
