...
- **Privilege and secrets**: TODO-651 now stores SecretString in a private String owner, validates the checked SecretBytes-to-SecretString boundary, and retains zeroization on drop; native runtime proof remains environment-specific. Privilege `assume_init`/`CStr` validation remains TODO-652/TODO-653; the completed TODO-684 audit also found forged identity and Windows `CurrentIds` portability boundaries (TODO-849), incomplete post-drop state proof (TODO-850), process-lock policy and embedded propagation gaps (TODO-851, TODO-852), and TLS identity/key-output lifecycle gaps (TODO-643, TODO-853); fsutil TOCTOU is resolved by TODO-591 (duplicate TODO-667).
