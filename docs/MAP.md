...
- **Tauri to desktop:** `PersistedTunnel.created_at:u64` and `BufferedLogLine.ts_ms:u64` are serialized as Unix milliseconds. Tauri performs checked wall-clock repair and reports timestamp validity/error metadata. The desktop bridge currently accepts positive numbers without a unit type, replaces invalid `createdAt` with browser wall time, and drops Tauri log validity metadata. TODO-868 owns this boundary.
- **Admin API consumers:** QKey `created_at`/`expires_at` values are consumed as Unix seconds and multiplied by 1000; admin `ts` log values are consumed as Unix milliseconds. Both remain plain `number` types and display paths do not validate range or invalid state. TODO-868 owns this boundary.
- **Elapsed graph:** Desktop bridge throughput uses `Date.now()`; admin Dashboard and SmoothTrafficValue use `performance.now()`; desktop paste suppression uses `Date.now()`; ThroughputChart pauses on hidden tabs and restarts RAF work. TODO-869 owns one monotonic and visibility contract.
- **Scheduler graph:** Desktop pollers, persistence, dialogs, tunnel list, country select, and throughput chart; admin pollers, dialogs, feedback, QKey panel, login, text input, and animation; and shared ConfirmDialog, ripple, toast, and copy-feedback primitives are all classified. Existing cleanup and untracked handles are recorded in TODO-870.
- **Identity graph:** Admin CSRF fallback, shared toast IDs, Sparkline SVG gradient IDs, and test UUID doubles use Date/Math entropy or unbounded random identity. Production tunnel IDs already prefer `crypto.randomUUID()`. TODO-871 owns the identity contract; TODO-872 owns fixed-clock and collision proof.
...
- **Read-only boundary:** TODO-825 is audit-complete, while TODO-868 through TODO-872 remain open. No UI or frontend implementation, frontend check, or browser gate is represented as complete by this map.
...
- `AddTunnelDialog.svelte` and `ImportQKeyDialog.svelte` are the explicit browser-owned creation boundary and use `createDesktopCreatedAt()`; this is separate from Tauri persistence repair. TODO-869 through TODO-872 retain elapsed, lifecycle, identity, and deterministic clock coverage.
