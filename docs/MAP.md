...
- Standalone egress observation: `main.rs::flush_connected_outgoing` records only completed UDP sends when `--telemetry` is active; `optimize::telemetry` owns bounded per-process flush-source, burst, gap, heartbeat, loss, and RTT metrics exposed through `/telemetry`. This path is observational and has no timer or scheduler ownership, but exact ARM64 evidence shows the opt-in path is currently not timing-neutral and cannot be used as a production correction.
