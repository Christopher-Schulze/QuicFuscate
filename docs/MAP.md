...
- **Server data plane:** the shared client fan-out queue is uncapped and its drain materializes the complete backlog into a second container (TODO-612); TODO-613 now owns the explicit PTB-for-all repair for oversized IPv4 uplink packets before either TUN write. The focused DF=0/DF=1 PTB regression passes locally; complete module and native evidence remain separate gates.
