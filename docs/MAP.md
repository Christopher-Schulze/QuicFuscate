...
- TUN provisioning proof: `src/interface.rs`, `src/interface/wintun.rs`, and `src/implementations/client/platform/{linux,macos}.rs` enforce the shared address/prefix/MTU contract; Linux server routing in `src/implementations/server/routing.rs` verifies exact postconditions and bounded owned rollback; `scripts/tests/tun-provisioning-negative-netns.sh` owns the privileged negative/retry/residue proof.
