...
- **Transport/Stealth**: terminal close priority (TODO-697; first-close-wins idempotency resolved by TODO-606; local close error-kind semantics resolved by TODO-772), ConnectionId clone hot path (TODO-638), StealthShaper RNG fallback logging (TODO-639), H3 masquerade time source (TODO-640), domain fronting jitter (TODO-641), TLS cover zero-key fallback (TODO-642), qftls `munlock` (TODO-643), and brain escalation/histogram/config correctness (TODO-584). TODO-644 closes the bounded qf-stealth probe detector with hosted 127/127 crate and 1,619/1,619 root-library execution in macOS job `93669407470`; TODO-808 remains the separate EscalationState timestamp-bound owner. Reality session map timer-driven cleanup is resolved by TODO-570.
