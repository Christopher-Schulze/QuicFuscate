...
- `scripts/tests/tun-e2e-fec-netem-adversity.sh`: exact-artifact 25-scenario ping/liveness matrix whose six declared contracts own every executed qdisc input, phase timing, and loss bound. Each completed scenario reads both telemetry endpoints for active-FEC, observed/lost packets, repairs, and mode switches. An optional new `QF_E2E_ARTIFACT_DIR` preserves only those snapshots with a binary-hash and contract manifest; throughput, overhead, mode-stability, and recovery-latency claims remain under TODO-557.
