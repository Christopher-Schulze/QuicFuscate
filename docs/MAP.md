...
30. Standalone dual-stack TCP diagnostic path: `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh` receiver trial boundary -> `scripts/tests/utils/udp-socket-evidence.py` -> `/proc/net/udp` in the server namespace for port 4433 before/after snapshots -> fail-closed nonzero drop delta. Forward encrypted-flow capture is separately observed at client and server host-veth boundaries; the remaining boundary is server ACK emission through valid client packet processing.
