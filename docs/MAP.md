...
28. Server TUN downlink pressure path: `src/implementations/server/mod.rs` TUN reader channel -> `process_server_tun_packet()` per-target `send_masque_downlink()` -> deferred failures stored in `LiveServerState::pending_tun_downlinks` -> `drain_pending_tun_downlinks()` retries and flushes each housekeeping tick -> socket flush and client TUN delivery.
