...
- **Scope gate:** The current register has 679 tracker headings, 340 current detail files, and 374 archived detail files. Git enumerates 898 tracked and 26,350 ignored paths with no non-ignored untracked files. The fail-closed validator currently stops at tracked `archive/stealth/doh.rs`; TODO-773 owns the missing archive classifier and manifest boundary. This prevents a whole-project completeness claim until the archive disposition is encoded and the validator passes.
