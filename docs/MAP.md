...
26. FEC operator-policy and observability path: `src/main.rs` / `src/implementations/server/mod.rs` engine `FecMode` -> `src/fec/mod.rs` `FecConfig::apply_engine_mode()` -> `AdaptiveFec` controller, codec transitions, and repair emission -> `src/optimize/telemetry.rs` exported mode/loss/window/switch/packet metrics. TODO-558 owns the current gap where Off is only an initial Zero mode, Auto control remains active, and required metric producers/scopes are incomplete.
