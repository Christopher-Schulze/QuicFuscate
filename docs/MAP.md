...
- **Audit persistence:** `AuditLog::flush` bounds the producer acknowledgement wait, but the writer's `flush` and `sync_data` remain synchronous and uninterruptible. Flush/checkpoint errors do not enter the same terminal state as event-write errors; drop/shutdown results are not sticky, and concurrent event admission is not closed before the shutdown command. TODO-675 retains durability/cancellation and failure-state truth; TODO-726, TODO-815, and TODO-814 retain terminal admission, shutdown ordering, and payload-size boundaries. TODO-813 now closes the shared configuration and probe maxima boundary.
...
- The nominal retained-segment budget is at most 8 GiB. Event payload-size and existing-file read limits remain separately owned by TODO-814 and TODO-727.
