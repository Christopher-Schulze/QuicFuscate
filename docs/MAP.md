...
30. Standalone dual-stack TCP diagnostic path: `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh` receiver trial boundary -> persisted start/end window plus client exit status -> `scripts/tests/utils/summarize-throughput-boundaries.py` observes encrypted client-to-server and server-to-client UDP at `qf523h1` and `qf523hs` -> per-window counts and gaps, including explicit zero return traffic. `scripts/tests/utils/udp-socket-evidence.py` separately snapshots `/proc/net/udp` in the server namespace for port 4433 before/after each trial and fails on a nonzero drop delta.
    Exact ARM64 harness `57a2eed` has a clean full-run proof for all four observed boundaries and zero server socket drops. The remaining diagnostic boundary is the same evidence retained through a failing clean-path interval, followed only if necessary by client valid-packet processing.
