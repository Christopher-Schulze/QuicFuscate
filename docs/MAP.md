...
- The Linux CI path now invokes the proof suite with `--require-native-privilege`, compiles the integration executable as the runner user, and executes exactly the isolated privilege test through passwordless `sudo`. Success requires separate standard and Tokio state markers, the terminal native-proof marker, parent-root preservation, and no unavailable marker; the first hosted execution remains pending.
...
- `scripts/tests/audits/audit-runtime-guardrails.sh`, `scripts/tests/utils/util-run-full-suite.sh`, and `.github/workflows/ci.yml` inspect and execute the proof wiring. The Linux fastpath job requires the exact standard and Tokio root-regain proof through passwordless `sudo` and retains its artifact; hosted execution remains pending. Windows native fault injection remains an explicit external boundary and is not inferred from static results.
