...
- Post-handshake application cover uses H3-framed cover requests and Server Push. WebTransport cover opens through H3 Extended CONNECT; its typed unidirectional data stream carries a session prefix followed by opaque application bytes. QUIC Cover PING stays transport-owned; no raw fixed-stream payload or configuration-dependent H3 ignore path exists.
