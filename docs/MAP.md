...
- Rust module ownership: `src/core/`, `src/main/`, `src/qftls/`, `src/stealth/`, `src/transport/connection/`, `src/transport/h3/`, and `src/implementations/server/` are real module trees with explicit narrow re-exports. The active `src/` and `crates/` trees contain no source-assembly `include!` calls and no Rust source above 2,000 physical lines. `scripts/tests/audits/audit-rust-module-structure.sh` is the fail-closed structural gate, with positive and negative fixtures in the adjacent audit test.
...
- Client profile storage wiring (TODO-658, TODO-662): `src/implementations/client/profile.rs` remains a standalone exported API with no current runtime, CLI, desktop, or admin owner. `Profile::from_qkey()` derives 32 lowercase hexadecimal characters from 128 bits of fallible OS CSPRNG output; `add()` and `load()` reject empty or duplicate IDs before state replacement, and non-empty legacy IDs remain unchanged. `save()` sorts IDs, uses a same-directory create-new temporary with creation-time mode `0600`, syncs its bytes, atomically replaces the destination, syncs the Unix parent, and retains dirty state plus the prior destination on failure. Hosted macOS job `93669407470` passes all current profile tests `14/14` and the root library 1,619/1,619 at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f`; implementation commits `251dcf31c79e61c26d02628fe5ec584cdadc762b` and `1197eb9c40b5eca9ca44705576825cbe43255893` are included. TODO-658 and TODO-662 are closed.
- Unified configuration boundary: `config/quicfuscate.toml` -> strict `EngineConfig` parse -> complete section validation -> `src/engine/app_config.rs` -> dedicated transport/client/server projections. `AppConfig` retains only validated FEC, stealth, optimization, and anti-replay runtime state; transport policies and startup-owned sections remain on their canonical owners, and invalid or unknown submitted values fail before admin persistence or runtime construction.
...
- Production VPN carrier: authenticated Core H3/MASQUE CONNECT-UDP carries TUN IP packets for standalone, embedded generic, and live server paths. One registered Flow-ID (`0`) is checked against the active CONNECT-UDP stream before decoded MASQUE DATAGRAM payloads reach TUN; oversized/unavailable datagrams use the bounded `QFT1` length-framed H3 fallback. `crates/qf-control-plane/src/lib.rs` owns the bounded versioned assignment capsule; the client sets a reconnect generation in the CONNECT request, waits for the authenticated assignment before opening TUN, and uses one bounded H3/MASQUE ingress owner for downlink. The public QKey ID in the QUIC Initial selects the server record; the bearer is presented only through the encrypted H3 `x-qf-auth` header. The server gates MASQUE DATAGRAM-to-TUN delivery on the current authenticated state and emits the assignment from the authenticated session allocation. TODO-866 owns the implemented assignment lifecycle and TODO-867 owns the implemented carrier integration; both retain native/external authenticated evidence gates. The retired stealth manager and stealth-local DoH resolver are archived, not compiled.
- TODO-866 native assignment closure: Main CI run `31455010980`, Linux traffic job `93666880449`, applies authenticated `10.0.1.2/24` and `fd00::2/64` assignment before the client TUN carries successful bidirectional traffic at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`. Windows job `93666880422` executes the pre-open projection and disabled-assignment rejection boundaries; unavailable privileged platform lanes remain explicit.
- TODO-867 native carrier closure uses the same Linux job: the bounded framed H3 fallback phase passes, then authenticated MASQUE CONNECT-UDP stream `0` and Flow-ID `0` carry bidirectional tunnel traffic through the live server. The final proof passes without a parallel raw-DATAGRAM consumer or control/data capsule confusion.
...
- Tunnel MTU ownership: `transport::PmtuState` discovers a validated 1280-1500 outer packetization budget; `core::QuicFuscateConnection` derives the FEC/QUIC/MASQUE datagram payload and a separate IPv6-safe inner tunnel MTU. Standalone clients open the TUN from the authenticated server assignment and apply later live TUN MTU changes. The platform normally enforces that interface MTU before the reader; any frame the reader does receive is admitted against the current tunnel-carrier boundary, and only packets above that carrier return local IPv4/IPv6 PTB. The server's `allow_client_uplink()` returns IPv4 Fragmentation Needed for both DF states before either MASQUE or framed-H3 TUN write, intentionally avoiding platform-dependent oversized writes and userspace fragmentation. The Linux CI native job first runs a separate 1280-byte carrier phase for bidirectional framed-H3 fallback, then gives its PTB phase a 1472-byte carrier on both ends, temporarily raises only the client TUN to 1500 after the 1280-byte assignment, and leaves the server TUN at 1280 so the 1,328-byte probe crosses only the server TUN boundary; `prove_server_ptb_from_client()` captures the server-sourced PTB for both IPv4 DF states and IPv6 and retains the exact wire and metric evidence. The two IPv4 probe destinations are isolated to prevent the client's learned PMTU from fragmenting the later DF=0 probe. Run `30823185685` proved that keeping the server carrier at 1280 truncated the probe before routing and produced `MalformedPacket`; run `30823826169` exposed same-destination PMTU-cache contamination; run `30824438300`, job `91722362887`, passed the complete PTB gate with unfragmented 1,328-byte DF=1 and DF=0 probes, server-generated IPv4 PTB for both, IPv6 Packet Too Big, and metric deltas `packet_too_big=3` plus `icmpv6=1`. Tunnel-ingress fingerprint normalization now preserves valid IPv4 TTL 0/1 packets until routing can emit `TIME_EXCEEDED`; the native proof of that correction is closed under TODO-806, while the independent queue failure remains separate.
...
- PKI identity wiring (TODO-577, TODO-656): `crates/qf-pki/src/lib.rs` owns `ensure_pki()`, captures one checked `PkiTime` from the canonical or injected clock, and passes it unchanged to existing leaf/intermediate/root validation, quarantine naming, and fresh root/intermediate/leaf generation; `src/lib.rs` retains the root compatibility re-export. Rustls/WebPKI verifies hostname, validity, trusted chain, and leaf/private-key match before reuse; invalid or incomplete material moves to a unique quarantine directory before a fresh hierarchy is written. Pre-epoch, unrepresentable, overflowed, and non-positive validity timestamps fail through typed PKI errors. Hosted macOS job `93669407470` passes the extracted qf-pki crate `19/19` at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f`, including every TODO-656 clock and regeneration boundary; implementation commit `860b958af544be8aa86e74214adbbf54c6028f36` is included and TODO-656 is closed.
...
- TODO-560 is archived after current-source revalidation. `qf-engine-types`, Engine, Core, `qf-fec`, `ClientRuntime`, and standalone reload retain the same typed transition, queue ownership, fresh-state, reconnect, and next-connection-only boundaries. No new native run is inferred from the archive pass.
...
- Post-handshake application cover uses H3-framed cover requests and Server Push. WebTransport cover negotiates a bounded draft-16 H3 SETTINGS profile before client-initiated `webtransport-h3` Extended CONNECT, publishes readiness only after a 2xx response, and carries opaque cover bytes on typed unidirectional and bidirectional streams. QUIC Cover PING stays transport-owned; no raw fixed-stream payload or configuration-dependent H3 ignore path exists.
- Server Push cover uses bounded seed-varied resource plans.
- WebTransport cover is H3 application cover only, active for Anti-DPI or Intelligent level 2, never a competing VPN carrier. External draft-16 interoperability remains unclaimed because the root TLS surface does not yet negotiate the required QUIC `max_datagram_frame_size` and `reset_stream_at` transport parameters.
...
- `scripts/tests/tun-e2e-netns.sh`: real server/client netns TUN over authenticated H3/MASQUE, pre-open durable routing recovery, durable routing-record publication, SIGKILL/restart stale recovery, exact server-owned and authenticated client-assigned IPv4 TUN provisioning without harness-side repair, 5/5 ping, 0% tunnel loss, graceful record removal, pre-deletion forwarding/TUN/firewall residue assertions, exit-scoped owned-PID cleanup, and fail-closed pre-existing-runtime isolation.
...
- TODO-559 is archived after current-source revalidation. The typed pressure path, bounded TUN/MASQUE ownership, 10,000-PPS limit, heartbeat owner, overload telemetry, and fail-closed native harness contracts remain present. The archive pass adds no new native execution claim.
...
25. Base Linux TUN proof lifecycle: `scripts/tests/tun-e2e-netns.sh` shared `flock` -> fail-closed pre-existing process/namespace check -> pre-open stale routing recovery -> exact server PID and durable routing-record capture -> SIGKILL/restart stale recovery -> authenticated server assignment provisions the client TUN without local address flags or harness-side repair -> exact server/client IPv4 address assertions -> TLS/H3/MASQUE TUN assertions -> graceful shutdown record removal -> exact child reap and owned namespace teardown; `scripts/tests/audits/audit-runtime-guardrails.sh` rejects global product-name process reapers on this path.
...
- `TunConfig.ip6` / `TunConfig.prefix6` (`src/interface.rs`): IPv6 TUN interface address fields consumed by the native Linux/macOS/Wintun backends. Standalone server flags define the server network; standalone clients receive their address, prefix, and MTU through the authenticated `ClientAssignment`. `InterfaceConfig::client_tunnel_addresses()` provides the canonical IPv4-only, IPv6-only, or dual-stack model for generic embedded `ClientRuntime` projection, while the public compatibility `ClientBackend` remains single-family and rejects canonical IPv6 fields.
...
- `main.rs`:
  - Client: authenticated `ClientAssignment` → `TunConfig.ip` / `TunConfig.netmask` / `TunConfig.ip6` / `TunConfig.prefix6` / `TunConfig.mtu`; local address and MTU flags are rejected.
...
- Generic and native TUN result ownership is now fail-closed across the wrapper and syscall boundaries: `TunInterface` rejects zero or oversized reads before pooled-block slicing, requires complete writes before caller success or accepted-byte telemetry, and applies the same invariant to borrowed and owned readers; TODO-845 adds bounded Linux read/write results, macOS `readv`/`writev` progress and total checks, bounded kernel-name parsing, exact Linux rollback identity, and terminalized close ownership with observable failures. Hosted macOS/Linux target tests and the process-real Linux rollback harness prove those contracts. TODO-846 adds a retryable Wintun owner ledger, structured constructor rollback, exact upstream packet-thread-safety wording, and a blocked-reader close assertion. Live privileged macOS utun packet I/O remains unclaimed.
...
- **Crypto data plane**: constant-time tag comparison (TODO-626, complete), key/IV length validation (closed by TODO-627), AEGIS mutex/`unwrap` path (TODO-628, resolved by TODO-582), AEAD header-protection sample validation (TODO-629), GHASH dispatch configuration (TODO-630), target-specific `AesGcm128` schedule zeroization (TODO-631, complete), nonce/IV uniqueness (TODO-632), QUIC KDF input validation (TODO-633, complete), and TODO-681's implementation pass across all seven crypto primitives. TODO-626 routes all 13 production tag-verification call sites through `subtle::ConstantTimeEq` and keeps byte-position-complete mismatch coverage. TODO-633 closure evidence is KDF 26/26, packet 29/29, key-update 4/4, fully green Clippy Matrix run `31471819294`, and Main CI macOS feature-matrix job `93716959602`. Retained and temporary AES schedules, ChaCha nonce/one-time-key material, and constructor copies now have local zeroization owners. The AES table fallback is explicitly not constant-time; x86 GHASH and AArch64 PMULL controls are documented as independent feature-checked release surfaces. TODO-834's exact dispatch delta is reconciled, checked seal-length arithmetic is closed by TODO-716, and primitive AEAD boundaries reject packet numbers above QUIC's 62-bit limit. Native ISA, compiler-erasure, copied-AEGIS-state, and side-channel proof remain open.
- **FEC recovery**: Fountain decoder admission/storage (TODO-634, complete), adaptive emitted-ID cap (TODO-635, complete), decoder peeling complexity (TODO-636, complete), and Wiedemann buffer reuse (TODO-637, complete). TODO-635 closes as stale because the canonical `qf-fec` controller evicts the oldest repair ID above depth `4096` and the current 10,000-send resource regression proves both order depth and unique-ID telemetry remain bounded. TODO-634 closure evidence is Fountain 25/25, wire 22/22, FEC 241/241, the 100,000-symbol adversarial bound, fully green Clippy Matrix run `31471819294`, and Main CI macOS feature-matrix job `93716959602`; TODO-636 closure evidence is isolated `k=256` Criterion improvement of approximately 8.6%, and TODO-637 closure evidence is focused Wiedemann 2/2, FEC 242/242, telemetry 1/1, and high-loss `k=128/256` Criterion coverage in the same hosted lanes.
- **Transport/Stealth**: terminal close priority (TODO-697; first-close-wins idempotency resolved by TODO-606; local close error-kind semantics resolved by TODO-772), receive-side Retry/destination-CID ownership (TODO-638, complete), StealthShaper seed lifecycle (TODO-639, complete), H3 masquerade time source (TODO-640, complete), domain fronting selection (TODO-641, complete), TLS Cover entropy failure (TODO-642, complete), and brain escalation/histogram/config correctness (TODO-584). TODO-642 closes as stale because `qf-stealth` returns secure-entropy failure before HKDF and the root provider maps it to typed `ConnectionError::CryptoError`; the forced-failure regression executes that exact boundary. TODO-638 closure evidence is authenticated Retry/CID coverage, transport 538/538, focused Criterion coverage, fully green Clippy Matrix run `31471819294`, and Main CI macOS feature-matrix job `93716959602`; TODO-639 closure evidence is stealth-focused 266/266, CC/Recovery integration 20/20 plus 2/2 in the same hosted lanes; TODO-640 closure evidence is H3 9/9, persona headers 13/13, and complete stealth 268/268; TODO-641 closure evidence is focused domain-fronting 9/9, complete stealth 269/269, stealth config 9/9, and stealth mode 7/7 in the same hosted revision. TODO-643 closes qftls page-exclusive `munlock` ownership with native macOS/Linux execution. TODO-644 closes the bounded qf-stealth probe detector and TODO-808 closes the separate bounded EscalationState history; macOS job `93669407470` passes their named regressions inside qf-stealth 127/127 and the root library 1,619/1,619. Reality session map timer-driven cleanup is resolved by TODO-570.
...
- **Optimize/Engine/Admin**: engine config reload (TODO-645, complete), io_uring executor ownership (TODO-646, complete), io_uring partial-send disposition (TODO-798, complete), admin HTTP capacity (TODO-647, complete), admin HTTP operation ownership (TODO-661, complete), config write validation (TODO-648, complete), MemoryPool lock release ownership (TODO-516), unsafe raw-pool ownership (TODO-826), safe-pool origin/capacity/TLS/ephemeral accounting (TODO-827), zstd FFI/context synchronization (TODO-828, complete), allocation layouts and recoverability (TODO-829), zero-copy syscall conversion (TODO-830), generic pooled-buffer failure cleanup (TODO-831), FEC pooled-buffer cleanup and symbol lengths (TODO-832, complete), zero-copy DATAGRAM queue return (TODO-833, complete locally), metrics export allocations (TODO-587, TODO-615), and TUN interface unaligned/fcntl safety (TODO-654, TODO-655, both complete). TODO-645 closure evidence is focused Engine/config/QKey 61/61, ClientRuntime 6/6, control-plane integration 5/5, fully green Clippy Matrix run `31471819294`, and Main CI macOS feature-matrix job `93716959602`; TODO-724 and TODO-876 separately close transactional standalone generation publication. Admin-web capacity is CLI-owned with default `16` and maximum `1024`; admission is acquired after `accept()` but before task creation, excess sockets are dropped without a user-space pending queue, and accepted tasks are joined on shutdown. Each accepted request uses a validated `50..=120000` ms operation deadline, a bounded command/result channel, an owned blocking worker `JoinSet`, explicit timeout/cancellation/panic/late-completion counters, and a one-second shutdown drain. Effective operation state is included in runtime admin status and health. TODO-647 closure evidence is focused 68/68, real contract 2/2, fully green Clippy Matrix run `31471819294`, and Main CI macOS feature-matrix job `93716959602`; TODO-661 closure evidence is focused 72/72 and server 467/467 in the same hosted revision. TODO-687 closed the io_uring unsafe/lifetime and receive-slot audit boundary; TODO-801 closed the additional Linux kernel evidence boundary.
- **Privilege and secrets**: TODO-651 now stores SecretString in a private String owner, validates the checked SecretBytes-to-SecretString boundary, and retains zeroization on drop; native runtime proof remains environment-specific. TODO-652 validates libc result-pointer identity before `assume_init`, and TODO-653 bounds account-name pointers to the returned lookup buffer and a verified NUL before `CStr::from_bytes_with_nul`; both boundaries have local tests. TODO-643 closes preloaded-key lock/publication lifecycle: individually locked Unix keys use page-exclusive anonymous mappings, rejected duplicate/conflict values zeroize before their own `munlock` and `munmap`, and process-wide/disabled/degraded paths remain distinct. Main CI run `31461410134` proves the native cleanup path on macOS job `93685519639` and Linux job `93685519734` at revision `c3432072b9066febd2a7d64227a35894ef8c7eea`. TODO-849 now closes the public forged-identity and cross-target `CurrentIds` source contracts, adds local FFI safety comments, and rejects malformed `getgroups()` counts; TODO-850 closes the source-level filesystem-ID/post-drop state contract and local standard/Tokio probe assertions; TODO-851 closes process-lock policy propagation and embedded ordering; TODO-852 closes process-lock failure/readiness policy, TODO-853 closes certificate-key correspondence and zeroizing key-output ownership, and TODO-854 closes local deterministic privilege, memory-lock, TLS, ordering, and portability-proof wiring. Native Linux root-regain and Windows native fault lanes remain explicit unavailable boundaries. fsutil TOCTOU is resolved by TODO-591 (duplicate TODO-667).
- **TODO-651 hosted closure**: Main CI run `31455010980`, macOS job `93666880395`, executes all checked UTF-8 construction, cloning, source-owner erasure, owned-buffer erasure, malformed-byte rejection, and real QKey parser cases at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`; Windows job `93666880422` independently executes the QKey rejection path.
- **TODO-652 hosted closure**: The same macOS job executes status, null-result, caller-owned pointer identity, `ERANGE` growth, and real unknown-account lookup regressions twice, including the dedicated privilege/memory/TLS proof suite. The root-only transition remains a separate native boundary.
- **TODO-653 hosted closure**: The same workspace and privilege proof gates execute in-buffer NUL acceptance plus null, outside-buffer, and unterminated account-name rejection. No conversion can scan outside the returned lookup buffer.
...
- **SIMD Reed-Solomon compatibility path**: TODO-594 corrects the standalone x86 AVX2/GFNI Reed-Solomon encode/decode implementations, including cross-shard accumulation, canonical GF(256) coefficients, full matrix inversion, dynamic LUT multiplication, safe vector tails, and runtime shard metadata checks. Rosetta-executed x86 tests pass; production recovery remains wired through `src/fec/`, while the completed FEC/GF16 audit is TODO-686 and GF16 polynomial correctness is closed by TODO-715. TODO-679's audit confirmed that the old AVX2-to-GFNI delegation claim is stale and split the remaining SIMD work into TODO-834, TODO-835, and TODO-836; FEC-specific SIMD remediation is TODO-855.
- **FEC SIMD contract**: TODO-855 closes the current FEC target-feature and release-safe length implementation boundary. The shared matrix selects distinct VBMI2/VBMI/AVX2/SSE2/SVE2/NEON/scalar levels, private helpers bound lengths in release builds, PCLMUL and compile-time SVE2 paths use runtime matrix gates, and the guardrail inventory includes `src/fec`. TODO-859 closes local negative/differential proof and documentation truth while preserving unavailable native lanes; TODO-715 closes GF16 PCLMUL mathematics.
- **Unsafe memory and pooled-buffer boundaries**: the feature-gated `UnsafeMemoryPool` cache/raw-pointer ownership contract is implemented with synchronized exact-base registry checks and focused `unsafe_rust` coverage under TODO-826; TODO-827 implements the safe `MemoryPool` origin/capacity/TLS/ephemeral ownership and accounting state machine, including bounded TLS-aware shrink, pool/TLS lifetime cleanup, and release-safe cleanup for rejected blocks. TODO-828 implements the checked native/fallback zstd context, mutex-serialized `Sync`, explicit failure typing, checked header lengths, and pool cleanup. TODO-829 provides checked 64-byte layouts, `isize::MAX` capacity bounds, fallible constructors/allocation/resize APIs, partial-construction cleanup, and explicit compatibility-wrapper panic policy. TODO-830 owns the typed Unix/Windows zero-copy send/receive boundary, exclusive receive borrows, checked iovec/WSABUF conversions, and direct-caller progress policy; Main CI run `31458252191`, Windows job `93676432308`, executes the exact Windows ABI regression within a `23/23` passing leaf at revision `bd159d2762ad2c242da165718d33ba1e7d064015`. Generic pooled-buffer failure cleanup (TODO-831), FEC pooled-buffer cleanup and symbol lengths (TODO-832), and zero-copy DATAGRAM queue return (TODO-833) are complete on the current ARM64 macOS workspace. TODO-826 through TODO-833 close the implementation split; umbrella TODO-678 remains blocked only by unavailable Miri and native Linux/Windows/ISA evidence. TODO-679 is the completed SIMD audit owner; TODO-834's feature-intersection implementation and local ARM64 verification are complete, TODO-835's release-safe unsafe-boundary implementation is complete, TODO-836's shared SIMD safety-contract guardrail is complete, and TODO-855 now closes the FEC-specific matrix, threshold, telemetry, length, and local safety-contract implementation boundary. Native safety proof remains unclaimed. TODO-680's Optimize audit is also complete and recorded the P1f-to-AVX2 reduction route, P4a-to-AVX512 moving-average route, test-only BMI2 bitmap boundary, short-pattern overwrite, overflow-prone pattern positions, SVE2 base64 output coverage, packet-number/VNNI contracts, Linux batch FFI proof gaps, and percentile/profile test gaps. Remediation remains open for the separate owners.
- **Unsafe crypto/transport/interface/privilege**: crypto primitives (TODO-681 source-and-owner reconciliation complete; schedule/transient-state, AES table, GHASH-control, copied-AEGIS-state, and native-proof findings remain; checked lengths, primitive nonce rejection, and the MORUS loader precondition are source-closed), transport batching/UDP/AF_XDP removal/frame-packet/PMTU surfaces (TODO-682 and remediation owners TODO-837 through TODO-842 complete, including native Linux kernel/syscall and x86_64 AVX2 proof), interface/Wintun/platform/WFP (TODO-683 audit complete; remediation split into TODO-843-TODO-848), and privilege/mlock/qftls/secret handling (TODO-684 audit complete; TODO-643, TODO-652, and TODO-653 are closed, TODO-516 retains pooled-block ownership, TODO-849-TODO-854 close the current implementation and proof split, and broader native Linux/Windows privilege execution remains unclaimed; TODO-651's local String-backed UTF-8 remediation is implemented). No frontend or Tauri implementation crosses these backend boundaries.
- TODO-683 is archived as a completed source-and-owner audit after current-path revalidation. TODO-654, TODO-664, and TODO-843 through TODO-848 are archived; unavailable Windows injected-failure and privileged residue lanes remain unclaimed.
- TODO-684 is archived as a completed source-and-owner audit after current-path revalidation. TODO-516 and TODO-678 remain explicit blocked pooled-lock owners and are not absorbed by the audit archive.
- **Unsafe FEC/io_uring/auxiliary**: TODO-686 and its FEC remediation owners TODO-634, TODO-636, TODO-637, TODO-690, TODO-715, TODO-832, TODO-834 through TODO-836, and TODO-855 through TODO-860 are archived. Native SIMD, AMX, sanitizer, Miri, privileged Linux, and unavailable cross-platform proof remain unclaimed. io_uring/io_driver lifetime ownership is TODO-687 and unordered partial-send disposition is TODO-798. TODO-688/TODO-861 and TODO-689 with TODO-670, TODO-752, TODO-811, TODO-826, TODO-827, TODO-834 through TODO-836, TODO-841, TODO-843, and TODO-862 through TODO-865 are archived as the audit-file and auxiliary unsafe audit/remediation owners. TODO-730 and TODO-754 remain separate blocked audit/register owners.
- **Unsafe QKey/admin**: TODO-685 is archived after current-source reconciliation found no raw-memory unsafe operation in QKey registry or admin session logic. Registry storage contains the only inspected Rust `unsafe` block, a Windows `MoveFileExW` call whose path encoding, pointer lifetime, aliasing, flags, result handling, and portable replacement regressions are closed by TODO-873 at unchanged source commit `b532ecd`.
  Native Windows execution remains explicitly unclaimed. TODO-861 and TODO-728 separately close audit-file FFI and audit pathname binding; TODO-647/TODO-661/TODO-665 retain the safe admin session lifecycle and replay ownership.
...
- **Client TUN projection:** TODO-663 closes the static portion of the split ownership boundary. The generic EngineConfig schema has canonical `tun_ip6`/`tun_prefix6` fields, `InterfaceConfig::client_tunnel_addresses()` validates and normalizes legacy and canonical sources, and `ClientRuntime` projects the result into typed `TunConfig` IPv4 and IPv6 fields. The compatibility `ClientBackend` remains explicitly single-family and fails before platform mutation when canonical IPv6 fields are supplied. TODO-731 closes standalone server CLI parse handling, and TODO-866 binds `AssignedClientIps` to a versioned authenticated client assignment before TUN open. TODO-663, TODO-731, and TODO-866 are archived; unavailable native evidence remains unclaimed. Original projection evidence remains config 3/3, generic client 5/5, compatibility 6/6, library 2,295/2,295, and all-target binary 41/43 with two then-known unrelated reload failures.
- **Linux resolver ownership:** TODO-623 remains the owner of absent-original, stale-ownership, and crash recovery. TODO-649 supplies the typed `LinuxResolverPaths` owner with standard `/etc/resolv.conf` defaults, backup-derived state/lock paths, schema-3 source/target/backup identities, create-only and atomic state publication, and fail-closed broken-link, replaced-target, foreign-backup, and read-only-parent handling. Valid symlinks remain symlinks through verified restore. Main CI run `31471819318`, Linux job `93716959582`, closes the Native Linux resolver ownership proof on revision `3e242d3c49aba65d3e2919370aa5021098411c94`: the unprivileged runner passes all 15 restore fixtures, all three path/lock fixtures, and the exact terminal marker.
...
- **Engine reload:** The generic `QuicFuscateEngine` exposes validated in-memory `update_config` and file-backed `reload_config_from_file`. Created/stopped engines replace the complete config; running clients synchronize the next-connection projection and preserve active non-FEC sessions, while startup-owned engine/interface/telemetry/logging/audit/crypto/optimization/security sections require a stopped engine. Running generic servers reject these mutations. The standalone server has a separate validated file reload path with explicit `NextConnectionOnly` scope, startup-owned fields, and a shared-policy-before-best-effort-transport publication boundary. That path reads `EngineConfig` only, so construction-time `ServerConfig` including blacklist/GeoIP/DDoS/auth policy is not replaced. TODO-645 owns the generic contract; TODO-660 closes blacklist worker lifecycle; TODO-724 owns standalone transactional publication.
- **io_uring:** `UringBatchSender` has no cross-call pending queue: scratch vectors are reused and submissions are chunked to SQ capacity. Public sender and worker methods reject batches above 256 packets or 524,288 aggregate payload bytes before their owned copies. Client and standalone server packet paths use one bounded `UringBatchWorker` per runtime, with a joined blocking thread, controlled completion deadline, quarantine on cancellation/timeout, and typed failure without ambiguous replay. Generic client TUN reads now use a trait-level `TunReadContract`; native backends declare nonblocking and custom backends default to a separately owned blocking-reader contract. TODO-687 closes pointer/lifetime safety, TODO-798 closes exact partial-send disposition, and TODO-646 is closed by Main CI run `31463107578`, Linux job `93690492040`: the transport matrix passes 20/20 and kernel integration passes 1/1.
- **Config writes:** The active `write_runtime_config` handler parses `AppConfig`, validates it, validates transport overrides, and only then calls `atomic_write_file`. TODO-648 is closed as stale; the earlier `config.rs` location was not the TOML write path. The direct invalid-candidate regression passes 1/1 and preserves the existing target byte-for-byte across full-config parse rejection and separate QUIC-varint rejection; the server test surface passes 163/163.
- **Linux DNS backup:** Restore copies the backup before removing it, verifies source/target identity and restored bytes, and deletes the backup only after successful read-back. TODO-649 owns the standard and typed alternate source/backup contract, derives state/lock paths from the backup, and rejects broken or replaced symlinks plus foreign backup/state entries without overwriting or removing them. TODO-623 owns absent-original and crash recovery. Main CI run `31471819318`, Linux job `93716959582`, closes the Native Linux resolver ownership proof.
- **TUN fcntl:** The canonical Linux and macOS constructors check `F_GETFL` and `F_SETFL`, route failure through their descriptor/interface rollback owners, and return before device publication. The macOS compatibility constructor keeps its `FdGuard` armed through both calls. TODO-655 is closed as stale with interface coverage 28/28 and compatibility close-failure coverage 1/1. TODO-654 owns the alignment-safe BMI2 load and unaligned-subslice regression proof; broader interface/platform remediation remains TODO-843 through TODO-848.
- **TODO-654 hosted closure:** Main CI run `31455010980` executes the intentionally unaligned portable write on macOS job `93666880395` and both portable plus direct BMI2 unaligned paths on Windows x64 job `93666880422` at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`. Dispatch selection remains separately owned by TODO-843.
- **Probe parsing:** `qf-privilege-probe`, `--initial-count`, and `--initial-interval-us` already return parse errors. TODO-657 makes `--timeout-ms` and `--hold-ms` in `qf-e2e-client` and `qf-e2e-desktop` explicit and testable: malformed or missing values fail, omitted values retain defaults, zero is valid, and duplicates remain last-value-wins. Hosted macOS job `93669407470` at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f` passes the exact bin tests `8/8` and `3/3` plus the complete `quicfuscate` binary `47/47`; implementation commit `e791bc0` is included and TODO-657 is closed.
- **Retry token:** Per-field bounds already exist. TODO-618 owns the corrected 192-byte aggregate capacity and 169-byte worst-case round-trip. TODO-659 is archived as stale because accepted inputs cannot exceed the active aggregate limit and oversized fields fail before body allocation and HMAC.
- **Blacklist sync:** TODO-660 is closed. The only current production caller is `ServerRuntime::run_loop()` housekeeping -> `LiveServerState::maybe_sync_blacklist()`. `BlacklistSyncOwner` stores the owned task plus `BlacklistSyncControl`, performs one atomic due/in-flight claim, observes success/failure/cancellation, and closes on drain. The synchronizer retains strict HTTPS, redirect, request-timeout, body, UTF-8, entry, custom-CA, cache-format, expiry, and last-known-good checks. Parsing, serialization, temporary cache writes, and `sync_all` run in `spawn_blocking`; one phase transition and commit mutex admit atomic cache rename plus active-state replacement together or reject both on cancellation. An overdue fetch is abortable, while an already admitted publication remains owned until the barrier exits. Retry delays are bounded at `5` through `300` seconds; absolute caps are `300` seconds request timeout, `16777216` bytes body/cache, `250000` entries, and `604800` seconds TTL/interval. Lifecycle events, freshness ages, stale state, active entries, and in-flight state are exported through Prometheus, health, and admin status. Blacklist tests pass 21/21, atomic commit rejection 1/1, the server surface 539/539, all-target checking, and strict library Clippy. Controlled external HTTPS and native Linux execution remain unclaimed environment evidence.
...
- **Log rotation:** `crates/qf-logging/src/lib.rs` owns FIFO `Rotate` and `Reopen` commands with bounded acknowledgements. Authenticated `POST /api/logs/rotate` force-rotates through the writer owner and emits a typed admin audit event. Unix SIGHUP preserves the validated next-connection-only config reload, then independently reopens the file sink and audits/logs the result. External rename and copytruncate are supported by sending SIGHUP after the pathname operation; reopen refreshes tracked size from the current pathname. TODO-672 is complete, and archived TODO-812 closes failed logger-installation worker cleanup by shutting down and joining the temporary writer.
...
- **Privilege and FFI:** TODO-683 completed the full read-only source, caller, platform-gate, cleanup, test, script, documentation, related-TODO, and history pass for interface and platform boundaries. TODO-684 completed the corresponding privilege, memory-lock, qftls, secret, caller, platform, cleanup, test, script, documentation, related-TODO, and history pass. TODO-652 closes returned-pointer identity and TODO-653 closes bounded account-name conversion in `crates/qf-privilege/src/drop.rs`, both with local deterministic tests; TODO-849-TODO-850 now close the source-level final-boundary identity, portable `CurrentIds`, complete filesystem-ID, typed partial-transition, and local probe contracts. TODO-851 closes standalone/embedded policy propagation and ordering, and TODO-852 closes process-lock failure/readiness policy. Pool unlock ownership remains TODO-516; page-exclusive qftls key unlock ownership is closed by TODO-643, and TODO-853 closes certificate/key-output ownership. TODO-854 closes local deterministic privilege, memory-lock, TLS, ordering, and portability-proof wiring. Native Linux root-regain and Windows native fault lanes remain environment-specific and unclaimed.
- **FEC unsafe inventory:** TODO-686 completed the full FEC audit. Active GF(256)/GF(16) wrappers clamp slice lengths before private vector calls; the old public raw-slice and SSSE3 claims are stale. TODO-855 closes the 12-declaration local `# Safety` inventory, release-safe length bounds, shared PCLMUL/NEON/SVE2 matrix gates, distinct GF16 SIMD levels, threshold mapping, and FEC guardrail scope. TODO-834 closes the shared feature-intersection implementation for FEC/GF16 dispatch; TODO-859 closes local negative/differential proof and documentation truth while preserving unavailable native lanes. TODO-832 closes pooled production packet ownership and declared-length transfer; TODO-856 closes direct compatibility-constructor lengths, Decoder4/8/16 dimension and active-block checks, public matrix shape validation, direct wire-helper validation, and source-only small-pool truncation. TODO-857 closes direct Fountain constructor bounds, source-index admission, symbol-size parity, and propagation validation. TODO-858 closes direct FEC policy construction, runtime-policy limits, sent/acked/lost tuple invariants, non-finite feedback, and controller conversion boundaries. TODO-860 closes direct counter wrap, interleave-shape arithmetic, sequence narrowing, repair-ID packing, Fountain wire source-ID arithmetic, and decoder/Wiedemann/Fountain size conversions. TODO-676 and TODO-816 through TODO-819 close active AMX kernel, dispatch, detector, proof-lane, and profile ownership while recording the absent native arithmetic backend.
- **io_uring:** The current sender copies payloads into owned slots before publishing pointers and quarantines after submit/protocol errors; SendMsgZc tracks `CQE_F_MORE` and waits for every announced notification, including errored primaries. Receive completion metadata is range-checked, zero-length and error receives are re-armed, and ring destruction precedes pool-block return. Client eventfd reads require exactly eight bytes. TODO-687 closed the unsafe/lifetime boundary and TODO-801 closed the additional Linux kernel evidence boundary. TODO-646 closes bounded sender admission, runtime-worker, shutdown, and generic client TUN ownership with the hosted Linux transport matrix passing 20/20 and kernel integration passing 1/1. TODO-798 closes unordered partial-send retry disposition.
- **Auxiliary unsafe surface:** TODO-689 and its environment, lint, pool, SIMD, PMTU, interface, prefetch, NUMA, auto-tuner, and constant-pool remediation owners are archived. Current sources provide non-faulting architecture prefetch hints, off-platform-provable NUMA classification, non-creating telemetry observation, explicit auto-tuner lifecycle, full-capacity zero-safe constant rings, and shared environment-test serialization. Telemetry, crate-root, and transport/config matches remain classified as source-text false positives. TODO-730 and TODO-754 remain separate blocked audit/register owners; unavailable native execution remains unclaimed.
...
- **Historical HTTP/3 control and parser snapshot:** This 2026-08-02 audit preceded implementation commit `3a50234`. Current H3 emits transactional client/server control-stream prologues, validates bounded local and peer SETTINGS, parses full QUIC-varint frame headers, and enforces stream ownership and legality. TODO-691 and TODO-692 retain the current closure evidence.
...
- **Archive boundary:** TODO-741 is archived after current workflow and negative-fixture revalidation. Release `v0.4.4` contains the complete three-platform signed updater map but predates `d047747`, so it is retained only as published asset-shape evidence for this owner.
...
- **Archive boundary:** TODO-877 is archived after the current benchmark-cell, fast/full-mode, shared-writer, shell-syntax, and implementation-ancestry gates passed again. Dry-run and fixture results remain contract evidence, not performance measurements.
...
- **Scope boundary:** No UI or Omega state was changed. TODO-633 is closed by the exact KDF rejection matrix and hosted all-target/Clippy evidence; TODO-758 closes hosted nightly sanitizer execution after local metadata and inventory closure, and the broader project audit remains open under its existing task owners.
...
- The audit separates current product-wire validation from direct public API reachability. TODO-634, TODO-636, TODO-637, TODO-690, TODO-715, TODO-832, TODO-855 through TODO-860, and the shared SIMD/AMX/environment owners identified by the audit are archived. Native ISA, sanitizer, Miri, privileged Linux, and unavailable cross-platform proof remain explicitly unclaimed.
- No production implementation, build, test, native probe, privileged network run, commit, or push was performed for TODO-686. The complete findings and current evidence boundary are in `docs/todo/done/todo-686-fec-unsafe-audit.md`.
...
- TODO-671, TODO-675, TODO-726, TODO-728, TODO-813, TODO-814, TODO-815, and TODO-861 close the file-mode, writer-lifecycle, terminal-admission, pathname-binding, configuration, payload, shutdown-order, and FFI/security-failure boundaries and are archived. Native Windows replacement execution and privileged root ownership-failure execution remain unclaimed.
- No production implementation, build, test, native Windows/root probe, privileged network run, commit, or push was performed for TODO-688. The complete evidence boundary is in `docs/todo/done/todo-688-audit-server-unsafe.md`.
...
- TODO-670, TODO-752, TODO-811, TODO-826, TODO-827, TODO-834 through TODO-836,
  TODO-841, TODO-843, and TODO-862 through TODO-865 close the environment,
  lint, pool, SIMD, PMTU, interface, prefetch, NUMA, auto-tuner, and
  constant-pool boundaries and are archived.
- The whole-project source audit is read-complete through this owner. TODO-730
  and TODO-754 remain blocked under their current fail-closed audit/register
  evidence; TODO-799 is archived after closing the canonical `Blocked`-section
  validator contract.
- No production implementation, build, test, native architecture probe,
  privileged run, commit, or push was performed for TODO-689. TODO-730 and
  TODO-754 remain separate blocked machine-checkable audit/register owners;
  unavailable native runtime proof remains explicitly unclaimed. The complete
  evidence boundary is in `docs/todo/done/todo-689-remaining-unsafe-audit.md`.
...
- **Hosted closure:** Local library check, strict library Clippy, 26 KDF tests, 29 packet tests, and 4 connection key-update tests pass. Full historical workspace/all-target tests completed with 2,204/2,206 passing; the two failures were TODO-807 DNS endpoint resolution and TODO-768 Rustls ClientHello readiness. Implementation commit `06de1cd` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` and Main CI macOS feature-matrix job `93716959602` are green for the platform-independent KDF contract.
...
- **Hosted closure:** Fountain 25/25, wire 22/22, complete FEC 241/241, strict library Clippy, and all-target workspace check pass. The full historical all-target workspace test run was 2,207/2,209 because TODO-807 DNS and TODO-768 Rustls baseline tests failed. Implementation commit `827bd3c` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` and Main CI macOS feature-matrix job `93716959602` are green for the platform-independent bounded-state contract. No UI, Omega, or remote state was changed.
...
- **Closure boundary:** FEC 241/241, workspace check, strict library Clippy, format, and diff checks pass. Full historical workspace/all-target tests were 2,207/2,209 due only to TODO-807 DNS and TODO-768 Rustls failures. Implementation commit `3a73209` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` and Main CI macOS feature-matrix job `93716959602` are green for the platform-independent representation contract.
...
- **Closure boundary:** No FEC equation mathematics, validation/fallback semantics, UI, Omega state, or remote state is changed by this wiring. Implementation commit `56f088a` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` and Main CI macOS feature-matrix job `93716959602` are green for the platform-independent scratch-ownership contract.
...
- **Hosted closure:** TODO-639 is closed. Implementation commit `e172069` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` is fully green and Main CI macOS feature-matrix job `93716959602` passes the default all-target Rust lane. Native Omega execution is not required for this platform-independent seed-lifecycle contract.
...
- **Hosted closure:** TODO-641 is closed. Implementation commit `7e53d15` is included in hosted revision `3e242d3c49aba65d3e2919370aa5021098411c94`; Clippy Matrix run `31471819294` is fully green and Main CI macOS feature-matrix job `93716959602` passes the default all-target Rust lane. Native Omega execution is not required for this platform-independent selection contract.
...
- **Detector boundary:** `FeatureDetector` uses stable in-process CPUID leaf `07H/00H` decoding for x86_64 AMX-BF16, AMX-TILE, and AMX-INT8 capability bits, with maximum-leaf validation and an SGX fail-closed path. The former PATH-resolved `cpuid` process, textual output parser, and unstable AMX feature-detection macros are absent.
...
- **Guardrail and tests:** The runtime guardrail includes `amx_detector_process_free`. Focused detector tests pass `3/3`, the stable Linux x86_64 default cross-check passes, and strict leaf Clippy, format, and diff checks pass. Product dispatch remains ineligible because the AMX arithmetic backend is still unverified.
- **Current workspace evidence:** `scripts/out/audits/workspace-seams-20260810T-stable-amx-detection-postpush/workspace-seams.json` records published source `b4ef855f4d20fd78bc0cb72fb820e2d821b44ba3`, 36 packages, 336 Rust files, 207,713 source lines, 106 module edges, 123 workspace dependency edges, no strongly connected components, and `protected_changes=[]`.
...
- Evidence boundary: local execution covers transfer classification, Unix invalid-descriptor and iovec-count checks. Main CI run `31458252191`, Windows job `93676432308`, executes `memory_pool_growth_tests::windows_zero_copy_checks_u32_abi_bounds` at revision `bd159d2762ad2c242da165718d33ba1e7d064015`; the complete Windows leaf passes `23/23` with no ignored tests.
- Verification: release zero-copy focus `18/18`, native `unsafe_rust,compression_zstd_ffi` zero-copy focus `18/18`, default-feature release library `2424/2424`, `unsafe_rust` release library `2455/2455`, current local `qf-memory-pool` `23/23`, hosted Windows `qf-memory-pool` `23/23`, all-target checking, strict library/all-target Clippy, formatting, diff hygiene, and locked metadata pass.
...
- Archive revalidation on 2026-08-11 confirms implementation commit `f6e20ff` remains in `main`; the current ownership path is `crates/qf-transport-frames/src/lib.rs` -> `src/transport/frames.rs` -> `Connection::preflight_frame_payload()` and the send reservation/serialization path. The runtime guardrail passes with Critical 0 and Warnings 0, and Clippy Matrix run `31501355668` is green at revision `c33da56f4a62b09723b9e9f0fa2a6cf12f0bfbe5`; protected frontend and Tauri paths remain unchanged.
...
- H3: `src/transport/h3_parts/connection.rs` emits transactional client/server control SETTINGS, caps local field sections and QPACK table capacity at 16 MiB and local blocked streams at 64, and parses full QUIC-varint stream prefixes, frame types, lengths, and fixed control payloads. Receive wiring is peer STREAM bytes -> fragmented prefix retention -> endpoint-owned classification -> per-stream placement/message validation -> staged H3 events or raw WebTransport data. `src/transport/h3_parts/qpack.rs` owns the RFC 9204 static table, byte-bounded dynamic tables, absolute insert indices, Required Insert Count/Base field sections, fragmented encoder/decoder instructions, blocked-stream release, acknowledgements, cancellation, and distinct QPACK error classes. Encoder and decoder streams are opened lazily with their first transactional instruction batch; zero capacity creates neither. Transport RESET_STREAM notifications retain stream ID/error ownership so a blocked field section can release decoder state and emit Stream Cancellation. Unknown streams discard every chunk; control and QPACK streams are unique and critical. Request and push streams retain awaiting-headers, body, and trailers state, including interim responses. WebTransport cover connections advertise and validate bounded draft-16 H3 settings, allow only client-initiated CONNECT session IDs, move sessions from pending to established only at a final 2xx response, classify fragmented `0x54` unidirectional and `0x41` bidirectional prefixes, and bound sessions, streams, buffered early data, and cumulative bytes. Invalid negotiation, initiator, session, duplicate, reset, close, and capacity transitions fail closed. The profile remains internal application cover because the root TLS transport parameters do not yet claim external draft-16 capability. Server Push wiring remains peer `MAX_PUSH_ID` plus latest request stream -> monotonic push ID -> request-stream `PUSH_PROMISE` -> independent server-unidirectional push stream with response `HEADERS` and cover `DATA`. TODO-691/TODO-692 own the preceding control/state closure, TODO-882 owns QPACK synchronization, and TODO-471 owns the bounded WebTransport cover state. TODO-882 is closed through final source commit `6dba2a3`: all 23 jobs in Clippy Matrix run `31516652440` pass, and Main CI macOS feature-matrix job `93863449188` passes the complete root test surface.
- TODO-879 archive revalidation anchors that baseline to commit `3a50234` immediately after handoff `a96bef9`; both remain ancestors of current `main`. Current wiring is `Connection::with_transport()` -> transactional `init_control_stream()` -> transport STREAM queue, while peer reads flow through buffered unidirectional classification and checked frame dispatch. Runtime guardrails pass with Critical 0 and Warnings 0, current hosted Clippy is green, and the archive pass leaves protected frontend and Tauri paths unchanged.
- TODO-691 archive revalidation adds the missing local-config boundary in `c2b891c`: all three advertised SETTINGS values are checked against the shared safety ceiling before QPACK construction and `init_control_stream()`. Clippy Matrix run `31503056901`, the locked workspace release build, and the complete workspace all-target `rust-tests` step in CI job `93818015386` pass; protected frontend and Tauri paths remain unchanged.
- TODO-471 is closed through Rust 2021 compatibility commit `eeb1538`. Clippy Matrix run `31518610863` passes all 23 jobs, and Main CI macOS feature-matrix job `93870588204` passes the complete workspace all-target `rust-tests` surface with root library `1,661/1,661`, including all seven new WebTransport regressions.
...
- Evidence boundary: the 2026-08-07 reconciliation itself was source-backed and ran no runtime, native, wire, build, test, or Clippy gate. Subsequent closure evidence is authoritative in each linked task detail and the task register.
- TODO-698 archive revalidation on 2026-08-11 confirms implementation commits `5ce6b57` and `9d5de06` remain in `main`; the current ownership flow is `crates/qf-fec::FecPacket::to_raw()` -> both Core front/write/pop commit sites and transport DATAGRAM front/stage -> padding -> short-header seal -> FIFO/telemetry commit. Runtime guardrails pass with Critical 0 and Warnings 0, and Clippy Matrix run `31501777499` is green at revision `a84476d8359db4842504ce4827ce56c88e07ae2b`; protected frontend and Tauri paths remain unchanged.
...
- TODO-839 native closure: Main CI run `31455640097`, Linux fastpath job `93669407582`, runs the packet-header target with `-C target-feature=+avx2` at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f`. The exact AVX2/scalar unaligned-output regression passes inside a `9/9` target, while the portable packet-header target passes `8/8`; this closes the packet-number ISA gate without relabeling any skip.
- TODO-842 native closure uses the same artifact: io_uring initialization, zero-length CQE rearm, opt-in SendMsgZc, the real Linux kernel hotpath, malformed UDP metadata, Linux invalid-fd, all transport integration targets, and harness loopback pass. The x86_64 host records the AArch64-only rerun as `SKIP`, preserving the previously executed ARM64 boundary instead of treating the skip as coverage.
- TODO-840 closure: the Linux fastpath frame target passes `8/8` at revision `abb9c8149ee5d4e46983e129720fc2d88698a45f`, covering malformed ACK/CID/STREAM input, cumulative capacity, and normal encoding. Prior guarded ARM64 execution covers the bounded AArch64 cursor path; the x86 runner keeps that architecture-only rerun explicit rather than manufacturing coverage.
...
- Main CI run `31463107578` contains implementation commit `19cc4769286ab1d6a1eb054fcf2d418ab432d51b`. macOS job `93690492100` passes qf-privilege `23/23`, including the forged-root and over-capacity `getgroups()` regressions, plus the portable privilege integration target `3/3`. Windows job `93690492145` successfully checks the core and compiles the Rust test library with `tun-windows,rust-tests`, including qf-privilege's non-Unix branch without the former `CurrentIds` error. Proof artifact `9090674020` is unexpired. TODO-850 separately closes native Linux filesystem-ID, post-drop, and root-regain proof; Windows native privilege transitions are not inferred.
...
- Main CI run `31469067924`, Linux fastpath job `93708224053`, passes the required isolated test through passwordless `sudo`. Standard mode verifies one thread, Tokio mode verifies nine threads, the terminal marker covers both UID/GID root-regain checks with the parent still root, and no unavailable marker is present. Artifact `9093181072` is unexpired and its versioned manifest reports Linux x86_64, native status `PASS`, and zero failures.
...
- `crates/qf-privilege/src/drop.rs` adds a pure Linux root-regain errno contract test, while existing TODO-849/TODO-850 malformed FFI, post-drop, filesystem-ID, and partial-transition tests are included through exact filters. `src/memory_lock.rs` adds deferred-state and panic-unwind cleanup proof. TODO-853 qftls mismatch, duplicate/conflict publication, and zeroizing exporter coverage is included through its isolated target.
- `scripts/tests/audits/audit-runtime-guardrails.sh`, `scripts/tests/utils/util-run-full-suite.sh`, and `.github/workflows/ci.yml` inspect and execute the proof wiring. Main CI run `31469067924`, Linux fastpath job `93708224053`, passes the exact standard and Tokio root-regain proof through passwordless `sudo`; artifact `9093181072` reports native status `PASS` with zero failures. Windows native fault injection remains an explicit external boundary and is not inferred from static results.
...
- Main CI run `31458252191`, Windows job `93676432308`, executes revision `bd159d2762ad2c242da165718d33ba1e7d064015`. The root suite passes `1,601/1,601`; the deterministic dispatch test and direct native unaligned BMI2 parser test each pass exactly once, with zero matching `SIMD_SKIP` records. This closes TODO-843's native target-feature and profile/runtime intersection proof. TODO-848 retains only its broader platform fault-injection and negative-proof manifest scope.
...
- Hosted closure uses Main CI run `31459436098` at revision `d0dca375d934bfad7d8adfe02ed4cd1f939ad0f2`: macOS job `93679888152` and Linux fastpath job `93679888252` each pass the exact Unix TUN syscall-contract step. Linux native job `93679888172` additionally proves process-real provisioning rollback with zero owned residue and publishes artifact `tun-provisioning-negative-native` (`9089352187`). The local macOS exact targets pass `5/5`. Live privileged macOS utun packet I/O is not inferred.
...
- TODO-845 is closed by the later hosted macOS/Linux target tests and process-real Linux rollback proof. Live privileged macOS utun packet I/O remains unclaimed. Windows Win32/WFP fault injection and verified-DLL residue belong to TODO-846 through TODO-848 and are recorded below without inference.
...
- Hosted completion at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`: Main CI run `31455010980`, Windows job `93666880422`, passes the complete `tun-windows,rust-tests` library (`1,601` passed) and `2/2` privileged verified-DLL Wintun lifecycle targets with bidirectional packet I/O, bounded blocked-reader close, repeated open/close, and zero adapter residue. Direct Win32 cleanup fault injection remains explicitly `UNAVAILABLE` in the versioned negative-proof manifest.
...
- Hosted completion at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`: Main CI run `31455010980`, Windows job `93666880422`, passes the exact deterministic WFP failure/retry tests (`2/2`), privileged IPv4/IPv6 packet policy, endpoint and Wintun-LUID exceptions, disconnect/disable behavior, process-exit persistence, stale cleanup, and the exact managed-object absence target. Artifact evidence reports `managed_object_residue=0`; direct BFE/Fwpm native failure injection remains explicitly `UNAVAILABLE`.
...
- Hosted completion at revision `3dd681d510778b6d72bc4bf086b37ae443f13aac`: Main CI run `31455010980`, Windows job `93666880422`, executes the Windows library, deterministic Wintun/WFP boundaries, privileged Wintun lifecycle, privileged WFP policy, and zero-residue assertions. Its versioned manifest records Windows x64 and exact prerequisites while preserving Unix `NOT_APPLICABLE`, ISA `HOST_CONDITIONAL`, and direct Win32/BFE fault injection as `UNAVAILABLE`; no skipped surface is represented as green.
...
- `rust-tests`-only sender injection invalidates a selected middle `msghdr.msg_iov` before real Linux submission. Main CI run `31463107578`, Linux fastpath job `93690492040`, passes the required exact-once gates at revision `20ae52561eea425f8675ae6be4fc1df5525fb422`: connected and unconnected SendMsg preserve `[Sent, Failed, Sent]`, retry only the failed slot, and produce no fourth delivery; opt-in SendMsgZc records two kernel sends, one fallback, three deliveries, zero duplicates, and three notifications. The complete Clippy matrix and macOS build/test job `93690492100` also pass.
...
- Deterministic routing regressions cover generation binding, cross-TUN/live/stale/resource collision decisions, owner-shape tampering, nftables replacement refusal, owner markers, and expected rule shape. `scripts/tests/tun-e2e-killswitch-netns.sh` now fingerprints one live server owner, rejects a second TUN identity before interface or admin-socket publication, preserves the original and unrelated firewall fingerprints, recovers process-loss state, and requires zero selected-backend residue after graceful teardown.
- `.github/workflows/ci.yml` executes the privileged release-binary proof independently for explicit nftables and iptables backends and requires the exact machine-readable ownership marker. Main CI run `31467150282` passes at revision `4a7dae10f9228248fcfcf7962d7dcdd39d80e202`: nftables job `93703336561` records `wall=15097ms`, `runtime_idle=15216ms`, and artifact `9092106520`; iptables job `93703336575` records `wall=14938ms`, `runtime_idle=15064ms`, and artifact `9092142078`. Both reject the second TUN without policy mutation, recover process-loss state, preserve unrelated resources, and leave zero graceful residue. Clippy Matrix run `31467150276` also passes.
...
- Linux routing reaches the public qf-firewall `verify_nft_table_owner` leaf API through a crate-private `quicfuscate::firewall` wrapper. The one-way dependency remains `quicfuscate -> qf-firewall`; routing implementation details do not enter the child.
...
- `crates/qf-stealth/src/chaff.rs` and `crates/qf-stealth/src/probe_detector.rs` are the canonical owners for the former root traffic-analysis chaff and active-probe detector implementations: bounded lifecycle scheduling, jittered and phase-locked cadence, one-slot chaff admission, QUIC PING/PADDING plaintext generation, probe matching, bounded history retention, threshold escalation, and response modes. Root files remain public compatibility projections; qf-stealth also owns the H3-coupled `CoverTrafficScheduler`.
- qf-stealth depends on `qf-common`, `qf-cpu`, `qf-transport-types`, `log`, and `rand`; `quicfuscate -> qf-stealth` remains one-way. H3 header generation crosses only the root-independent transport contract; concrete transport, engine, frontend, and Tauri implementations remain outside the child.
...
- `crates/qf-transport-frames/src/lib.rs` uses `TransportVarInt`, delegating every direct frame-leaf varint operation to the canonical SIMD-dispatched codec in `crates/qf-transport-pn/src/varint.rs`; the historical public `ScalarVarInt` name remains a compatibility alias.
- The one-way workspace edge is `qf-transport-frames -> qf-transport-pn`; the root frame adapter retains only its narrow varint and acceleration bridges. Serialization, zero-copy parsing, packet-space legality, and wire behavior are unchanged.
- qf-transport-frames `21/21`, qf-transport-pn `27/27`, qf-transport-version `7/7`, root frame filter `6/6`, strict frame/root/workspace Clippy, all-feature workspace checking, formatting, diff hygiene, and the complete workspace all-target `rust-tests` command pass with no failures. Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-frames-varint-postpush/workspace-seams.json` at source revision `252a27b859d271d1d606236803eedf2ee31f0f17`: `36` packages, `326` Rust files, `206,514` source lines, `125` module edges, `110` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`.
- The post-test disk guard records `9,120,256 KiB` target usage and `3,104,604 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Frontend/Tauri paths remain untouched.

## H3 Varint Adapter Ownership (2026-08-10, TODO-562)

- `src/transport/h3_parts/connection.rs` routes all H3 and MASQUE varint operations through `qf-transport-pn::varint`; the local SIMD adapter duplicate is removed while the historical H3 error categories and append behavior remain stable.
- `BufferTooShort` maps to `Error::BufferTooShort`, `InvalidPacket` maps to `Error::FrameError`, and any future child error fails closed as `Error::InternalError`. The eight-byte QUIC buffer is sufficient for the RFC 9000 maximum and no wire, stream, capsule, frontend, or Tauri contract changes.
- H3 `76/76`, root strict Clippy, workspace all-feature checking, workspace strict `rust-tests` Clippy, formatting, diff hygiene, and the complete workspace all-target `rust-tests` run pass with no failures; the root library contributes `1,658` passed tests. Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-h3-varint-postpush/workspace-seams.json` at revision `c9c4caaee05899c90e093d38345e032efd75851c`: `36` packages, `326` Rust files, `206,509` source lines, `125` module edges, `110` Cargo dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`.
- The disk guard rejected one Clippy start at `1,751,328 KiB` free; `cargo clean` removed `9.1 GiB`. The final guard records `3,352,804 KiB` target usage and `8,689,472 KiB` free, below the 12-GiB cleanup threshold and above the 2-GiB floor. Frontend/Tauri paths remain untouched.

## H3 Config Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/h3.rs` now owns the root-independent HTTP/3 `Config` contract for QPACK table capacity, blocked streams, field-section defaults, mutation, and accessors. `src/transport/h3.rs` preserves the historical root path; connection validation and concrete QPACK/stream behavior remain root-owned.
- The only new workspace edge is `qf-transport-types -> qf-error`, needed to preserve the existing `Config::new()` result type. Root private-field tests now use the child accessors; defaults, H3 settings wire output, error categories, and frontend/Tauri surfaces are unchanged.
...
- Post-push seam evidence: `scripts/out/audits/workspace-seams-20260810T-http3-masquerade-postpush/workspace-seams.json` at source revision `48fa4b2978dc2dc0034f5e200f849df022c6cc98`; `36` packages, `330` Rust files, `206,816` source lines, `125` module edges, `113` workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`.

## H3 Cover Traffic Scheduler Workspace Ownership (2026-08-10, TODO-562)

- Canonical owner: `crates/qf-stealth/src/cover_traffic.rs` (`CoverTrafficScheduler`, weighted H3 request selection, interval state, qf-common `ProtocolClock`, and qf-transport-types headers); compatibility owner: `src/stealth/parts/cover_traffic.rs`.
...
- Projection flow: engine TOML -> `EngineConfig::from_toml` -> complete validation -> `engine::app_config::AppConfig` -> FEC/stealth/optimization/anti-replay runtime consumers.
- Canonical owner: `src/engine/app_config.rs`; compatibility owners: `src/interface.rs` for `interface::app_config` and `src/lib.rs` for the root `app_config` export. Internal callers no longer route through the interface module.
...
- Post-push seam evidence: `scripts/out/audits/workspace-seams-20260810T-interface-compatibility-isolation-postpush/workspace-seams.json` at source revision `9d4bb39fa5f80c4f1bb1977e04331a4dfcf1a92e`; `36` packages, `334` Rust files, `207,161` source lines, `109` module edges, `116` workspace dependency edges, and `protected_changes=[]`. Interface is outside every SCC; remaining SCCs are `engine/implementations` and `qftls/transport`.

## Implementation Engine-Contract Dependency (2026-08-10, TODO-562)

- Contract flow: canonical leaf owners -> implementation client/server runtime -> root `EngineConfig` aggregate. QKey, lifecycle, tunnel, interface, FEC, stealth, rotation, congestion-control, anti-replay, logging, and AppConfig contracts no longer traverse the concrete engine module.
- The direct `implementations -> engine` edge is reduced from `97` references to the `6` real `EngineConfig` imports. The remaining aggregate ownership is intentionally not disguised by a compatibility alias and keeps the `engine/implementations` SCC open until the config owner moves.
- Verification: root all-feature check; strict workspace library/binary/example Clippy; exact isolated transient admin-auth regression `1/1`; unchanged complete all-feature library rerun `1,693/1,693`; formatting and diff hygiene. Frontend and Tauri are unaffected.
- Post-push seam evidence: `scripts/out/audits/workspace-seams-20260810T-implementation-engine-contracts-postpush/workspace-seams.json` at source revision `5c2e421b313b3e3f2ec91a0f03d42a819a9af85c`; `36` packages, `334` Rust files, `207,167` source lines, `109` module edges, `116` workspace dependency edges, SCCs `engine/implementations` and `qftls/transport`, and `protected_changes=[]`. Final target/free space is `8,049,484 / 11,472,652 KiB`.

## Engine Configuration Workspace Ownership (2026-08-10, TODO-562)

- Ownership: `qf-engine-types::config` owns `EngineConfig`, `StealthSection`, aggregate parsing/validation/normalization, its builder, and the QKey projection. Root `engine::config` owns only compatibility exports and the concrete-runtime `DataAeadConfig` adapter.
- Flow: configuration file or builder -> `qf-engine-types::EngineConfig` -> client/server implementation runtime -> concrete root engine orchestration. Implementations no longer import the root engine module; the historical public engine configuration path remains an exact re-export.
...
- Linux receive flow: optional `Duration` -> checked `time_t` seconds plus bounded nanoseconds cast to signed Linux `c_long` -> explicitly typed `std::io::Result<libc::timespec>` -> `Option::transpose` -> borrowed timeout pointer for `recvmmsg`. The explicit error owner removes feature-dependent inference without changing the syscall boundary.
...
## qf-transport-batch Linux Timeout Conversion Cleanup (2026-08-10, TODO-562)
...
- Inventory ownership: an unfiltered successful process must expose a parseable Vitest summary and retain at least Web Admin 27 files/385 tests or Desktop 36 files/442 tests. Filtered diagnostic runs remain bounded but explicitly skip the full-inventory assertion.
...
## Web Admin Runtime Response Contracts (2026-08-11, TODO-878)

- Request boundary: `apps/svelte-admin/src/lib/api.ts` successful response -> JSON decode -> required `RuntimeSchema<T>` -> validated value or endpoint-named `ApiError`. Generic type parameters no longer assert unvalidated `resp.json()` values.
- Schema ownership: `apps/svelte-admin/src/lib/admin-api-contracts.ts` owns 19 distinct method/endpoint contracts across login/logout, admin auth, config, status, clients, metrics, blocked IPs, logging, logs, and QKeys. Each data schema validates the exact fields consumed downstream; action schemas validate the common success/message envelope.
- State wiring: named schemas -> layout auth probe, LoginModal, Sidebar, Dashboard, Configuration, Logs, AdminSettings, and QKey panels -> request coordinators -> stores. Invalid JSON, missing required fields, wrong field types, malformed list members, and raw QKey list credentials stop before state publication.
- Verification: 76 table-driven boundary cases cover valid, missing-field, wrong-type, and non-JSON responses for every contract; the complete bounded Web Admin suite passes 27 files/385 tests; Svelte checking reports zero errors and zero warnings; the production static build passes. No dependency, styling, layout, Desktop, Tauri, or Rust path changed.

## Bounded Frontend Preview Wiring (2026-08-11, TODO-748)

- Command path: Admin `serve:codex` -> Admin build -> shared preview owner -> package-local Vite on `127.0.0.1:1430`; Tauri `serve:codex` -> Desktop `serve:codex` -> Desktop build -> the same owner -> package-local Vite on `127.0.0.1:4173`.
- Lifecycle path: validated loopback host and port -> bind-availability preflight -> Vite `preview --strictPort` child -> bounded HTTP readiness plus process-stability check -> 30-second serving window -> SIGTERM -> bounded wait -> SIGKILL escalation -> reaped exit.
- Failure path: port/spawn/early server exit=`1`; invalid contract=`2`; readiness timeout=`124`; cleanup failure=`125`; caller SIGINT/SIGTERM=`130`/`143`. Failure and interruption paths pass through the same cleanup owner before the wrapper exits.
- Proof path: CI `frontend-checks` and `test-desktop-webadmin-rust-integration.sh` -> Admin `test:serve:codex` -> `run-bounded-preview.test.ts` -> real Bun HTTP subprocess fixture -> success, never-ready, occupied-port, and SIGTERM-resistant modes -> five passing lifecycle cases. Final Admin and Tauri-delegated Desktop runs completed their builds, readiness probes, bounded lifetimes, and graceful cleanup; listener checks found no residue on 1430 or 4173.
