...
19. Audit logging path: `src/main.rs::run_server()` pre-resolves the privilege target -> `--audit-log <path>` -> `src/audit/mod.rs::init_audit_log()` applies mode `0o600` and the exact resolved UID/GID owner (global `OnceLock<Arc<AuditLog>>`) -> `crate::audit::audit()` calls at lifecycle, privilege, authentication, QKey, admin, connection, configuration, and firewall boundaries -> `src/main.rs` `verify-audit-log <path>` -> `AuditLog::verify_chain()`.
