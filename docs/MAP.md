...
- `rust-tests`-only sender injection invalidates a selected middle `msghdr.msg_iov` before real Linux submission. The required transport gates expect genuine `[Sent, Failed, Sent]` kernel results, retry only the failed slot, and verify exactly-once UDP delivery for connected `SendMsg`, unconnected server `SendMsg`, and opt-in `SendMsgZc` primary CQEs. Local formatting, Bash syntax, locked metadata, and diff hygiene pass; hosted Linux compilation and execution remain the closure gate under the local 2 GiB disk floor.
