...
- Client IO ownership: inbound flushes reuse caller-owned 65,535-byte buffers, Linux outbound dispatch reuses one batch-reference vector per loop, and the client TUN reader transfers pool-backed `interface::TunPacket` blocks through its bounded channel so backpressure preserves ownership without a per-packet `Vec` allocation. Admin HTTP auth/session state uses `parking_lot`; its login limiter is a 10,000-key LRU with all-attempt accounting, and session replay uses a `HashSet` plus bounded FIFO eviction. The FIFO is count-based rather than time-based (TODO-665), while the outer live-session map has no explicit count cap (TODO-809).
...
- Memory locking wiring (TODO-516): `src/main.rs::run_server()` applies `mlockall(MCL_CURRENT | MCL_FUTURE)` when `[security] lock_memory = true` (default), but defers it until after a configured Linux UID/GID transition so glibc never broadcasts setxid across pre-locked runtime stacks. The preloaded TLS private-key allocation and `src/optimize/parts/memory_pool.rs` `MemoryPool` blocks use individual `mlock()` before the transition. The current MemoryPool source has no matching `munlock` release path; TODO-516 is reopened for that ownership gap. `LimitMEMLOCK=infinity` in systemd enables full process locking; finite limits retain explicit failure reporting and the individually locked boundary. The process caller currently logs and continues after `mlockall` failure, so readiness and fail-closed policy remain TODO-852. The embedded engine path does not yet propagate the same lock settings before TLS identity loading; TODO-851 owns that entry-point gap. qftls preloaded-key ownership is tracked separately in TODO-643 and TODO-853.
...
20. Memory locking path: `src/engine/config.rs` `[security] lock_memory/lock_blocks` -> standalone `src/main.rs::run_server()` `RLIMIT_MEMLOCK` gate -> unlimited `mlockall(MCL_CURRENT | MCL_FUTURE)` or finite-limit `MCL_CURRENT` -> `src/optimize/parts/memory_pool.rs` `MemoryPool::set_lock_blocks()` -> best-effort `mlock_block()` in `alloc_numa_block()`; the process caller currently continues after lock failure and the embedded engine does not yet apply the same settings, tracked in TODO-852 and TODO-851. The MemoryPool `munlock` release owner remains TODO-516.
...
- `TunConfig.ip6` / `TunConfig.prefix6` (`src/interface.rs`): IPv6 TUN interface address fields, wired to standalone CLI flags `--tun-ip6` / `--tun-prefix6` and consumed by the native Linux/macOS/Wintun backends. TODO-663 records that generic `ClientRuntime` still hardcodes these fields to `None`, `EngineConfig::InterfaceConfig` has no separate dual-stack pair, server-assigned IPv6 is not propagated to the client, and the public compatibility `ClientBackend` remains single-family.
...
- **Crypto data plane**: constant-time tag comparison (TODO-626), key/IV length validation (closed by TODO-627), AEGIS mutex/`unwrap` path (TODO-628, resolved by TODO-582), AEAD header-protection sample validation (TODO-629), GHASH dispatch configuration (TODO-630), target-specific `AesGcm128` schedule zeroization (TODO-631), nonce/IV uniqueness (TODO-632), QUIC KDF input validation (TODO-633), and the completed TODO-681 unsafe-crypto audit. TODO-681 leaves separate `Aes128Ctx` and temporary-schedule erasure, checked seal-length arithmetic, owner-only packet-number enforcement, AEGIS copied-state erasure, AES table side-channel scope, GHASH release-control proof, and native ISA proof open.
...
- **Transport/Stealth**: terminal close priority (TODO-697; first-close-wins idempotency resolved by TODO-606; local close error-kind semantics resolved by TODO-772), ConnectionId clone hot path (TODO-638), StealthShaper RNG fallback logging (TODO-639), H3 masquerade time source (TODO-640), domain fronting jitter (TODO-641), TLS cover zero-key fallback (TODO-642), qftls `munlock` (TODO-643), probe detector history cap (TODO-644), EscalationState timestamp bound (TODO-808), and brain escalation/histogram/config correctness (TODO-584). Reality session map timer-driven cleanup is resolved by TODO-570.
...
- **Unsafe crypto/transport/interface/privilege**: crypto primitives (TODO-681 audit complete with lifecycle, checked-length, nonce-owner, AES table, GHASH-control, and native-proof findings), transport batching/UDP FFI/AF_XDP/frame-packet/PMTU surfaces (TODO-682 audit complete; remediation split into TODO-837-TODO-842), interface/Wintun/platform/WFP (TODO-683 audit complete; remediation split into TODO-843-TODO-848), and privilege/mlock/qftls/secret handling (TODO-684 audit complete; remediation split across TODO-516, TODO-643, TODO-651, TODO-652, TODO-653, and TODO-849-TODO-854). No production implementation or verification command was performed for these audit passes.
...
- **Client TUN projection:** TODO-663 confirms a split ownership boundary. The standalone CLI maps explicit IPv4 and IPv6 options into the canonical typed `TunConfig`, while generic `ClientRuntime` sets `ip6`/`prefix6` to `None`; the generic EngineConfig schema cannot carry a simultaneous dual-stack address pair. Server `AssignedClientIps` is used for server-side policy/routing only, with no client-facing address propagation found. The compatibility `ClientBackend` trait carries one address family. The former generic string-reparse finding was corrected as unconfirmed; TODO-731 owns CLI parse failure handling. No source implementation or test execution occurred in this audit pass.
- **Linux resolver ownership:** TODO-623 is locally implemented but blocked on native Linux verification. The legacy path now carries an explicit absent-original state, create-only boot/PID/start-time ownership, an advisory inter-process lock, and a resolver-content ownership marker used to refuse foreign or missing current state during stale recovery. The focused resolver suite passes 8/8; the full local library reached 2,190/2,192 but remains red on an external-DNS-dependent DoH cache test and a qftls readiness-jitter assertion outside this task. Native Linux verification is unavailable because this macOS host lacks a Linux C sysroot and the configured Omega SSH path is unavailable. TODO-649 owns fixed backup/state/lock path configuration, resolver symlink/target identity, and broken-link rollback semantics.
...
- **Engine reload:** The generic `QuicFuscateEngine` exposes validated in-memory `update_config` but no file-backed reload owner. After `start()`, generic client/server runtime configuration is duplicated into runtime-owned copies; only FEC has explicit post-start propagation, while the other setters do not update the runtime copy or active connection. The standalone server has a separate validated file reload path with explicit `NextConnectionOnly` scope, startup-owned fields, and a shared-policy-before-best-effort-transport publication boundary. That path reads `EngineConfig` only, so construction-time `ServerConfig` including blacklist/GeoIP/DDoS/auth policy is not replaced. TODO-645 remains open for one supported reload and propagation contract; TODO-660 owns blacklist worker lifecycle; TODO-724 owns transactional publication.
- **io_uring:** `UringBatchSender` has no cross-call pending queue: scratch vectors are reused and submissions are chunked to SQ capacity, but public send methods materialize arbitrary per-call input before the first chunk. Synchronous submit, wait, and CQE draining still run from client/server packet paths; generic client TUN reads also rely on the native Linux nonblocking setup rather than a trait-level readiness contract. TODO-646 owns admission, executor, shutdown, and generic-reader boundaries; TODO-687 covers pointer/lifetime safety and TODO-798 covers exact partial-send disposition.
...
- **Blacklist sync:** The only current production caller is `ServerRuntime::run_loop()` housekeeping -> `LiveServerState::maybe_sync_blacklist()`. `BlacklistSync` enforces HTTPS, redirect, request timeout, body, UTF-8, entry, custom-CA, cache-format, expiry, and atomic last-known-good publication checks. TODO-660 remains open because the detached `JoinHandle` has no `ServerRuntime` owner or shutdown barrier, due-check and timestamp publication are not one atomic in-flight claim, configured body/entry caps have no absolute ceiling, synchronous cache publication runs inside the async worker, and freshness/failure/cancellation/retry state is not observable beyond logs. No production implementation or test execution was performed in this audit phase.
...
- **File modes:** `fsutil`, audit persistence, QKey registry, resolver ownership state, routing ownership state, and PKI private-key writers set restrictive creation modes before opening files. TODO-671 retains the Linux resolver source/lock/backup contract, rotating-log initial/reopen creation, credential-bearing standalone profile persistence, and direct public audit reopen mode; TODO-662 owns profile atomic publication.
...
- **Interface and platform:** TODO-683 completed the full read-only source,
  ...
  retryable. TODO-654 retains the unaligned BMI2 load; TODO-843 through
  TODO-848 own the remediation and negative proof boundaries. No product
