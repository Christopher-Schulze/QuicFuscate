...
- **Linux resolver ownership:** TODO-623 is locally implemented but blocked on native Linux verification. The legacy path now carries an explicit absent-original state, create-only boot/PID/start-time ownership, an advisory inter-process lock, and a resolver-content ownership marker used to refuse foreign or missing current state during stale recovery. The focused resolver suite passes 8/8; the full local library reached 2,190/2,192 but remains red on an external-DNS-dependent DoH cache test and a qftls readiness-jitter assertion outside this task. Native Linux verification is unavailable because this macOS host lacks a Linux C sysroot and the configured Omega SSH path is unavailable. TODO-649 owns backup-path and symlink/target semantics.
