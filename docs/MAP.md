...
- Windows Wintun ownership: `src/interface.rs` selects the built-in backend only with `tun-windows` -> `src/interface/wintun.rs` securely loads the upstream DLL, creates one adapter/session, captures its LUID and session-owned read event, configures addresses and active MTU, and serializes packet operations against one shutdown event and one exactly-once teardown -> ignored native tests prove dual-stack state, bidirectional UDP, blocked-read close, repeated lifecycle, and residue -> `scripts/utils/provision-wintun.ps1` pins archive/DLL hashes plus Authenticode -> CI and Tauri MSI paths provision the untracked DLL beside their executable. Current privileged MSVC/MSI evidence, authenticated QuicFuscate traffic, and WFP remain open under TODO-528.
...
- Automation wiring: scripts in `scripts/` orchestrate build/test/benchmark/audit tasks; `scripts/tests/suites/test-qkey-registry-encryption.sh` owns the process-real encrypted-registry migration, rejection, rotation, secrecy, and cleanup contract; GitHub workflows own cross-platform core checks and signed release packaging; generated local artifact directories are intentionally outside this map.
...
- Windows: Kill-switch activation fails with `NotSupported`; stale exact legacy-rule cleanup remains. The legacy `WindowsPlatform` adapter path also fails before host mutation because it cannot prove ownership and exact restoration of a pre-existing adapter. `src/interface/wintun.rs` is the only valid native data-plane owner and has secure loading, LUID/read-event capture, exact error mapping, blocked-read wakeup, operation drain, and exactly-once teardown. The tracked CI/MSI path now pins and Authenticode-verifies the upstream DLL, exercises the privileged data-plane lifecycle, and rejects exact test residue, but current workflow evidence is still pending. TODO-528 still owns that execution proof and WFP because broad `netsh` block rules cannot be safely overridden by endpoint/TUN allow rules.
