...
- The root package depends on `qf-common` through one normal Cargo edge. `src/env_utils.rs`, `src/time_source.rs`, `src/rng.rs`, and `src/secret.rs` remain explicit compatibility surfaces: production builds re-export the common crate, while root unit tests include the moved implementation under `cfg(test)` so existing test-only controls remain scoped to tests. Secret owners and observers remain root-private.
...
- The former `src/firewall/mod.rs` and `src/firewall/cleanup.rs` sources are staged for a true move into `crates/qf-firewall/`. The child owns backend selection, iptables/nftables command operations, platform-specific cleanup, owned-resource inspection, and the existing firewall regression tests. The root `quicfuscate::firewall` module keeps the public API and internal `crate::firewall` caller paths through a compatibility boundary.
