...
**Production readiness closure (TODO-508..514)**: The runtime and CI are very far along, but the repository is not allowed to claim 100% production-ready until the final closure wave is done. TODO-508 DONE (docs SSOT cleanup, release checkpoint aligned to `f1ec566`, retired worklog references gone, audit-todo-consistency PASS). Open items: post-clean local gate replay, Docker release artifact validation without local Docker dependency, security/ops acceptance audit, long-running Broderick soak/chaos proof, signed release/install/rollback proof, and real-world stealth traffic realism validation.
...
| TODO-448 | I | P1 | Graceful shutdown (SIGTERM, drain mode, connection handoff) | **DONE** - `wait_shutdown_signal()` helper handles both SIGINT (ctrl_c) and SIGTERM on Unix, Ctrl+C on Windows. Server run loop and client run loop both use unified handler. `systemctl stop`, `docker stop`, and orchestrated container termination all trigger graceful shutdown with CONNECTION_CLOSE + TUN cleanup + kill switch disable. | - |
...
| TODO-515 | S | P0 | Wire AuditLogger into server runtime | **DONE** - global `OnceLock<Arc<AuditLog>>` accessor in `src/audit/mod.rs` (same pattern as `ADMIN_LOG_BUFFER`); `--audit-log <path>` CLI flag; events wired at server start, privilege drop (success+failure), auth success/failure in `commit_qkey_auth_result`, QKey issued in `insert_with_ttl`, QKey revoked/admin kick/config reload/server shutdown in `handle_admin_action`; 2 new tests (noop-when-uninitialized + init-and-emit), all 8 audit tests pass; build+clippy+full test suite PASS. | TODO-439, TODO-511 |
| TODO-516 | S | P1 | Implement mlock/mlockall for key material and memory pools | **DONE** - `SecurityConfig` extended with `lock_memory`/`lock_blocks` (default true); `mlockall(MCL_CURRENT\|MCL_FUTURE)` called in `run_server()` before key material is loaded; `MemoryPool::set_lock_blocks()` + `mlock_block()` helper in `alloc_numa_block()`; `LimitMEMLOCK=infinity` already in systemd service (TODO-511); 3 new tests pass; build+clippy+full test suite PASS. | TODO-440, TODO-511 |
...
| TODO-519 | I | P2 | Windows desktop build: cfg-gate Unix-specific core library code | **OPEN** - Core QUIC library uses Unix-specific syscalls (`libc::iovec`, `libc::msghdr`, `std::os::unix`) in `transport/batch.rs` and other modules without cfg gates. Windows desktop job removed from `release.yml` until the core library compiles on Windows. See detail file for the full error list and proposed approach. | - |

Detail files: `docs/todo/todo-{508,509,510,511,512,513,514,515,516,517,518,519}-*.md`.

**Closure rule:** Production-ready means all TODO-508 through TODO-518 are DONE. DONE = locally verified with tests or remotely verified on Broderick (ARM64, Ubuntu 24.04). All 3 previously OPEN (prepared) TODOs (510, 512, 513) were closed on 2026-07-07 by executing the prepared commands on Broderick — see execution evidence in their detail files and `docs/remote-proof-runbook.md`. TODO-412 (real-world profiling) was closed on 2026-07-07 after Oracle Cloud Security List was opened for UDP 4433. First GitHub Release v0.4.0 published with GPG-signed checksums and Tauri updater manifest. Desktop builds (macOS, Linux) are wired in CI as non-blocking jobs with `bundle.createUpdaterArtifacts: true` enabling Tauri updater bundle generation; Windows desktop is blocked on core library Windows porting (TODO-519, OPEN). No OPEN items remain in the 508–518 wave; TODO-519 is a separate post-release wave item.
