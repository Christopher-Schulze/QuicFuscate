...
| TODO-516 | S | P1 | Implement mlock/mlockall for key material and memory pools | **DONE** - `SecurityConfig` extends `lock_memory`/`lock_blocks` (default true); server startup uses `MCL_CURRENT\|MCL_FUTURE` only with unlimited `RLIMIT_MEMLOCK` and safely degrades to `MCL_CURRENT` for finite limits; `MemoryPool::set_lock_blocks()` + best-effort `mlock_block()` protect pool blocks; `LimitMEMLOCK=infinity` remains in the systemd service. | TODO-440, TODO-511 |
...
| TODO-520 | S | P1 | Remove dead QKey transport-parameter channel and false confidentiality claims | **OPEN** - Dead transport-parameter auth is removed. Local real-UDP proof now authenticates through trusted TLS plus encrypted HTTP/3 and fails fast on untrusted certificates; false pre-header auth failure, pre-TLS readiness, finite-memlock allocation failure, and 5 ms stats-log spam found by the first Omega probe are fixed. Full native gates, CI, and repeated native ARM64 Omega proof remain. | TODO-415 |

Detail files: `docs/todo/todo-{508,509,510,511,512,513,514,515,516,517,518,519,520}-*.md`.

**Closure rule:** The v0.4.0 readiness wave required TODO-508 through TODO-518 and TODO-412 to be DONE through local or Broderick evidence. TODO-519 is also DONE through native MSVC, signed Windows MSI, updater-manifest, Linux, and macOS release evidence. Current end-to-end production readiness requires TODO-520 removal of the dead QKey transport-parameter channel and false confidentiality claims. Documentation claims alone are not completion evidence.
