...
| TODO-447 | I | P1 | Container deployment (Docker only; stale cluster manifests removed) | **DEFERRED** - Docker artifacts are retained for GitHub/CI image work, but cluster deployment manifests are no longer active surfaces in this repository. The stale manifest directories were removed to avoid false production-readiness signals. Local Docker is not required. | - |
