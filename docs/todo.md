...
| ID | Phase | Priority | Title | Status | Depends On |
|----|-------|----------|-------|--------|------------|
| TODO-435 | H | P1 | DNS through tunnel (DoH wire-in, DNS proxy, server forwarding) | **DONE** — `src/dns/mod.rs`: DNS query parser (RFC 1035), DoH proxy (`resolve_via_doh`), server-side DNS forwarding (`forward_dns_query`), `DnsProxyConfig` with DoH endpoints + upstream resolvers, `process_dns_query()` entry point, NXDOMAIN response builder. | TODO-429 |
| TODO-436 | H | P1 | Key rotation & immediate revocation (incl. race condition fix) | **DONE** — `src/implementations/server/revocation.rs`: `RevocationManager` (O(1) `is_revoked()`, terminate callback), `QKeyConnectionTracker` (O(1) QKey↔conn mapping, `drain_connections_for_key`), `KeyRotationManager` (auto-rotation with overlap window, pending revocation processing). | TODO-434 |
...
| TODO-447 | I | P1 | Container deployment (Docker, docker-compose, K8s) | **DONE** — `Dockerfile` (multi-stage, rust:1.93-bookworm → debian:bookworm-slim), `docker-compose.yml` (TUN + NET_ADMIN), `k8s/` (deployment, service, configmap, secret), `helm/` (Chart.yaml, values.yaml, templates). | — |
...
| TODO-357 | MODERATE | CONTRIBUTING.md says "Rust stable (latest)" instead of 1.93.0 | **DONE** - changed to "Rust 1.93.0 stable (pinned in rust-toolchain.toml)" |
...
| TODO-342 | LOW | ci.yml profile:minimal on dtolnay/rust-toolchain (no-op) | **DONE** - removed, switched to @1.93.0 |
| TODO-343 | LOW | ci.yml redundant msrv-check job | **DONE** - removed (build-test already covers 1.93.0) |
| TODO-344 | LOW | ci.yml benchmarks job @stable+toolchain override | **DONE** - switched to @1.93.0 |
