...
| TODO-520 | S | P1 | Remove dead QKey transport-parameter channel and false confidentiality claims | **OPEN** - The claimed early QKey channel is dead: rustls constructors receive empty transport parameters and no runtime caller reads the later provider-local mutation. Remove the inert credential copy, parser, server branch, and false `EncryptedExtensions` claims while preserving the real encrypted HTTP/3 authentication path and its fail-closed data gate. | TODO-415 |
