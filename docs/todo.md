...
| TODO-519 | I | P2 | Windows desktop build: cfg-gate Unix-specific core library code | **OPEN** - Active implementation. The backend-only source and dependency correction closes all 24 audited platform-contract errors plus the test-only raw-socket truncation found by the expanded gate. Local Windows GNU check/test compilation/Clippy and native macOS all-target test/Clippy regression gates pass. Native MSVC test execution, Tauri MSI packaging/upload, and signed updater-manifest proof remain open. | - |

Detail files: `docs/todo/todo-{508,509,510,511,512,513,514,515,516,517,518,519}-*.md`.

**Closure rule:** The v0.4.0 readiness wave required TODO-508 through TODO-518 and TODO-412 to be DONE through local or Broderick evidence. Current end-to-end production readiness additionally requires TODO-519 to close with native MSVC check/Clippy, Windows MSI/signature upload, `latest.json` `windows-x86_64` proof, and no Linux/macOS regression. TODO-519 remains OPEN; tracked workflow configuration alone is not completion evidence.
