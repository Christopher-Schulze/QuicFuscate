...
| TODO-448 | I | P1 | Graceful shutdown (SIGTERM, drain mode, connection handoff) | **OPEN** - Persistent SIGTERM/SIGINT listeners are now live-proven under authenticated load, but the original drain contract remains incomplete: shutdown still closes clients immediately and lacks the `Running -> Draining -> Stopped` lifecycle, grace period, SIGHUP reload, admin drain/status API, and live systemd notify wiring. | - |
