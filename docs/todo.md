...
- **Diagnosis (2026-08-23):** Root cause is cumulative PTO cascade in nested 3-hop QUIC-over-QUIC tunnels under 1% loss + 2% reorder. Each hop has its own QUIC stack with independent PTO; under loss, PTOs trigger on all hops, backoff accumulates (up to 2^16x per RFC 9002), and the tunnel progressively stalls until TCP connect times out. The recovery code is RFC 9002 conformant - this is an architectural property of nested QUIC tunnels, not a code bug. Evidence: underlay packet capture shows exponentially increasing inter-packet gaps (50ms -> 1s -> 2s -> 3s -> 5s), matching PTO backoff pattern. The circuit establishes and carries ICMP/DNS reliably, but TCP-over-tunnel fails under sustained loss. Candidate fixes: (a) reduce PTO backoff cap for inner tunnel hops, (b) FEC repair for ACK-eliciting packets on inner hops, (c) ACK prioritization in relay forwarding, (d) reduce impairment profile (0.5% loss is still meaningful). Recommended: (d) as immediate test-infra adjustment plus (b) as a separate transport improvement TASK.
