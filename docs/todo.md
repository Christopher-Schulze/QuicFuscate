...
**Current execution:** Finish the exact-commit native closure for TODO-554, then execute TODO-555 and TODO-556 before the dependency-ordered product register. Dioxus parity starts only after the UI-facing backend contracts stabilize. During Dioxus work, the existing Svelte web-admin and Svelte/Tauri desktop UI are read-only references and must remain byte- and pixel-identical.
...
|---:|---|---|---|---|---|
| 1 | TODO-554 | Evidence | Make the base TUN E2E harness own its process cleanup | **OPEN** - implementation, local gates, release, artifact, and two clean Omega runs pass; a replacement exact-commit CI run is the final closure gate after one hosted fuzz runner stalled. | - |
| 2 | TODO-555 | Evidence | Replace broad process reapers in specialized TUN E2E harnesses | **OPEN** - four FEC/loss harnesses retain unsafe global product-name cleanup. | TODO-554 |
