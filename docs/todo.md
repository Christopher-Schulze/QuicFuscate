...
**Production readiness closure (TODO-508..520 + TODO-448)**: REOPENED by TODO-521. TODO-448 remains fully proven on `bef00fe6501baa1d5fc99ad25f5e8e89c9b6d4a3`, but the exhaustive backlog audit found 31 `DONE` detail files with 441 unclassified unchecked acceptance items. TODO-515 and TODO-516 are now reopened for missing runtime integration events and missing operating-system memory-lock evidence; TODO-457 and TODO-449 also contain contracts that are not implemented as written or were superseded without task-truth reconciliation. Global production readiness is blocked until TODO-521 classifies and resolves the complete set.
...
| TODO-516 | S | P1 | Implement mlock/mlockall for key material and memory pools | **OPEN** - Runtime/config/pool wiring exists, but no test invokes the production `mlockall` boundary and proves `VmLck > 0` under sufficient privileges or explicitly covers its graceful-skip path. Reopened by TODO-521. | TODO-440, TODO-511 |
