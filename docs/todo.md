...
**Current State addendum (TODO-505)**: FEC repair telemetry fastpath is DONE. The R-wave now also keeps systematic-only FEC send paths free of repair-history HashSet/VecDeque maintenance while preserving repair diagnostics.

**Latest checkpoint (TODO-505)**: FEC repair telemetry fastpath is DONE. `AdaptiveFec::on_send_into()` now updates repair-symbol uniqueness/order diagnostics only for emitted repairs, so systematic-only send paths avoid HashSet/VecDeque repair-history maintenance. Broderick `fec_send_reuse_hot_path/light/1400B` improved from about `1.7126 us` to `1.6522 us`, `normal/1400B` improved from about `1.1290 us` to `1.0435 us`, and `streaming/1400B` improved from about `374.48 ns` to `314.54 ns`.
...
| TODO-503 | R | P1 | Svelte unit harness stability | **DONE** - admin/desktop Vitest configs now avoid file-level worker contention, setup hooks clean up real timers deterministically, and app checks/builds/unit tests are green without touching UI source | TODO-192, TODO-197, TODO-472 |
...
Detail files: `docs/todo/todo-{464,465,466,467,468,469,470,471,472,473,474,475,476,477,478,479,480,481,482,483,484,485,486,487,488,489,490,491,492,493,494,495,496,497,498,499,500,501,502,503,504,505}-*.md`.
