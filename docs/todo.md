...
| TODO-510 | S | P1 | Docker release artifact validation without local Docker dependency | **OPEN (prepared)** - `.github/workflows/docker-validation.yml` created with 6 validation steps (build, image size, --help, networking tools, secret scan, config template); triggers on Docker path PRs + workflow_dispatch; awaiting first GitHub Actions run; privileged TUN/UDP bind documented as real limitation. | TODO-447, TODO-509 |
...
| TODO-512 | S | P0 | Broderick long-running production soak and chaos proof | **OPEN (prepared)** - `scripts/tests/suites/test-runtime-soak-chaos.sh` exists with configurable iterations, admin iterations, fast/dry-run modes, output directory; DNS leak assertion already proven by `tun-e2e-dns-leak-netns.sh`; awaiting ~3h remote execution on Broderick with netns + root privileges. | TODO-473, TODO-474, TODO-509, TODO-511 |
| TODO-513 | S | P1 | Signed release, install, upgrade, and rollback proof | **OPEN (prepared)** - `release.yml` extended with SHA-256 checksum generation + optional GPG detached signature (when `RELEASE_GPG_KEY_ID` secret configured); checksums uploaded as separate artifact; `install-server-linux.sh` + hardened systemd service ready; awaiting clean VM execution for install/upgrade/rollback proof. | TODO-448, TODO-460, TODO-461, TODO-509 |
