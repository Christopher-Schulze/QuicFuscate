...
- DONE. Default-build `dgram_send`/`enqueue_received_datagram` paid `to_vec`/`into_owned` + free per queued DATAGRAM (hot on the VPN dataplane). Per-connection `dgram_*_freelist` (cap 64) now recycles drained buffers: enqueue extends a retained-capacity `Vec`, commit/recv return it — malloc+free per datagram eliminated, payload memcpy kept (queue must own bytes). `zero_copy_dgram` deliberately stays non-default (64 KiB block × 1024-deep queue ≈ 64 MiB/conn worst case); the zc lane is untouched. Adjacent: `PooledBlock::pool_ref()` removes an `Arc` clone per `from_pooled_blocks` validation. 142/142 default + 145/145 zc transport tests, both modes check-clean on Omega.
