...
| TODO-448 | I | P1 | Graceful shutdown (SIGTERM, drain mode, connection handoff) | **OPEN** - Shared drain lifecycle, existing `engine.shutdown_timeout_ms` grace control, persistent SIGINT/SIGTERM/SIGHUP handling, admin drain/status, close-frame flush, and systemd notify wiring are locally green. The live two-client proof completes in 5118 ms; full check, clippy, and test gates pass. CI and Omega proof remain. | - |
...
**Closure rule:** The v0.4.0 readiness wave required TODO-508 through TODO-518 and TODO-412 to be DONE through local or Broderick evidence. TODO-519 and TODO-520 are also DONE through native cross-platform CI, signed release artifacts, and live ARM64 evidence. Current end-to-end production readiness requires TODO-448 completion of its reopened graceful-drain contract. Documentation claims alone are not completion evidence.
