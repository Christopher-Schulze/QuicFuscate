...
**Current State (2026-07-23):** The protocol optimization, FEC performance, stealth coherence, TUN data-plane, CI/release, and completed production-evidence waves retain their individual proof. The exhaustive legacy-acceptance reconciliation is complete; every genuine remaining gap is represented directly by an open TASK in the ordered production register below.
...
**Current execution:** Close the already-implemented native evidence tasks first, then follow the dependency-ordered register. Dioxus parity starts only after the UI-facing backend contracts stabilize. During Dioxus work, the existing Svelte web-admin and Svelte/Tauri desktop UI are read-only references and must remain byte- and pixel-identical.
...
|---:|---|---|---|---|---|
| 1 | TODO-362 | Evidence | Audit retained FEC dead-code suppressions | **OPEN** - source cleanup and local gates are complete; native CI and exact artifact evidence remain. | - |
| 2 | TODO-545 | Evidence | Prove cipher reinstallation state safety | **OPEN** - adversarial implementation and local gates are complete; native security and release evidence remain. | - |
...
TODO-362 is intentionally excluded from this historical deferred list because its remaining native and exact-artifact evidence is active in the production-readiness register.
