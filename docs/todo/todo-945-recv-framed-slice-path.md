...
## Befund
`recv_on_path` (Slice-Eingang — der Standard-Einlieferpfad für Server- und
...
— Checkout + Memcpy + Free waren reine Verschwendung auf dem häufigsten Pfad
(FEC aktiviert → alles ist geframed).

## Fix
- `recv_on_path` prüft `wire::is_framed(data)` vorab: geframete Datagramme
...
- Malformed-Semantik identisch: Parse-Fehler → debug-log + `Ok(len)`
  (consumed), Scratch wird zurückgelegt.
...
- Pro framed Ingress-Datagramm: 1 Pool-Checkout, 1 Payload-Memcpy, 1 Pool-Free
  ...
  profitieren beide.
- Der ungeframete Pfad (rohe QUIC-Datagramme) behält den Block — Ownership wird
  dort tatsächlich gebraucht (`FecPacket::from_pooled_blocks`).

## Verifikation
- Lokal: `cargo check` + `clippy --lib` sauber; `cargo test --lib connection`
  326/326, `--lib fec` grün.
- Omega (aarch64, Kernel 6.17, io_uring-Feature): `cargo check` sauber;
  `cargo test --lib connection` 326/326 grün.
