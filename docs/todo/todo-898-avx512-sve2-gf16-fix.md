...
- **Miri found a latent MORUS scalar-path bug (2026-08-23, commit `39e0578`):** the scalar fallback in `Morus1280State::update()` Round 3 (`morus.rs:534-537`) omitted the message-block XOR (`^m`) that all SIMD backends (NEON, SSE2/4.1/4.2/SSSE3) include. This produced wrong ciphertext and authentication tags on any platform hitting the scalar fallback (Miri, or future CPUs without NEON/SSE). Native tests never caught it because they always dispatch to SIMD. The full qf-crypto Miri suite now passes **151/151** after the fix. The earlier 33/33 subset did not include the `morus_official_caesar_morus1280_128_vectors` test that exposed the mismatch.
- **Brain histogram Miri guard (2026-08-23, commit `39e0578`):** the NEON/SVE2 dispatch paths in `decay_histogram()` and `jensen_shannon_divergence()` lacked `cfg!(miri)` guards, causing Miri to attempt `vcvtq_u64_f64` (llvm.fptoui.sat.v2i64.v2f64) which it cannot emulate. Added `!cfg!(miri)` gates to force scalar fallback under Miri, matching the MORUS pattern.
