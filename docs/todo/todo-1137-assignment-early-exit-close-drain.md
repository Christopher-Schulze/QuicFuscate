...
- [ ] Trace return sites in `negotiate_assignment` and signatures/side effects
      of `flush_outbound`, `AssignmentReception`,
      `finalize_authenticated_assignment`, `begin_masque_control_tunnel`,
      `ClientDataPlane::drive`, and Core H3 polling. Build an exact exit/cause
      matrix in this file before editing code.
