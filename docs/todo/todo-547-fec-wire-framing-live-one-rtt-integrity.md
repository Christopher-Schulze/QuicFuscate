...
- A later native Windows gate on commit `a0e9bba` exposed a CPU-specific production defect in the AVX-512 VBMI2 GF16 kernel: `_mm512_permutex2var_epi16(a, idx, b)` was called with the index as `a`, corrupting odd-length GF16 recovery on a VBMI2-capable runner. The four lookups now pass their tables as `a` and `b`, and a direct hardware-gated scalar-parity regression covers the private kernel. The original Windows job `88932400005` and failed-job rerun `88935087624` reproduce both affected wire tests before the correction; final native proof is pending the corrective commit.
