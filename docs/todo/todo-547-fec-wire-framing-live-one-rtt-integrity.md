...
- A later native Windows gate on commit `a0e9bba` exposed a CPU-specific production defect in the AVX-512 VBMI2 GF16 kernel: `_mm512_permutex2var_epi16(a, idx, b)` was called with the index as `a`, corrupting odd-length GF16 recovery on a VBMI2-capable runner. The original Windows job `88932400005` and failed-job rerun `88935087624` reproduce both affected wire tests. Corrective commit `a10cdb5` passes its tables as `a` and `b`, adds a direct hardware-gated scalar-parity regression for the private kernel, and passes native Windows core check/test/Clippy job `88937360219` plus complete CI `29924295737`.
