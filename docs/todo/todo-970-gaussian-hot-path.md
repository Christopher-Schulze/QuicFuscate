...
2. **RHS build O(min_len × k)**: for every payload byte it rescanned all
   `k` coefficients and did a scalar `gf_mul_table` lookup per hit —
   ~300k table lookups per equation at min_len≈1200.
3. **`yb[row].clone()` per (pivot × eliminated row)**: each pivot pass
   cloned the min_len-byte RHS row for *every* eliminated row —
   ~u × m clones ≈ 20 MB of churn at u=m=128, min_len≈1200. The follow-up
...
- RHS rows are bit-identical: `gf_mul_scalar_slice` XOR-accumulates
  `coeff × src` over the row, which is exactly `rhs ^= cj * kd[b]` per
