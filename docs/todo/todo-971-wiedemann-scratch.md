---
id: TODO-971
title: Wiedemann per-worker matrix/rhs scratch + lookup build
status: DONE
created: 2026-09-18
---
...
1. **Per-byte matrix + rhs allocation**: inside the Rayon byte loop, every
   ...
   that is ~1200 x (m x n) matrix allocations plus 1200 RHS vectors, all
   charged to `WIEDEMANN_MATRIX_RHS_ALLOCS` telemetry.
2. **O(m x n x k) coefficient lookup build**: `eq_coeff_lookup` nested-
...
- `matrix` and `rhs` moved into the `map_init` worker state
  ...
  (~8 allocations) instead of once per byte (~1200 x workers).
...
- `eq_coeff_lookup` now walks each coefficient row once and places
  entries via `unknowns.binary_search(&sid)` - O(m x k). Subtlety: the
...
- `cargo bench --bench fec_pipeline --features benches -- wiedemann` -
  baseline: high_loss/128 ~ 7.0 ms, high_loss/256 ~ 181 us
