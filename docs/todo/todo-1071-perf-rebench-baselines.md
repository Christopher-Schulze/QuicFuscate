...
- Omega primary host: Linux 6.17, one Neoverse-N1 vCPU, rustc/cargo 1.97.1, batch SSH and non-interactive sudo available. Its clean checkout was fast-forwarded from `560b7e2` to `f6a867fb`; the portable preflight and metadata-contract fix is now pushed as `b0aae929` and awaits a fast-forward before measurement. At 2026-09-23 04:24:11Z, 61 GiB were free and its old `target/` measured 566 MiB.
...
- Omega's TUN E2E runner is available, but its fixed `/tmp/ns-*.log` paths already exist. The runner's optional traffic capture starts after its synchronous iperf ready hook, so it cannot currently measure loaded wire overhead. Add opt-in isolated log paths and hook-level pcap capture, preserving the default runner behavior, before collecting mode overhead.
...
1. Fast-forward Omega's clean checkout to `b0aae929`, validate the toolchain, and start from a measured clean Cargo cache.
2. Run the same-API R-RING/R-LC/S-AEGIS bakeoff on Omega and the M1 reference with the same owners, sizes, release flags, and three repetitions. Prioritize P1 1200/1400-byte cells; retain full supporting sizes and p95/p99.
   ...
   Omega repaired rerun uses a new `run02` output directory after the runner fix is pushed.

   Mac reference command (run01, pre-run free space 13 GiB at 04:47:48Z): `QF_BAKEOFF_AWS_LC=1 bash scripts/benchmarks/suites/bench-aead-bakeoff.sh --full --owners R-RING,R-LC,S-AEGIS --output-dir scripts/out/benchmarks/todo-1071-aead-macos-arm-b0aae929-run01`. Source code is pinned at `b0aae929`; only this TODO detail file was dirty.
