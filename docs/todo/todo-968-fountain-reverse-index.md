...
`indices.contains(&decoded_idx)` — O(retained_symbols × avg_degree) per
propagation step. For a window holding S encoded symbols and D decodes the
peeling loop paid O(D × S × deg): trivial at k ≤ 256 (~µs) but quadratic at
...
actually affected, O(adj_len) instead of O(S × deg). After the update
