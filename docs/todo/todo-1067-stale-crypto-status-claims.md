...
5. `docs/todo/todo-1049-delete-dead-first-party-aead.md` section "Current code". It still lists `aes.rs`, `gcm.rs`, `chacha.rs`, `poly1305.rs`, and the first-party QKey sealer as present. The Result section says they are gone. Rewrite "Current code" into past tense, or move it under a "Plan at open" heading and leave Result as the status. A reader must not see both as true.
