...
unbedingter Aufwand fuer einen Low-Rate-Cover-Traffic-Pfad.

## Fix
- `prepare_http3_poll_iteration` gibt nur noch `intelligent_level` zurueck.
- `emit_server_push_cover_burst` holt `conn.stats()` selbst - nach dem
  Early-Return, nur wenn ein Cover-Burst tatsaechlich faellig ist; `sent`/`lost`
...
- Ein ~200-B-Struct-Clone pro Poll-Iteration eliminiert (N Iterationen pro
  Datagramm-Poll).
- Semantik identisch: Stats werden erst bei tatsaechlichem Cover-Burst gelesen.
