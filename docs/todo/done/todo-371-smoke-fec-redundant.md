...
`scripts/benchmarks/smoke/smoke-fec-quick.sh` is absent. No non-document
script references `smoke-fec-quick.sh`. The requested redundant wrapper has
already been removed, so the correct disposition is DONE rather than SCRAP.
