...
- `FecPacket::from_pooled_blocks()` requires both payload and coefficient guards to originate from the packet's `MemoryPool`, validates declared lengths before transfer, and frees both guards together when construction is rejected. The compatibility `FecPacket::new()` remains available for raw and foreign test inputs; mismatched or foreign buffers are directly released by the pool boundary without entering accounting, and failed upsizing normalizes the corresponding logical length to zero.
...
- `FecPacket::new()` no longer allocates an unbounded replacement buffer for overstated compatibility lengths. `try_new()` and `try_from_block()` provide checked rejection, systematic packets cannot retain coefficient metadata, stream flags are exact, and clone/stream slices remain bounded by pool-backed storage.
