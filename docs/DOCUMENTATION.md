...
- TUN reader lifecycle: `TunInterface::reader_loop_with_shutdown()` checks an atomic shutdown flag and waits for Unix descriptors with bounded `poll(2)` instead of a 1 ms sleep. The client uses `reader_loop_with_shutdown_owned()` to transfer pool-backed `TunPacket` blocks through its bounded channel without a per-packet `Vec` copy; packet ownership returns the block to the originating pool after carrier acceptance or terminal drop. Client and standalone server runtimes own the reader cancellation flag plus `JoinHandle`; they drop the bounded receiver before joining, use the native Wintun shutdown event where required, release the descriptor after the join, and use `Notify` to wake the event loop when a frame is queued.
...
- **macOS pf activation:** `MacOSKillSwitch::ensure_pf_enabled()` already fails fast when the main ruleset lacks the required anchor reference. The remaining client gap is cleanup of a just-loaded anchor after that check fails and consistency between `enabled` and backend state. The internal server `RoutingManager::setup_pf()` does not prove active-ruleset reference before reporting success; the shipped server TUN path rejects non-Linux, and TODO-607 owns broader internal macOS teardown. Tracked in TODO-624.
...
- **Cargo target inventory:** The root package declares 71 integration-test targets and every declared source path exists. Static script/CI inspection finds 70 declared target invocations; the sole undeclared invocation is `it-masque-runtime-integration` in `scripts/tests/suites/test-desktop-webadmin-rust-integration.sh`, while the matching source survives only under `archive/tests/`. TODO-774 owns the stale runner contract.
...
- **Audit register:** The current local corpus contains 679 tracker headings, 340 current detail files, and 374 archived detail files. Git scope currently enumerates 898 tracked, 26,350 ignored, and zero non-ignored untracked paths. `bash scripts/tests/audits/verify-audit-completeness.sh` exits nonzero at `archive/stealth/doh.rs` because the tracked archive tree has no classifier or coverage-manifest row; TODO-773 owns that blocker. Therefore the whole-project audit remains open and no passing completeness claim is valid yet.
