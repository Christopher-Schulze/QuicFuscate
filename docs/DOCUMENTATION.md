...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline, so the generic I/O driver does not oversleep an outer-pacer release. The standalone client and server retain their established 5 ms housekeeping cadence: two exact ARM64 candidates that added direct release-time Tokio wake-ups both failed closed with client heartbeat timeouts. The direct runtime integration was removed; PMTU performance qualification remains open.
...
- Persistent-congestion accounting now follows the RFC boundary across ACK frames: an acknowledged ack-eliciting packet at or after a pending loss-run start resets that run, so a later loss cannot bridge across it; ACK-only losses cannot establish persistent congestion. Four focused recovery regressions cover collapse, in-window ACKs, cross-frame ACKs, and ACK-only loss.
...
- Persistent-congestion logs now retain the triggering packet-number space, ACK-frame largest packet number, terminal lost packet number, run duration, and RFC recovery period. The evidence is emitted only when cwnd collapses and is covered by the focused recovery regression set.
...
- Native evidence then confirmed the probe path itself must not become congestion evidence: 1328 and 1400 were reconfirmed after the reset, but a later probe loss entered persistent congestion. `Recovery` now tracks isolated DPLPMTUD probes as ack-eliciting loss-detection state without charging bytes in flight, congestion-control loss, or persistent-congestion runs. The probe-loss regression proves the same ACK sequence leaves the congestion state equal to a no-probe control. Exact ARM64 proof remains required.
