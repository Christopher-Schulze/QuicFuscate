...
- `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh` now has an opt-in external diagnostic, `QF_E2E_EXTERNAL_EGRESS_CAPTURE=1`. It captures only client 1's underlay UDP packets to the server on the host-side `qf523h1` veth, only during each three-trial throughput phase, and writes bounded packet-count and inter-packet-gap summaries. The product runtime, normal gate, packet payloads, and credentials remain untouched. Native evidence is pending.
