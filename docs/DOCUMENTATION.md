...
- Accepted connection tasks are owned by a `JoinSet`; shutdown aborts and joins them before `AdminHttpServer::run()` returns. The outer standalone service-task handle remains TODO-700, synchronous handler cancellation remains TODO-661, and request-body memory admission remains TODO-712.
...
- **Admin HTTP admission and capacity**: `AdminHttpServer` now owns a validated CLI-only capacity with default `16` and maximum `1024`. It acquires admission immediately after `accept()` and before `tokio::spawn`; excess sockets are dropped without a user-space pending queue, and `admission_snapshot()` exposes active, pending, admitted, rejected, and joined-completed counts. Accepted connection tasks are owned by a `JoinSet` and aborted/joined during server shutdown. TODO-700 still owns the outer standalone service-task handle, TODO-661 the synchronous-handler deadline/cancellation boundary, and TODO-712 request-body memory admission.
