...
- The current `verify-audit-completeness.sh` run enumerates `991` tracked paths, `26,731` ignored paths, `0` non-ignored untracked paths, and `27,722` accounted paths. It fails closed only because the latest Graphify evidence manifest is stale relative to the current Git revision; TODO-759 owns that provenance boundary.
...
- **Detection scope:** The current local run at source revision `5c635a9c7ecd37d3e904457e22789569f366cea7` enumerates 729 files and 1,279,132 words: 676 code, 29 documents, 24 images, and 3 sensitive files accounted for as redacted. The detected code surface contains 333 Rust files, 142 shell/PowerShell scripts, 115 TypeScript files, and 50 Svelte files. Generated, dependency, agent-state, audit-output, and local-sensitive Git paths remain counted by the machine-readable Git scope without being treated as source inputs.
...
- **Current result:** The latest local run produced 14,885 raw nodes and 40,442 raw edges, 13,252 normalized nodes and 40,442 normalized edges, zero normalized dangling edges, stable IDs, and relative source paths. Raw evidence still contains 1,484 dangling edges and 2,125 duplicate IDs; normalized evidence retains 350 ambiguous and 1,471 unresolved edge statuses. Six detected files have no AST nodes, so the extraction contract is `BLOCKED` rather than a relationship pass.
