...
- Native transport and interface ownership remains source-audited but not fully cross-platform proven. TODO-837 now implements the bounded UDP batch result, address-length, partial-send, timeout, aligned-buffer, and caller-fd contracts; TODO-845 implements the Unix TUN syscall/name/rollback/close source contracts and static proof wiring; TODO-846 implements the Wintun cleanup ledger, constructor rollback, and lifecycle contract; local ARM64 macOS gates pass, while native Linux/Windows/Omega execution remains unclaimed. PMTU/prefetch, WFP transactions, and privilege/memory-lock negative proof remain owned by TODO-841, TODO-842, TODO-847 through TODO-848, and TODO-850 through TODO-854; TODO-844's generic TUN result contract is implemented and statically wired.
...
- Static verification passes `cargo fmt --all -- --check`, `cargo metadata --locked --no-deps`, Bash syntax, and `git diff --check`. The privilege unit target passes `19/19` and `it-privilege-boundary` passes `3/3` on ARM64 macOS with `rust-tests`; the aggregate runtime guardrail remains `FAIL` only for the known pre-existing findings outside TODO-849. Windows/native privilege proof and the Linux-specific root/filesystem-field lane remain unclaimed. TODO-850 owns the post-drop proof implementation; TODO-854 retains native/negative proof.
...
- Focused qftls verification passes `24/24`, including the isolated mismatch/duplicate/conflict preload contract and zeroization-before-drop regression. Native and negative privilege proof remains TODO-854; pooled-block unlock/accounting remains TODO-516/TODO-678.
