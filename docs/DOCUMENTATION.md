...
- **Native evidence boundary:** The corrected source has not yet been run through the privileged Linux native harness. TODO-806 remains open until the wire capture validates server source/destination, ICMP type/code, quote, checksums, and telemetry on the native path; the earlier PTB evidence remains independently closed under TODO-613.
