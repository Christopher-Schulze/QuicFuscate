...
Environment parsing has a deterministic helper contract but is not a universal live-reload contract. `src/env_utils.rs` owns the shared `EnvSnapshot` and helper paths used by the active Brain, FEC, Reality, stealth, TLS Cover, core, and transport construction boundaries. Invalid helper values warn and retain the configured default or skip the invalid override; ordered aliases skip empty or invalid values and continue to the next alias. Several active compression, memory-pool, zstd, trusted-proxy, and other subsystem paths still parse environment values directly with their own fallback rules; TODO-811 owns that separate parser-authority inventory. Dedicated server auth, DDoS, rate-limit, GeoIP, blacklist, and QKey policy loaders return typed errors or warnings and remain their own validated boundaries.
...
- Direct production parsers outside `src/env_utils.rs` are not silently counted as migrated. TODO-811 owns their authority inventory, validation, warning, and lazy-initialization reconciliation.
