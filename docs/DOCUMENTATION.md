...
- `.github/workflows/release.yml` builds native x86_64 and ARM64 Linux server bundles plus Tauri ed25519-signed desktop artifacts for macOS (DMG plus `.app.tar.gz` updater), Linux (deb plus directly signed `.AppImage`), and Windows (directly signed MSI). Both server architectures and all three desktop updater jobs are required dependencies of tagged release publication.
- Updater integration is configured in `tauri.conf.json` with `bundle.createUpdaterArtifacts: true`, a GitHub Releases endpoint, and an embedded ed25519 pubkey. `scripts/audits/verify-release-updater.sh` owns the required/optional platform policy, discovers exactly one non-empty bundle/signature pair per required platform, verifies native runtime activation, and generates `latest.json` only after the complete map is present.
...
- **Release pipeline**: `release.yml` first rejects any mismatch between the release tag, root product version, and Tauri bundle version through `scripts/audits/verify-release-version.sh`, then builds required native x86_64 and ARM64 Linux server bundles and required signed macOS/Linux/Windows desktop updater artifacts. `publish-release` cannot publish a `v*` tag unless every server and desktop dependency succeeds; it then runs `scripts/audits/verify-release-updater.sh` before creating the GitHub Release. GPG signing key: `07484E2F6ED688BC` (ed25519, expires 2028-07-06). Tauri updater signing uses the `TAURI_SIGNING_PRIVATE_KEY` secret.
- **Tauri updater**: `bundle.createUpdaterArtifacts: true` produces platform-specific updater artifacts: macOS `.app.tar.gz` plus `.sig`, Linux `.AppImage` plus `.sig`, and Windows `.msi` plus `.sig`. The manifest contract requires `darwin-aarch64`, `linux-x86_64`, and `windows-x86_64`; missing, empty, duplicate, or mismatched pairs fail the release instead of producing an empty or partial `latest.json`. Release `v0.4.4` proves all three signed updater entries.
- **Desktop platforms**: macOS, Linux, and Windows are required release jobs for updater publication. The native host keeps debug updater activation opt-in, while release jobs set `QUICFUSCATE_DESKTOP_UPDATER_ACTIVE=true` at compile time so the packaged runtime includes the updater plugin.
...
| low | Signed updater availability is platform-dependent | tagged publication requires exactly one signed updater pair for each declared desktop platform; a missing platform blocks publication | controlled by release workflow and `verify-release-updater.sh` | desktop release |
...
- Enable memory locking (`[security] lock_memory = true`, `lock_blocks = true`) to prevent key material and crypto buffers from being swapped to disk. The shared `src/memory_lock.rs::MemoryLockPolicy` maps these settings without normalization and is applied by both standalone and embedded server startup before TLS identity construction: it applies process locking when the startup boundary permits it, configures `MemoryPool::set_lock_blocks()`, and only then allows runtime transport/TLS identity construction. Standalone Linux startup with a configured UID/GID transition explicitly defers process-wide locking until verified setxid completion while individually locking the TLS key before the transition. Standalone reload rejects changes to these startup-owned fields and requires restart. `LimitMEMLOCK=infinity` in systemd enables current-and-future process locking; finite limits retain the individually locked boundary and attempt current-page locking with explicit failure reporting.
...
The server runtime emits tamper-evident audit events to a hash-chained NDJSON segment set when `--audit-log <path>` is provided. One global `OnceLock<Arc<AuditLog>>` owner is initialized before privilege dropping. Producers validate JSON-encoded UTF-8 bounds before cloning dynamic fields, then submit fully owned events through non-blocking `try_send`; the single `qf-audit-writer` assigns sequence and timestamp, serializes, hashes, rotates, checkpoints, and performs all file I/O. Queue saturation, lifecycle closing, or disconnection rejects the newest event and increments `quicfuscate_audit_dropped_events_total` as the aggregate plus `quicfuscate_audit_dropped_events_by_cause_total` under `cause="queue_full"`, `"worker_closing"`, or `"worker_disconnected"` (TODO-726), because a writer that is merely behind, one that is shutting down, and one that is gone until restart demand different operator responses and a single counter cannot tell them apart; oversized source IP, client ID, reason, message, or combined dynamic payload is rejected before queue admission and increments `quicfuscate_audit_payload_rejections_total`; write and checkpoint failures increment `quicfuscate_audit_persistence_errors_total` and publish one terminal failure to producers. Producers rejected after that terminal state increment `quicfuscate_audit_terminal_dropped_events_total`; the writer-side durability watchdog increments `quicfuscate_audit_slow_flushes_total` when a flush exceeds the configured bound, while `quicfuscate_audit_shutdown_failures_total` preserves failed shutdown outcomes. If the system clock is before the Unix epoch, the writer rejects the event and enters its terminal persistence-error state instead of silently recording timestamp zero. TODO-813 centralizes queue, segment, retention, and lifecycle timeout bounds before any audit resource is acquired. Startup reads of existing segments are bounded (TODO-727): `read_first_entry`, `read_first_sequence`, `verify_segment`, and `read_tail_state` stream line by line instead of loading whole files, a segment above the `MAX_AUDIT_SEGMENT_BYTES` retention ceiling is rejected from its metadata before any content is allocated, and a single entry above `MAX_AUDIT_ENTRY_BYTES` (64 KiB) is refused rather than read. Tail recovery streams forward and retains one entry, so startup memory no longer scales with the retained file. Hash-chain verification is unchanged and still reads every entry. Every audit file operation is bound to an opened object rather than a name (TODO-728): the append file is opened with `O_NOFOLLOW`, so a symlink at the audit path is refused atomically with the open instead of by a separate check that a replacement could invalidate, and the regular-file test is made through the returned handle. Hardening opens once with `O_NOFOLLOW` and performs both the mode change and `fchown` through that descriptor, so a replacement cannot tighten permissions on one inode and hand ownership to another; a newly created parent directory is chowned through an `O_NOFOLLOW|O_DIRECTORY` handle. The checkpoint staging file is created with `O_NOFOLLOW` and published by rename, which does not follow a link at its final component.

**File security:** The active log, immutable segments, and checkpoint are regular files created with mode `0o600` (owner read/write only) on the global startup path. Direct public reopen also reasserts `0o600` on the opened active-file handle before the writer can append. Symlinks and special files are rejected before reading. When running as root, the audit set is chowned to the resolved runtime user/group so logging survives privilege reduction. The parent directory is chowned only if the server created it; pre-existing system directories are never re-owned. Hardening is fail-closed: `secure_audit_file()` returns a typed `AuditError` when the mode cannot be tightened or the ownership transfer fails, `init_audit_log_with_options()` propagates it before publishing the global audit owner, and `init_audit_log()` returns the same status instead of downgrading it to a warning. A process therefore cannot report the documented secure state after a required hardening step failed. On Windows, checkpoint replacement rejects a path containing an interior NUL before calling `MoveFileExW`, so the kernel cannot act on a truncated prefix of the named path. Pathname binding remains TODO-728.
...
**Verification and lifecycle:** `AuditLog::verify_chain()` and `quicfuscate verify-audit-log <path>` validate ordered segment identity, sequence and hash continuity, checkpoint anchor/tail, and detect mutation, interior deletion, reordering, truncation, and tail loss. `flush()` bounds the producer acknowledgement wait, while a writer-side watchdog publishes a typed durability failure within the same configured timeout; the underlying filesystem syscall cannot be forcibly interrupted, so the writer remains owned and shutdown reports a bounded timeout rather than claiming durability. Write, checkpoint, slow-flush, terminal-discard, and shutdown-failure counters are exposed through `AuditStats` and Prometheus. Shutdown atomically changes admission from `Open` to `Closing`, waits for already-admitted producers, then sends the final barrier and joins the writer within the configured bound; the first durability or shutdown failure is sticky across later calls and drop cleanup.
...
**Memory locking (TODO-516, TODO-851, TODO-852, TODO-854):** When `[security] lock_memory = true` (default), an unlimited `RLIMIT_MEMLOCK` uses `mlockall(MCL_CURRENT | MCL_FUTURE)` and a finite budget uses `MCL_CURRENT`; a failed budget query is explicit typed state rather than an opaque `.ok()` fallback. `SecurityConfig.memory_lock_failure_policy` selects `best-effort` or `fail-closed`: best-effort keeps startup available with degraded health after a query, syscall, or unsupported-platform failure, while fail-closed aborts before TLS identity publication and service readiness. `src/memory_lock.rs::MemoryLockPolicy` is the shared server startup owner for standalone and embedded paths. It resets the qftls process-coverage marker, applies process locking when not explicitly deferred, configures `MemoryPool::set_lock_blocks()`, and publishes `MemoryLockStartupStatus` to runtime Metrics, admin status/health, and systemd `/health`, `/ready`, and `/live`. With Linux privilege reduction, standalone startup individually locks the TLS private-key allocation before the drop and defers process-wide locking until the verified setxid transition; the deferred result is propagated before runtime services and `READY=1`. `BlockLockLedger` records only successful block locks per pool. `MemoryPool::free()` is the caller-owned return boundary; queue shrink, full-queue disposal, pool `Drop`, and thread-local cache `Drop` all zeroize blocks, call `munlock(2)` through the ledger, and only then release the `AlignedBox` allocation. Direct `AlignedBox` drops still bypass pool ownership and remain separate pooled-buffer cleanup boundaries. Standalone reload compares the current and candidate startup policy before runtime mutation and rejects changes to `security.lock_memory`, `security.lock_blocks`, or `security.memory_lock_failure_policy`; restart is required. Finite budgets remain current-only and emit explicit diagnostics; `LimitMEMLOCK=infinity` enables current-and-future protection. TODO-854 closes local deterministic negative-proof wiring; native Linux root-regain and Windows native fault lanes remain explicit unavailable boundaries. TODO-853 closes certificate/key correspondence and zeroizing exporter ownership; TODO-643 retains lower-level preloaded qftls-key lock/publication ownership.

**Retained-secret erasure:** `src/secret.rs` owns heap-backed secret byte and UTF-8 representations and overwrites the live range before clearing and deallocation. QKey generation, parsing, configuration, profile, connection, registry insertion, and binary-token hashing use these owners; persisted registry state contains only the non-secret QKey identifier and SHA-256 verifier. QuicFuscate's copied 1-RTT traffic secrets, session-cache ticket material, test-bound ticket keys/sessions, returned session-ticket copies, private-key PEM read buffers, and AES header-protection key use zeroizing owners. AEGIS L/X4/X8 wrappers retain only their copied key and IV, wipe both on wrapper drop, and create local derived cipher state for each packet or batch; `AesBlock` is non-`Copy` and zeroizes each value on `Drop`, while the local state is zeroized on scope exit and is never shared behind a mutex. Test-only pre-deallocation observers make normal, error, eviction, replacement, and partial-initialization erasure assertions failable without reading freed memory. TODO-681's completed crypto unsafe audit leaves separate proof work for `Aes128Ctx` round-key storage, temporary AES schedules, ChaCha nonce/clone copies, and the exact compiler-level register-erasure boundary.
...
- `src/qftls.rs`: Boundary split between rustls real TLS protocol and optional TLS Cover overlay
  ...
  - `src/optimize/`: Optimization submodules remain under `src/optimize/*`; the UDP owner is `crates/qf-transport-udp/src/lib.rs` + `fastpath.rs` with `src/optimize/udp.rs` and `src/transport/udpfast.rs` as compatibility projections, and retained helpers are re-exported through `src/accelerate.rs` to keep the public `accelerate::*` API stable.
...
- `src/pki/mod.rs`: Production CA hierarchy generation and validated reuse of the server leaf, including Rustls/WebPKI chain, hostname, expiry, and private-key matching checks; invalid or incomplete material is quarantined before regeneration. Every key, chain, and CA writer goes through one `write_pki_file()` boundary under TODO-723: an existing symlink at the output name is rejected as `PkiError::UnsafePath` without being followed, content is staged in a sibling file that cannot pre-exist (`create_new` plus `O_NOFOLLOW`, mode 0600 for private keys and 0644 for certificates), and the staged file is flushed and renamed onto the target. `rename` does not follow a symlink at its final component, so a link planted after the check replaces the link rather than writing through it; the mode is carried by the staging file, so replacing a pre-existing permissive key file cannot inherit its wider mode. Existence checks in `ensure_pki` and quarantine use `symlink_metadata`, because `Path::exists()` resolves the link and reports a dangling one as absent.
...
- Standalone runtime config reload: the server module owns runtime reload normalization for stealth overrides, optimize normalization, and `transport.*` TOML overrides. Reload is explicitly `NextConnectionOnly`: the single live server loop serializes profile replacement with connection construction, while every existing session remains immutable. `RuntimePolicyGeneration` gates publication and `RuntimePolicySnapshot` clones transport, FEC, optimization, and stealth together, so a new connection cannot mix generations; successful reloads advance and expose the generation in `StandaloneReloadOutcome`, runtime logs, and audit text. Admin acknowledgement, runtime logs, and audit records state this scope and the unchanged active-session count. `main.rs` only forwards reload intent and transport state into that server-owned path.
...
CPU feature detection is centralized. The runtime dispatch surface uses exact compile-time/runtime subfeature intersections; native architecture proof remains a separate gate:
...
`CpuFeatures::simd_dispatch_matrix()` in `crates/qf-cpu/src/lib.rs` is the single runtime conjunction table for target-feature callers. It requires AVX-512F plus VL for ACK canonicalization, F plus BW for byte/word paths, F plus BW plus VBMI2 for VBMI2 paths, F plus CD plus VPOPCNTDQ for histogram population count, F plus VL plus VNNI for SHA-256 VNNI, F plus VAES plus AES-NI plus SSE2 for the delegated VAES/AES path, VPCLMUL/PCLMUL plus SSE4.1 for GF16, FMA with AVX-512 or AVX2 for neural dot products, and AVX plus SSE4.1 plus SSSE3 for the AVX ChaCha path. GHASH, AEGIS, AES-GCM, and MORUS direct crypto dispatch use the exact `features_full()` fields for their target attributes. The same matrix drives profile selection, FEC kernel thresholds, string dispatch, ACK dispatch, test wrappers, and telemetry expectations.
...
#### SIMD Gap Status
 ...
 - **FEC (large-window decode acceleration)**: The active Wiedemann path uses a checked scalar GF(256) SpMV fallback. The former raw AMX staging and unverified INT8 kernels are removed because they did not perform GF(256) arithmetic and had no proven tile contract. TODO-818 owns `scripts/tests/suites/test-amx-proof.sh`, which compiles the integration target with exactly `-Ctarget-feature=+amx-tile,+amx-int8`, records compiler/OS/CPU/source-revision evidence, emits machine-readable `AVAILABLE` or `UNAVAILABLE`, and runs scalar parity, concurrency, dimension, scratch-reset, and telemetry coverage. BF16 is not an eligibility requirement. Native AMX arithmetic and supported-host execution remain fail-closed until a verified backend exists; the current production boundary has no tile state to share or release. Intel AMX remains independent of `X86_P3e`; `Apple_M` is a NEON/crypto profile whose current callers do not execute AMX. TODO-676 closes the planner/runtime, concurrent ownership, and scalar-versus-AMX telemetry boundary; TODO-817 and TODO-818 retain detector and native-proof boundaries.
 - **Utility (RVV)**: Infrastructure for RISC-V Vector (`RVV`) and additional iterator backends are not active in the current build.
 - **SIMD safety (TODO-593, TODO-835, TODO-836)**: x86 GF(256) matrix kernels reject checked dimension overflow and all source/destination slice mismatches in release mode, BMI2 varint encoding returns zero without touching an undersized output buffer, the SSE4.2 short-needle path loads from owned sixteen-byte padding, and every Berlekamp-Massey entry rejects an overlong syndrome prefix before indexing. AVX2 header dispatch avoids a discarded 32-byte load by delegating to the scalar first-byte check, and scalar/NEON/GFNI Reed-Solomon encoders zero-pad partial input shards instead of truncating them. Windows-only SHA-256 compression stubs fail loudly if reached. TODO-836 adds local `# Safety` sections to all current 131 unsafe SIMD declarations, removes the blanket suppression, verifies exact target-feature wording, and requires explicit `SIMD_SKIP` accounting for unsupported ISA tests. Native x86/Linux, Windows, SVE2, and sanitizer/Miri proof remain unclaimed target-specific evidence boundaries.
 - **SIMD dispatch intersections (TODO-834)**: implementation and local delivery are complete. The centralized matrix, exact profile/FEC/string/transport/crypto gates, safe test wrappers, packet-number endian regression test, and honest scalar telemetry owners are implemented. ARM64 macOS verification passes the complete serial library suite at `2,440/2,440` plus strict library/all-target Clippy and formatting/diff gates. Native x86/Linux/Windows ISA execution remains an external evidence boundary. TODO-836 records the local safety and unsupported-ISA guardrails; TODO-835's release-safe boundary implementation is complete, TODO-855's FEC-specific dispatch/contract implementation is complete, and TODO-859/TODO-715 retain FEC proof and GF16 polynomial boundaries.
 - **FEC SIMD contract (TODO-855)**: FEC GF8/GF16 target-feature helpers now have local `# Safety` contracts and release-safe shared-length bounds. The live GF16 selector consumes one matrix-derived level: AVX-512 VBMI2 requires AVX-512F + AVX-512BW + AVX-512VBMI2, AVX-512 VBMI remains distinct, and NEON requires only NEON. Thresholds and connection telemetry expose the selected implementation instead of collapsing VBMI2 into generic AVX-512. GF16 PCLMUL and ARM SVE2 paths consume the shared runtime matrix, including a runtime check for compile-time SVE2 builds. Focused unit and self-check coverage exercises matrix-negative selection, unequal lengths, and short/tail boundaries; native x86/Windows/SVE2, sanitizer/Miri, and complete per-backend differential proof remain TODO-859/TODO-715 evidence boundaries.
 - **SIMD Reed-Solomon compatibility path (TODO-594)**: standalone x86 AVX2 and GFNI Reed-Solomon encode/decode now use the canonical GF(256) generator, full augmented-matrix inversion, dynamic per-coefficient nibble LUTs, release-safe shard metadata validation, and scalar tails for non-vector-aligned shards. Rosetta-executed x86 tests cover AVX2/GFNI roundtrips and the AVX2 matrix kernel. These helpers remain internal/test-only; production FEC stays on `src/fec/`. TODO-679 confirmed the old AVX2-to-GFNI delegation claim is stale. TODO-836's local safety-contract implementation is complete; native SIMD proof remains unclaimed, and TODO-835's boundary implementation is complete.
...
- **sendmmsg/recvmmsg**: runtime-owned Linux batching in `crates/qf-transport-udp/src/lib.rs` and `fastpath.rs`, with `src/optimize/udp.rs` and `src/transport/udpfast.rs` as root projections
...
runtime owner for UDP GSO policy and `UdpFastPath` is `crates/qf-transport-udp/src/lib.rs` and
`crates/qf-transport-udp/src/fastpath.rs`, while the root projections remain
...
- **Global pool lifecycle:** `global_pool()` is the authoritative initialization path. Its first call captures one environment snapshot, builds the adaptive pool, and starts the process-global auto-tuner when the resolved runtime enables it. `init_global_pool(capacity, block_size)` is the explicit alternative and now produces the same worker behaviour, so lazy and explicit initialization no longer tune differently depending on which ran first; it returns `false` and changes nothing if a pool is already published. `global_pool_if_initialized()` is the non-creating accessor for observers: telemetry resource refresh uses it, so a metrics scrape can no longer construct the pool and its worker as a side effect of being asked for numbers. `MemoryPool::shutdown_auto_tuner()` is a process-final or test-teardown operation that stops and joins the single worker while leaving the published pool valid and allocatable; it is a no-op when no worker runs, and tuning can be restarted explicitly with `start_auto_tuner()`. `MemoryPool::drop` deliberately does not stop the worker, because the worker is process-global while a pool is not.
- `MemoryPool::try_new()` and `MemoryPool::try_new_adaptive()` validate non-zero capacity, non-zero requested size, 64-byte layout validity, and the `isize::MAX` total-byte bound before allocating. `try_alloc()`, `try_alloc_from_slice()`, and `try_set_capacity()` return typed `MemoryPoolError` values for allocator, ownership, or growth failures. The existing `new()`, `alloc()`, `alloc_from_slice()`, and `set_capacity()` methods are infallible compatibility wrappers that panic with the exact failure instead of constructing an invalid layout; recoverable callers must use the `try_*` APIs. `OptimizationManager::try_new*()` and `try_alloc_block()` expose the same boundary to manager callers.
...
- Linux `SystemIoHotpathAdapter` performs one-time batch socket capability initialization on first `sendmmsg` use via the hidden runtime helper `transport::init_socket_acceleration`, keeping runtime hotpath independent from the test-only `qf-transport-batch::BatchProcessor` surface projected at `transport::batch`.
...
- The shared `TunInterface` result boundary validates generic read lengths and
  complete writes under TODO-844. TODO-845 now validates raw `readv` and
  `writev` totals and progress, bounds and terminates the kernel-reported
  interface name, and routes setup through explicit descriptor rollback; native
  privileged execution remains a separate proof boundary.
...
- Wintun cleanup now has an explicit resource ledger: shutdown signaling,
  session end, adapter close, shutdown-event close, and DLL unload advance only
  after their own step succeeds. Event and DLL failures remain pending for an
  explicit retry; startup rollback retains every acquired owner, and Drop makes
  one bounded retry with a durable pending-resource diagnostic. The safety
  comment follows the exact upstream thread-safety guarantees, while the
  compile gate and the existing blocked-reader native lifecycle test cover the
  local Send/Sync and close ordering contract.
- Deterministic cleanup-state fault coverage is wired, but Windows compilation,
  injected Win32 failure execution, and privileged native residue execution for
  this change remain unclaimed. Historical native lifecycle evidence does not
  close those new failure-injection cases.
...
- `TunDevice::read_contract()` makes executor ownership explicit. Native Linux and macOS descriptors publish `NonBlocking`; external/custom backends default to `Blocking` and the generic client `IoDriver` rejects them before entering its async outbound loop, so they must use an owned reader boundary. `TunInterface` rejects zero or oversized reads, zero, short, or oversized writes, and incomplete packet results under TODO-844; TODO-845 closes the source-level native Unix syscall progress, raw vectored-result, kernel-name, rollback, and close-ownership contracts, while privileged execution remains unclaimed.
...
- Active codec cascade
  ...
  - Block modes above 255 sources per lane: GF(2^16) with deterministic Cauchy repair rows and exact odd-length recovery. A process-wide 65,536-entry, 128 KiB inverse table is built once in linear field order, preserving every wire coefficient while removing repeated exponentiation from eager row-cache construction. Construction is transiently larger than the retained table: it also allocates a 65,535-entry exponent vector (131,070 bytes) and a 65,536-entry logarithm vector (131,072 bytes), so the one-time peak is 393,214 bytes (about 384 KiB) across all three allocations. Both temporaries are dropped when the initializer returns, leaving the documented 131,072-byte (128 KiB) steady state.
...
- `src/implementations/server/systemd.rs` - systemd-oriented service/unit integration helpers. `ServiceConfig` emitted `--mode server`, a CLI form that no longer exists, plus a placeholder repository URL, so a generated unit could not start while the separately shipped installer unit worked (TODO-743). Both now describe the same invocation: the `server` subcommand with `--listen`, `--cert`, `--key`, and `--config`, from named constants, and the real project URL. `scripts/install/quicfuscate-server.service` remains the reference; the generator is checked against it by a drift test that includes the shipped unit at compile time, so a rename in either place fails the suite instead of a customer's first boot.

Audit module (`src/audit/`):
- `src/audit/mod.rs` - one bounded asynchronous audit owner with producer-side JSON-encoded UTF-8 payload bounds, non-blocking producers, schema-v2 typed events, SHA-256 chaining, deterministic segment rotation/retention, atomic durability checkpoints, restart recovery, writer-side durability watchdog, sticky terminal/shutdown failures, observable queue/payload/persistence/terminal-discard/slow-flush counters, and schema-v1 compatibility. `verify-audit-log <path>` verifies the complete retained set. Every audit artifact is a mode-`0o600` regular file owned by the runtime identity.
...
- `crates/qf-telemetry/src/lib.rs` - global telemetry counters and snapshot/export helpers; root optimize/telemetry paths are compatibility projections.
- `crates/qf-cpu/src/transport.rs` - transport acceleration helpers; `src/optimize/transport.rs` is the root compatibility projection.
...
- `crates/qf-transport-udp/src/lib.rs` + `fastpath.rs` - UDP syscall/GSO and `UdpFastPath` owners; `src/optimize/udp.rs` and `src/transport/udpfast.rs` are root compatibility projections.
- `crates/qf-transport-batch/src/lib.rs` - explicit rust parity/test-only `BatchProcessor` owner; `src/transport/batch.rs` is the root compatibility projection.
...
- `crates/qf-simd/src/arm_varint.rs` - ARM varint SIMD helpers.
- `crates/qf-simd/src/x86_ack.rs` - x86 ACK-related SIMD helper path.
- `crates/qf-simd/src/x86_header.rs` - x86 header parse/validate SIMD helper path.
...
- `crates/qf-transport-frames/src/lib.rs` - frame encoders/decoders and canonical ACK block logic. `src/transport/frames.rs` is the compatibility adapter. `wire_len()` returns `Result<usize, ConnectionError>` and validates ACK ranges, checked QUIC-varint CRYPTO/STREAM/DATAGRAM payload lengths (up to 64 KiB), New Connection ID invariants, and checked length arithmetic before serialization. `from_bytes()` returns `Frame<'a>` with `Cow::Borrowed` data fields for zero-copy parsing, accepts both RFC 9221 DATAGRAM forms, consumes no-LEN STREAM payloads to the packet boundary, and enforces packet-space legality; construction sites use `Cow::Owned`.
- `src/transport/h3.rs` - HTTP/3 state machine (streams, QPACK, events, MASQUE wiring).
- `src/transport/packet.rs` - QUIC packet parse/build, encryption/decryption glue; `CryptoStream` is owned by `crates/qf-transport-crypto-stream/src/lib.rs` and re-exported here.
...
`qf-transport-crypto-stream::CryptoStream::{send,next_crypto_frame,ack_crypto,requeue_crypto,recv}` return
...
- The `windows-core-checks` job caps Cargo at two jobs, checks and lints `tun-windows,rust-tests`, compiles its unit-test binary, provisions the integrity-checked upstream DLL beside that binary, executes ordinary tests plus the deterministic WFP ownership filter and serial privileged Wintun adapter/I/O/close and WFP packet-outcome/process-exit suites, independently verifies zero managed WFP objects even after a failed behavior step, rejects owned adapter or legacy firewall-rule residue, emits `quicfuscate.interface_platform_negative_proof.v1` with unavailable native fault-injection prerequisites explicit, and uploads provenance plus Windows-build evidence. Run `30508948149`, job `90764941801` predates the negative-proof manifest and proves commit `afe46e0` with the complete native Wintun and WFP lifecycle green. Manual Windows-Omega runs `30535603045` and `30536002374` add two consecutive authenticated dual-stack Wintun/WFP data-plane proofs against one unchanged ARM64 server process. The `release-contract` job runs `scripts/audits/verify-release-version.sh` on pushes and pull requests.
- `.github/workflows/clippy-matrix.yml` runs the Rust clippy feature matrix on stable Rust with `-D warnings`.
- `.github/workflows/release.yml` runs only for `v*` tags or explicit manual dispatch. It builds required native x86_64 and ARM64 server bundles and required signed macOS/Linux/Windows desktop updater artifacts. Each desktop job enables `QUICFUSCATE_DESKTOP_UPDATER_ACTIVE=true` at build time and verifies its signed pair before upload; the Windows job additionally enables `tun-windows`, provisions the same verified upstream DLL as a Windows-only Tauri resource beside the executable, administratively extracts every produced MSI, and verifies exactly one byte-identical DLL. Tagged publication requires all server and desktop jobs, then maps the complete platform set into `latest.json` through `verify-release-updater.sh`.
...
Linux server startup resolves `--drop-user` and `--drop-group` before opening privileged resources. Selectors containing only decimal digits are treated strictly as numeric UID/GID values; all other selectors are resolved as names through NSS. TLS certificate and private-key PEM are validated and retained in memory before the transition, so new connections never reopen a root-owned key file. After socket, TUN, and routing setup, the process clears supplementary groups, sets real/effective/saved GID and UID, clears effective/permitted/inheritable/ambient capabilities, and verifies every Linux thread has the target IDs, empty groups, zero capability sets, and `PR_SET_NO_NEW_PRIVS`. The destructive root-regain proof is isolated in `qf-privilege-probe`; it is not run inside the multi-threaded service. The shipped systemd unit root-starts with only `CAP_NET_ADMIN`, `CAP_NET_BIND_SERVICE`, `CAP_NET_RAW`, `CAP_CHOWN`, `CAP_SETGID`, and `CAP_SETUID` in `CapabilityBoundingSet`, and the process removes all capabilities before accepting traffic. Service-manager confinement and privileged host-routing teardown remain platform responsibilities. The non-Linux Unix path now clears supplementary groups with the same `setgroups(0, NULL)` operation before `setgid`, because after `setuid` the process no longer holds the privilege to drop them and a retained membership keeps granting access to everything reachable through it. A clearing failure is propagated and cannot produce a successful drop report. Post-drop verification there accepts only an empty group set or the new primary GID alone, because POSIX leaves it unspecified whether `getgroups()` reports the effective GID and the BSD-derived platforms do; any other entry fails the drop and is named. Linux keeps its stricter empty-set contract.
...
- **Desktop IPC contracts (TODO-746):** `apps/svelte-desktop/src/lib/ipc-contracts.ts` validates every value crossing the Tauri boundary, because `invoke<T>()` is a cast rather than a check. `engine_status` requires a usable state and nulls malformed optional fields so a bad error string cannot hide a real transition; `engine_stats` defaults only absent numbers and drops the whole sample when a present one is invalid, since `?? 0` substitutes for null and undefined only and previously let `NaN`, `Infinity`, a negative counter, or a string reach the throughput calculation; the updater plugin result must carry both versions and a callable installer before it becomes a local update contract. The helpers reject rather than coerce, because a malformed field means the two sides disagree and repairing it silently hides that. `parseRemote` also rejects trailing text after a bracketed IPv6 host, which used to be read as an absent port and normalized to :4433, turning an invalid endpoint into a different valid-looking one. Admin API response schemas remain open under TODO-878.
...
- **Blocked IPs** (`<config>.blocked.json`): loaded on startup, written on every block/unblock action. The load is fail closed (TODO-733): a missing file is `Absent`, a valid file is `Valid`, and an unreadable file, malformed JSON, or an entry that admission could never match is an error that stops startup rather than becoming an empty allow-all set. `Absent` and an explicitly empty policy are distinguished and both logged, because they are identical in memory and only the log tells an operator which one happened. Block and unblock apply to the live set first and then persist; if persistence fails the live change deliberately stands, since rolling back a block would readmit an address the operator just denied, but the admin response returns failure and states that the change applies to the running server only and will be lost on restart. Success therefore means durable.
...
All memory-pool controls in this section are parsed into one `MemoryPoolRuntimeConfig` from the pool's immutable construction snapshot. `MemoryPool::new()` and `MemoryPool::new_adaptive()` capture their own snapshot when no parent runtime supplies one; the global pool and body pool pass their existing snapshot through. The auto-tuner stores the typed values at startup and never rereads process environment variables. Positive controls reject zero with a warning; utilization values are clamped to their documented percentages with a warning; `POOL_MAX_CAP` is normalized to at least the effective minimum capacity and clamped to the representable `isize::MAX / block_size` block count.
...
Notes:
- Allocation layout contract (TODO-829): the effective block size is checked against a 64-byte `Layout` and the total initial capacity is checked against `isize::MAX` before any allocation. `MemoryPool::alloc_numa_block()` has no invalid-alignment fallback and returns typed `MemoryPoolError` values on allocator or ownership failure; partially constructed queues are zeroized, unlocked, and released before a fallible constructor returns. The feature-gated `UnsafeMemoryPool::try_new()` applies checked cache-line rounding, total-byte validation, `Vec`/`HashMap` reservation, and partial-allocation cleanup. The legacy infallible wrappers retain explicit panic policy for callers that cannot propagate errors. TODO-516 remains the sole mlock/munlock lifecycle owner.
...
- `check_invariants()` remains a diagnostic warning surface with configurable debug slack/grace, but the synchronized ownership ledger is authoritative for exact block, origin, location, capacity, available, and in-use state and is covered by focused tests. `PooledBlock` closes the generic, FEC, and zero-copy DATAGRAM early-return and unwind cleanup boundaries under TODO-831, TODO-832, and TODO-833.
...
- `smoke/smoke-shell-portability.sh` - Shell portability gate (TODO-745): macOS ships Bash 3.2 as `/bin/bash`, so `env bash` resolves to it on a host without a newer Bash and Bash 4 builtins abort the script before it does any work. The gate scans every shipped script for `mapfile`/`readarray` outside comments, exempting only the Linux-only installer suites, and runs the utilities under `/bin/bash` itself. BSD `find` on macOS does support `-mindepth` and `-delete`, verified directly, so those are not portability hazards.
- `smoke/smoke-netfilter-fastpath.sh` - Ownership and port-policy contract for the netfilter fast-path helper (TODO-744): proves every emitted iptables command carries the owner comment, that no delete could match a rule the helper did not create, and that out-of-policy and malformed ports are refused before any mutation.
- `smoke/smoke-bench-example-cli.sh` - Negative CLI contract for the benchmark and probe examples (TODO-738): 14 cases proving that a malformed number, a zero workload, an out-of-range ratio, an unknown backend, an unknown option, an all-unsupported length set, and a unitless jitter each exit nonzero with a diagnostic and without a panic. The shared contract lives in `examples/bench_cli/mod.rs`: typed parse errors instead of `unwrap`, a 1 GiB buffer budget, a 10^9 iteration budget, a checked bytes-times-iterations product, and zero treated as an error because a zero-work run emits a record that contains no measurement.
...
- `test-benchmark-cell-contract.sh` - Negative benchmark artifact contract for empty filters, per-cell Criterion failure, export failure, and platform skips
- `test-release-updater-artifact-contract.sh` - Negative release updater contract for required platform completeness, matching signatures, build-bound native activation, workflow hard failures, and missing-artifact rejection
...
- `bench-ci-regression.sh` - CI regression benchmark gate (Criterion). Benchmark preflight is classified, not guessed (TODO-735): `qf_bench_preflight` in `scripts/tests/lib/lib-common.sh` reads the declared benchmark targets from `cargo metadata` and separates two outcomes that a nonzero `cargo bench --no-run` used to merge. No declared target is a legitimate SKIP with exit 0; a declared target that fails to build is a FAIL with the build output, so a compile error can no longer produce a green skip that reads as a completed performance check. Every suite and the CI regression gate use it. A failed baseline creation in the gate now exits nonzero instead of reporting a created baseline, and the orchestrator exits nonzero when its selection is empty. TODO-877 now closes per-cell result status, identity, metric, filter, and export truth.
...
- `util-dev-uis-start.sh`, `util-dev-uis-stop.sh` - start/stop local frontend dev servers under `scripts/out/run/dev-uis`. Ownership is recorded rather than assumed (TODO-747): each record carries the PID, the process group, and a process identity built from the start time and command line. Start runs the child under job control so it leads its own process group, and stop signals that group, so Bun and Vite descendants die with it instead of surviving while the helper reported success. Stop refuses to signal a PID whose identity no longer matches the record, since PIDs are reused and the previous version would have signalled whatever now held the number, and it reports surviving descendants as a failure with a nonzero exit.
- `util-run-local-ui.sh`, `util-stop-local-ui.sh` - local stack orchestration helpers for UI + server workflows. `util-run-local-ui.sh` and `util-run-local-admin-web.sh` wait for each service to accept a TCP connection before reporting its URL. `tmux send-keys` only queues text into a pane, so they previously printed started URLs while the build had not run; a readiness timeout is now a distinct nonzero state and the session is left running for inspection. Override the bound with `QF_READY_TIMEOUT_SECS` (default 240).
...
- **Metrics/counters (98 of 117 raw statics, plus 270 wrapped telemetry statics)**: These are read-only observation surfaces (95 in `crates/qf-telemetry/src/lib.rs` + 3 in `transport/batch.rs`) and are appropriate as globals. No change planned.
...
- **Runtime config (15 across qf-telemetry, qf-cpu, optimize/, crypto/, qftls.rs)**: 6 telemetry collection gates + 5 optimize/ hardware-adaptive + 2 crypto/ AEAD/AES + 2 qftls/ TLS gates. Could migrate to a shared `RuntimeConfig` struct passed through the call chain, but current usage is stable and well-bounded.
...
- QuicFuscate refuses to claim kill-switch support unless a successful `pfctl -sr` query exposes an actual `anchor "com.quicfuscate.killswitch"` or matching `com.quicfuscate/*` statement. If the anchor was loaded before that check fails, the client flushes the just-loaded anchor and removes its nonce-scoped config; if rollback fails, the failure is reported as retained fail-closed ownership for explicit cleanup.
- Installers own only the exact marked `anchor "com.quicfuscate.killswitch" all` line in `/etc/pf.conf`. `scripts/install/install-macos-pf-anchor.sh install|remove|check` uses an atomic marked block, a mode-0600 `/var/db/quicfuscate/pf/managed-anchor.state`, a private pre-install backup, and a state-directory lock. It refuses foreign exact/wildcard anchors, modified markers, symlinks, orphaned state, and unverifiable active-ruleset state; it never enables/disables PF or flushes unrelated rules. `--root PATH` is a non-privileged fixture boundary that skips `pfctl` and is covered by `scripts/tests/fast/test-macos-pf-anchor-installer.sh`.
- The read-only privileged runtime proof remains `sudo scripts/tests/macos-pf-anchor-proof.sh`. Packet, coexistence, crash-retention, stale-cleanup, and uninstall proof still require an authorized native macOS runner; this ARM64 session does not claim those outcomes.

**Temp file conflicts:**
- Kill-switch config uses secure random nonce-scoped temp files (`/tmp/quicfuscate_killswitch_<nonce>.conf`) with exclusive creation and no-follow semantics to avoid multi-instance conflicts
...
- **Privilege drop FFI boundary**: TODO-652 keeps libc status, null-result, and exact returned-pointer identity checks adjacent to `MaybeUninit::assume_init`, retains the lookup buffer until `passwd`/`group` names are copied, and adds deterministic status, `ERANGE`, null, pointer-mismatch, and unknown-account tests. TODO-653 replaces both raw account-name scans with a bounded buffer-range and NUL check before `CStr::from_bytes_with_nul`, covering normal, null, out-of-buffer, and unterminated records. TODO-849 now makes `ResolvedIdentity` opaque, revalidates non-root selectors and canonical account mappings at the final drop boundary, defines `CurrentIds` for non-Unix compilation, rejects malformed `getgroups()` counts, and documents every remaining privilege FFI block locally. TODO-850 closes the source-level filesystem-ID/post-drop state contract and local probe assertions. TODO-851 closes process-lock policy propagation and ordering; TODO-852 closes process-lock failure/readiness policy, TODO-853 closes certificate-key correspondence and zeroizing exporter ownership, and TODO-854 closes local deterministic privilege, memory-lock, TLS, ordering, and portability-proof wiring. Native Linux root-regain and Windows native fault lanes remain environment-specific and unclaimed.
...
- **Audit log blocking flush**: `src/audit/mod.rs` retains synchronous filesystem operations but wraps each durability barrier in a bounded writer-side watchdog. A stalled operation publishes terminal persistence failure, producers reject subsequent events, and shutdown reports a bounded timeout without claiming durable completion. Existing-file read bounds remain TODO-727 and pathname binding remains TODO-728.
- **AMX capability contract**: TODO-816 removed the former `static mut` tile config and unverified raw kernels from the active source. TODO-817 removes the external `cpuid` process and exposes separate in-process CPU, OS permission, compiler target, and product-backend evidence. TODO-676 closes the final fail-closed dispatch intersection and concurrent ownership boundary: the product path remains scalar and `product_dispatch_eligible` is false without every required signal. The production FEC path reports only scalar Wiedemann operations until a future verified AMX arithmetic backend exists; profile/documentation truth is closed by TODO-819 and native proof remains the explicit TODO-818 lane.
...
- **Interface/platform unsafe**: TODO-683 completed the read-only audit of
  ...
  CI/audit script, documentation claim, and relevant history. TODO-843 closes
  the BMI2 dispatch source contract, and TODO-844 closes the generic TUN
  wrapper result contract. TODO-845 now closes the source-level Unix syscall
  result, progress, length, kernel-name, rollback, and close-ownership
  contracts with deterministic fault fixtures and explicit host skips. Wintun
  lifecycle and concurrency proof (TODO-846) is implemented at the source
  boundary, and TODO-847 now retains WFP engine/transaction ownership after
  failed native release with deterministic fault-injected status tests.
  Windows compilation, BFE fault execution, and privileged residue proof for
  TODO-846/TODO-847 remain unclaimed. TODO-848 emits a versioned
  negative-proof manifest that separates executed local fixtures, conditional
  host skips, and unavailable Windows Win32/BFE fault lanes.
- **Privilege/mlock/secret unsafe**: TODO-684 completed the current-source and owner reconciliation across privilege drop, memory locking, qftls, secret handling, callers, platform gates, tests, scripts, documentation, related TODOs, and history. TODO-651 closes the checked `SecretString` boundary, TODO-652 closes lookup result-pointer validation, TODO-653 closes bounded account-name conversion, TODO-643 closes preloaded-key lock/publication ownership, and TODO-849 closes the source-level forged-identity, cross-target `CurrentIds`, local FFI-safety, and group-count contracts. TODO-850 closes the source-level post-drop/filesystem-ID contract and local standard/Tokio probe assertions. TODO-851 closes the shared standalone/embedded process-and-pool policy, pre-identity ordering, deferred privilege boundary, and startup-owned reload rejection; TODO-852 closes readiness/failure semantics, TODO-853 closes certificate-key correspondence and zeroizing key-output ownership, and TODO-854 closes local deterministic privilege, memory-lock, TLS, ordering, and portability-proof wiring. Native Linux root-regain and Windows native fault lanes remain explicit unclaimed boundaries. TODO-516 and TODO-678 retain pool lock ownership.
- **QKey/admin unsafe**: TODO-685 is closed as a stale unsafe-site inventory after current-source reconciliation found no raw-memory unsafe operation in QKey registry or admin session logic. One current Windows `MoveFileExW` path-encoding and native-proof boundary remains in QKey registry storage under TODO-873; current failpoint coverage exercises only the host `rename()` branch and does not prove Windows success, failure, or interior-NUL rejection. TODO-861 owns the separate audit-file FFI boundary, TODO-728 owns audit pathname binding, and TODO-647/TODO-661/TODO-665 retain admin session lifecycle and replay ownership.
- **FEC unsafe and boundary audit**: TODO-686 is complete as a read-only audit across FEC unsafe SIMD/AMX sites, direct core/runtime callers, decoder/matrix/wire/Fountain public inputs, feature gates, malformed tests, fuzz and shell/benchmark/netns proof, documentation, and history. Open remediation is split across TODO-634, TODO-636, TODO-637, TODO-690, TODO-715, and TODO-855 through TODO-860; TODO-832 closes the pooled-buffer ownership boundary, while the remaining FEC arithmetic, SIMD, public-input, and runtime-proof owners remain open. TODO-860's current reconciliation confirms unchecked zero/Fountain counter boundaries, non-divisible interleave shape changes, sequence narrowing before direct lane routing, unbounded repair bit-packing, Fountain wire source-ID subtraction, and unchecked decoder/Wiedemann/Fountain size arithmetic. No implementation or runtime verification was performed.
...
- **Audit/limits unsafe**: TODO-688 completed the read-only audit of the complete audit implementation and tests, direct startup/runtime callers, audit probe, limits false positive, suites, guardrails, documentation, related owners, and history. The current inventory is three production FFI sites plus one Unix-only test guard. TODO-861's current reconciliation confirms warning-only permission/ownership hardening, a void-return security helper, missing local `geteuid` safety wording, Windows interior-NUL acceptance, and absent root/Windows negative proof; TODO-671, TODO-675, TODO-726, TODO-728, TODO-813, and TODO-814 retain their separate mode, lifecycle, path-binding, and bound owners. TODO-815 now closes the shutdown admission-order owner. No production implementation or runtime verification was performed for TODO-688.
- **Remaining auxiliary unsafe audit**: TODO-689 completed the read-only audit of `cpu_dispatch`, the shared prefetch facade and direct callers, Windows NUMA FFI, global-pool/auto-tuner initialization, test-environment mutation, the test-only constant-buffer helper, telemetry, `lib.rs`, and the transport/config false-positive matches. TODO-862 confirms that non-iOS AArch64 still uses faulting `read_volatile` behind a null-only pointer API; TODO-863 confirms ignored processor-query status and node-zero availability semantics; TODO-864 confirms lazy/explicit tuner divergence and telemetry initialization side effects; TODO-865 confirms N-1/test-only capacity and N=0 gaps. Remediation is TODO-862 through TODO-865, with TODO-670/TODO-811, TODO-826/TODO-827, TODO-834/TODO-835/TODO-836, TODO-841, TODO-843, and TODO-752 retaining adjacent ownership. No architecture, Windows, lifecycle, or rust-tests gate was run for these reconciliations.
...
- **Audit persistence:** `AuditLog::flush` bounds the producer acknowledgement wait and a writer-side watchdog publishes terminal failure when synchronous `flush` or `sync_data` exceeds the configured deadline. Write, checkpoint, slow-flush, terminal-discard, and shutdown-failure counters are exposed, and the first durability/shutdown failure is sticky. TODO-727 and TODO-728 retain existing-file read limits and pathname binding; TODO-814 closes producer-side payload bounds, and TODO-815 closes shutdown admission ordering.
...
- **AMX:** TODO-816 removes the compile-time-absent production AMX branch from `src/fec/parts/decoders.rs`, restores scalar SpMV on every x86 build, and removes the uncalled raw kernels from `src/simd/amx.rs`. TODO-817 removes the external detector process and makes product eligibility fail closed until OS permission and a verified backend exist. TODO-676 closes the broader planner/runtime intersection and concurrent ownership boundary; the current production path no longer claims AMX operations, allocates AMX scratch, or owns tile state. TODO-818 owns the explicit future native proof lane, and TODO-819 owns profile/documentation truth.
- **Unsafe memory and pooled-buffer boundaries:** TODO-826 replaces the former non-TLS `UnsafeCell` cache in `src/optimize/unsafe.rs` with a synchronized exact-base ownership registry, separates fallback allocations, checks packet invariants in release builds, and bounds prefetch pointer arithmetic. TODO-828 now uses a checked native/fallback zstd context behind a mutex, applies Advanced API parameters through `ZSTD_compress2()`, rejects invalid lengths before header narrowing, and returns every failed output block to its pool. TODO-827 owns the completed safe `MemoryPool` origin/capacity/TLS/ephemeral accounting contract. TODO-831, TODO-832, and TODO-833 now route generic, FEC, and zero-copy DATAGRAM ownership through `PooledBlock`; compatibility raw/foreign boundaries remain explicitly owned by their callers. The historical `copy_to_block` inventory is absent from the current source. TODO-678 is the umbrella index; TODO-829 through TODO-833 own the completed split remediation boundaries, with TODO-516, TODO-646, TODO-682, TODO-683, TODO-687, TODO-689, TODO-730, TODO-734, and TODO-767 retaining their adjacent contracts.
- **SIMD:** The old AVX-512/GFNI Reed-Solomon delegation claim is stale for the active decoder. TODO-834's completed dispatch implementation now covers the exact SVE2 decode, ACK AVX512VL, SHA-VNNI, AES/VAES, GF16, AVX-512 compression/pattern/histogram, neural FMA, optimization string, stale BMI2 profile, scalar-claim, FEC, and packet transport boundaries. TODO-835 closes the critical `find_pattern_sse42_short` short-needle load, release-safe matrix and BMI2 output checks, overlong Berlekamp-Massey rejection, and private key/XOR caller contracts; focused malformed-input tests are part of the owner. TODO-836's proof implementation is complete: all 131 unsafe SIMD declarations have local `# Safety` contracts, blanket suppression is absent, the audit matches private and restricted-visibility helpers, and unsupported ISA lanes report explicit `SIMD_SKIP` markers. Native x86/Linux, Windows, SVE2, sanitizer, and Miri evidence remain unclaimed and are not represented as local proof.
...
- **Interface and platform:** TODO-683's current-source and owner reconciliation covers the full interface, caller, platform-gate, cleanup, test, script, documentation, and history scope. TODO-834 now gates the BMI2 parser with `features_full().bmi2` and scalar fallback, and TODO-654 uses `std::ptr::read_unaligned`; native/negative proof remains open. TODO-844 closes the shared TUN wrapper result contract, and TODO-845 closes the source-level Unix raw-result, progress, bounded-name, rollback, and close-ownership contracts with deterministic fault fixtures and explicit platform runners. TODO-846 now closes the Wintun source-level cleanup ledger, startup-owner rollback, and Send/Sync contract; Windows injected-failure and privileged native execution remain unclaimed. WFP engine/transaction failures are not retryable under TODO-847, and TODO-848 owns the remaining interface/platform negative proof. No native test execution was claimed in this audit reconciliation.
- **Privilege and FFI:** TODO-684 completed the current-source and owner reconciliation across the full privilege, memory-lock, qftls, secret, caller, platform, cleanup, test, script, documentation, related-TODO, and history scope. TODO-651 closes the checked `SecretString` boundary, TODO-652 closes returned-pointer identity validation, TODO-653 closes bounded account-name conversion, and TODO-643 closes preloaded-key lock/publication ownership, all with the recorded local source evidence. TODO-849 now closes final-boundary identity validation, the portable `CurrentIds` type, local FFI safety contracts, and malformed group-count handling. TODO-850 now closes source-level four-field filesystem-ID/post-drop verification, typed partial-transition failure, explicit non-Linux scope, and local standard/Tokio probe assertions. TODO-851 closes the shared standalone/embedded policy and startup ordering; TODO-852 closes process-lock readiness/failure semantics. TODO-853 closes certificate-key correspondence and public key-output ownership; TODO-854 closes local deterministic privilege, memory-lock, TLS, ordering, and portability-proof wiring. Native Linux root-regain and Windows native fault lanes remain environment-specific and unclaimed. Pool lock ownership remains TODO-516/TODO-678. Windows audit-file path FFI, local safety comments, and security-hardening failure semantics are TODO-861; Unix audit pathname TOCTOU remains TODO-728.
- **FEC unsafe inventory:** TODO-686 completed the full FEC source, caller, public-input, feature, test, script, documentation, related-owner, and history audit. Active GF(256)/GF(16) wrappers clamp slice lengths before private vector calls; the old public raw-slice and SSSE3 claims are stale. TODO-834's shared matrix-based GF16 threshold selection is current, and TODO-855 closes the current FEC target-feature, release-safe length, PCLMUL, NEON, SVE2, telemetry, and local safety-contract implementation boundary. TODO-856 now closes direct `FecPacket` compatibility lengths, Decoder4/8/16 dimension and active-window checks, matrix shape validation, direct wire-helper validation, and source-only small-pool truncation. TODO-857 now closes direct Fountain constructor bounds, source-index admission, symbol-size parity, and propagation validation. TODO-858 now closes FEC configuration, runtime-policy, transport-feedback, Kalman, and mode-manager validation. TODO-859 now closes FEC test, script, and documentation truth: the vacuous streaming-dedup and ordering assertions are non-vacuous, the mode-transition loop matches its documented 100 transitions, the GF16 slice kernels have a lane-boundary length matrix plus an overlong-request clamping case, the FEC fuzz target covers the block, wire, and matrix input boundaries, and the simulation, netns, and GF16 table-footprint proof boundaries are stated where they are consumed. Native x86 AVX-512 VBMI2 differential/throughput proof, the AMX negative matrix, and privileged Linux netns gates stay explicitly unavailable on ARM64 macOS. TODO-715 retains GF16 PCLMUL mathematics. TODO-860 now closes the FEC arithmetic boundaries: `InterleavedEncoder::params()` reports the shape its lanes represent instead of a floored request, interleaved repair ordinals are bounded by `MAX_REPAIR_ORDINAL` so they cannot shift out of the `u64` identity and alias, non-finite redundancy falls back to systematic-only instead of the maximum repair budget, the Fountain window offset uses a checked subtraction plus a `source_count` bound, Fountain source ids convert with `usize::try_from` so a 32-bit truncation cannot alias a valid index, and Decoder16 checks its `words * 2` product. The Wiedemann mathematics remain TODO-690. TODO-816 and TODO-676 close the active AMX kernel, dispatch, and concurrent ownership boundaries; TODO-817-TODO-819 retain detector, proof, and profile/documentation boundaries. No implementation or runtime verification was performed for TODO-686 itself.
...
- **Telemetry and scope:** `WIEDEMANN_SCALAR_OPS` records the active solver path. `WIEDEMANN_AMX_OPS` and `WIEDEMANN_AMX_SCRATCH_ALLOCS` remain reserved and zero until a future native AMX GF(256) backend passes the TODO-818 compiler/runtime lane. TODO-676 closes tile ownership and the broader dispatch/race boundary; TODO-817 retains detector execution bounds; TODO-819 closes profile/documentation mapping; TODO-690 retains Wiedemann equation correctness.
...
- **Detector boundary:** `FeatureDetector` uses in-process `is_x86_feature_detected!("amx-tile")`, `is_x86_feature_detected!("amx-int8")`, and `is_x86_feature_detected!("amx-bf16")` on x86_64. The former PATH-resolved `Command::new("cpuid").output()` dependency and textual substring parser are removed.
- **Evidence separation:** `AmxCapability` records CPU instruction support, `os_tile_state_permitted`, compiler target-feature flags, and `product_dispatch_eligible` separately. The current production contract reports no OS permission proof and no verified backend, so product eligibility is false even when CPU instruction bits are present.
- **Fallback and diagnostics:** Missing or unavailable OS/backend evidence is represented explicitly as `None` or `false`, and the detector logs the complete AMX contract. Apple Silicon's existing `CpuFeature::APPLE_AMX` label remains a separate platform capability and is not treated as Intel AMX proof.
- **Guardrail and tests:** `amx_detector_process_free` fails closed if a `cpuid` dependency reappears. The focused detector tests pass `2/2`; strict library Clippy, format, and diff checks pass. The full runtime-guardrail aggregate still reports three unrelated critical failures and one existing warning, so that aggregate is not a clean project gate.
...
- **Pushed-source proof:** The isolated Omega checkout at exact commit `5c635a9c7ecd37d3e904457e22789569f366cea7` passes the complete library suite `2409/2409` and strict library Clippy with `-D warnings`; direct remote source guards pass for the removed detector helper and legacy AMX symbols. Remote rustfmt is unavailable because `cargo-fmt` is missing from toolchain `1.97.1-aarch64-unknown-linux-gnu`.
- **Ownership:** TODO-676 closes the final compiled/runtime/OS dispatch intersection and tile ownership for the current scalar fail-closed product path. TODO-817 closes detector execution bounds; TODO-818 retains native AMX arithmetic/build/runtime proof for any future backend; TODO-819 closes profile/documentation mapping; TODO-760 remains the Cargo-feature contract owner.
...
- **Other CI failures:** The historical Windows core check failed at `src/privilege/drop.rs:676` because `CurrentIds` was unavailable; TODO-849 now defines the type on every target, while the Windows compile gate itself remains unexecuted on this ARM64 macOS host. Strict macOS Clippy still has the unrelated diagnostics at `src/stealth/tls_cover.rs:393` plus `src/implementations/client/dns_runtime.rs:279,289` (TODO-709/TODO-752/TODO-787). These boundaries are not represented as local green native evidence.
...
- **Comprehensive audit evidence:** The revision `1b91a55` run is a historical snapshot with exit `1`, `4` Critical classifications, and `7` Warnings; its `1` `unwrap`, `20` `expect`, and `1` `panic` strict diagnostics are not the current source result. The 2026-08-08 run records separate production, benchmark, and all-feature strict lanes as `PASS`; its remaining failures are the unrelated web-admin publish contract, Linux-only all-target integration compilation on macOS, four pre-existing runtime-guardrail findings, and two unavailable native lanes. TODO-730 owns result integrity and heuristic classification, TODO-676 retains the dispatch/runtime boundary, TODO-816 now closes the active kernel-semantics finding, TODO-817 owns detector process bounds, TODO-818 owns the AMX proof lane, TODO-819 owns profile/documentation truth, and TODO-803 owns the two redundant clones.
...
## Deep Audit Reconciliation (2026-08-07, FEC SIMD contract implementation)

- **Scope:** TODO-855 converts the audited FEC SIMD inventory into one implementation contract across GF8/GF16 wrappers, the shared `SimdFeatureMatrix`, `dispatch_bitslice()`, connection-level FEC telemetry, tests, and runtime guardrails.
- **Implementation:** private FEC target-feature helpers now document ISA and storage preconditions, bound independent lengths in release builds, and receive equal-length slices from the safe GF16 dispatcher. AVX-512 VBMI2, AVX-512 VBMI, AVX2, SSE2, SVE2, NEON, and scalar levels are selected from the same matrix-derived owner; NEON no longer requires unrelated AES/PMULL features. GF16 PCLMUL and compile-time SVE2 paths use the shared runtime matrix, and FEC safety-contract/unsupported-ISA scans include `src/fec` and its GF16 test.
- **Evidence boundary:** Focused matrix-negative, threshold, unequal-length, and tail tests plus the SIMD self-check are part of this owner. Native x86/Windows/SVE2 execution, sanitizer/Miri proof, complete per-backend differential coverage, and GF16 PCLMUL polynomial proof remain TODO-859/TODO-715 boundaries until their owners run.

## Deep Audit Reconciliation (2026-08-07, FEC current-source reconciliation)

- **Scope:** TODO-686 remains complete as a read-only audit across every current FEC source and test module, all FEC `unsafe` sites and direct callers, public decoder/matrix/wire/Fountain boundaries, feature gates, malformed-input tests, fuzz and shell/benchmark/netns proof, documentation, related owners, and history.
- **Source closures:** TODO-832 closes production pooled-buffer ownership, oversized allocation, exact-length propagation, and decoder teardown. TODO-834 through TODO-836 reconcile shared SIMD intersections, release-safe slice/dimension guardrails, and local unsafe-boundary proof. TODO-676 and TODO-816 through TODO-819 reconcile the active AMX path as a checked scalar fallback while retaining the native proof lane. Current Fountain source checks reject empty/oversized/out-of-range source-index sets and make `k == 0` progress finite; direct constructor/resource, fixed symbol-size, and product-adapter equality contracts are now closed under TODO-857.
- **Current evidence:** The validated product wire path has real profile, epoch, length, and repair-identity checks. Direct `FecPacket` compatibility, decoder, matrix, wire-helper, Fountain, and FEC configuration/feedback boundaries are now fail-closed with focused malformed/boundary tests. Existing tests and scripts prove selected recovery, liveness, bounded telemetry, and live Linux transitions, but native feature intersections and platform-negative proof remain unclaimed.
- **Open remediation:** TODO-634 retains Fountain storage/queue growth, TODO-636 peeling complexity, TODO-637 Wiedemann scratch reuse, TODO-715 retains GF16 PCLMUL mathematics. TODO-690 is closed: `solve_wiedemann_system()` now performs a real GF(256) solve. Berlekamp-Massey reduced with the AES polynomial `0x11B` while the FEC codec field is `0x11D`, so the minimal polynomial was computed in a different field from the system it described; its shift bookkeeping was also wrong, so it did not annihilate its own input. Both are fixed in the single scalar implementation that every dispatched lane delegates to. The solver builds the Krylov sequence from the right-hand side, recovers `x = lambda[d]^-1 * XOR_j lambda[j] A^(d-1-j) b` by Horner back-substitution, retries a bounded number of independent deterministic projectors because scalar Wiedemann can fail on a full-rank system for one projector, and verifies `A x = b` before returning. The caller's per-equation validation is retained as the second gate. TODO-860 now closes the FEC arithmetic boundaries: `InterleavedEncoder::params()` reports the shape its lanes represent instead of a floored request, interleaved repair ordinals are bounded by `MAX_REPAIR_ORDINAL` so they cannot shift out of the `u64` identity and alias, non-finite redundancy falls back to systematic-only instead of the maximum repair budget, the Fountain window offset uses a checked subtraction plus a `source_count` bound, Fountain source ids convert with `usize::try_from` so a 32-bit truncation cannot alias a valid index, and Decoder16 checks its `words * 2` product. The Wiedemann mathematics remain TODO-690. Shared transport and environment owners remain in force.
- **Documentation truth:** The accepted product Fountain maximum is 128 sources and the example uses 128. The steady-state GF16 inverse table is 65,536 `u16` entries, or 128 KiB; temporary exponent/logarithm build allocations are separate. Production FEC paths use live `PooledBlock` guards, while `FecPacket::new()` is a crate-internal compatibility constructor for raw/foreign test inputs and is no longer a public panic surface.
- **Evidence boundary:** Current source and owner reconciliation is recorded in `docs/todo/todo-686-fec-unsafe-audit.md`. It does not claim remaining remediation, native ISA, sanitizer, Miri, privileged, local runtime, or external gates are closed. No production implementation, build, test, native probe, or privileged network run was performed for TODO-686.
...
- **Scope boundary:** No UI or Omega state was changed. The TODO-633 KDF implementation is local-only until its full matrix and native/external proof gates close; TODO-758 now owns hosted nightly sanitizer execution after local metadata and inventory closure, and the broader project audit remains open under its existing task owners.
...
- The final `verify-audit-completeness.sh` refresh enumerates the complete local register/detail/archive/Git scope and passes structural integrity with `graphify=BLOCKED`; stale or missing Graphify manifests fail closed rather than being promoted to green. TODO-759 owns the remaining provenance boundary.
- The current strict `audit-all-comprehensive.sh` run at `/tmp/quicfuscate-audit-current-20260806.8UshkJ` completes all `38` result objects and returns `FAIL`: `31 PASS`, `3 FAIL`, and `2 UNAVAILABLE`, plus the aggregate fail record. The failed checks are strict runtime Clippy, all-target quality Clippy, and runtime guardrails. The unavailable checks are PowerShell dialect parsing and the ARM64-host AMX proof lane. The run records `5` critical classifications and `10` warnings, `233` Rust production files, `858` unsafe locations, `4` leak-pattern locations, zero secret findings, `1,130` SIMD feature conditionals, and a `63%` source-marker metric explicitly not equivalent to executed coverage.
- This is current audit evidence, not a completion claim. Native Linux/Windows, Graphify freshness and semantic relationships, frontend browser execution, Omega attribution, privileged networking, and other external proof boundaries remain owned by their named TODOs.
- Historical post-staging validator snapshot (2026-08-04): the tracked/ignored scope was `956`/`28,123` with zero non-ignored untracked paths and the register validator passing. The current live result is recorded above and fails closed on stale Graphify evidence.
...
- **Terminal close has send priority:** before the control queue is flushed, any queued CONNECTION_CLOSE or APPLICATION_CLOSE is hoisted to the front. Close frames are not ack-eliciting, so once in front they are emitted even under congestion bypass, where the flush otherwise stops at the first ack-eliciting frame. A PING or MAX_DATA queued earlier could previously hide a later close until congestion reopened or the idle timeout fired. Hoisting preserves the relative order of the remaining frames, is idempotent when a close is already in front, and does not change which frames congestion bypass is allowed to emit.
- **Terminal timeout has one recovery owner:** `Connection::on_timeout()` calls `Recovery::discard_all_spaces()`, which retires every packet-number space at once: sent maps, retained byte accounting, time-threshold timers, PTO bases, largest-acked marks, PTO backoff, and the congestion controller's in-flight accounting. The connection previously zeroed only its own `bytes_in_flight` while the three recovery spaces kept their packets and timers, so a later poll could observe state belonging to a connection that already reported itself closed. The discard is idempotent, so repeated timeouts neither double-count a loss nor resurrect state, and nothing remains that could emit a loss callback, probe, or retransmission afterwards.
- **Recovery scan and retention bounds:** `detect_lost_packets()` stops at the first survivor instead of materializing every retained packet number up to `largest_acked` and sorting the result. Packet numbers ascend through the map and send times are non-decreasing in packet number, so the loss set is a contiguous prefix and the cost is `O(log n + k)` in the number of losses rather than in the in-flight window. Timer re-arming takes the first usable deadline for the same reason. Each packet-number space additionally enforces explicit retention budgets of `16,384` packets and `64 MiB`, evicting the oldest record first so current traffic stays tracked, and every eviction increments `RECOVERY_SENT_RETENTION_EVICTIONS`. A non-zero value means loss detection lost visibility of the oldest unacknowledged packets in that space.
- **ACK emission is transactional:** `PktNumSpace` separates inspection from commit. `peek_ack_at()` reports the `(ack_delay, ranges)` a frame would carry without touching `ack_elicited`, `recvd_since_ack`, or `ack_deadline`; `commit_ack_at()` clears them and records the send time, and is called only after the frame bytes are in the packet. Both the application space in `impl_recv.rs` and the Initial/Handshake space in `impl_send.rs` use that order, so an undersized output buffer or a `frames::to_bytes()` failure leaves the ACK pending with its deadline and ranges intact instead of discarding an ACK that no further inbound packet is guaranteed to re-trigger. `take_ack_at()` remains as peek-then-commit for callers that cannot fail in between.
- **Receive flow-control accounting:** connection and stream byte counters advance by the newly covered byte union, not by raw payload length. `newly_covered_bytes()` measures an incoming `[offset, end)` range against the stream's contiguous delivered prefix and every buffered out-of-order fragment, so a duplicate retransmission contributes zero and a partial overlap contributes only its extension. Counting whole payloads previously let reordering or ordinary loss recovery exhaust the connection window and drive MAX_DATA and MAX_STREAM_DATA thresholds without a single new byte being delivered.
...
- **Configuration wiring:** `src/memory_lock.rs::MemoryLockPolicy` maps `SecurityConfig.lock_memory` and `lock_blocks` for standalone and embedded server startup. The policy resets qftls process-coverage state, applies process locking before embedded identity construction, and preserves the standalone Linux deferred process-lock boundary; qftls receives the configured `lock_memory` value for individual TLS-key ownership. Standalone startup reports successful `MCL_FUTURE` coverage to qftls; finite `MCL_CURRENT` protection leaves later key allocations on the individual-lock path.
...
- **Scope boundary:** CPU-profile/BMI2 feature dispatch and Unix native source contracts are reconciled by TODO-843 and TODO-845; Wintun, WFP, and negative/native platform proof remain TODO-846 through TODO-848-owned boundaries; TODO-844 owns the implemented shared generic TUN result contract.
...
## Implementation Reconciliation (2026-08-05, Graphify relationship evidence contract)
...
- **Detection scope:** The current local run at source revision `f09fc454c24aa6dcc1047d1c3aebf48d27a8cbc3` enumerates 750 files and 1,320,086 words: 697 code, 29 documents, 24 images, and 3 sensitive files accounted for as redacted. The detected code surface contains 334 Rust files and 146 shell/PowerShell scripts. Generated, dependency, agent-state, audit-output, and local-sensitive Git paths remain counted by the machine-readable Git scope without being treated as source inputs.
...
- **Current result:** The latest local run produced 15,370 raw nodes and 42,508 raw edges, 13,699 normalized nodes and 42,508 normalized edges, zero normalized dangling edges, stable IDs, and relative source paths. Raw evidence still contains 1,486 dangling edges; normalized evidence retains 385 ambiguous and 1,462 unresolved edge statuses. Six detected files have no AST nodes, so the extraction contract is `BLOCKED` rather than a relationship pass. Semantic extraction remains `UNAVAILABLE` for 53 uncached content files.
...
- **Scope boundary:** This reconciliation fixes source comments, operator logging, telemetry labeling, and canonical documentation only. It does not introduce an AMX backend, change profile variant names, or claim native Intel/Apple AMX execution. TODO-676 closes the broader dispatch/tile ownership for the inactive production backend; TODO-818 retains the build/runtime proof lane for any future native backend; TODO-690 retains Wiedemann equation correctness.

## Implementation Reconciliation (2026-08-07, TODO-676 AMX dispatch and tile-state boundary)

- **Static-state closure:** The former `static mut TILE_CONFIG`, raw AMX kernels, scalar-after-tile-load/store path, and compile-time-absent decoder branch are absent from the active source. The production path has no AMX tile configuration or tile-register state to share, release, or race.
- **Eligibility closure:** `AmxCapability::from_signals()` intersects CPU AMX-TILE/INT8 support, compiler target features, platform OS tile-state permission, and `VERIFIED_BACKEND` before setting `product_dispatch_eligible`. The active FEC caller does not dispatch to an AMX branch; it always uses checked scalar GF(256) SpMV.
- **Concurrency and telemetry:** Rayon workers own separate `WiedemannScratch` state, so concurrent solver calls do not touch shared AMX state. `WIEDEMANN_SCALAR_OPS` records the active path, while AMX operation and scratch counters remain zero. No native AMX arithmetic or supported-host execution is claimed.
- **Proof:** The AMX contract checker passes; the current source passes the focused Wiedemann `6/6` and AMX capability/scalar-concurrency `3/3` tests plus format/diff checks. The repository-owned x86 proof lane remains explicit and unavailable on this ARM64 macOS host; a local attempt was stopped at the configured free-disk safety floor, so no native AMX result is inferred.
- **Ownership:** TODO-676 closes the current compiled/runtime/OS intersection, scalar-versus-AMX dispatch, and concurrent tile-ownership boundary. TODO-817 closes detector execution, TODO-819 closes profile/documentation truth, TODO-818 retains future native backend proof, and TODO-690 retains Wiedemann equation correctness.

## Implementation Reconciliation (2026-08-05, TODO-820 protocol clock contract)

- **Clock owner:** `src/time_source.rs::ProtocolClock` now clones one `TimeSource` handle for each protocol connection or runtime. `elapsed_since()` saturates backwards manual-clock movement, while `SystemTime` remains a separate wall-clock domain.
- **Transport propagation:** `transport::Connection` owns the clock and passes it to packet-number spaces, Recovery, BBR2/BBR3/CUBIC construction, path validation, NAT discovery, anti-replay cleanup, ACK scheduling, send/receive timestamps, migration, and idle accounting. `CongestionController::update_rtt_at()` preserves the measured sample timestamp through Recovery, BBR2/BBR3, and StealthShaper.
- **Higher layers:** The same owner reaches H3 push scheduling, Core telemetry and diagnostics, engine uptime, Stealth manager/rate-choker/flow/probe/cover/chaff state, and qftls handshake readiness, rebuild, profile jitter, and duration measurements. Compatibility constructors retain an explicit `ProtocolClock::default()` fallback; active connection constructors use `new_with_role_and_clock()`, `connect_with_clock()`, and `accept_with_clock()`.
- **Static boundary:** The final inventory has no production ambient monotonic read in the scoped protocol graph. Test fixtures retain local clocks. qftls session fixtures and H3/Stealth epoch values retain their separate wall-clock owners. The engine handshake Condvar still uses the native OS `Instant` domain at `src/engine/engine.rs:1235` because `ClientRuntime::wait_handshake()` accepts that domain; TODO-822 owns its runtime-clock boundary.
- **Proof:** Local ARM64 macOS `cargo check --locked --lib`, the complete library suite `2390/2390`, the focused owned-clock ACK test `1/1`, the BBR3 filter `23/23`, and strict library Clippy with `--features rust-tests -- -D warnings` pass. `cargo fmt --all -- --check` and `git diff --check` pass. A generated-target cleanup removed 17,092 files and 5.7 GiB; `scripts/out` was not modified.
- **Ownership:** TODO-821 owns server/client stateful timing, TODO-822 owns Tokio and OS watchdog domains, TODO-823 owns wall-clock provenance, TODO-824 owns broader injection and test isolation, and TODO-825 owns frontend/browser clocks. TODO-640, TODO-768, TODO-584, and TODO-588 retain their narrower timestamp owners.

## Implementation Reconciliation (2026-08-06, TODO-821 server/client state clock ownership)

- **Runtime owner:** `QuicFuscateEngine::clock` is passed into `ClientRuntime::new_with_clock()` and the standalone server initialization path. `ServerRuntime` retains that handle and propagates it to `LiveServerState`, shared server domains, metrics, admin state, accepted connections, and standalone admin HTTP.
- **State owners:** Server rate limits, quotas, blacklist state, sessions, login/session stores, DNS admission, TUN pending queues, client snapshots, qkey post-handshake state, client quality tracking, backend connection state, and I/O driver accounting now use explicit `ProtocolClock` handles or explicit timestamp parameters. Compatibility constructors retain a documented default-clock wrapper only at API boundaries.
- **Call-chain proof target:** Accepted server connections use the runtime clock through `LiveClientBuildRequest` and `create_live_server_connection_with_runtime_and_clock()`. Client connections, DNS admission, subsystem initialization, bandwidth tracking, metrics, and connection uptime use the same client runtime clock.
- **Clock-domain boundary:** Native `std::time::Instant` Condvar waits, systemd watchdog scheduling, Tokio request/deadline timers, DNS forwarding deadlines in `src/dns/mod.rs`, and other runtime/OS deadline decisions remain explicit TODO-822 boundaries. Epoch and profile/persistence timestamps remain TODO-823/TODO-658/TODO-662 boundaries.
- **Local proof:** On ARM64 macOS, `CARGO_BUILD_JOBS=2 CARGO_INCREMENTAL=0 cargo check --locked --lib`, `cargo check --locked --tests --features rust-tests`, the complete library suite (`2398/2398`), the focused client-backend clock test (`1/1`), strict library Clippy with `--features rust-tests -- -D warnings`, `cargo fmt --all -- --check`, and `git diff --check` pass after the final constructor cleanup. The existing no-feature check emits two unchanged dead-code warnings outside this change. External runtime proof remains open and is not claimed.
- **Omega proof boundary:** The clean isolated checkout `/home/ubuntu/CODE/QuicFuscate-verify-dc4de71` was fetched and pinned to exact commit `ff0a2d3b50d55977457e6c654036af770f0f941c`. Its locked test-target check and strict library Clippy pass. Full Omega library-test and focused-test attempts were terminated by the remote host before any result summary, so no remote test pass is claimed. Dirty `SOFTWARE/QuicFuscate` and `CODE/QuicFuscate` checkouts were not modified.

## Implementation Reconciliation (2026-08-06, TODO-822 runtime clock-domain boundary)

- **Injected product monotonic:** `ProtocolClock` remains the owner for transport, Core, H3, stealth, server/client state, QUIC release/recovery deadlines, client I/O protocol deadlines, and server product housekeeping. `src/main_parts/runtime.rs::client_housekeeping_delay()` now reads that connection clock and crosses into Tokio only as a `Duration`.
- **Tokio runtime:** Client heartbeat/stat scheduling, server accept and admin HTTP deadlines, stealth worker joins, DNS intercept worker reaping, and DoH endpoint fallback use `tokio::time::Instant` and Tokio timeout APIs. These deadlines stay live while product time is manually controlled.
- **OS/native runtime:** `GracefulShutdown` drain timing and systemd watchdog scheduling remain native monotonic boundaries. The engine handshake Condvar, Wintun activation, native adapter waits, and blocking UDP DNS sockets retain their native deadline contracts. Tokio receives only a bounded duration when it joins a blocking DNS worker, so no implicit `std::Instant` to Tokio `Instant` conversion remains.
- **Wall and diagnostic domains:** `SystemTime` epoch/profile/persistence values remain separate wall-clock owners. Runtime branch/phase measurements, instrumentation uptime, harness timing, FEC solve/telemetry timestamps, and benchmark samples remain live native diagnostic measurements. Client heartbeat diagnostics now use the product clock for QUIC deadline remaining values and the native clock only for diagnostic elapsed values.
- **Cross-domain proof:** The traced direct mixed-clock comparisons in client housekeeping, client deadline diagnostics, server drain, stealth shutdown, DNS worker shutdown, DoH fallback, and blocking-DNS async cancellation were removed or made explicit as duration-only boundaries. Engine handshake, systemd, Wintun/native, accept/admin, and admin HTTP paths remain intentionally owned by their respective runtime domains.
- **Local proof:** `cargo fmt --all -- --check`, `git diff --check`, locked library checking, locked test-target checking with `rust-tests`, and strict library Clippy with `--features rust-tests -- -D warnings` pass on ARM64 macOS. The Cargo-built library test binary exits successfully with `2399/2399` tests passed, including the new live-drain-clock regression. The no-feature check retains only the two unchanged dead-code warnings in `transport/recovery.rs` and `implementations/server/metrics.rs`.
- **Build-space anomaly:** A later redundant Cargo relink attempt with `-- --test-threads=2` failed at the macOS linker with `ld: write() failed, errno=28` after the generated target exceeded the available disk budget. It produced no test failure; the already-built test binary had passed all 2399 tests, and the failed generated target was removed with `cargo clean`. `scripts/out` was not modified.
- **Omega proof:** Clean isolated `/home/ubuntu/CODE/QuicFuscate-verify-dc4de71` is pinned to exact commit `1d7d56f2e039cfcf2c500fc5948c6f4933273aa7`. Locked test-target checking and strict library Clippy pass; Omega reports one unchanged `RoutingManager::pf_rules` dead-code warning under the test-target check. The full Omega library-test attempt exits `255` when the SSH host closes the connection before any test summary, so no remote test pass is claimed. Dirty `/home/ubuntu/CODE/QuicFuscate` and `/home/ubuntu/SOFTWARE/QuicFuscate` remain untouched.
...
- **Wall-clock boundary note:** TODO-823 owns checked wall-clock provenance for server security/expiry, client profile metadata, logs, audit entries, and Tauri persistence. PKI, H3 cookies, Reality capture, Rust test isolation, Stealth escalation, and frontend/browser clocks retain their narrower owners.

## Implementation Reconciliation (2026-08-06, TODO-823 wall-clock provenance)

- **Shared conversion contract:** `src/time_source.rs` is the single checked conversion boundary for Unix seconds and milliseconds. `WallClockError` distinguishes pre-epoch input from millisecond overflow and calendar conversion overflow; Unix epoch zero remains a valid value only for an actual `UNIX_EPOCH` input.
- **Server security and expiry:** Blacklist cache persistence, quota period/reset, retry-token issuance/validation, qkey registry metadata, revocation retention, and admin-auth persistence now capture the configured wall-clock source and reject conversion failure. Security and expiry paths return typed errors or fail closed instead of treating a clock failure as epoch zero.
- **Quota admission:** A wall-clock conversion failure produces the explicit `clock_unavailable` bandwidth outcome, increments dedicated metrics, and drops the affected TUN decision without accounting bytes against a fabricated period.
- **User-facing records:** `AdminLogBuffer` and the Tauri log buffer preserve explicit timestamp validity and error metadata. Audit and RFC3339 logging reject pre-epoch conversion rather than formatting a valid-looking epoch-zero timestamp.
- **Tauri persisted-state limits (TODO-742):** the `MAX_*_CHARS` constants mean Unicode scalar values and are now measured and applied as such. They were compared against `String::len()`, a byte count, and the resulting byte index was handed to `String::truncate`, which panics when it falls inside a code point, so non-English or hostile persisted state could abort the host during startup sanitization. Only display fields are truncated: `name` and `location` are shortened at a scalar boundary, while an over-long `id`, `remote`, or `sni` drops the tunnel and an over-long `qkey` is cleared, because a shortened identifier, endpoint, or credential silently means something different rather than the same thing abbreviated. Values exactly at a limit are accepted unchanged.
- **Client and desktop state:** Profile connection metadata propagates wall-clock errors. Tauri persisted-state sanitization fails when a missing creation timestamp cannot be filled, while test-only temporary paths use deterministic timestamp/nonce helpers and no longer depend on the production wall-clock helper.
- **Consistency capture:** Quota reset computes the new period before mutating usage, and the per-client reset captures one wall value for both directional quotas. qkey, revocation, profile, persistence, and audit failures do not silently mutate the protected state with a fallback timestamp.
- **Deterministic coverage:** Focused negative tests cover the shared conversion, blacklist cache, quota, retry token, qkey registry, revocation, admin log, admin-auth persistence, profile metadata, audit timestamp, and Tauri timestamp boundaries. Local root library tests pass `2410/2410`, Tauri host tests pass `42/42`, and the exact pushed Omega library passes `2434/2434`; detailed commands and warnings are recorded in the TODO detail.
- **Owner boundaries:** PKI remains TODO-656; H3 cookies remain TODO-640; profile identity remains TODO-658; atomic profile persistence remains TODO-662; audit durability remains TODO-675; sensitive file modes remain TODO-671; logger rotation/lifecycle remains TODO-672/TODO-812; Reality capture/cache timestamps remain TODO-584; Stealth escalation timestamp history remains TODO-584; broader injection/test isolation remains TODO-824; and frontend/browser clocks remain TODO-825. This reconciliation does not claim those narrower contracts migrated.

## Implementation Reconciliation (2026-08-06, TODO-824 time-source isolation and inventory)

- **Clock contract:** `src/time_source.rs::TimeSource` now states the production non-decreasing monotonic contract, permitted backwards movement for manual clocks, checked monotonic deadline overflow, independent wall-clock error handling, and the non-equivalence of monotonic and wall-clock domains. Owners must capture one value and pass it through an operation when a coherent sample is required.
- **Test isolation:** `install_for_test()` uses a thread-local nested override instead of replacing the process-global source. Explicit `ProtocolClock` owners remain independent of later overrides; spawned threads see the production source unless the owner passes its explicit clock. Drop restores the prior override during normal scope exit and unwinding.
- **Negative coverage:** The time-source tests cover explicit-owner immunity to an installed override, cross-thread visibility isolation, restoration after a caught panic, backwards elapsed saturation, independent wall-clock values, and `Instant` deadline overflow rejection.
- **Machine-checkable inventory:** `scripts/audits/verify-time-source-inventory.sh` reuses the existing Rust lexer/masking helper and scans all tracked Rust, browser, and shell clock patterns. The completion snapshot before the frontend follow-through contained 957 locations: production 153, test 387, benchmark 58, probe 31, script 262, browser 63, and archived 3. Every location had an explicit scope, clock domain, owner, path, line, and evidence; the fail-closed result reported zero unclassified entries.
- **Owner classification:** TODO-820 and TODO-821 remain the protocol and server/client owner classes; TODO-822 remains the Tokio/native runtime boundary; TODO-823 remains the checked wall-clock provenance boundary; TODO-584, TODO-640, and TODO-656 retain their narrower Reality/Stealth, H3, and PKI owners; TODO-825 retains browser clocks. Benchmarks, probes, scripts, tests, and `archive/stealth/masque_manager.rs` are classified as non-production evidence rather than silently promoted into runtime ownership.
- **Verification boundary:** The inventory gate passes on the current source. The focused time-source filter passes 7/7 and the complete root-library suite passes 2413/2413 with serial test execution. Locked test-target checking, default library checking, strict feature Clippy, format/diff hygiene, and the Tauri host suite at 42/42 also pass. The pushed commit `b2ef7719b94c860446d375486f858135e12bec32` is verified in the isolated Omega checkout: inventory 957/0, locked test-target checking `RC=0`, strict feature Clippy `RC=0`, and full root-library tests `2437/2437`; the only warning is the unchanged `RoutingManager::pf_rules` dead-code warning.

## Audit Reconciliation (2026-08-06, TODO-825 frontend and browser clock boundary)

- **Audit scope:** The complete active browser/Tauri timestamp and scheduling surface was read across `apps/tauri/src-tauri`, `apps/svelte-desktop`, `apps/svelte-admin`, `packages/ui`, frontend unit setups, and relevant Playwright fixtures. The protected UI implementation boundary remained in force, so no Svelte component, frontend test, Tauri command, API implementation, or visual surface was changed.
- **Machine inventory:** The pre-remediation audit baseline reports 962 locations with zero unclassified entries. The active browser scope in that baseline is 65 locations: 10 wall-clock reads, 2 monotonic reads, and 53 scheduler/animation locations. Frontend tests account for 390 classified locations. The earlier 957-location TODO-824 snapshot missed five `Date.now()` expressions inside template-literal interpolations; `mask_javascript()` now preserves `${...}` expressions and `scripts/tests/audits/test-time-source-inventory.sh` covers the regression.
- **Boundary ownership:** Tauri persistence owns checked Unix milliseconds for `createdAt` and log `tsMs`; the desktop bridge validates those values, preserves invalid-state metadata, and never repairs backend-owned records with browser time. Admin QKey metadata is validated as Unix seconds and admin log values as Unix milliseconds, with invalid metadata remaining structurally visible. TODO-868 owns the unit, provenance, validation, and invalid-state contract.
- **Elapsed ownership:** Desktop and admin throughput use the shared monotonic `performance.now()` evaluator with a bounded 5,000 ms sample gap, counter rollback handling, and visibility rebasing. Paste suppression uses a bounded timer-owned window, and chart/animation consumers reset or cancel on hidden transitions. TODO-869 owns the verified monotonic elapsed and visibility behavior.
- **Lifecycle ownership:** Poll intervals, selected RAFs, ResizeObserver cleanup, copy feedback, QKey reconciliation, delayed dialog actions, focus callbacks, error feedback, root persistence debounce, CountrySelect typeahead/focus, ConfirmDialog, toast, and ripple fallback handles now have explicit owners. TODO-870 owns the completed lifecycle implementation and its regression/browser proof.
- **Identifier ownership:** Admin `createCsrfNonce()` uses `crypto.randomUUID()` or 32 secure random bytes plus a process-local sequence and returns an explicit unavailable result when no secure browser source exists. Shared toast IDs and Sparkline gradient IDs use monotonic module-owned sequences rather than wall-clock or `Math.random()` values. Frontend UUID test doubles use explicit deterministic sequence values. TODO-871 owns the cryptographic/fail-closed and collision contract; TODO-872 owns the deterministic wall/monotonic/RAF/visibility test harness and suspension matrix.
- **Negative search:** No active app/package/frontend-test source uses zero-argument `new Date()`, `performance.timeOrigin`, `performance.mark`, `performance.measure`, `requestIdleCallback`, `cancelIdleCallback`, `queueMicrotask`, `AbortSignal.timeout`, or `setImmediate`. `new Date(value)` display consumers remain boundary consumers under TODO-868.
- **Status:** TODO-825 is complete as the parent audit and follow-through owner. TODO-868 through TODO-872 are implemented and verified in their reconciliation sections; their frontend checks, builds, Chromium preflight, E2E, and scoped unit/regression gates close the implementation boundary without changing visual UI composition.

## Audit Reconciliation (2026-08-07, TODO-677 current time-source closure)

- **Current tracked-source result:** `bash scripts/audits/verify-time-source-inventory.sh` reports `919` locations and `0` unclassified. Scope counts are production `156`, tests `384`, benchmarks `58`, probes `31`, scripts `262`, browser `25`, and archived `3`. The `962`-location TODO-825 report and `957`-location TODO-824 report are historical pre-follow-through snapshots, not current-source contradictions.
- **Ownership closure:** TODO-820 owns the injected protocol graph, TODO-821 server/client state, TODO-822 Tokio and OS/native runtime domains, TODO-823 wall-clock provenance, TODO-824 Rust test isolation and inventory semantics, and TODO-825 browser/Tauri clock boundaries. TODO-868 through TODO-872 close the frontend timestamp, elapsed, lifecycle, identity, and deterministic-test follow-through. TODO-584, TODO-640, TODO-656, TODO-658, TODO-662, TODO-671, TODO-675, TODO-672/TODO-812, TODO-768, and TODO-588 retain their narrower contracts.
- **Explicit runtime exceptions:** `src/firewall/cleanup.rs:185` retains a synchronous retry-policy delay under `native-cleanup-runtime`; `src/stealth/parts/tls_cover_provider.rs:379` retains deliberate synchronous TLS-cover jitter under `qftls-stealth-jitter`. These are runtime delays, not clock reads, and are not mechanically replaced by `crate::time_source`.
- **Classification integrity:** The current inventory includes direct and implicit Rust/Tokio/browser monotonic and wall-clock patterns plus script delays, probes, benchmarks, tests, and retired archive evidence. The inventory test passes, and no unclassified location remains. No unavailable native Linux/Windows/AMX/Miri/Omega result is inferred from the local ARM64 macOS gates.
- **Graph evidence:** The post-push refresh at `scripts/out/audits/graphify-20260807T012815Z/graphify-evidence.json` is explicitly `BLOCKED` for unavailable semantic extraction, incomplete/unsupported coverage, and raw/normalized/legacy relationship limitations. It is not promoted to a green audit result.
- **Closure:** TODO-677 is complete as the umbrella audit. The parent audit changed no product or UI implementation; the owning successor tasks contain the actual remediation and their exact verification boundaries.

## Frontend and Tauri Timestamp Boundary (2026-08-06, TODO-868)

- `packages/time/index.ts` owns the dependency-free branded timestamp contract. `UnixMilliseconds<source>` is used for Tauri persisted tunnels, Tauri log lines, and browser-created desktop tunnels; `UnixSeconds<source>` is used for admin QKey metadata. Admin log timestamps use branded Unix milliseconds.
- `parseUnixMilliseconds()` and `parseUnixSeconds()` reject non-numeric, non-finite, fractional, unsafe, pre-epoch, zero, out-of-Date-range, and clearly unit-mismatched values. `unixSecondsToMilliseconds()` and `unixMillisecondsToDate()` are the only shared conversion/display boundaries.
- Tauri remains the owner of persisted-tunnel repair and log timestamp validity. The desktop bridge validates `createdAt` as Tauri Unix milliseconds and skips invalid persisted records with an explicit error; it does not invent a browser timestamp for backend-owned data. Newly created desktop tunnels use an explicitly browser-owned `desktop-created` timestamp.
- Tauri log `timestampValid` and `timestampError` metadata reaches the desktop log model. Invalid timestamps retain the log message but render as `Time unavailable`, never as epoch zero or current time.
- Admin QKey and log responses are parsed at their API boundaries. Invalid QKey metadata keeps a structurally valid row visible with unavailable metadata; invalid log timestamps keep the message visible with unavailable time. The admin API uses `timestamp_valid`/`timestamp_error`, while the Tauri bridge uses camelCase serialization.
- The contract changes timestamp validation and provenance only. No visual surface, style, asset, or layout is changed. Verification passes: Admin and Desktop checks report 0 errors and 0 warnings; Admin unit tests pass 292/292, Desktop 389/389, and shared UI 82/82; both production builds pass; Playwright Chromium 1.58.2 preflight passes; Admin E2E passes 70/70 and isolated Desktop E2E passes 23/23; the locked Tauri host suite passes 42/42. The first parallel Desktop E2E run had one DOM-detach timeout and the isolated rerun passed 23/23.

## Browser Elapsed and Visibility Contract (2026-08-06, TODO-869)

- `packages/time/index.ts` owns the shared byte-rate policy. Browser samples use `performance.now()` through `readBrowserMonotonicMilliseconds()`; no wall-clock fallback is allowed. A sample is accepted only when elapsed time is positive and at most 5,000 ms, counters are finite and non-decreasing, and the calculated rates remain finite.
- A first sample, clock regression, renderer gap above 5,000 ms, invalid clock/counter, or counter rollback rebases on the current counters and emits zero rates. The same policy is used by the desktop Tauri bridge and the admin Dashboard.
- Desktop throughput samples are reset on every document visibility transition and while hidden. Admin Dashboard samples and history are reset on every visibility transition and do not calculate hidden responses. A visible response after suspension becomes a new baseline, so a hidden interval cannot fabricate a rate.
- Desktop `ThroughputChart` clears its smoothed circular buffers and restarts its RAF loop only after a visible transition. `SmoothTrafficValue` cancels in-flight animation on hide and snaps to the current target. Desktop QKey paste-click suppression uses a shared 400 ms timer-owned window instead of wall-clock deadlines.
- This contract changes elapsed/sample behavior only. Existing visual composition, styles, assets, and layout remain unchanged. Deterministic clock, counter, gap, visibility, RAF, paste-order, and timer-cleanup regressions cover the task's acceptance boundary.
- Verification: Admin and Desktop checks report 0 errors and 0 warnings; full Admin units pass 293/293, full Desktop units 402/402, and shared UI 82/82; both production builds pass; Chromium 1.58.2 preflight passes; Admin E2E passes 70/70; Desktop E2E passes 23/23; the focused elapsed, visibility, chart, SmoothTrafficValue, and paste-order regressions pass; the post-cleanup desktop dialog rerun passes 16/16; and `git diff --check` passes. No Rust source changed in this task, so no new Rust gate is claimed.

## Browser Timer and RAF Lifecycle Contract (2026-08-06, TODO-870)

- `packages/ui/owned-scheduling.ts` owns replaceable delayed callbacks and animation frames. `schedule()` cancels the previous handle, `cancel()` releases a pending handle, and `destroy()` permanently prevents post-unmount execution.
- Desktop and admin delayed dialog actions, login feedback/focus, QKey animation, error-reset feedback, CountrySelect focus/typeahead, and root persistence use explicit owner cleanup. Root persistence serializes saves: a hidden or before-unload flush cancels the debounce, preserves the in-flight save, and queues one latest-state follow-up instead of starting concurrent writes.
- Shared `ConfirmDialog`, toast state, and ripple actions retain and clear their own timers. Ripple destruction removes active circles and safety handles; toast removal clears the matching expiry handle.
- Desktop Tauri, admin Dashboard, Logs, and Configuration pollers skip new work while hidden, invalidate in-flight generations on hide, and restart the visible work immediately. Throughput, sampling, RAF, and ResizeObserver owners retain their existing visibility and teardown boundaries.
- Delayed async continuations are also lifecycle-owned: shared copy feedback, admin fatal-error clipboard and login responses, and desktop QKey clipboard/parser responses stop before post-unmount state or timer work.
- No visual surface, style, asset, or layout was intentionally changed. Full unit suites pass Shared UI `88/88`, Admin `301/301`, and Desktop `411/411`; checks report 0 errors and 0 warnings; both production builds pass; Chromium `1.58.2` preflight passes; Admin E2E passes `70/70`; Desktop E2E passes `23/23`; and `git diff --check` passes. No Rust source changed, so no new Rust gate is claimed.

## Browser Identifier and CSRF Entropy Contract (2026-08-06, TODO-871)

- Admin POST requests obtain a CSRF nonce from `crypto.randomUUID()` when available, otherwise from 32 bytes returned by `crypto.getRandomValues()`. The nonce carries a process-local sequence suffix so repeated provider output cannot reuse an active request identity; if both secure sources are absent or throw, `createCsrfNonce()` returns `secure-randomness-unavailable` and the request fails before network dispatch.
- Shared toast IDs are monotonic `toast-*` sequence values owned by `packages/ui/toast-store.svelte.ts`. Their expiry handles remain in the existing per-ID owner map, so repeated IDs cannot remove a different toast and removal clears only the matching timer.
- Admin Sparkline gradient IDs are monotonic `sparkline-grad-*` sequence values in the component module scope. The module owner is document-wide for the loaded bundle, so concurrent component instances cannot bind an SVG fill to another instance's gradient.
- Frontend unit setups install `test-uuid-*` sequence identities only when the environment lacks `crypto.randomUUID()`. The deterministic test identity is separate from production secure-source capability and fallback tests; no test UUID derives from `Date.now()` or `Math.random()`.
- Fixed-clock, repeated-random, unavailable-source, concurrent toast, and concurrent Sparkline regressions define the collision and failure boundary. Full unit suites pass Shared UI `89/89`, Admin `307/307`, and Desktop `411/411`; Admin/Desktop checks report 0 errors and 0 warnings; both production builds pass; Chromium `1.58.2` preflight passes; Admin E2E passes `70/70`; Desktop E2E passes `23/23`; the focused API nonce suite passes `15/15`; and `git diff --check` passes. No visual composition, style, asset, or layout was changed. No Rust source changed, so no new Rust gate is claimed.

## Deterministic Frontend Clock Test Contract (2026-08-06, TODO-872)

- `scripts/tests/frontend/test-clock.ts` owns the isolated unit-test harness for wall time, monotonic time, fake-timer advancement, requestAnimationFrame/cancellation, and document visibility. The harness exposes explicit setters/advancers and restores every installed spy, descriptor, timer mode, callback, and visibility override after each test.
- The Shared UI, Web Admin, and Desktop Vitest setup files create one fresh harness per test without globally replacing time sources. Tests opt into only the clock domains they exercise, so unrelated tests retain their existing timer and browser behavior.
- Timing owners use the harness for the suspension matrix: Tauri pollers and Dashboard polling cover delayed responses, hidden invalidation, visible restart, monotonic rebasing, and cleanup; ThroughputChart and SmoothTrafficValue cover RAF cancellation; elapsed-time coverage proves a wall-clock jump cannot corrupt a monotonic sample.
- Browser E2E fixtures use named, unit-labeled constants for QKey Unix seconds and admin-log Unix milliseconds. Unit timestamp fixtures use a fixed valid Unix-millisecond default; no fixture depends on the host wall clock.
- Existing TODO-871 tests remain the identifier/CSRF collision and fail-closed proof surface. TODO-872 adds the shared control surface and cross-owner timing controls without changing product or visual implementation.
- Unit verification: Shared UI `92/92`, Admin `307/307`, and Desktop `412/412` pass. Focused harness/owner regressions pass Shared UI `3/3`, Admin `46/46`, and Desktop `21/21`; the first Auto-Advance regression was corrected by preserving the prior explicit fake-timer option. Admin and Desktop `bun run check` pass with 0 errors and 0 warnings, and both production builds pass. Chromium preflight passes with Playwright `1.58.2`; Admin E2E passes `70/70` and Desktop E2E passes `23/23`; `git diff --check` passes. The Desktop build retains the known Tauri `core.js` dynamic/static import warning. No Rust source changed, so no Rust gate is claimed.

## UnsafeMemoryPool Ownership Contract (2026-08-06, TODO-826)

- `src/optimize/unsafe.rs::UnsafeMemoryPool` uses one mutex-protected registry keyed by exact allocation base address. Each record carries preallocated-versus-fallback origin and available-versus-in-use state. The pool has no manual `Send`/`Sync` implementation; compiler-derived pool traits rely on the mutex, with the raw pointer record marked `Send` only because it is moved under that ownership lock.
- `alloc_uninit()` admits only registered preallocated blocks or newly registered fallback blocks. `free()` checks alignment, exact pool membership, live state, and pointer origin in release builds and returns a typed error for invalid, foreign, or double-free input before mutating state. Sub-slice bases cannot enter the registry.
- Fallback blocks are never inserted into the preallocated available cache. They are removed from the registry and deallocated directly on return, so fallback demand cannot increase preallocated capacity or available-slot accounting. Pool drop deallocates only available owned blocks and logs plus leaks checked-out blocks if the explicit no-live-user precondition is violated.
- `UnsafePacket::from_raw_parts()` checks length, capacity, alignment, exact live ownership, and pool identity before construction. `extend_from_slice()` uses checked addition and overlap-safe copying. `prefetch_block()` derives its maximum offset from the actual cache-line count, including one-line blocks. `copy_from_slice()` applies the same live-block proof and overlap-safe copy semantics.
- Focused debug `unsafe_rust` verification passes `24/24`, including cross-thread allocation/free, foreign-pool and sub-slice rejection, double-free rejection, fallback accounting, runtime packet rejection, checked overflow, overlap, and undersized-prefetch cases. The full debug `unsafe_rust` library passes `2437/2437`; the optimized release focused lane passes `24/24`; the optional `compression_zstd_ffi` focused lane passes `24/24`; strict all-target Clippy with `unsafe_rust`, formatting, and diff hygiene pass. Miri is unavailable on the pinned `1.97.1-aarch64-apple-darwin` toolchain, so no Miri result is claimed. The Rust target was cleaned after verification to preserve the disk floor.

## MemoryPool Ownership and Accounting Contract (2026-08-06, TODO-827)
...
- `ZeroCopyBuffer<'a>` owns immutable send slices only. `ZeroCopyRecvBuffer<'a>` owns exclusively borrowed mutable receive slices only; the outer slice and its inner mutable slices remain borrowed for the wrapper lifetime, so a receive syscall cannot alias caller access through the original slice.
- Unix constructors check runtime `IOV_MAX` and the signed `msg_iovlen` ABI ceiling before storing iovec counts. Windows constructors check every `WSABUF.len`, the buffer count, and the `WSASendTo` address length before the Winsock call. Aggregate buffer lengths use checked `usize` addition.
- `ZeroCopyResult<T>` returns `ZeroCopyError` for boundary failures and underlying syscall errors. `ZeroCopyTransfer` stores the requested and transferred byte counts; `is_zero()`, `is_partial()`, and `is_complete()` are the shared progress classification. A negative Unix return or Winsock failure is never represented as a byte count, and successful Windows `u32` counts widen to `usize` without `i32` narrowing.
...
- The pre-FEC core send path keeps a `PooledBlock` until all buffer-size, QUIC send, path-control, stealth, and wire-length checks pass; only then does it transfer the raw block and its originating pool into `FecPacket`. FEC-internal failure cleanup is closed by TODO-832.
- `batch_encode_frames()` no longer allocates an unused intermediate pool block. The compatibility pool argument remains accepted, but frame encoding has no hidden allocation or cleanup obligation. Cumulative output positions use checked tails and return `BufferTooShort` before a slice can escape the output buffer.
...
## FEC Pooled-Buffer Failure Cleanup Contract (2026-08-06, TODO-832)

- FEC production allocations use a live `PooledBlock` guard until construction is committed. The streaming raw parser, active wire source/repair parser, GF4/GF8/GF16 repair generators, decoder equation/known storage, and Fountain adapters transfer only validated blocks into `FecPacket`; rejected lengths and fallible coefficient or row work return through guard drop.
- `FecPacket::from_pooled_blocks()` requires both payload and coefficient guards to originate from the packet's `MemoryPool`, validates declared lengths before transfer, and frees both guards together when construction is rejected. The compatibility `FecPacket::new()` is crate-internal for raw and foreign test inputs; mismatched or foreign buffers are directly released by the pool boundary without entering accounting, and failed length assertions panic only inside the crate.
- Decoder `known` maps and pending equations retain `PooledBlock` values, so decoder teardown and occupied-entry recovery no longer directly drop pool buffers. Fountain symbol output rejects oversized symbols instead of pairing truncated storage with an overstated length. GF16 coefficient stride, row extent, coefficient-byte, and even-symbol calculations use checked arithmetic.
- `MemoryPool::try_alloc_from_slice()` rejects oversized input with `MemoryPoolError::SliceTooLarge` instead of truncating it. The final ARM64 macOS evidence is green: the complete FEC library matrix passes `252/252`, the complete debug library passes `2,436/2,436`, the complete release library passes `2,436/2,436`, strict `unsafe_rust` library and all-target Clippy pass, formatting, diff hygiene, and locked metadata pass, and exact parser, generator, decoder-teardown, symbol-length, and wire-error counter assertions are included in the FEC matrix. TODO-833 closes the separate feature-gated DATAGRAM queue ownership boundary; TODO-682/TODO-683 remain the deeper raw UDP/interface FFI boundaries. The final `target/` directory is `2.6G`, below the 13-GiB task ceiling.

## Zero-Copy Datagram Pool Return Contract (2026-08-06, TODO-833)

- Feature-gated `DatagramBuffer` owns a `PooledBlock` instead of a raw `AlignedBox`. Direct receive/send queue removal, purge, successful frame serialization, serialization-error restoration, and `Connection` teardown therefore return the checked-out block through `MemoryPool::free()` on normal and unwind paths.
- `enqueue_received_datagram()` rejects a full receive queue and rejects an inbound payload larger than the pool block before allocation or copying. `dgram_send()` applies the same fail-closed block-size check before allocation, so the feature-on path never silently truncates a payload. The feature-off path retains `Vec<u8>` storage and the accepted-payload tests assert identical bytes and lengths in both modes.
- Exact feature-gated tests cover pool counter recovery for send purge, frame success, receive pop, `dgram_recv_vec()`, send/receive queue rejection, oversized inbound/send rejection, serialization-error restoration, and connection drop. The bounded optimization suite's literal `zero_copy` filter now matches real `zero_copy_dgram_*` tests; its full release run is not claimed because the suite hit an unrelated telemetry baseline failure and the later release attempt reached the mandatory disk floor.
- Verification on the current ARM64 macOS source: feature-on focused `5/5`, feature-on full debug library `2,441/2,441` before the final test-only queue-rejection assertion expansion, feature-off full debug library `2,437/2,437`, strict `zero_copy_dgram,unsafe_rust` library/all-target Clippy, formatting, diff hygiene, and locked metadata pass. Native Linux/Windows execution remains outside this boundary and stays owned by TODO-682/TODO-683.

## UDP Batch FFI Result and Ownership Contract (2026-08-07, TODO-837)

- Linux `sendmmsg`/`recvmmsg` and Apple `sendmsg_x` batching admit at most 64 messages at the raw syscall boundary. Returned completion counts are checked against the prepared message array before indexing, accounting, or success is reported.
- Every Linux send completion must report the complete payload length; a short datagram is a typed `WriteZero` error. Receive `msg_len` values are checked against the corresponding buffer, zero-length UDP datagrams remain valid results, and peer `msg_namelen` values are checked against the full `sockaddr_storage` and family-specific ABI size before conversion.
- `crates/qf-transport-udp/src/lib.rs` owns one zero-initialized, size/alignment-asserted `sockaddr_storage` conversion for IPv4 and IPv6 on Unix. Linux, Apple, and the root `src/optimize/udp.rs` compatibility projection use the same conversion and reject unsupported families or malformed lengths as `InvalidData`.
- Apple scalar `sendmsg` fallback checks exact datagram completion, while `UdpFastPath` delegates its Apple batch path to the shared owner. Single sends and Linux GSO validate exact byte completion; aligned receive-buffer construction is fallible and rejects arithmetic overflow or failed reservation.
- `qf-transport-batch::BatchProcessor` validates Linux timeout conversion into `timespec`, raw fallback `sendto` byte counts, receive metadata, and caller-owned descriptor lifetime. The root `src/transport/batch.rs` adapter preserves the historical namespace only. Unix/Windows temporary `UdpSocket` views use `ManuallyDrop`, so success and early errors do not close caller-owned handles. Accelerated `InvalidData` and `WriteZero` results are propagated instead of retrying a potentially partially executed batch through the scalar fallback. The Linux client hotpath retains the raw `io::Error` kind and applies the same no-duplicate retry rule.
- Local ARM64 macOS verification passes the feature-gated library `2448/2448`, Optimize/UDP unit tests `8/8`, Unix caller-fd ownership `1/1`, UDP-fastpath unit tests `3/3`, `rt-udp-batch-send` `3/3`, `rt-transport-udpfast` `2/2`, and the macOS branch of `rt-transport-batch-processor` `1/1`; formatting and diff hygiene pass. The final Linux-only error-propagation change was source-reviewed and formatted but not compiled on this macOS host. Linux-only integration cases, Windows target compilation, and all-target rebuilding were not run because the prior build reached the 2-GiB disk floor; no Omega mutation is claimed.
...
- Native transport and interface ownership remains source-audited but not fully cross-platform proven. TODO-837 now implements the bounded UDP batch result, address-length, partial-send, timeout, aligned-buffer, and caller-fd contracts; TODO-845 implements the Unix TUN syscall/name/rollback/close source contracts and static proof wiring; TODO-846 implements the Wintun cleanup ledger, constructor rollback, and lifecycle contract; local ARM64 macOS gates pass, while native Linux/Windows/Omega execution remains unclaimed. PMTU/prefetch and WFP transactions remain owned by TODO-841, TODO-842, and TODO-847 through TODO-848; TODO-854 closes local privilege, memory-lock, TLS, ordering, and portability-proof wiring while native Linux/Windows execution remains unclaimed; TODO-844's generic TUN result contract is implemented and statically wired.
...
## PMTU Validation and Portable Prefetch Contract (2026-08-07, TODO-841)

- `PmtuPolicy::validate()` is the shared configuration boundary for `Config::set_pmtu_policy()` and `PmtuState::new()`. The PMTU state constructor now returns `Result<PmtuState, ConnectionError>`, and transport connection creation propagates invalid policy errors.
- PMTU probe admission rejects sizes outside the validated floor/ceiling. Probe timing uses `saturating_duration_since()` so an earlier `Instant` is deterministic and non-panicking. Midpoint and black-hole reset targets use ordered, saturating arithmetic and remain within the configured bounds.
- `prefetch_frame_parse_window()` accepts a slice rather than an unbounded raw base/end pair. Empty slices return without a hint; offsets clamp to the final in-bounds byte; the helper never forms a one-past-end pointer. The non-iOS AArch64 prefetch implementation emits `prfm` and does not dereference the supplied pointer; the previous `read_volatile` fallback is removed.
- Source regressions cover invalid construction, earlier probe time, maximum validated bounds, empty receive input, exact cache-line input, and over-bound frame offsets without a network runtime. `cargo fmt --all -- --check`, `git diff --check`, and `CARGO_BUILD_JOBS=1 cargo check --lib --features rust-tests` pass, with only the pre-existing `memory_pool.rs:1071` dead-code warning. The focused `pmtu_tests` test-target build was intentionally interrupted at 1.7 GiB free and `cargo clean` restored 2.6 GiB; no test result is claimed from the partial build.

## Transport Malformed-Boundary and Native ISA Proof Coverage (2026-08-07, TODO-842)

- The shared UDP boundary has deterministic direct regressions for syscall-count, receive-length, partial-send, batch-size, datagram-size, and malformed-address metadata. `qf-transport-batch::BatchProcessor::batch_send()` also has a real invalid-caller-fd regression expecting `EBADF`; non-Linux hosts record an explicit skip. No production syscall mock seam was introduced.
- The transport frame target covers malformed ACK ranges, malformed Connection IDs, AArch64 stream-cursor truncation, and cumulative batch-output capacity overflow. The packet-header target compares scalar and compile-gated AVX2 packet-number encoding for 1- to 4-byte values at an unaligned offset and verifies sentinel preservation. The suite now invokes exact library tests, the packet-number decode parity target, and explicit ARM/AVX2 platform lanes with unique evidence artifacts.
- The aggregate runtime guardrail now fails closed when the new UDP, Linux-fd, malformed-frame, ARM, or AVX2 proof wiring disappears. The new checks pass. The complete guardrail remains `FAIL` with four pre-existing critical findings and one pre-existing warning owned outside TODO-842.
- TODO-838 makes AF_XDP feature-on evidence not applicable because the incomplete implementation, Cargo feature, and target were removed. TODO-831 makes the former batch pool-cleanup finding not applicable because `batch_encode_frames()` is allocation-free. TODO-841 retains the PMTU source regressions and blocked focused test gate.
- The current host is ARM64 macOS. Native Linux, Windows, x86_64 AVX2, privileged, and Omega execution is unavailable or outside this workspace; no such proof is inferred. The focused release library compile reached dependency compilation and was interrupted with exit 130 at approximately 2.0 GiB free before test execution; `cargo clean` restored 2.4 GiB. No test result is represented as passed. The adjacent stale `InterfaceType::Xdp`/`XdpMode` configuration surface is reconciled by TODO-874 in the section below.
- The TODO completeness validator validates tracker/detail/archive/path coverage and then stops at the pre-existing Graphify freshness check because the current manifest is stale relative to `HEAD`. Graphify remains explicitly `BLOCKED`; this is not treated as a TODO-842 implementation or register pass.

## Privilege Identity and Cross-Platform FFI Contract (2026-08-07, TODO-849)

- `ResolvedIdentity` fields are private and the type remains constructible only through `resolve_identity()` or `inspect_identity()`. Read-only accessors preserve the CLI, audit, server, probe, and integration-test surfaces without allowing a caller to fabricate UID/GID or canonical account-name state; the type derives `Serialize` only and has no deserialization constructor.
- `drop_privileges_resolved()` performs a final non-root shape check and re-resolves both selectors through the account database, requiring the canonical names and numeric IDs to match before `setresgid`/`setresuid` or `setgid`/`setuid` executes. Root UID/GID, empty selectors, empty canonical names, account replacement, and selector-to-ID mismatches fail closed.
- `CurrentIds` is defined outside the Unix cfg so the non-Unix `current_ids()` stub has a valid Windows-compilation signature. Unix `current_groups()` rejects negative results and any returned count larger than the prepared capacity before truncating or sorting.
- Every privilege FFI block has an adjacent safety contract for pointer ownership, scalar-only libc calls, null-pointer zero-count operations, the Linux capability v3 ABI, and the isolated root-regain probe. Deterministic source tests cover forged root identity and malformed group counts; the runtime guardrail reports the opaque-identity and FFI contract as green.
- Static verification passes `cargo fmt --all -- --check`, `cargo metadata --locked --no-deps`, Bash syntax, and `git diff --check`. The privilege unit target passes `19/19`, and the dedicated portable `it-privilege-boundary` target passes `1/1` on ARM64 macOS with `rust-tests`; the broader integration target previously passed `3/3` in the TODO-849/850 evidence. The TODO-854 manifest records native Linux privilege proof as unavailable on this host; Windows compilation is declared in CI and statically guarded, while Windows native privilege proof remains unclaimed.

## Privilege Post-Drop State and Failure Proof (2026-08-07, TODO-850)

- Linux `/proc/self/task/<tid>/status` verification now requires exactly four `Uid:` and `Gid:` values and compares real, effective, saved, and filesystem IDs for every enumerated live thread. The same per-thread proof retains empty supplementary groups, zero effective/permitted/inheritable/ambient capabilities, and `NoNewPrivs: 1` requirements.
- `PrivilegeTransitionState` and `DropError::PartialTransition` make every failure after final identity validation fail closed with the last completed transition point. Linux wraps failures from no-new-privileges, supplementary-group removal, UID/GID changes, capability clearing, state inspection, and process/thread verification; non-Linux Unix wraps its narrower `setgid`/`setuid` path and effective-ID verification.
- `prove_root_cannot_be_regained(&identity)` now denies both `setresuid(0,0,0)` and `setresgid(0,0,0)` with `EPERM`, then repeats the complete process and per-thread proof. The isolated standard and Tokio probe paths shut down all extra threads before running this process-wide probe; both integration branches assert the same serialized IDs, groups, capabilities, and no-new-privileges state.
- Non-Linux Unix does not claim Linux `/proc`, saved-ID, capability, supplementary-group, or root-regain semantics. `verify_process_privilege_state()` returns `NotSupported` outside Linux instead of returning a zero-thread success. Root-only integration runs emit `PRIVILEGE_PROOF_UNAVAILABLE reason=requires_root` when the host is not privileged.
- The runtime guardrail checks the four-field parser, partial-transition state, complete isolated regain probe, deterministic probe evidence, and explicit non-Linux boundary. Focused Rust verification passes `19/19` privilege unit tests and `3/3` privilege integration tests on ARM64 macOS with one Cargo job. The Linux cross-target check is unavailable because the host lacks both `x86_64-linux-gnu-gcc` and a Linux sysroot (`assert.h`); Windows compilation, native privilege proof, and Omega evidence remain unclaimed. `cargo clean` removed the generated target artifacts after verification, leaving `target/` absent and the free-space margin restored.

## Embedded Server Memory-Lock Policy and Reload Ownership (2026-08-07, TODO-851)

- **Shared server startup memory-lock policy:** `src/memory_lock.rs::MemoryLockPolicy` is the single mapping from `SecurityConfig.lock_memory`, `SecurityConfig.lock_blocks`, and `SecurityConfig.memory_lock_failure_policy` for standalone and embedded server startup. The policy preserves the configured values, resets qftls process-coverage state, applies process locking when the boundary permits it, and configures `MemoryPool::set_lock_blocks()` before TLS identity construction.
- Embedded `QuicFuscateEngine::start()` applies the policy before `global_pool()` and before `build_runtime_transport_config()` can load the server TLS identity. Standalone `run_server()` applies the same policy before `load_server_identity()`; when a Linux UID/GID transition is configured, it individually protects the TLS key before the transition and applies the deferred process-wide lock after verified privilege state.
- Runtime reload builds a complete candidate for every domain before publishing any of it (TODO-724). Signed TOML integers in the transport override section are range-validated rather than clamped (TODO-725): `max_idle_timeout`, the four `initial_max_*` flow-control limits, both `initial_max_streams_*` limits, and both `dgram_*_queue_len` values reject a negative with a field-specific error instead of turning it into zero, which would disable liveness detection or permit no data while reload reported success. Zero stays accepted because an operator can mean it; the upper bound is the QUIC varint maximum, since a larger value cannot be encoded on the wire at all. A rejected value aborts the whole override set before any transport policy is mutated. `apply_transport_overrides_from_toml()` returns setter failures instead of logging and skipping them, and it applies the overrides to a private copy that is committed only after every setter has succeeded, so a rejected constraint cannot leave transport half-updated. Transport is applied first and the FEC, optimization, and stealth locks are written only after it succeeds, so a failure leaves every domain on the prior generation instead of publishing three of four. `RuntimePolicyGeneration` now serializes the successful publication and profile-rotation mutation, while `RuntimePolicySnapshot` holds one read lease while cloning all four domains. `apply_transport_overrides_from_file()` treats absence as the only acceptable reason to keep the configured defaults: a present but unreadable or invalid override file fails startup rather than silently downgrading transport semantics the operator wrote. Complete `EngineConfig` validation and strict typed-string projection are closed by TODO-794; TODO-876 owns the generation identity and mixed-reader boundary.
- Standalone runtime reload compares current and candidate startup-owned memory-lock settings before `apply_runtime_config_reload()`. Unchanged values are accepted, but changing any of the three settings returns a typed rejection and requires restart, so no submitted value is silently retained or ignored.
...
- `SecurityConfig.memory_lock_failure_policy` is an explicit `best-effort` or `fail-closed` startup choice. The default remains best-effort for portable and embedded configurations; `config/server-linux.default.toml` selects fail-closed for the production Linux server path.
- `MemoryLockStartupError` preserves whether the failure came from the `RLIMIT_MEMLOCK` query, `mlockall`, or an unsupported platform, together with the observed budget and OS message. Unlimited budgets request current-and-future locking; finite and unknown budgets request current-only locking, with unknown-budget best-effort fallback visible as degraded state.
- `MemoryLockStartupStatus` is published globally and copied into runtime Metrics. Admin status/health and systemd `/health`, `/ready`, and `/live` expose the policy, state, process mode, budget classification, and failure cause. Best-effort degradation remains ready; deferred and fail-closed failure states are not ready. The liveness endpoint remains HTTP 200 while readiness fails closed.
- Embedded startup propagates a fail-closed error before `global_pool()` and server TLS construction. Standalone startup propagates the same error before TLS/service setup, and deferred standalone locking is applied after verified privilege transition and before the runtime can emit service readiness. The process policy does not claim pooled-block unlock/accounting, which remains owned by TODO-516/TODO-678, or TLS key correspondence/output proof, which is closed by TODO-853; lower-level key lock/publication remains owned by TODO-643.
- The process-wide lock test uses a Drop guard that calls `munlockall()` on every exit path, including panic unwinding. Deterministic policy and flag tests cover best-effort versus fail-closed decisions, finite/unlimited/unknown budgets, and the production boundary. TODO-854 includes these tests in the negative-proof manifest; native Linux root-regain execution remains explicitly unavailable on the current host.
...
- `preload_tls_server_identity()` now validates the parsed certificate chain and private key through rustls `ServerConfig::with_single_cert()` before process-global `OnceLock` publication. Rustls' end-entity SPKI correspondence check rejects a parseable but mismatched certificate/key pair before publication.
- Same-identity preload calls remain idempotent and conflicting identity calls fail closed. `LockedKeyMaterial` and rejected `OnceLock` values retain their lower-level zeroize-before-`munlock` ownership under TODO-643; process-wide lock readiness/failure policy remains under TODO-852.
- `SensitiveKeyingMaterial = Zeroizing<Vec<u8>>` is now the `export_keying_material()` result through the public trait, combined provider, rustls provider, and wrapper. Rustls receives and returns the same erasing owner, and the exporter audit found no production caller outside `qftls` that could discard it.
- Focused qftls verification passes `24/24`, including the isolated mismatch/duplicate/conflict preload contract and zeroization-before-drop regression. TODO-854 includes a dedicated qftls negative-proof target and explicit unavailable native-lane reporting; pooled-block unlock/accounting remains TODO-516/TODO-678.
...
## Interface BMI2 Dispatch and CPU Profile Proof (2026-08-07, TODO-843)

- `FeatureDetector` derives and caches the automatic `CpuProfile` from the exact `CpuFeatures` snapshot. The pure `profile_from_features()` path preserves the explicit `avx2 && bmi2` requirement for `X86_P2b`; the higher x86 profiles `X86_P3a` through `X86_P3e` and `X86_P4a`/`X86_P4b` do not imply BMI2.
- `TunInterface::write_packet()` passes the cached profile and full runtime feature snapshot through `bmi2_parser_is_allowed()`. The target-feature parser is selected only for an x86 profile with `features.bmi2 == true`; all other profiles and missing-BMI2 snapshots use `parse_ip_header_scalar()`. The BMI2 parser continues to load its four-byte IPv4 header with `std::ptr::read_unaligned`.
- Failable library tests cover all x86 automatic profile selectors with BMI2 absent and present, all selecting and non-selecting interface profiles with synthetic feature snapshots, and a portable intentionally unaligned IPv4 slice. The direct native BMI2 parser test emits `SIMD_SKIP` when the host lacks BMI2 rather than silently returning green. The core suite invokes the exact library tests and records architecture/CPU skip objects.
- The runtime guardrail checks the cached profile handoff, exact profile/runtime gate, automatic-profile test, interface profile test, unaligned test, native skip marker, and core-suite runners. This proves source and execution wiring, not native x86 execution.
- Formatting, Bash syntax, diff hygiene, the BMI2 guardrail, and one-job `cargo check --lib --features rust-tests` pass with the pre-existing `memory_pool.rs:1071` dead-code warning. The exact focused unaligned test build was stopped with exit 130 at 1.5 GiB free before test execution; `cargo clean` removed 390.6 MiB and left 1.7 GiB free. The current host is ARM64 macOS. Native x86 BMI2 execution, native Linux/Windows, and external Omega evidence remain unclaimed under TODO-848, and no test result is inferred from the interrupted build.
...
- `FecPacket::new()` is now a crate-internal compatibility constructor; it no longer allocates an unbounded replacement buffer for overstated compatibility lengths and its panic surface is no longer exposed publicly. `try_new()` and `try_from_block()` provide checked rejection, systematic packets cannot retain coefficient metadata, stream flags are exact, and clone/stream slices remain bounded by pool-backed storage.
- Decoder8, Decoder4, and Decoder16 now fail closed for zero, oversized, field-incompatible, or invalid interleave dimensions. Repair equations require the exact coefficient width and a valid source-window anchor; Decoder16 establishes and retains an active anchor before admitting an equation, filters unrelated systematic IDs, and only completes inside that active window. Lazy/interleaved compatibility constructors cap invalid capacity inputs without changing their existing return shape.
...
- Evidence: `cargo check` and `cargo clippy` pass with five pre-existing warnings; the serial stream/decoder filter passes `17/17`, the Wire FEC filter passes `24/24`, and the malformed matrix test passes `1/1`. Tests now acquire the shared `ENV_MUTEX` where they read or mutate FEC environment variables, so the suite also passes with `--test-threads=16`. Native x86/SVE2, sanitizer/Miri, privileged, Omega, and full external proof remain unclaimed and are owned by their adjacent tasks where applicable.
...
## Runtime Policy Generation and Reload Snapshot (2026-08-08, TODO-876)

- `RuntimePolicyGeneration` is the shared read/write publication gate for the standalone server's transport, FEC, optimization, and stealth state. Successful reload publication advances the counter exactly once after transport and all three shared policy locks have committed; rejected candidates leave the prior generation unchanged.
...
## Implementation Reconciliation (2026-08-08, TODO-758 fuzz manifest, CI, and corpus contract)

- The current fuzz manifest resolves the root crate through `../../..`; locked metadata succeeds and `cargo fuzz list --fuzz-dir scripts/tests/fuzz` returns exactly `connection_handling`, `crypto_operations`, `fec_encoding`, `frame_decoding`, `packet_parsing`, and `varint_parsing`. The path repair predates this reconciliation in commit `35c90fa`.
- CI now has a bounded PR lane (60 seconds per target), a longer main-push lane (120 seconds per target), and a scheduled Sunday lane (1,800 seconds per target). Both workflows install Cargo Fuzz `0.13.2` with a lockfile, require nightly Rust, set `RUSTFLAGS=-Zsanitizer=address`, run the shared fail-closed runner, and upload crash artifacts on failure.
- `scripts/tests/fuzz/run-ci-fuzz.sh` validates toolchain, metadata, exact target inventory, numeric bounds, curated-seed staging, and every target's nonzero failure propagation before invoking libFuzzer. Runtime corpora and artifacts are ignored; the tracked corpus is reduced to eight deterministic files per target, selected by size-ranked coverage points.
- `crypto_operations.rs` enumerates every accepted public `force_aead` spelling separately from planner-owned `aegis-128x4` and `aegis-128x8` internal names. The internal values intentionally exercise the runtime fallback and are not presented as product configuration.
- Local ARM64 macOS evidence covers metadata, target listing, Bash syntax, static contract checks, formatting, and diff hygiene. The pinned host has no nightly toolchain, so sanitizer-instrumented fuzz compilation and execution remain a hosted CI gate; no green result is inferred locally.

## Strict Panic and Invariant Contract (2026-08-08, TODO-757)

- The strict Rust contract is split into three explicit audit records. `strict_production_clippy` covers the non-test runtime feature set, `strict_benchmark_clippy` covers the `server,benches` target class, and `strict_runtime_clippy` retains the full `--all-features --lib --bins` aggregate. Each invokes `clippy::unwrap_used`, `clippy::expect_used`, and `clippy::panic` as deny-level lints.
- Recoverable production boundaries now propagate typed errors: GF table initialization, shared FEC buffer access, packet stealth-mode transitions, server-domain construction, MASQUE queue installation, replay-window fixed-width hashing, and PEM conversion. Probe and benchmark binaries also return worker, refresh, and packet-construction failures instead of panicking.
- Compatibility APIs with no error channel retain only narrow function-level dispositions. HKDF/HMAC fixed-size dependency contracts, benchmark setup, validated legacy DDoS/blacklist constructors, infallible `MemoryPool` and `OptimizationManager` wrappers, `LiveServerState::default`, and standalone runtime `live` accessors document their invariants and expose fallible alternatives where callers can recover.
- Regression coverage includes the RFC 4231 HMAC vector, the HKDF 8160-byte RFC limit, GF table identities, replay-window hashing, PEM conversion, default bandwidth construction, standalone runtime ownership, and benchmark transport setup. The default library suite passes `2,659/2,659`; the all-feature library suite passes `2,701/2,701`.
- Verification passes `cargo check --all-features`, `cargo fmt --all -- --check`, `git diff --check`, and all three strict Clippy lanes. The comprehensive audit still reports unrelated baseline boundaries: the generated web-admin publish contract, the Linux-only all-target integration compile error on macOS, four pre-existing runtime-guardrail findings, and two unavailable native lanes. The completeness validator reaches coverage but fails closed on the stale Graphify evidence manifest for the current revision. Frontend files were not modified.

## io_uring Per-Slot Send Disposition (2026-08-08, TODO-798)

- `src/optimize/uring_batch.rs` provides `BatchSendDisposition` (`Sent`, `Failed`, `NotSubmitted`, `Quarantined`) and `BatchSendResult` as the exact ownership contract for connected and unconnected batch sends. `UringBatchSender` and `UringBatchWorker` expose detailed methods; legacy count methods remain compatibility wrappers over `sent_count()`.
- Standard SendMsg CQEs are indexed by user data and mapped independently, so a later successful CQE is never hidden behind an earlier failure. SendMsgZc uses the primary CQE result for the same per-slot map while retaining notification lifetime draining. Submission refusal is `NotSubmitted`; accepted-but-unprovable state after submit, protocol, completion, cancellation, timeout, or dropped-response failure is `Quarantined` and must not be retried.
- Worker admission errors retain the original input width. A bounded queue or pre-submit shutdown is `NotSubmitted`; a response timeout or dropped response is full-width `Quarantined` because the worker may already own kernel completion state. The worker remains available after recoverable `WouldBlock` submission and becomes failed after other operation errors.
- The Linux client outbound path constructs an index-preserving retry subset before sendmmsg. The Linux server live flush path applies the same per-slot selection before async socket fallback. Each path records counters only for slots actually accepted by io_uring, sendmmsg, or an individual socket send. The synchronous `server_send_batch_to_with_disposition()` helper preserves the same contract for non-runtime callers.
- Regression coverage includes out-of-order `[Sent, Failed, Sent]` mapping and all-slot quarantine. ARM64 macOS evidence passes `cargo check --all-features`, strict `cargo clippy --all-features --lib --bins` with panic/unwrap/expect denied, `cargo test --all-features --lib -- --test-threads=1` (`2,701/2,701`), `cargo fmt --all -- --check`, and `git diff --check`.
- Linux native or equivalent kernel-backed duplicate-delivery proof is not claimed locally. The installed GNU target cannot compile its C dependencies on this macOS host: clang stops in `ring`/`zstd-sys` on missing GNU/Linux sysroot headers `assert.h`, `string.h`, `stdio.h`, and `stdlib.h`. Frontend paths remain outside this implementation and no UI field was added.
...
- Evidence: `cargo test --features rust-tests --lib` passes `2,663/2,663`; all-feature `cargo test --all-features --lib` passes `2,705/2,705`; routing focus passes `26/26`; `cargo check --all-features`, strict all-feature library/bin Clippy with panic/unwrap/expect denied, formatting, and diff hygiene pass. The installed `x86_64-unknown-linux-gnu` target reaches `ring` but stops because this host has no GNU/Linux sysroot (`assert.h`); privileged Linux iptables/nftables collision, process-loss, zero-residue, and Linux source-compile evidence remain unavailable. Frontend paths are unchanged.

## Managed macOS PF Anchor Installer and Proof Boundary (2026-08-08, TODO-548)

- `scripts/install/install-macos-pf-anchor.sh` owns one fixed main-ruleset reference, `anchor "com.quicfuscate.killswitch" all`, inside exact begin/end markers. Install, remove, and check are idempotent where ownership is proven; the installer atomically stages `/etc/pf.conf`, preserves its mode and owner, keeps a mode-0600 pre-install backup, and publishes a mode-0600 ownership record under `/var/db/quicfuscate/pf/`.
- The installer uses one state-directory lock, refuses foreign exact or wildcard QuicFuscate anchors, modified or duplicated markers, symlinks, orphaned state/backup, and unverifiable active-ruleset state. It never enables or disables PF, flushes an unrelated anchor, or replaces the main ruleset wholesale. Reload or postcondition failure restores the original file and removes managed residue only after a verified rollback.
- `--root PATH` is a deterministic non-privileged fixture boundary that maps `PATH/etc/pf.conf` and `PATH/var/db/quicfuscate/pf` and skips `pfctl`. `scripts/tests/fast/test-macos-pf-anchor-installer.sh` covers install, idempotence, read-only check, marker tampering, foreign exact/wildcard refusal, lock exclusion, cleanup, and symlink rejection. The runtime anchor identity is shared through `MACOS_PF_ANCHOR` in `src/implementations/client/killswitch.rs`.
- Local shell syntax, ShellCheck, fixture contract, and diff checks pass. Privileged packet outcomes for block-only, endpoint-only, connected TUN, selected DNS, IPv4/IPv6, unexpected-loss retention, stale cleanup, coexistence, and uninstall remain unavailable on this ARM64 macOS session and are not inferred. Frontend surfaces and fields remain untouched.

## Workspace Seam Contract (2026-08-08, TODO-880)

- `scripts/audits/audit-workspace-seams.py` is the deterministic backend-only inventory for the planned TODO-562 workspace decomposition. It invokes `cargo metadata --no-deps --format-version 1`, scans the root `src/**/*.rs` tree and each workspace `crates/*/src/**/*.rs` tree, records top-level owner sizes, extracts `crate::` edges with source-file attribution, computes strongly connected components, and refuses any tracked or untracked change under the protected Svelte/Tauri prefixes.
- The current revision is one `quicfuscate` workspace package at version `0.4.4`: 50 dependencies (48 normal and 2 dev), 29 features, and 95 Cargo targets (1 lib, 1 custom-build, 9 bins, 9 examples, 71 tests, and 4 benches). The source inventory is 234 Rust files and 203,361 lines, with one 16-node SCC spanning `accelerate`, `brain`, `compress`, `core`, `crypto`, `engine`, `fec`, `implementations`, `interface`, `logging`, `memory_lock`, `optimize`, `qftls`, `simd`, `stealth`, and `transport`.
- `env_utils`, `time_source`, `rng`, and `secret` are the first leaf-contract candidates. TODO-881 owns their `qf-common` extraction with root compatibility re-exports and test-helper parity. This is a prerequisite boundary, not a claim that the leaf extraction resolves the higher-order transport/crypto/qftls/stealth/fec, engine/implementations, or interface/implementations cycles.
- The staged rule is one independently buildable boundary per child task: extract the leaf contract, re-run the inventory and API gates, then split one measured cycle family at a time. Lower-level crates must not depend on `implementations` or the root binary; frontend and Tauri paths remain immutable and any required UI field is deferred to a later frontend task.

## Common Workspace Contract Crate (2026-08-08, TODO-881)

- `crates/qf-common/` is the first independently buildable workspace boundary. It owns environment snapshots, protocol time, secure randomness, and zeroizing secret wrappers, and depends only on `getrandom`, `log`, and `zeroize`. No product subsystem, frontend, or Tauri dependency crosses into this crate.
...
- The gate never resets or edits a remote checkout, removes candidate data, or signals a process. Ambiguous checkout selection, dirty state, unreadable Git history/diffs, unrelated runtime processes, and incomplete artifact binding all resolve to `UNAVAILABLE`.
- Current live evidence is `scripts/out/audits/omega-proof-ownership-20260808T195315Z/ownership.json`: two checkouts are present; `/home/ubuntu/SOFTWARE/QuicFuscate` has 43,722 untracked paths; `/home/ubuntu/CODE/QuicFuscate` has 20 tracked modifications and an unreadable object `c7831a90bd47c77be57fb345fdf4a47a6022d3e1`; PID `1363976` is a running QuicFuscate server outside an owned proof lifecycle. The result is explicitly `UNAVAILABLE`, and no remote mutation occurred.

## PKI Workspace Crate (2026-08-08, TODO-562)

- `crates/qf-pki/` owns the former `src/pki/mod.rs` production PKI implementation: CA hierarchy generation, checked validity windows, chain validation, symlink-safe PEM output, invalid-material quarantine, and the existing PKI regression suite. The root `quicfuscate::pki` module remains a direct compatibility re-export for qftls and server callers.
- The child uses `qf-common::time_source` as its only product boundary. Its manifest keeps the explicit PKI dependencies (`base64`, `libc`, `log`, `rustls`, `zeroize`, plus optional `rcgen` and `time`), and root `server`/`dev-certs` features forward `qf-pki/rcgen` without changing the root feature names. No unnecessary crate-internal PKI item became public and no frontend/Tauri path changed.
- `scripts/audits/audit-workspace-seams.py` reports seven workspace packages (`quicfuscate`, `qf-common`, `qf-control-plane`, `qf-error`, `qf-instrumentation`, `qf-pki`, `qf-privilege`), 240 Rust files, 203,481 source lines, the same 16-module product SCC, the one-way edge `qf-pki -> qf-common`, root edges to all six leaves, and `protected_changes=[]`. Post-commit evidence at revision `e61c21ea6edeca608ce239d315ef941815f4cc21` is `scripts/out/audits/workspace-seams-20260808T212716Z/workspace-seams.json`.
- Every leaf passes its independent Cargo check; qf-pki passes both no-feature and all-feature checks. Workspace all-target check, strict all-target Clippy with `rust-tests`, formatting, and the full all-target `rust-tests` suite pass with 28, 8, 2, 5, 19, 23, and 2,608 root library tests respectively, plus green integration targets. The touched qf-pki dev rebuild is 0.75 seconds real time.
- The all-feature all-target Clippy acceptance lane is explicitly blocked on macOS by the existing Linux-only `compile_error!` in `scripts/tests/rust/rt-transport-uring.rs:8`; it was not weakened. The release binary gate is locally green: `cargo build --release --bin quicfuscate --locked --offline` succeeded, `target/release/quicfuscate --help` returned the canonical CLI, and the 9,815,328-byte binary SHA-256 is `002a79251e7469930412e295c17306ced9996b282abb105c122f85ba2a14f61b`. A pre/post SHA-256 manifest over all 149 protected Svelte/Tauri paths is identical. Post-commit Graphify evidence is fail-closed `BLOCKED` at `scripts/out/audits/graphify-20260808T212644Z/graphify-evidence.json`; CI, native Linux/Omega, and higher-order SCC decomposition remain open under TODO-562.

## REALITY Workspace Crate (2026-08-08, TODO-562)

- `crates/qf-reality/` owns the former `src/reality.rs` implementation: active-probe UDP fallback sessions, bounded target selection, REALITY configuration, cover-site TLS capture/cache, raw-flight parsing, and the complete existing unit-test module. The root `quicfuscate::reality` module remains a direct compatibility re-export for stealth runtime and manager callers.
- The child has no product dependency beyond `qf-common`. Its manifest keeps the explicit runtime boundary (`log`, `parking_lot`, `tokio`, `tokio-rustls`, `rustls-native-certs`, and `webpki-roots`), while `qf-common` owns the canonical environment snapshot and protocol clock. Snapshot construction, stale-session cleanup, and shutdown are explicit child APIs required by the root stealth lifecycle; no frontend or Tauri path is part of this boundary.
- Root compatibility now uses the single qf-common `EnvSnapshot` and `ProtocolClock` types in production and test builds. Deterministic `EnvSnapshot::from_pairs`, `ManualTimeSource`, and thread-local test-clock helpers are feature-gated by `rust-tests` and supplied to child/root test targets through dev-dependency feature wiring, so test helpers do not become part of the default production API.
- Verification passes qf-reality all-target/all-feature checking, `23/23` qf-reality tests, workspace all-target checking with default features and `rust-tests`, strict workspace Clippy with `rust-tests`, formatting, and diff hygiene. The target-size guard triggered one `cargo clean` during the feature matrix and removed 13.9 GiB. Post-commit seam evidence at `scripts/out/audits/workspace-seams-20260808T235531/workspace-seams.json` records source revision `88cbcb67fd916ec6b91ef1f191e947f5b2164546`, eight workspace packages, 240 Rust files, 203,473 source lines, the unchanged 16-module product SCC, eleven dependency edges including the qf-reality normal/dev edges to qf-common, and `protected_changes=[]`. The full all-target `rust-tests` workspace run completed without failures with package counts qf-common=28, qf-control-plane=8, qf-error=2, qf-instrumentation=5, qf-pki=19, qf-privilege=23, qf-reality=23, and root=2,570, with all registered integration targets green. The release binary gate is green after the release-only TLS warning cleanup: `target/release/quicfuscate --help` returned the canonical CLI, the binary is 9,832,176 bytes, and SHA-256 is `086eb07262347ae0f686eb872831c44428dacec09282512bb299868a100e6bf5`; the protected frontend/Tauri path diff is empty. Graphify remains fail-closed `BLOCKED` at `scripts/out/audits/graphify-20260808T215553Z/graphify-evidence.json`.

## DNS Workspace Crate (2026-08-09, TODO-562)

- `crates/qf-dns/` owns the former `src/dns/mod.rs` implementation: bounded DNS query parsing and wire-preserving synthetic responses, DoH client construction with endpoint pre-resolution, pooled DoH requests, authenticated UDP response matching, aggregate deadlines, and bounded admission control. The root `quicfuscate::dns` module remains a direct compatibility re-export for client runtime callers.
- The child depends on `qf-common` for the canonical `ProtocolClock`, plus its explicit `log`, `parking_lot`, `reqwest`, `tokio`, and `url` boundary. The root and child test targets share qf-common's feature-gated manual clock helpers; no frontend or Tauri path is part of this boundary.
- Isolated qf-dns all-target/all-feature checking and `41/41` tests pass. Workspace default/all-target and `rust-tests` checks plus strict workspace Clippy pass; formatting and diff hygiene are clean. Post-commit seam evidence at `scripts/out/audits/workspace-seams-20260809T001834/workspace-seams.json` records source revision `26bc6bb08fa15d63c0b6c408d33382ed51ab447d`, nine workspace packages, 240 Rust files, 203,467 source lines, the unchanged 16-module product SCC, fourteen dependency edges, and `protected_changes=[]`. The full all-target `rust-tests` workspace run completed without failures with package counts qf-common=28, qf-control-plane=8, qf-dns=41, qf-error=2, qf-instrumentation=5, qf-pki=19, qf-privilege=23, qf-reality=23, and root=2,522, plus green integration targets. The release gate is green: canonical `--help`, 9,865,392-byte binary, SHA-256 `23bf3bea7b26dc21c6db12b3669a6d9d44c561d3918a7bf3ad9c1efbd459480b`, and no release warnings. The all-feature all-target Clippy lane remains platform-blocked only by the existing Linux-only `compile_error!` in `scripts/tests/rust/rt-transport-uring.rs:8`; the protected frontend/Tauri path diff is empty. Graphify remains fail-closed `BLOCKED` at `scripts/out/audits/graphify-20260808T221834Z/graphify-evidence.json`.

## Firewall Workspace Crate (2026-08-09, TODO-562)

- `crates/qf-firewall/` owns the backend selection implementation and the root-independent serde/default `FirewallConfig` contract alongside firewall backend selection, iptables/nftables command operations, platform-specific cleanup, owned-resource inspection, and the existing firewall regression tests. Root `quicfuscate::firewall` and `engine::FirewallConfig` remain compatibility reexports, preserving internal `crate::firewall` caller paths.
...
## Telemetry Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-telemetry/` now owns the former `src/optimize/telemetry.rs` implementation and the root-independent serde/default `TelemetryConfig` contract: atomic counters and gauges, plain-text export, CPU-profile mask calculation, memory-usage refresh, and the existing telemetry tests. The root `engine::TelemetryConfig`, `quicfuscate::telemetry`, and `quicfuscate::optimize::telemetry` paths remain compatibility projections; no frontend or Tauri path changed.
- The child depends only on `sysinfo`. Its CPU-profile boundary uses a stable `CpuProfileId` enum, while the optimize adapter maps the existing `CpuProfile` without widening or changing the root API. Pool metrics are refreshed through one owner-installed function hook, so telemetry no longer imports the optimizer or creates a pool as a side effect.
...
- Isolated qf-telemetry all-target/all-feature checking, `15/15` unit tests, and strict all-target/all-feature Clippy pass. The root library compatibility suite passes `2,398/2,398`. Working-tree seam evidence is `scripts/out/audits/workspace-seams-20260809T014342Z/workspace-seams.json`: sixteen workspace packages, 245 Rust files, 203,986 source lines, the 13-module product SCC, one new one-way `quicfuscate -> qf-telemetry` edge, and `protected_changes=[]`.
- The complete serial workspace all-target `rust-tests` run passes 99 result blocks with 2,999 passed, 0 failed, and 6 ignored; qf-telemetry contributes `15/15` and the root library contributes `2,398/2,398`. Workspace all-target checking and strict `rust-tests` Clippy pass. The release gate is green: `cargo build --release --bin quicfuscate --locked --offline` succeeded, canonical `--help` returned, the binary is 9,949,296 bytes, and SHA-256 is `62b12fb50dc3aefd2bdb75c823f339e6e388d098ac20b3af886f3d002b6a0970`. Full all-feature/all-target Clippy is platform-blocked by the existing Linux-only guard at `scripts/tests/rust/rt-transport-uring.rs:8`; no guard was weakened. The post-code seam report is `scripts/out/audits/workspace-seams-20260809T015627Z/workspace-seams.json` with sixteen workspace packages, 245 Rust files, 203,986 source lines, 141 product-module edges, 25 Cargo workspace dependency edges, the unchanged 13-module product SCC, and `protected_changes=[]`. Graphify remains fail-closed `BLOCKED` with exit 2 at `scripts/out/audits/graphify-20260809T015552Z/graphify-evidence.json`; semantic extraction and complete relationship proof remain unavailable. The release cycle ended at 6,710,076 KiB, below the 12-GiB cleanup threshold. No frontend or Tauri path changed.
...
## CPU Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-cpu/` now owns the former `src/optimize/parts/cpu_dispatch.rs` implementation, the former `src/simd/planner.rs` module, the former compression SIMD helpers from `src/optimize/compress.rs` and `src/optimize/simd/compress.rs`, the former `src/optimize/iter.rs` and `src/optimize/sort.rs` implementations, and the runtime substring-search implementation formerly in `src/optimize/string.rs`: runtime CPU feature detection, cache hierarchy, prefetch policy, SIMD dispatch contracts, FEC bitslice dispatch, AEAD planning, byte classification, histograms, pattern search, f32/u32/u64 iterator reductions, sorting, argsort, and accelerated `string_contains`. Root `optimize` and `simd` paths retain compatibility imports while test/rust-tests-gated Base64 helpers remain root-owned.
...
- The extraction removes direct crypto-to-optimize coupling and preserves the root `crate::optimize`/`crate::simd` compatibility surface; runtime guardrails resolve qf-cpu, qf-crypto, qf-privilege, qf-memory-lock, and qf-instrumentation owners directly. The current guardrail audit is clean apart from the existing fail-closed AMX native proof blocker at `scripts/out/audits/runtime-guardrails-20260809T090900Z/results.json`; no guard was weakened. Full all-feature/all-target Clippy remains platform-blocked by the existing Linux-only guards, stopping at `scripts/tests/rust/rt-transport-uring.rs:8` in this run and also covering `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4`, while the all-feature library/binary Clippy lane passes. Graphify remains fail-closed `BLOCKED` at `scripts/out/audits/graphify-20260809T015552Z/graphify-evidence.json`; semantic extraction and complete relationship proof remain unavailable. Current seam evidence is `scripts/out/audits/workspace-seams-20260809T082400Z/workspace-seams.json`: twenty-one workspace packages, 253 Rust files, 204,103 source lines, 134 product-module edges, 45 Cargo workspace dependency edges, and the unchanged 11-module product SCC; `protected_changes=[]`. No frontend or Tauri path changed.
## Transport Types Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-transport-types/` owns the dependency-free transport value contracts formerly defined in `src/transport.rs`: `ConnectionId`, `RecvInfo`, `SendInfo`, `EcnMark`, `CongestionControlAlgorithm`, `Frame`, `EcnCounts`, `Stats`, `PathStats`, and `MAX_CONN_ID_LEN`. The root `quicfuscate::transport` namespace re-exports them unchanged, preserving packet, FEC, brain, engine, and implementation caller paths. The child has no product, crypto, FEC, stealth, or frontend/Tauri dependencies and its six tests cover CID bounds/identity, oversized-input rejection, datagram metadata/flags, borrowed frame payloads, ECN counters, and statistics defaults. Isolated all-target/all-feature checking, strict Clippy, root compatibility checking, and formatting pass. The complete workspace suite passes 102 result blocks with 3,005 passed, 0 failed, and 6 ignored; workspace strict `rust-tests` Clippy is green. The post-cycle-break release gate passes with canonical `--help`, a 9,966,912-byte binary, and SHA-256 `b833e97790379c46eaffa531e771dacce8850a5859fd77aa6ba6a9063aa49857`; the target ended at 8,359,384 KiB, below the 12-GiB cleanup threshold. Current seam evidence is `scripts/out/audits/workspace-seams-20260809T062828Z/workspace-seams.json`: nineteen workspace packages, 247 Rust files, 204,125 source lines, 136 product-module edges, 35 Cargo workspace dependency edges, a 12-module product SCC, `quicfuscate -> qf-transport-types`, and `protected_changes=[]`. Full all-feature Clippy remains blocked by the existing macOS-incompatible Linux guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8`; Graphify remains fail-closed `BLOCKED`. No frontend field/API addition was needed.

## Memory-Pool Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-memory-pool/` owns the former `src/optimize/parts/memory_pool.rs` implementation: adaptive pooled allocation, exact ownership/accounting ledgers, TLS and locked caches, NUMA policy/classification, and Unix/Windows zero-copy buffers. Root `quicfuscate::optimize` re-exports the pool and zero-copy contracts unchanged; the transport-dependent Linux UDP batch adapter remains root-owned.
- The child depends on `qf-common`, `qf-cpu`, `qf-memory-lock`, and `qf-telemetry` plus its explicit allocation/socket libraries. NUMA classification proof tests moved with the implementation. Isolated `23/23` tests, strict all-target/all-feature Clippy, root all-target compatibility checking, root strict Clippy, and formatting pass. Seam evidence is `scripts/out/audits/workspace-seams-20260809T050634Z/workspace-seams.json`: twenty workspace packages, 247 Rust files, 204,066 source lines, 136 product-module edges, 40 Cargo workspace dependency edges, the unchanged 12-module product SCC, and `protected_changes=[]`. This is backend-only and exposes no frontend or Tauri field/API requirement.

## Compression Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-compress/` owns the former `src/compress.rs` implementation: compression analysis and policy, zstd pooled and dictionary frames, entropy/chunk heuristics, persona dictionary registry/persistence, and the large-body pool. Root `quicfuscate::compress` remains a compatibility projection with a private adapter for the root-only snapshot call site.
- qf-compress depends on `qf-common`, `qf-cpu`, `qf-memory-pool`, and `qf-telemetry` plus `zstd` and `log`; qf-cpu owns the SIMD classification, histogram, pattern-search, iterator-reduction, sorting, argsort, and runtime substring-search implementations. Isolated qf-compress all-target/all-feature checking, `17/17` tests, strict Clippy, root compatibility checking, root strict Clippy, formatting, and the complete workspace all-target `rust-tests` suite (`3,005` passed, `0` failed, `6` ignored) pass; qf-cpu contributes `58/58` isolated tests after the sort slice. Current seam evidence is `scripts/out/audits/workspace-seams-20260809T082400Z/workspace-seams.json`: twenty-one workspace packages, 253 Rust files, 204,103 source lines, 134 product-module edges, 45 Cargo workspace dependency edges, and an 11-module product SCC. This is backend-only with no frontend or Tauri field/API projection.
...
- Warning-free release verification passes: `target/release/quicfuscate --help` exits 0, the binary is `10,017,024` bytes, SHA-256 is `b422f25c19e3fc7ccc9eddbe914682488d4cbb06df2687d69ae16d11d31fe357`, and the target ends at `5,912,068 KiB` with `7,522,288 KiB` free. The latest seam report is `scripts/out/audits/workspace-seams-20260809T065128Z/workspace-seams.json`: twenty-two workspace packages, 254 Rust files, 204,107 source lines, 134 product-module edges, 47 Cargo workspace dependency edges, and the unchanged 11-module product SCC; `protected_changes=[]`.
- The runtime guardrail audit is fully green with zero critical failures and zero warnings: `scripts/out/audits/runtime-guardrails-20260809T065928Z/audit-runtime-guardrails.log`. The AMX contract checker now follows the qf-cpu owner paths after the workspace extraction; no platform guard was weakened and no frontend/Tauri path changed.
- The complete `cargo clippy --workspace --all-targets --all-features --locked --offline` lane remains intentionally platform-blocked at the repository-owned Linux-only guard `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` on this macOS ARM64 host; no guard was weakened. The all-feature library/binary Clippy lane is green.
...
- `qf-cpu::count_ascii_printable` now owns the former scalar implementation from `src/accelerate.rs`; the root `quicfuscate::accelerate::count_ascii_printable` path is a compatibility re-export. This removes the last implementation body from the root acceleration compatibility surface without changing callers or behavior.
- The move adds no dependency and remains backend-only. Isolated qf-cpu all-target/all-feature tests pass `66/66`, the root `rt-stealth-ascii-count` integration target passes `2/2`, root all-target checking and strict `rust-tests` Clippy pass, and no frontend or Tauri field/API projection is involved.
- Current seam evidence is `scripts/out/audits/workspace-seams-20260809T073000Z/workspace-seams.json`: twenty-two workspace packages, 255 Rust files, 204,122 source lines, 134 product-module edges, 47 Cargo workspace dependency edges, and the unchanged 11-module product SCC; `protected_changes=[]`.
...
- `crates/qf-transport-version/` owns the QUIC version-negotiation implementation formerly held by `src/transport/version.rs`: v1/v2 constants and support selection, Version Information transport-parameter codec, downgrade-safe negotiation state, GREASE/reserved-version generation, and long-header type-bit mapping. The root `transport::version` namespace is a compatibility adapter translating the child `LongHeaderPacketType` to the established root `PacketType` API.
- The child also owns the root-independent `QuicVersion` configuration enum and its V1/V2 wire mapping; `src/engine/config.rs` re-exports it through the established engine path. Its dependencies are `qf-common`, `qf-error`, and serde. The private scalar varint codec remains confined to the low-frequency Version Information parameter path; packet-number and H3 varints remain on the existing SIMD-dispatched root hotpath, so this extraction introduces no data-plane performance downgrade.
- Isolated qf-transport-version all-target/all-feature tests pass `7/7`; strict all-target/all-feature Clippy passes. Root all-target checking, strict `rust-tests` Clippy, and EngineConfig tests pass `39/39`. Full all-feature/all-target Clippy remains intentionally platform-blocked at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` on macOS ARM64; no Linux guard was weakened.
- Historical warning-free release verification passes with `target/release/quicfuscate --help` exit 0, `10,017,104` bytes, and SHA-256 `93b4d4ae832af30c57788d196b99f93bfed772ce64b3571faf764f4113f30bca`. Fresh seam evidence after the configuration-owner cut is `scripts/out/audits/workspace-seams-20260809T-quic-version-config/workspace-seams.json`: `35` packages, `307` Rust files, `205,449` source lines, `129` module edges, `94` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`; target usage is `9,924,628 KiB` with `7,133,388 KiB` free. Runtime guardrails remain green and no frontend/Tauri path changed.
...
- `crates/qf-transport-pn/` owns the packet-number implementation formerly embedded in `src/transport/pn.rs`: packet-number spaces and ACK scheduling, connection-ID sets, adaptive ACK ranges, stream reassembly buffers, and transport random helpers. The root `transport::pn` module remains a compatibility projection. The SIMD-dispatched varint codec stays root-owned because it depends on the root SIMD transport backend; no scalar fallback or duplicate codec was introduced.
- The child depends only on `qf-common` for protocol time and secure entropy and `qf-transport-types` for `ConnectionId`. Isolated all-target/all-feature checking, `23/23` tests, and strict all-target/all-feature Clippy pass. Root library compatibility checking passes with only the cross-crate `PktNumSpace::MAX_PACKET_NUMBER` contract made public for existing connection code. The complete workspace all-target `rust-tests` suite exits 0 with qf-transport-pn `23/23`, root `1,966/1,966`, and no failure markers across the remaining registered targets. The post-test target reached `9,522,700 KiB`; the mandatory cleanup then removed `10.2 GiB` before the next gate.
...
- `crates/qf-cpu/src/transport.rs` now owns the transport acceleration implementation formerly embedded in `src/optimize/transport.rs`: congestion sample aggregation with VNNI/AVX2/NEON dispatch, bounded bitmap range updates, ECN mark counting, and packet-number reconstruction with scalar parity fallbacks. The root `src/optimize/transport.rs` is a compatibility projection, preserving the established `ConnectionStats` and packet parser paths.
- The qf-cpu boundary now includes the dependency-free `qf-transport-types` statistics contract. The child imports no root transport, FEC, stealth, crypto, or frontend/Tauri implementation. The runtime guardrail owner check now follows `crates/qf-cpu/src/transport.rs`.
...
## TLS Cover Cipher and Audit Test Isolation (2026-08-10, TODO-562)

- `qf-stealth` is the canonical owner of `TlsCoverCipherSuite`, including the stable cipher-name and TLS-ID mappings. The later fingerprint-profile extraction also moved preference parsing and hardware-aware selection into the child; the root provider keeps only runtime crypto installation and compatibility calls.
- The TLS-cover move is root-independent, preserves wire behavior, and has no frontend or Tauri projection. qf-stealth passes `99/99` tests and strict all-target/all-feature Clippy; root Stealth passes `190/190` tests.
- qf-audit test fixtures now derive unique paths and clean the active file, checkpoint, and rotated-segment set before and after each test. This removes stale process-ID artifacts without changing production audit behavior. qf-audit passes `42/42` tests with strict all-target/all-feature Clippy.
- The complete workspace all-target `rust-tests` command exits `0`; workspace strict Clippy and `cargo check --workspace --all-features` pass. All-feature/all-target lanes remain bounded only by the repository-owned Linux-only guards on macOS ARM64. Protected frontend and Tauri paths are unchanged.

## Individual Memory-Lock Status Contract (2026-08-10, TODO-562)

- `qf-memory-lock` is the canonical owner of `MemoryLockAllocationStatus` and its stable diagnostic names. The TLS module keeps `TlsKeyLockStatus` as a compatibility alias, while TLS identity publication and key-buffer cleanup remain qftls-owned.
- This is a contract-only move. Lock syscalls, process-wide coverage detection, zeroization, publication idempotency, and failure behavior are unchanged. qf-memory-lock passes `12/12` tests and strict all-target/all-feature Clippy; root qftls passes `24/24` tests and strict library Clippy.
- No frontend or Tauri path changed and no frontend field/API projection is required.
...
- TODO-680's local release proof is now green: the guarded Optimize suite records `5/5` suite records, `43` executed tests, zero failures, and zero skips in `scripts/out/tests/test-optimization-20260810T-backend-continuation-fast/results.json`. The prior storage-floor interruption is historical only. Native x86/BMI2, AVX10/VNNI, SVE2, Linux, sanitizer, Miri, and Omega proof remain unavailable on this ARM64 macOS host.
- TODO-840's release frame integration target passes `8/8`, including malformed ACK/CID, ARM cursor, and cumulative-capacity cases. TODO-841's guarded release PMTU/prefetch filter passes `1/1`. TODO-842's guarded release transport matrix passes `56/56` across twelve local integration targets; the raw evidence is `/tmp/qf-transport-release-20260810.log`.
- The transport matrix covers packet-number boundaries, unaligned portable output, malformed frame/packet inputs, ARM cursor checks, UDP batch behavior, connection/config/recovery/H3 contracts, PN-space ACK policy, and harness loopback. Native x86_64 AVX2, Linux kernel/invalid-fd, Windows, privileged, and Omega lanes remain explicit unavailable evidence boundaries, not inferred passes.
- The post-matrix target usage is `628,476 KiB` with `19,980,464 KiB` free. The 12-GiB cleanup threshold and 2-GiB free-space floor were not crossed. No frontend or Tauri path changed and no frontend field/API projection is required.
...
## FEC Packet, Decoder, and Zero-Path Workspace Ownership

- The canonical qf-fec implementation now owns pooled `FecPacket` storage, FEC mode/control contracts, GF(2^4)/GF(2^8)/GF(2^16) encoders, the matching decoder families, bounded decoder-dimension validation, Wiedemann scratch/solver helpers, and the zero-overhead encoder/decoder path.
- Root FEC modules retain only compatibility projections and product-specific observer/adaptive integration. The root adaptive/config layer does not extend child-private decoder internals. This preserves existing `quicfuscate::fec` callers without a second production implementation owner.
- Verified gates for this slice: qf-fec all-target/all-feature tests `56/56`, qf-fec strict all-target/all-feature Clippy, root all-target `rust-tests` checking, root strict `rust-tests` Clippy, and focused root FEC tests `235/235`. The seam audit is `scripts/out/audits/workspace-seams-20260809T-continuation-fec-zero/workspace-seams.json`; frontend/Tauri paths remain unchanged and no frontend field/API projection is required.

## FEC Wire Contract Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-fec/src/wire.rs` now owns the root-independent FEC wire contract: `WireMode` codec selection, repair-coefficient generation, profile and packet metadata validation, fixed-header serialization/parsing, source-symbol framing, and typed wire reports/errors. The root `src/fec/wire.rs` retains the pool-backed `WireFecReceiver`, decoder windows, admission telemetry, and the compatibility `FecMode -> WireMode` adapter.
- `crates/qf-fec/src/policy.rs` now owns bounded FEC environment snapshots and runtime-policy parsing; the root exposes only the compatibility alias used by decoders, observers, and adaptive control. The policy leaf depends on `qf-common` and child wire limits, with no engine or transport dependency. Final post-extraction tests, audits, Clippy, and release gates are green below.
- The product dependency remains one-way (`quicfuscate -> qf-fec`); no frontend or Tauri implementation crosses into the leaf, and receiver/decoder ownership remains root-local.
- Isolated qf-fec all-target/all-feature tests pass `56/56`; strict qf-fec Clippy, workspace `rust-tests` checking, strict workspace `rust-tests` Clippy, and the complete workspace all-target `rust-tests` matrix pass. The complete matrix exits 0 with `118` result blocks, `3,016` passed, `0` failed, and `6` ignored; `protected_changes=[]`.
...
## FEC Mode-Manager Workspace Ownership

- `crates/qf-fec/src/manager.rs` owns the root-independent `ModeManager` contract: bounded loss history, target ranking, hysteresis and dwell-time switching, explicit mode/window parameter resolution, finite-redundancy admission, and protocol-time access through `qf-common`.
- The root `src/fec/mod.rs` exposes a doc-hidden compatibility re-export. `src/fec/internal.rs` continues to own product-coupled encoder/decoder variants, lazy decoding, interleaving, packet orchestration, and telemetry.
- This boundary is backend-only and introduces no frontend or Tauri field/API projection. qf-fec all-target/all-feature checking, strict Clippy, and tests pass (`56/56`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, and focused root FEC tests pass (`235/235`).
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-manager/workspace-seams.json`: 35 workspace packages, 289 Rust files, 205,121 source lines, 129 module edges, 93 Cargo workspace dependency edges, and the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), with `protected_changes=[]`. The target is `4,583,968 KiB` and free space is `13,599,680 KiB`, below the `12,582,912 KiB` cleanup threshold.
- The remaining FEC dispatch/adaptive boundary and external Linux/native/CI acceptance gates are still open; any later frontend projection remains explicitly deferred to the frontend phase.

## FEC Variant-Dispatch Workspace Ownership

- `crates/qf-fec/src/variants.rs` owns `EncoderVariant` and `DecoderVariant`: mode-to-codec dispatch, fountain repair identity allocation, pooled repair/result conversion, and decoder progress telemetry.
- The root `src/fec/internal.rs` keeps the product-coupled `LazyDecoder`, `InterleavedEncoder`, and `InterleavedDecoder` orchestration and re-exports the child variants for historical callers. This backend boundary introduces no frontend or Tauri field/API projection.
- Post-move qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`56/56`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, the focused root FEC suite (`235/235`), and `git diff --check` pass. Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-variants/workspace-seams.json`: 35 workspace packages, 290 Rust files, 205,128 source lines, 129 module edges, 93 Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`. Target usage is `4,802,520 KiB` with `12,309,232 KiB` free, below the 12-GiB cleanup threshold. The root lazy/interleaved orchestration and broader product-core decomposition remain open; the frontend projection stays deferred to the frontend phase.

## FEC Interleaved-Encoder Workspace Ownership

- `crates/qf-fec/src/interleaved.rs` owns `InterleavedEncoder`, lane-aware repair identity allocation, represented `(k, n)` shape reporting, and bounded repair-ordinal constants. The root `src/fec/internal.rs` re-exports this encoder for compatibility, while `LazyDecoder` and `InterleavedDecoder` remain root-owned for the next coupled extraction.
- Isolated qf-fec all-target/all-feature checking, strict Clippy, and tests pass (`56/56`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, formatting, `git diff --check`, and focused FEC tests pass (`235/235`). Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-interleaved-encoder/workspace-seams.json`: 35 packages, 291 Rust files, 205,116 source lines, 129 module edges, 93 workspace edges, the unchanged 9-module product SCC, and `protected_changes=[]`. Target usage is `5,018,204 KiB` with `13,094,456 KiB` free, below the 12-GiB cleanup threshold.
- This is a backend-only ownership boundary and requires no frontend or Tauri field/API projection; any UI projection remains deferred to the frontend phase.

## FEC Lazy-Decoder Workspace Ownership

- `crates/qf-fec/src/lazy.rs` owns `LazyDecoder`: bounded pending source/repair state, gap and tail-loss detection, streaming repair admission, lazy/full recovery decisions, partial-result draining, fountain-seed propagation, and lazy-skip telemetry. The root `src/fec/internal.rs` re-exports it for `InterleavedDecoder` and historical callers.
- Isolated qf-fec all-target/all-feature checking, strict Clippy, and tests pass (`56/56`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, formatting, `git diff --check`, and focused FEC tests pass (`235/235`). Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-lazy-decoder/workspace-seams.json`: 35 packages, 292 Rust files, 205,082 source lines, 129 module edges, 93 workspace edges, the unchanged 9-module product SCC, and `protected_changes=[]`. Target usage is `5,258,896 KiB` with `12,844,788 KiB` free, below the 12-GiB cleanup threshold.
- This is a backend-only ownership boundary and requires no frontend or Tauri field/API projection. Root `InterleavedDecoder` remains the next coupled FEC extraction; any UI projection stays deferred to the frontend phase.

## FEC Interleaved-Decoder Workspace Ownership

- `crates/qf-fec/src/interleaved_decoder.rs` owns `InterleavedDecoder`: validated wire-profile construction, source/repair lane routing, full and partial result aggregation, recovery-state queries, seed propagation, and bounded test introspection. The root `src/fec/internal.rs` re-exports it for wire receive, adaptive control, and historical callers.
- Isolated qf-fec all-target/all-feature checking, strict Clippy, and tests pass (`56/56`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, formatting, `git diff --check`, and focused FEC tests pass (`235/235`). Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-interleaved-decoder/workspace-seams.json`: 35 packages, 293 Rust files, 205,084 source lines, 129 workspace/module metrics as recorded (`129` module edges, `93` workspace edges), the unchanged 9-module product SCC, and `protected_changes=[]`. Target usage is `5,510,796 KiB` with `12,330,412 KiB` free, below the 12-GiB cleanup threshold.
- This is a backend-only ownership boundary and requires no frontend or Tauri field/API projection. The remaining FEC root surface is compatibility tests and adaptive/observer integration; any UI projection stays deferred to the frontend phase.
...
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings cargo build --release --bin quicfuscate --offline`; `target/release/quicfuscate --help` exits 0, the binary is `9,937,120` bytes, SHA-256 is `1a647be6e7f9ecdd2a8c46a654b71d8942012796772caade4faaf05db2a371a7`, and final target usage is `9,732,580 KiB` with `4,883,940 KiB` free, below the 12-GiB cleanup threshold.

## Stealth Scheduler Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-stealth/src/chaff.rs` and `crates/qf-stealth/src/probe_detector.rs` now own the former root `src/stealth/parts/chaff.rs` and `src/stealth/parts/probe_detector.rs`: bounded traffic-analysis lifecycle scheduling, jittered and phase-locked cadence, one-slot chaff admission, QUIC PING/PADDING plaintext generation, active-probe matching, bounded timestamp retention, threshold escalation, and response-mode selection. Root files remain public compatibility projections; H3-coupled `CoverTrafficScheduler` stays root-local.
- qf-stealth depends on `qf-common`, `log`, and `rand`; the product edge remains one-way (`quicfuscate -> qf-stealth`). No transport, H3, engine, frontend, or Tauri implementation crosses into the child, and no frontend field/API projection is required.
...
- The all-feature library/binary Clippy lane passes with `RUSTFLAGS=-Dwarnings`; the complete all-feature/all-target Clippy lane remains intentionally platform-blocked by the repository-owned Linux-only guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8` on macOS ARM64; no guard was weakened. Runtime guardrails are fully green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T-qf-stealth-chaff-probe-final/audit-runtime-guardrails.log`; AMX proof and SIMD feature contracts pass.
...
- `crates/qf-stealth/src/profiles.rs` is the canonical owner for the root-independent `BrowserProfile` and `OsProfile` persona identifiers, serde/CLI derives, case-insensitive parsers, and legacy macOS aliases. The later `fingerprint_profile.rs` extraction moved the complete persona matrix into the same child; the root `src/stealth/parts/browser_profiles.rs` is now only a compatibility projection.
...
- `crates/qf-fec/` now owns the former `src/fec/fountain_codes.rs` and `src/fec/gf_tables.rs` implementations plus connection-local seed, decode-prefetch, GF16 slice, Brain/FEC hint, and Kalman smoothing primitives: deterministic LT source selection, robust-soliton distributions, HMAC-derived fountain seeds, GF(2^8)/GF(2^16) arithmetic and inverse tables, architecture-dispatched GF16 slice kernels, SIMD-dispatched XOR combinations, bounded decoder admission and eviction, belief-propagation recovery, partial indexed results, and the moved FEC regression tests. Root FEC and Brain modules preserve historical callers through doc-hidden compatibility projections.
- The child depends on `qf-crypto`, `qf-cpu`, `qf-memory-pool`, and `qf-telemetry` plus `log`; it owns `BrainFecHints` and `KalmanFilter` alongside the default fountain seed, source-symbol bound, connection-local HMAC seed derivation, decode-window prefetch, Galois tables, GF16 scalar byte helpers, and architecture-dispatched u16 slice multiplication used by the root Brain, FEC wire, and codec paths. No stealth, implementation, frontend, or Tauri implementation crosses into the child.
- Isolated qf-fec all-target/all-feature tests pass `51/51`, strict all-target/all-feature Clippy passes, the complete workspace all-target `rust-tests` matrix exits 0 with `117` result blocks, `3,007` passed, `0` failed, and `6` ignored, workspace checking and strict `rust-tests` Clippy pass, and the all-feature library/binary Clippy lane passes; `protected_changes=[]`.
...
- The complete all-feature/all-target Clippy lane remains intentionally platform-blocked by the repository-owned Linux-only integration guard at `scripts/tests/rust/rt-transport-uring.rs:8` on macOS ARM64; no guard was weakened. Runtime guardrails are fully green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T-brainfec-final/audit-runtime-guardrails.log`; AMX proof and SIMD feature contracts pass.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings cargo build --release --bin quicfuscate --offline`; `target/release/quicfuscate --help` exits 0, the binary is `9,920,400` bytes, SHA-256 is `ecf5255b1137fbbaf5a8fed80f3011c40b7bac9b0b8699ab835124df3874e915`, and final target usage is `6,676,268 KiB` with `9,258,820 KiB` free, below the 12-GiB cleanup threshold.

## SIMD Workspace Leaf (2026-08-09, TODO-562)

- `crates/qf-simd/` now owns the former `src/simd/` implementation: runtime CPU dispatch, core XOR/CRC/popcount, GF8/GF16 arithmetic, FEC solvers, QPACK Huffman encode/decode, QUIC varints, packet-header validation, bitstream operations, SHA-256/HMAC dispatch, architecture-specific ARM/x86 helpers, and the existing SIMD parity tests. The root `src/simd.rs` is a compatibility projection, and the root H3 QPACK codec delegates its Huffman tables and codec to this child without retaining duplicate tables or decoding logic.
- The child depends only on `qf-common`, `qf-cpu`, `qf-crypto`, and `qf-telemetry`, plus `sha2-asm` on non-Windows targets. Feature forwarding covers `rust-tests`, `benches`, `prefetch`, `aggressive_inline`, and `internal_avx10_preview`. Root transport frame compatibility uses only doc-hidden ACK and ARM STREAM helper exports; no frontend or Tauri surface is involved.
- Isolated qf-simd all-target/all-feature checking and strict all-target/all-feature Clippy pass; its all-target/all-feature test suite passes `61/61`. Workspace all-target checking with `rust-tests` passes after the adapter move. The seam audit records `33` workspace packages, `273` Rust files, `204,747` source lines, `132` module edges, `84` Cargo workspace dependency edges, and a reduced 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`): `scripts/out/audits/workspace-seams-20260809T143432-qf-simd-final/workspace-seams.json`. The qf-simd edges are only `qf-simd -> qf-common`, `qf-simd -> qf-cpu`, `qf-simd -> qf-crypto`, `qf-simd -> qf-telemetry`, and `quicfuscate -> qf-simd`; `protected_changes=[]`.
- Workspace strict `rust-tests` Clippy and the complete workspace all-target `rust-tests` suite pass: `116` result blocks, `3,007` passed, `0` failed, and `6` ignored. The all-feature library/binary Clippy lane passes; the complete all-feature/all-target Clippy lane remains intentionally platform-blocked by the repository-owned Linux-only guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8` on macOS ARM64, with no guard weakened. The warning-free release gate passes with `target/release/quicfuscate --help` exit 0, a `9,919,664`-byte binary, SHA-256 `e2fd68fca63f1674ada98063ad0a1eb3aebd9eb5445e044653422f152405132d`, and target usage `5,039,748 KiB` with `14,706,484 KiB` free. Runtime guardrails are fully green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T142109-qf-simd-final/audit-runtime-guardrails.log`.

## Stealth Persona Enum Contracts

- `crates/qf-stealth/src/config.rs` is the canonical owner for the root-independent `PaddingStrategy`, `StealthMode`, and `RotationMode` enums, including serde aliases and existing configuration spellings. `src/stealth/parts/config.rs` removes duplicate definitions, while `src/stealth/mod.rs` preserves the historical root re-export paths.
...
- `crates/qf-stealth/src/tls_client_hello.rs` now owns `TlsClientHelloProfileCatalog` and its exact 13 browser/OS metadata combinations. `src/stealth/parts/tls_client_hello.rs` is a compatibility projection for the historical root path; rustls remains the active wire ClientHello owner and no transport override setter is exposed.
- The child depends only on existing root-independent profile contracts and `qf-common`; no connection, transport, FEC, engine, implementation, frontend, or Tauri behavior crosses the boundary. The catalog regression locks the curated matrix and rejects unsupported Safari/Linux metadata.
- qf-stealth all-target/all-feature tests pass `23/23` with strict all-target/all-feature Clippy. Root all-target `rust-tests` checking, strict root `rust-tests` Clippy, and the root Stealth test filter pass `252/252`. Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T-stealth-tls-final-2/audit-runtime-guardrails.log`.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-stealth-tls-catalog-final-2/workspace-seams.json`: `35` workspace packages, `309` Rust files, `205,489` source lines, `128` module edges, `94` Cargo workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`. Warning-free release verification passes with a `9,972,752`-byte binary and SHA-256 `bf108ec19b2e3758ac3be307339b5acde66a649465867bd1c465fe1f83426ad9`; the target guard records `11,894,856 KiB` with `5,010,352 KiB` free, below the cleanup threshold. No frontend or Tauri path changed, and no frontend field/API projection is required.

## Stealth TLS Cover Builder Workspace Ownership

- `crates/qf-stealth/src/tls_cover.rs` now owns the root-independent synthetic TLS record and ClientHello extension builder and its regression tests. `src/stealth/tls_cover.rs` remains the compatibility projection for the historical root path; the root `TlsCoverProvider` retains crypto-context installation, entropy/key lifecycle, handshake state, and transport-facing provider behavior.
- The child depends only on existing browser/OS profile contracts and `qf-common::env_utils::EnvSnapshot`; the ultra-mode flag is read from the supplied snapshot. No provider, transport, frontend, or Tauri behavior crosses the boundary.
...
- The all-feature/all-target check remains platform-blocked on macOS ARM64 by `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4`, which requires Linux; no platform guard was weakened. Warning-free release verification passes with a `9,973,184`-byte binary and SHA-256 `7bef1cd1c79e42579849b237a1970c16eb02d3ff69eefae710aa46eccfd1e8fd`; the post-build target guard records `7,286,392 KiB`, below the `12,582,912 KiB` cleanup threshold. No frontend or Tauri path changed, and no frontend field/API projection is required.

## Stealth Profile Slot Parser Workspace Ownership

- `crates/qf-stealth/src/profiles.rs` now owns the pure `browser[@os]` fingerprint rotation-slot grammar, including trimming, default-OS inheritance, legacy-separator rejection, and bounded error reporting. The parser symbol in `src/stealth/parts/browser_profiles.rs` remains only a root compatibility re-export; the TLS-Cover-coupled `FingerprintProfile` stays root-local, so the parser has one source of truth.
...
- `crates/qf-transport-types/src/lib.rs` now canonically owns `QuicEncryptionLevel`, the root-independent QUIC Initial, EarlyData, Handshake, and Application encryption-level contract. `src/qftls.rs` re-exports it as the historical `crate::qftls::Level` name, so TLS providers, transport callers, and existing tests keep their established API without a duplicate enum.
- The child contract is `repr(C)`, preserves the existing discriminants `0..=3`, and has no connection, rustls, crypto, stealth, frontend, or Tauri dependency. TLS provider lifecycle, packet protection, and `crypto::aead::Level` remain separate owners.
- qf-transport-types all-target/all-feature tests pass `16/16` with strict Clippy. The root qftls filter passes `24/24`, root library checking and strict Clippy pass, and the complete workspace all-target `rust-tests` run exits `0` with `118` result blocks, `3,059` passed, `0` failed, and `6` ignored.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-level-final/workspace-seams.json`: source revision `d69e10446c92be684346387d613c28340f18b25f`, `35` workspace packages, `313` Rust files, `205,646` source lines, `127` module edges, `94` workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-level-final/audit-runtime-guardrails.log`.
...
- `crates/qf-transport-types/src/brain.rs` canonically owns `StealthBrainConfig`: the twelve-field adaptive Brain configuration, exact defaults, environment overrides, bounded clamping, cross-field validation, and the captured-environment constructors. `src/brain.rs` preserves the historical `crate::brain::StealthBrainConfig` path as a compatibility re-export; sensor fusion, FEC steering, transport mutation, and provider lifecycle remain root-owned.
- The child consumes `qf-common::env_utils::EnvSnapshot` only at the configuration boundary and retains no connection, FEC, stealth runtime, optimizer, frontend, or Tauri implementation dependency. Existing environment variable names, defaults, validation errors, and fallback-to-default behavior are unchanged.
- qf-transport-types tests pass `19/19` with strict Clippy; the focused root Brain filter passes `62/62`; root and workspace strict `rust-tests` Clippy plus the all-feature library/binary Clippy lane pass. The complete workspace all-target `rust-tests` run exits `0` with `118` result blocks, `3,062` passed, `0` failed, and `6` ignored.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-brain-config-final/workspace-seams.json`: source revision `d69e10446c92be684346387d613c28340f18b25f`, `35` workspace packages, `314` Rust files, `205,701` source lines, `127` module edges, `95` workspace dependency edges, unchanged 9-module product SCC, the new `qf-transport-types -> qf-common` configuration edge, and `protected_changes=[]`. Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-brain-config-final/audit-runtime-guardrails.log`.
- The all-feature/all-target check remains intentionally blocked by the repository-owned Linux-only guard `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` on macOS ARM64; no guard was weakened. Warning-free release verification passes with `--help` exit `0`, a `9,990,144`-byte binary, SHA-256 `9af45c7eb233c6bdcf0f8f89996102ba945726f84af80ebdbfa82f2f18a688eb`, and target usage `1,694,492 KiB` with `21,017,564 KiB` free, below the `12,582,912 KiB` cleanup threshold. The threshold guard reported `release_threshold_clean=not_needed`; the prior workspace gate automatically cleaned at the threshold. No frontend or Tauri path changed, and no frontend field/API projection is required.

## Stealth Network-Fingerprint Normalization Workspace Ownership


- `crates/qf-stealth/src/fingerprint.rs` now owns the root-independent TCP/ICMP fingerprint implementation: `OsFingerprintProfile`, `PacketNormalizer`, IP-ID behavior, bounded TCP option normalization, incremental IP/TCP checksum helpers, ICMP unreachable policy, and all moved regressions. `src/stealth/fingerprint.rs` remains the historical compatibility projection, including the doc-hidden `required_capacity` contract used by the root connection path.
- The child retains the existing `qf-common`, `clap`, `log`, `rand`, and `serde` dependencies and has no reverse product dependency. It consumes only the qf-stealth `OsProfile` contract; TLS Cover, adaptive FEC, H3, transport, engine, implementation, frontend, and Tauri behavior remain outside the leaf.
- qf-stealth all-target/all-feature tests pass `69/69` and strict Clippy passes. Root all-target `rust-tests` checking, strict root Clippy, and the focused root Stealth filter pass `206/206`; the complete workspace all-target `rust-tests` run exits `0` without failures and the all-feature library/binary Clippy lane passes.
- Seam evidence is `scripts/out/audits/workspace-seams-20260809T-stealth-fingerprint-final-2/workspace-seams.json`: `35` workspace packages, `310` Rust files, `205,501` source lines, `128` module edges, `94` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T-stealth-fingerprint-final-2/audit-runtime-guardrails.log`; the all-feature/all-target check remains blocked only by the Linux-only guard at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` on macOS ARM64.
- Release verification passes with `--help` exit `0`, a `9,972,768`-byte binary, SHA-256 `90f00497ea181c76104853196f577a4f18145e9439d3436c5aa38b0b7799f314`, and final target usage `2,179,060 KiB` with `22,922,776 KiB` free. No frontend or Tauri path changed and no frontend field/API projection is required.
...
- The child depends on `qf-cpu` for prefetch/inline policy plus `libc`, `smallvec`, and `sysinfo`; its low-level helper exports are `#[doc(hidden)]` compatibility contracts rather than a new product API. No transport connection, FEC, crypto, stealth, or frontend/Tauri implementation crosses this boundary.
...
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T112739Z/workspace-seams.json`: 32 workspace packages, 272 Rust files, 204,685 source lines, 133 module edges, 79 Cargo workspace dependency edges, and the unchanged 11-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `optimize`, `qftls`, `simd`, `stealth`, `transport`). qf-transport-batch depends on `qf-transport-udp`, qf-transport-udp depends on `qf-cpu`, and the root edge remains `quicfuscate -> qf-transport-udp`; `protected_changes=[]`.
- The final refreshed runtime guardrail audit is fully green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T114855Z/audit-runtime-guardrails.log`. Its owner-path assertions follow `crates/qf-transport-udp/src/lib.rs` and `fastpath.rs` while preserving both root compatibility adapters. `RUSTFLAGS=-Dwarnings cargo clippy --workspace --lib --bins --all-features --locked --offline -- -D warnings` exits 0; the complete all-feature/all-target Clippy lane remains intentionally blocked by the repository-owned Linux-only guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8` on macOS ARM64. The fresh release verification ended at `8,518,292 KiB` with `12,312,660 KiB` free, below the `12,582,912 KiB` cleanup threshold.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings cargo build --release --bin quicfuscate --locked --offline`; `target/release/quicfuscate --help` exits 0, the binary is `9,919,776` bytes with SHA-256 `449fe522168fecdb73f6a4bb76e3398480ef849b470047103171ce1821c0dc01`.
- No frontend or Tauri path changed, and no frontend field/API projection is required. TODO-562 remains blocked for the coordinated cyclic product-core split and external Linux/native/CI acceptance gates.

## QUIC Varint Codec Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-pn/src/varint.rs` is now the canonical owner of the RFC 9000 variable-length integer codec: SIMD-dispatched encoding and decoding, fixed-width encoding, bounded output handling, and typed `ConnectionError` failures. The root `src/transport/pn.rs` preserves the historical `transport::pn::varint` namespace as a compatibility re-export.
- `qf-transport-version` now consumes the same child codec for Version Information transport-parameter encoding and parsing, removing its private duplicate implementation. The child dependency direction is explicit: `qf-transport-pn -> qf-simd` for the existing runtime dispatch and `qf-error` for the shared error contract; no connection, crypto, FEC, stealth, engine, implementation, frontend, or Tauri implementation crosses this boundary.
- Isolated qf-transport-pn tests pass `27/27`; qf-transport-version tests pass `7/7`; the root packet-number filter passes `11/11`; strict child/root/workspace Clippy, `cargo check --workspace --all-features`, formatting, and `git diff --check` pass. The complete workspace all-target `rust-tests` command exits `0` with no failures, including the root library's `1,658` tests. No release build was required for this contract-only slice.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-varint-postpush/workspace-seams.json`: source revision `7ef0e55a1f048e555815801985f5e7c3c45314fa`, `36` workspace packages, `326` Rust files, `206,558` source lines, `125` module edges, `109` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The post-test disk guard records `7,118,568 KiB` target usage and `5,212,796 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Frontend/Tauri paths remain untouched and no frontend field/API projection is required.

## Transport Frame Varint Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-frames/src/lib.rs` no longer carries a second scalar QUIC varint implementation. Its direct API uses `TransportVarInt`, which delegates to the canonical SIMD-dispatched codec in `crates/qf-transport-pn/src/varint.rs`; the historical public `ScalarVarInt` name remains a type alias for source compatibility.
- The frame leaf now has the one-way dependency `qf-transport-frames -> qf-transport-pn`, while the root `src/transport/frames.rs` keeps its narrow `VarIntCodec` adapter for the root transport namespace and acceleration hooks. Frame serialization, zero-copy parsing, packet-space legality, and all wire behavior remain unchanged.
- Isolated qf-transport-frames tests pass `21/21`; qf-transport-pn `27/27`; qf-transport-version `7/7`; the root frame filter passes `6/6`; strict frame/root/workspace Clippy, `cargo check --workspace --all-features`, formatting, diff hygiene, and the complete workspace all-target `rust-tests` command exit `0` with no failures. The root library contributes `1,658` passed tests.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-frames-varint-postpush/workspace-seams.json`: source revision `252a27b859d271d1d606236803eedf2ee31f0f17`, `36` workspace packages, `326` Rust files, `206,514` source lines, `125` module edges, `110` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The post-test disk guard records `9,120,256 KiB` target usage and `3,104,604 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Frontend/Tauri paths remain untouched and no frontend field/API projection is required.

## H3 Varint Adapter Ownership (2026-08-10, TODO-562)

- `src/transport/h3_parts/connection.rs` now routes H3 frame, SETTINGS, stream, DATAGRAM, and MASQUE capsule varints through `qf-transport-pn::varint`; the local SIMD-dispatch duplicate is gone. The H3 helper retains its historical `Vec<u8>` append behavior for unrepresentable values and preserves the existing `BufferTooShort` and `FrameError` result categories.
- The adapter uses the canonical eight-byte QUIC varint buffer and maps the shared child error contract at the H3 boundary: `BufferTooShort` remains `Error::BufferTooShort`, `InvalidPacket` remains `Error::FrameError`, and any future child error fails closed as `Error::InternalError`. No H3 wire shape, capsule framing, stream classification, or frontend/Tauri surface changed.
- The focused root H3 filter passes `76/76`; root strict Clippy, `cargo check --workspace --all-features`, strict workspace `rust-tests` Clippy, formatting, diff hygiene, and the complete workspace all-target `rust-tests` command exit `0` with no failures. The root library contributes `1,658` passed tests; no release build was required for this adapter-only slice.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-h3-varint-postpush/workspace-seams.json`: source revision `c9c4caaee05899c90e093d38345e032efd75851c`, `36` workspace packages, `326` Rust files, `206,509` source lines, `125` module edges, `110` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The disk guard stopped the first post-test Clippy attempt when free space reached `1,751,328 KiB`; `cargo clean` removed `9.1 GiB` of generated target data before the remaining gates. The final post-test guard records `3,352,804 KiB` target usage and `8,689,472 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Frontend/Tauri paths remain untouched and no frontend field/API projection is required.

## H3 Config Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/h3.rs` is now the canonical owner of the root-independent HTTP/3 `Config` value contract: QPACK table capacity, blocked-stream limit, field-section default, setters, and read-only accessors. `src/transport/h3.rs` preserves the historical root path through a compatibility re-export; H3 connection validation and QPACK/stream behavior remain root-owned.
- The child adds only the direct `qf-error` dependency required to preserve `Config::new() -> Result<Config, ConnectionError>` across the crate boundary. Root tests use accessors instead of private-field reach-through; defaults, setter behavior, error type, and wire settings remain unchanged. No frontend or Tauri field/API projection is required.
- qf-transport-types tests pass `36/36`; the focused root H3 filter passes `76/76`; root check and strict Clippy, workspace all-feature checking, workspace all-target strict `rust-tests` Clippy, formatting, diff hygiene, and the complete workspace all-target `rust-tests` command exit `0` with no failures. The root library contributes `1,658` passed tests. No release build was required for this contract-only slice.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-h3-config-postpush/workspace-seams.json`: source revision `3b046e9f495099231a4d1696d01832bec1423d45`, `36` workspace packages, `326` Rust files, `206,555` source lines, `125` module edges, `111` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The post-test disk guard records `4,781,140 KiB` target usage and `6,572,096 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Frontend/Tauri paths remain untouched and no frontend field/API projection is required.
...
- `crates/qf-fec/src/runtime.rs` now owns process-wide FEC initialization: bounded Rayon thread-cap policy, one-time global pool setup, Galois-table initialization, and the adaptive cadence gate. The root keeps compatibility exports only, and the direct root Rayon dependency was removed after the owner move.
- qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`81/81`). Root all-target checking with `rust-tests`, strict `rust-tests` Clippy, focused FEC tests (`211/211`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-fec-runtime/workspace-seams.json` with `35` workspace packages, `298` Rust files, `205,096` source lines, `129` module edges, `93` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`.
- Target usage is `9,849,564 KiB` and free space is `6,619,596 KiB`, below the configured `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Any UI exposure remains deferred to the frontend phase.

## FEC Matrix Workspace Ownership

- `crates/qf-fec/src/matrix.rs` now owns the checked GF(2^8) matrix operation and `MatrixError` contract. Root FEC exports the child operation for historical callers; no second matrix implementation remains.
- The moved implementation retains input-shape and overflow validation, central GF-table initialization, and XOR-accumulating row multiplication. Its malformed-shape regression now runs in qf-fec alongside the existing wire and codec tests.
- Isolated qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`81/81`). Root all-target checking with `rust-tests`, strict `rust-tests` Clippy, focused FEC tests (`211/211`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-fec-matrix/workspace-seams.json` with `35` workspace packages, `297` Rust files, `205,080` source lines, `129` module edges, `93` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`.
- Target usage is `8,254,980 KiB` and free space is `7,300,432 KiB`, below the configured `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Any UI exposure remains deferred to the frontend phase.

## FEC Wire-Receiver Workspace Ownership

- `crates/qf-fec/src/receiver.rs` now owns the pool-backed wire receiver: bounded retained windows, epoch/profile validation, systematic/source admission, repair coefficient reconstruction, Fountain symbol identity limits, streaming missing-source checks, partial/full recovery draining, and source-only delivery. The root `src/fec/wire.rs` retains compatibility exports and the root `FecMode` mapping adapter.
- The moved receiver preserves pool ownership and all wire safety checks without importing transport, engine, implementations, frontend, or Tauri code. The root connection continues to use the same historical `WireFecReceiver` path and seed lifecycle.
- Isolated qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`80/80`). Root all-target checking with `rust-tests`, strict `rust-tests` Clippy, focused FEC tests (`211/211`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-fec-wire-receiver/workspace-seams.json` with `35` workspace packages, `296` Rust files, `205,152` source lines, `129` module edges, `93` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`.
- Target usage is `8,106,792 KiB` and free space is `6,338,748 KiB`, below the configured `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Any UI exposure remains deferred to the frontend phase.

## FEC Observer Workspace Ownership

- `crates/qf-fec/src/observer.rs` now owns connection-local FEC observer state: ACK-delay EWMA smoothing, ECN evidence, profile and platform resolution, immutable runtime-policy snapshots, Brain hint attachment, adaptive streaming interval computation, and one-shot redundancy-hint admission. The root compatibility file keeps the root `TransportObserver` implementation and applies the returned FEC-owned delta to the live transport Connection.
- The child uses only FEC/common contracts and `parking_lot`; the transport trait and Connection stay root-owned. No reverse dependency, public transport API widening, frontend path, or Tauri path was introduced.
- Isolated qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`56/56`). Root all-target checking with `rust-tests`, strict `rust-tests` Clippy, focused FEC tests (`235/235`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-fec-observer/workspace-seams.json` with `35` workspace packages, `295` Rust files, `205,140` source lines, `129` module edges, `93` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`.
- Target usage is `7,279,060 KiB` and free space is `10,410,632 KiB`, below the configured `12,582,912 KiB` cleanup threshold. Any later UI exposure remains deferred to the frontend phase.

## FEC Loss-Estimator Workspace Ownership

- `crates/qf-fec/src/loss.rs` now owns the connection-local `LossEstimator`: bounded EMA smoothing, optional Kalman updates, CUSUM change detection and retuning, bounded burst-window classification, clean-link confirmation, fountain-readiness admission, disturbance reporting, and burst-variance calculation. Root FEC retains a compatibility re-export, and adaptive control constructs the child from validated product policy plus ambient Kalman overrides.
- The move preserves existing loss semantics and keeps test-only constructors/probes feature-gated. No second production estimator remains in `src/fec/parts/codecs_and_observers.rs`, and no public frontend or Tauri contract is widened.
- Isolated qf-fec all-target/all-feature checking, strict all-target/all-feature Clippy, and tests pass (`56/56`). Root all-target checking with `rust-tests`, strict `rust-tests` Clippy, focused FEC tests (`235/235`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-loss-estimator/workspace-seams.json` with `35` workspace packages, `294` Rust files, `205,068` source lines, `129` module edges, `93` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`.
- Target usage is `5,697,220 KiB` and free space is `12,349,896 KiB`, below the configured `12,582,912 KiB` cleanup threshold. This backend-only slice requires no frontend or Tauri field/API projection; any UI exposure remains deferred to the frontend phase.

## FEC Configuration Workspace Ownership

- `crates/qf-fec/src/config.rs` owns the complete `FecConfig` implementation: adaptive-FEC TOML parsing, defaults, product defaults, validation, mode/window policy, and public configuration fields. `src/fec/mod.rs` re-exports the type as the historical root API.
- Engine compatibility is expressed through the root-independent `EngineFecMode` and `EngineFecSection` traits in qf-fec. `qf-engine-types::FecMode` and `qf-engine-types::FecSection` implement those traits, while `src/engine/config.rs` re-exports the engine contracts and `src/fec/mod.rs` preserves the historical qf-fec compatibility exports. qf-fec remains independent of engine, transport, implementations, frontend, and Tauri code; `serde` and `toml` are direct parsing dependencies.
- qf-fec all-target/all-feature checking, strict Clippy, and tests pass (`81/81`); root all-target `rust-tests` checking, strict `rust-tests` Clippy, focused FEC tests (`211/211`), formatting, and `git diff --check` pass. Seam evidence: `scripts/out/audits/workspace-seams-20260809T-fec-config/workspace-seams.json` with `35` workspace packages, `299` Rust files, `205,184` source lines, `129` module edges, `93` workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`.
- The post-gate target guard records `11,212,088 KiB` with `2,997,980 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. No frontend or Tauri path changed; UI projection remains deferred.

## FEC Runtime-Plan Workspace Ownership

- `crates/qf-fec/src/runtime_plan.rs` is the canonical owner for root-independent FEC runtime-plan resolution: compute-profile snapshots, explicit memory-pool and environment inputs, policy-aware mode/target selection, CPU-dependent stream cadence, wire-safe `(k, n, depth)` normalization, partial-recovery admission, and loss-estimator construction. The root adaptive controller supplies the existing global pool and keeps product lifecycle wiring.
- The child depends on qf-cpu, qf-common, qf-memory-pool, and its existing FEC policy/target/loss contracts without importing engine, transport, implementations, frontend, or Tauri. Root compatibility re-exports preserve the historical `FecAmbientInputs`, `FecComputeProfile`, `FecRuntimePlan`, and `wire_safe_encoder_params` paths.
...
- `crates/qf-fec/src/state.rs` is the canonical owner for the root-independent `FecTelemetrySnapshot` and `FecPolicyChange` value contracts. The root adaptive controller keeps live counters and policy-transition behavior, and root FEC re-exports both contracts unchanged for existing engine, connection, implementation, and test callers.
- The child state contract depends only on `FecControlPolicy` and `FecMode`; it imports no transport, engine, implementation, frontend, or Tauri code. Existing fields and telemetry semantics are preserved.

## Adaptive FEC Controller Workspace Ownership

- `crates/qf-fec/src/adaptive_controller.rs` and `gf16_and_config.rs` canonically own `AdaptiveFec`: pooled interleaved encode/decode, block-safe mode transitions, loss and transport feedback, wire-profile publication, telemetry, SIMD selection, and runtime control deltas. The root FEC module keeps compatibility re-exports for the historical type and switch-reason paths.
- Product construction injects the existing optimizer global pool through `AdaptiveFec::new_with_snapshot_and_pool`; standalone qf-fec construction creates an explicit child-owned pool. No root, transport, engine, implementation, frontend, or Tauri dependency crosses into qf-fec.
- Existing root FEC tests retain their field and method access through hidden compatibility surface. Verification is complete for this slice: qf-fec all-target/all-feature tests pass `81/81`; the focused root FEC suite passes `249/249`; qf-fec all-target/all-feature Clippy, root all-target `rust-tests` checking, strict workspace `rust-tests` Clippy, the complete workspace all-target `rust-tests` matrix, `cargo check --workspace --all-features`, all-feature library/binary Clippy, formatting, and diff hygiene pass. The complete all-feature/all-target check remains platform-blocked by `scripts/tests/rust/rt-transport-uring.rs:8` on macOS ARM64.
- Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260809T-adaptive-fec-final-2/audit-runtime-guardrails.log`. Warning-free release verification passes; `target/release/quicfuscate --help` exits `0`, the binary is `9,972,752` bytes, and SHA-256 is `a43b1782cd94e3afd07bced762954c0b1a9116470048e6aa56dd5610319f6b07`.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260809T-adaptive-fec-final-2/workspace-seams.json`: `35` workspace packages, `308` Rust files, `205,472` source lines, `128` module edges, `94` Cargo workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`. No frontend or Tauri path changed; any UI projection remains explicitly deferred to the frontend phase.

## Transport Traffic-Policy Workspace Ownership

- `crates/qf-transport-types/src/traffic.rs` owns `TrafficAnalysisDefense` and `TrafficAnalysisPolicy`, including parsing, serde aliases, defaults, validation, ceiling intersection, and wire-cost estimation. `src/transport/config.rs` re-exports the child types so the historical `transport::config` paths remain stable.
- The child has no connection, FEC, stealth, engine, implementation, frontend, or Tauri dependency. qf-transport-types uses serde for the existing configuration serialization contract. qf-transport-types all-target/all-feature tests pass `10/10`, and root transport configuration regressions pass `50/50`.
- Seam evidence is `scripts/out/audits/workspace-seams-20260809T-transport-traffic-policy/workspace-seams.json`: `35` packages, `302` Rust files, `205,269` source lines, `129` module edges, `93` workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. The free-space guard triggered cleanup at `1,555,740 KiB`; post-clean target usage was `0 KiB` with `15,491,160 KiB` free. Frontend/Tauri paths remain untouched and UI projection is deferred.

## Transport Runtime-Control Contracts

- `crates/qf-transport-types/src/runtime.rs` is the canonical owner for the root-independent `FecControlDelta` and `BrainRuntimePermissions` contracts. `src/transport.rs` re-exports them for existing Connection, Brain, StealthManager, engine, and test callers.
- The child has no product, connection, FEC implementation, frontend, or Tauri dependency; inert FEC-delta and fully permissive Brain defaults are preserved. qf-transport-types tests pass `12/12`, and the complete root transport test filter passes `312/312`.
...
- `crates/qf-transport-recovery/src/lib.rs` now owns `PmtuPolicy`, its defaults, and DPLPMTUD bound/timer validation. `src/transport/config.rs` re-exports the type to preserve the historical transport configuration API without a duplicate owner.
- qf-transport-recovery all-target/all-feature tests pass `38/38`; root all-target `rust-tests` checking, strict `rust-tests` Clippy, and the complete root transport test filter pass `312/312`. Formatting and `git diff --check` pass.
- Seam evidence is `scripts/out/audits/workspace-seams-20260809T-pmtu-policy/workspace-seams.json`: `35` workspace packages, `303` Rust files, `205,313` source lines, `129` module edges, `93` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. The post-gate target guard records `4,854,520 KiB` target usage and `13,195,432 KiB` free, below the cleanup threshold and above the free-space floor. No frontend or Tauri path changed; UI projection remains deferred.

## Transport Stealth-Contract Workspace Ownership

- `crates/qf-transport-types/src/stealth.rs` now owns the root-independent `BrowserProfile`, `StealthRuntimePolicy`, and `StealthRuntimeDelta` contracts. `src/transport.rs` and qf-transport-recovery preserve the historical re-export paths, and qf-transport-cc preserves `cc::stealth_shaper::BrowserProfile`.
- qf-transport-types all-target/all-feature tests pass `14/14`, qf-transport-cc tests pass `92/92`, qf-transport-recovery tests pass `38/38`, and the root transport filter passes `312/312`. Root all-target `rust-tests` checking, strict `rust-tests` Clippy, isolated checks/Clippy, formatting, and `git diff --check` pass.
- Seam evidence is `scripts/out/audits/workspace-seams-20260809T-transport-stealth-contracts/workspace-seams.json`: `35` workspace packages, `304` Rust files, `205,336` source lines, `129` module edges, `94` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. The post-gate target guard records `6,179,472 KiB` target usage and `12,314,976 KiB` free, below the cleanup threshold and above the free-space floor. No frontend or Tauri path changed; UI projection remains deferred.

## Transport PMTU-State Workspace Ownership

- `crates/qf-transport-recovery/src/pmtu.rs` now owns the DPLPMTUD `PmtuState` machine: validated construction, probe scheduling, congestion-bypass timing, binary-search confirmation/loss, black-hole reset, and above-floor ACK evidence. The root connection module keeps only the independent prefetch helpers.
- qf-transport-recovery all-target/all-feature checking, strict Clippy, and tests pass `41/41`; root all-target `rust-tests` checking, strict `rust-tests` Clippy, and the complete root transport test filter pass `309/309`. The child keeps state fields private while exposing only the safe packetization query and test-gated compatibility setter.
- Seam evidence is `scripts/out/audits/workspace-seams-20260809T-transport-pmtu-state/workspace-seams.json`: `35` workspace packages, `305` Rust files, `205,349` source lines, `129` module edges, `94` Cargo workspace dependency edges, unchanged 9-module product SCC, and `protected_changes=[]`. The post-gate target guard records `7,276,704 KiB` target usage and `11,557,068 KiB` free, below the cleanup threshold and above the free-space floor. No frontend or Tauri path changed; UI projection remains deferred.

## MASQUE Downlink Queue Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/masque.rs` is the canonical owner for the root-independent bounded `MasqueDownlinkQueue` FIFO and `MasqueDownlinkQueueReject` admission contract. It preserves packet-count and byte-budget bounds, FIFO order, byte accounting, retry ownership, and discard accounting.
- `src/core_parts/connection.rs` keeps the historical `crate::core::{MasqueDownlinkQueue, MasqueDownlinkQueueReject}` paths as compatibility re-exports. DNS interception, live authentication/routing, and server metrics consume the child contract directly; no second queue implementation remains.
- The child queue has no connection, transport runtime, FEC, stealth, engine, implementation, frontend, or Tauri dependency. This ownership move changes no wire format or frontend surface. The measured product SCC remains coordinated because implementations still use the concrete `QuicFuscateConnection`; the queue move alone is not a claim that the SCC is split.
- Validation evidence: qf-transport-types tests `22/22` and strict all-target/all-feature Clippy pass; qf-fec tests `81/81`; qf-transport-cc tests `94/94` and strict all-target/all-feature Clippy pass; the focused root core filter passes `33/33`.
- The complete workspace all-target `rust-tests` suite exits `0` with `118` result blocks, `3,067` passed, `0` failed, and `6` ignored. Strict workspace Clippy, `cargo check --workspace --all-features`, and all-feature library/binary Clippy pass with `RUSTFLAGS=-Dwarnings`.
- The complete all-target check remains platform-blocked only by the repository-owned macOS-inapplicable guard `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4`; the complete all-target Clippy lane remains platform-blocked only by `scripts/tests/rust/rt-transport-uring.rs:8`. Both are Linux-only compile guards and no guard was weakened.
- Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-connection-stats-final/audit-runtime-guardrails.log`. Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-connection-stats-final/workspace-seams.json`: `35` workspace packages, `317` Rust files, `205,846` source lines, `127` module edges, `96` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), `implementations -> core` at three concrete references, and `protected_changes=[]`.
- Warning-free release verification passes with `target/release/quicfuscate --help` exit `0`; the binary is `9,989,968` bytes with SHA-256 `32cb41223f784570f8be1f09e9dd251957f1f51e563764ae1672e2b232be2caa`. The final target guard records `11,448,400 KiB` with `9,508,052 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor.
- `crates/qf-fec/src/state.rs` now owns `ActiveFecPolicyChange`, `crates/qf-transport-types/src/handlers.rs` owns the MASQUE callback aliases, and `crates/qf-transport-cc/src/stats.rs` owns `ConnectionStats` plus bounded congestion-window projection. Root core and FEC namespaces retain compatibility re-exports; the wire/runtime behavior is unchanged. Frontend and Tauri paths remain untouched and no frontend field/API projection is required.

## TUN and Fastpath Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/tun.rs` now owns the root-independent TUN constants, `TunError`, `TunConfig`, shared address/MTU validation, `TunCapabilities`, `TunReadContract`, `TunDevice`, and `TunFactory` contracts. `crates/qf-transport-types/src/fastpath.rs` owns `FastpathMode` and its captured-environment policy parser.
- `src/interface.rs` keeps the platform backends, pooled `TunPacket`/`TunInterface` runtime, capability probing, factory registration, telemetry, and native lifecycle behavior. Its historical `quicfuscate::interface::*` contract paths remain compatibility re-exports, so Linux, macOS, Windows, iOS, and external-factory implementations do not change their public signatures.
- The child remains root-independent and imports only `std`, `qf-common`, and the existing logging boundary. No connection, engine, FEC, stealth runtime, frontend, or Tauri implementation crosses into the contract owner; no wire format or frontend projection changes.
- Backend-wide validation is green: qf-transport-types all-target/all-feature tests pass `27/27` with strict Clippy; the complete workspace all-target `rust-tests` suite exits `0` with `118` result blocks, `3,072` passed, `0` failed, and `6` ignored; strict workspace `rust-tests` Clippy, `cargo check --workspace --all-features`, and all-feature library/binary Clippy pass with `RUSTFLAGS=-Dwarnings`. The complete root library `rust-tests` filter remains `1,659/1,659`.
- The complete all-target check exits `101` only at the repository-owned macOS-inapplicable Linux guard `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4`; the all-target Clippy lane is likewise blocked by the Linux-only test guards, including `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8`. No guard was weakened.
- Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-tun-contract-final/audit-runtime-guardrails.log`. Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-tun-contract-final/workspace-seams.json`: `35` workspace packages, `319` Rust files, `205,949` source lines, `126` module edges, `96` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings cargo build --release --bin quicfuscate --locked --offline`; `target/release/quicfuscate --help` exits `0`, the binary is `9,990,080` bytes, and SHA-256 is `d1c8fdfb35a5936296ba4b3f20efa75440d8477152458a30de79953766ba99a2`. The final target guard records `3,106,692 KiB` with `20,537,516 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. No frontend or Tauri field/API projection is required by this backend slice; any UI projection remains explicitly deferred to the frontend phase.

## Transport Packet Protocol Contract Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/protocol.rs` now owns the root-independent `Epoch`, `PacketType`, decoded transport `Header`, and `TransportError` value contracts. The root `transport` namespace preserves `Epoch`, `PacketType`, `Header`, and the historical `Error` alias through compatibility re-exports.
- The protocol child contains no connection, packet I/O, crypto, FEC, stealth, engine, implementation, frontend, or Tauri dependency. Packet-type epoch mapping and indexed epoch ranges retain their historical behavior; packet parsing continues to use the separate `transport::packet::Header` wire adapter.
- This is a backend-only contract move with no wire-format or frontend surface change. qf-transport-types all-target/all-feature tests pass `30/30` with strict Clippy; the root library `rust-tests` filter passes `1,659/1,659`; the complete workspace all-target `rust-tests` suite exits `0` with `118` result blocks, `3,072` passed, `0` failed, and `6` ignored; strict workspace `rust-tests` Clippy, `cargo check --workspace --all-features`, and all-feature library/binary Clippy pass with `RUSTFLAGS=-Dwarnings`.
- The complete all-target check and all-target Clippy remain platform-blocked on macOS ARM64 by the repository-owned Linux-only guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8`; no guard was weakened.
- Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-protocol-final/audit-runtime-guardrails.log`. Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-protocol-final/workspace-seams.json`: `35` workspace packages, `320` Rust files, `205,975` source lines, `126` module edges, `96` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings cargo build --release --bin quicfuscate --locked --offline`; `target/release/quicfuscate --help` exits `0`, the binary is `9,990,080` bytes, and SHA-256 is `7e7e9cb010ecf8ad973d0d0a99cb55d068a5fe8f2d90d7881cff2f67909b38df`. The final target guard records `8,485,768 KiB` with `13,148,664 KiB` free, below the cleanup threshold and above the build floor. No frontend or Tauri field/API projection is required.

## Engine Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-engine-types/src/lib.rs` now owns the engine `EngineMode` and `StealthMode` enums, serialized `EngineSection` configuration and validation, the engine-facing `FecMode` adapter, the engine-facing `FecSection` runtime projection and `FecSectionError`, lifecycle `EngineState`, typed `DataPlaneFault`, `EngineError`, and `ConfigError` contracts, `EngineEvent`/`EngineCallback`, atomic `EngineStats`, immutable `StatsSnapshot`, and `DisconnectReason`. The root `engine` module re-exports the child contracts while retaining concrete engine orchestration and configuration-owned commands; the `ConfigError` to `EngineError` conversion is child-owned to preserve the dependency boundary.
- The child depends on the root-independent `qf-fec` contract, `serde`, and the standard library. It contains no client/server runtime, TUN, transport implementation, stealth, frontend, or Tauri implementation; no frontend field/API projection is required.
- `qf-engine-types` all-target/all-feature tests pass `8/8` with strict Clippy; the root library `rust-tests` filter passes `1,659/1,659`; the complete workspace all-target `rust-tests` suite exits `0` with `119` result blocks, `3,083` passed, `0` failed, and `6` ignored; strict workspace `rust-tests` Clippy, `cargo check --workspace --all-features`, and all-feature library/binary Clippy pass with `RUSTFLAGS=-Dwarnings`.
- The complete all-target check and Clippy remain platform-blocked on macOS ARM64 by repository-owned Linux-only guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8`; both are unchanged. Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-fec-section-final/audit-runtime-guardrails.log`.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-fec-section-final/workspace-seams.json`: `36` workspace packages, `321` Rust files, `206,097` source lines, `126` module edges, `98` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings CARGO_BUILD_JOBS=2 cargo build --release --bin quicfuscate --locked --offline`; `target/release/quicfuscate --help` exits `0`, the binary is `9,990,688` bytes, and SHA-256 is `3ede1c864adbbe8d8e06d8f46c9ab034f9ba5ddd758b159d83d67ccba59f08b3`. The final target guard records `2,827,664 KiB` with `20,052,236 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor.

## Engine Configuration Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-engine-types/src/lib.rs` now canonically owns the root-independent engine mode, lifecycle, error, event, callback, statistics, and disconnect contracts together with the serialized `EngineSection`, `SecurityConfig`, `OptimizationConfig`, `TransportConfig`, `ConnectionConfig`, `InterfaceConfig`, canonical client-tunnel address contracts, `QKeyConfig`/zeroizing `QKeyToken`, structured engine command/result contracts, and their validation/runtime projections. Historical root paths remain available through `src/engine/config.rs`, `src/engine/engine.rs`, and `src/engine/qkey.rs` compatibility reexports.
- The child dependency direction is explicit: `qf-engine-types -> qf-common`, `qf-cpu`, `qf-firewall`, `qf-fec`, `qf-memory-lock`, `qf-transport-cc`, `qf-transport-recovery`, `qf-transport-types`, and `qf-transport-version`, plus `serde`, `sha2`, and the existing direct support crates. The root owns the aggregate `EngineConfig`, `StealthSection`, concrete runtime adapters, QKey encoding/registry, and orchestration. No frontend or Tauri implementation crosses this boundary.
- qf-engine-types focused all-target/all-feature tests pass `16/16`; the root `engine::config` filter passes `38/38`, and the complete workspace all-target `rust-tests` command exits `0`. Strict child/root Clippy remains green; no frontend/Tauri path is involved.
- The complete `--all-features --all-targets` check and Clippy lanes remain intentionally unavailable on macOS ARM64: both stop at the repository-owned Linux-only compile guard `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` with its explicit `requires Linux` message. The all-feature library/binary lanes pass, and no platform guard was weakened.
- The refreshed runtime guardrail audit is fully green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-engine-config-final-final/audit-runtime-guardrails.log`. The audit owner probes were updated to follow the moved interface and memory-lock contracts; no guard was weakened.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-engine-config-final/workspace-seams.json`: source revision `d69e10446c92be684346387d613c28340f18b25f`, `36` workspace packages, `322` Rust files, `206,215` source lines, `125` module edges, `106` workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- Warning-free release verification passes with `RUSTFLAGS=-Dwarnings CARGO_BUILD_JOBS=2 cargo build --release --bin quicfuscate --locked --offline`; `target/release/quicfuscate --help` exits `0`, the binary is `9,991,520` bytes, and SHA-256 is `063730bf0d56250b09b3c9098c36517c0064a38e85b58e52e397d6016672e604`. The post-release guard recorded `9,816,268 KiB` target usage and `12,779,780 KiB` free; the final post-feature guard recorded `11,021,556 KiB` and `10,553,436 KiB`, respectively. Both remain below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. The mandatory initial `cargo clean` was already applied before this work; no threshold-triggered clean was needed afterward.
- TODO-562 remains blocked for the coordinated 9-module product-core split and external Linux/native/CI acceptance gates. This slice reduces contract ownership but does not claim to dissolve the cyclic runtime implementation. No frontend or Tauri field/API projection is required; the protected frontend paths remain unchanged.

## Backend Continuation Verification (2026-08-10)

- The backend continuation closes the locally actionable contract and crypto boundary work without touching the protected frontend/Tauri paths. `qf-engine-types` owns the serialized engine configuration, QKey, control-plane command/result, lifecycle, statistics, and runtime projection contracts; the root keeps orchestration, aggregate configuration, QKey encoding/registry, and compatibility re-exports. `qf-crypto` now rejects packet numbers above QUIC's 62-bit limit at every AEAD trait boundary, and the property-suite counter generators use the same valid range.
- Focused evidence is green: qf-engine-types all-target/all-feature tests `15/15`, qf-crypto all-feature library tests `137/137`, the AEAD property suite `12/12`, and the root engine/config/QKey filter `77/77`. The complete workspace all-target `rust-tests` run has `119` result blocks, `3,093` passed, `0` failed, and `6` ignored. Workspace strict Clippy and the workspace all-target check with `rust-tests` pass with `RUSTFLAGS=-Dwarnings`; all-feature lib/bin check, Clippy, and tests pass as well.
- The all-feature/all-target check and Clippy lanes remain platform-bounded on macOS ARM64 by the unchanged repository-owned Linux-only compile guards at `scripts/tests/rust/rt-io-hotpath-kernel-integration.rs:4` and `scripts/tests/rust/rt-transport-uring.rs:8`. The guard messages are the only observed blockers; no guard was weakened. Formatting, audit-shell syntax, and `git diff --check` pass.
- Runtime guardrails are green with `Critical: 0` and `Warnings: 0` at `scripts/out/audits/runtime-guardrails-20260810T-backend-final-docs/audit-runtime-guardrails.log`. Seam evidence is `scripts/out/audits/workspace-seams-20260810T-backend-final/workspace-seams.json`: source revision `d69e10446c92be684346387d613c28340f18b25f`, `36` workspace packages, `322` Rust files, `206,258` source lines, `125` module edges, `106` workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
...
- `crates/qf-transport-types/src/observer.rs` canonically owns the root-independent `TransportObserver`, `TransportPolicyTarget`, and `TransportPolicyError` contracts. `src/transport.rs` keeps compatibility re-exports, and `src/transport/connection/mod.rs` implements the concrete adapter for the live `Connection`.
- Brain policy application now targets the child contract, while the FEC observer consumes the same root-independent observer surface and retains only its concrete `Connection` mutation adapter. ACK, packet, ECN, and runtime-policy behavior are unchanged; no wire format, frontend, or Tauri path changed.
- qf-transport-types tests pass `35/35`; the focused root Brain filter passes `64/64`; strict child/root/workspace Clippy, `cargo check --workspace --all-features`, and the complete workspace all-target `rust-tests` command exit `0` with no failures. Formatting and `git diff --check` pass.
- Fresh seam evidence is `scripts/out/audits/workspace-seams-20260810T-observer-contract/workspace-seams.json`: `36` workspace packages, `324` Rust files, `206,549` source lines, `125` module edges, `106` workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`. The contract surface is decoupled, but the cyclic runtime implementation remains open. Frontend/Tauri paths remain untouched.

## QKey Codec Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-engine-types/src/qkey.rs` now canonically owns the bounded QKey prefix, stable identifier, URL-safe base64 codec, checksum validation, parser error taxonomy, and zeroizing encode/decode boundaries. `src/engine/qkey.rs` retains only the concrete `EngineConfig` conversion, historical root re-exports, and root compatibility tests.
- The move adds only the child crate's direct `base64` and `serde_json` dependencies. QKey wire spelling, 16 KiB encoded/decoded parser bounds, error messages, checksum semantics, token ownership, and existing root API paths remain unchanged. No frontend or Tauri field/API projection is required.
- qf-engine-types all-target/all-feature tests pass `18/18` with strict Clippy; the root QKey filter passes `88/88`; strict workspace `rust-tests` Clippy, `cargo check --workspace --all-features`, and the complete workspace all-target `rust-tests` command exit `0` with no failures. Formatting and `git diff --check` pass.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-qkey-codec-postpush/workspace-seams.json`: source revision `f2c95b3ca51d378dee91c108a78fcc259f35f520`, `36` workspace packages, `325` Rust files, `206,581` source lines, `125` module edges, `106` workspace dependency edges, the unchanged 9-module product SCC, and `protected_changes=[]`. The QKey ownership move reduces contract duplication but does not dissolve the cyclic runtime implementation or close external Linux/native/CI gates. Frontend/Tauri paths remain untouched.

## Path Event Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/path.rs` now owns the root-independent `PathEvent` value contract for new, validated, failed, closed, reused-CID, and peer-migration path events. `src/transport/connection/parts/pmtu.rs` no longer defines a duplicate enum; `src/transport/connection/mod.rs` and `src/transport.rs` preserve the historical `quicfuscate::transport::PathEvent` path through compatibility re-exports.
- The child contract contains only `SocketAddr` values and event payloads. Path-validation policy, pending-path state, PMTU probing, event ordering, and concrete Connection ownership remain root-local. No connection, FEC, crypto, stealth, frontend, or Tauri implementation crosses into qf-transport-types.
- qf-transport-types all-target/all-feature tests pass `37/37`; the focused root path/migration filter passes `47/47`; root library checking and strict `rust-tests` Clippy, workspace all-feature checking, workspace strict `rust-tests` Clippy, formatting, and diff hygiene pass. The complete workspace all-target `rust-tests` command exits `0` with no failure markers.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-path-event-postpush/workspace-seams.json` at source revision `403ef081d3a1befdeabb69871c81db7c9bf5be63`: `36` workspace packages, `327` Rust files, `206,596` source lines, `125` module edges, `111` workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The post-test disk guard records `4,807,672 KiB` target usage and `6,227,812 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor.
- This is a backend-only contract move with no wire-format, frontend, or Tauri surface change. TODO-562 remains blocked for the coordinated product-core SCC split and external Linux/native/CI acceptance gates; no frontend field/API projection is required.

## H3 Error and ALPN Contract Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-transport-types/src/h3.rs` now owns the root-independent HTTP/3 `Error` taxonomy, the `TransportError` conversion, the H3-to-`ConnectionError` projection, and the supported `APPLICATION_PROTOCOL` ALPN list. `src/transport/h3.rs` preserves the historical `quicfuscate::transport::h3::{Error, APPLICATION_PROTOCOL}` paths.
- Moving the `ConnectionError` conversion with the child-owned error type keeps Rust orphan coherence valid while preserving the exact `Done` terminal mapping and `H3 error: ...` transport diagnostics. Concrete H3 connection state, QPACK, frame parsing, stream handling, and error-production policy remain root-owned.
- qf-transport-types all-target/all-feature tests pass `38/38`; the focused root H3 filter passes `76/76`; root checking and strict `rust-tests` Clippy, workspace all-feature checking, workspace strict all-target `rust-tests` Clippy, formatting, and diff hygiene pass. The complete workspace all-target `rust-tests` command exits `0`; the root library contributes `1,658/1,658`.
- Post-push seam evidence is `scripts/out/audits/workspace-seams-20260810T-h3-contract-postpush/workspace-seams.json` at source revision `15c3a1c7e31c1a82ab0a4e36c8814dcff523afb7`: `36` workspace packages, `327` Rust files, `206,622` source lines, `125` module edges, `111` workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`.
- The post-test disk guard records `4,924,504 KiB` target usage and `21,896,264 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. No frontend or Tauri path changed, and no frontend field/API projection is required.

## Stealth Fingerprint Profile Workspace Ownership (2026-08-10, TODO-562)

- `crates/qf-stealth/src/fingerprint_profile.rs` now owns the complete `FingerprintProfile` contract: the curated browser/OS matrix, User-Agent and language metadata, QUIC persona parameters, deterministic ClientHello audit metadata, synthetic ServerHello metadata, strict rotation-slot validation, and legacy fallback construction.
- `TlsCoverCipherPreference` now lives beside the profile and resolves one immutable `EnvSnapshot` through the canonical `qf-cpu::FeatureDetector`. `src/stealth/parts/tls_cover_provider.rs` delegates preference parsing, hardware detection, and cipher selection to that owner while retaining only root crypto-context installation. `src/stealth/parts/browser_profiles.rs` is a three-line compatibility projection.
- Isolated qf-stealth all-target/all-feature tests pass `103/103`; root Stealth compatibility tests pass `82/82`; root checking, strict child and workspace Clippy, formatting, and diff hygiene pass. The complete workspace all-target `rust-tests` matrix exits `0`, including root `1,658/1,658`.
- The final disk guard records `8,571,612 KiB` target usage and `15,515,880 KiB` free, below the `12,582,912 KiB` cleanup threshold and above the `2,097,152 KiB` build floor. Protected frontend/Tauri paths remain untouched and no frontend field/API projection is required.
