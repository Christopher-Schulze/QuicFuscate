...
- Admin authentication and session surface:
  ...
  - bounded state: session replay fingerprints use a `HashSet` for constant-time membership beside a `VecDeque` that evicts one oldest fingerprint per insertion after the 4,096-entry ceiling; this is a count-based FIFO window, not a time-based full-session guarantee, so a fingerprint can be accepted again after enough distinct guarded POSTs while the sliding session remains valid (TODO-665). The outer live-session map has no explicit count cap and is tracked separately in TODO-809. Login keys are pruned by lockout age and evicted least-recently-used at the hard cap.
...
The server runtime emits tamper-evident audit events to a hash-chained NDJSON segment set when `--audit-log <path>` is provided. One global `OnceLock<Arc<AuditLog>>` owner is initialized before privilege dropping. Producers submit fully owned events through non-blocking `try_send`; the single `qf-audit-writer` assigns sequence and timestamp, serializes, hashes, rotates, checkpoints, and performs all file I/O. Queue saturation or disconnection rejects the newest event and increments `quicfuscate_audit_dropped_events_total`; sink or checkpoint failures increment `quicfuscate_audit_persistence_errors_total`. If the system clock is before the Unix epoch, the writer rejects the event and enters its terminal persistence-error state instead of silently recording timestamp zero. The producer-terminal admission contract remains open under TODO-726; synchronous durability cancellation and failure classification remain TODO-675; configuration, payload, and shutdown-order bounds remain TODO-813, TODO-814, and TODO-815.

**File security:** The active log, immutable segments, and checkpoint are regular files created with mode `0o600` (owner read/write only) on the global startup path. Symlinks and special files are rejected before reading. When running as root, the audit set is chowned to the resolved runtime user/group so logging survives privilege reduction. The parent directory is chowned only if the server created it; pre-existing system directories are never re-owned. Direct public reopen of an existing active file does not yet reassert its mode and remains TODO-671; pathname binding remains TODO-728.
...
- The live server TUN path forwards intercepted DNS with plain UDP `forward_dns_query()` rather than this DoH helper. That server ownership model is intentional; `resolve_via_dns_upstreams()` shares the SERVFAIL-versus-upstream-response contract with the client proxy. The server TUN intercept now applies bounded admission in `DnsInterceptAdmission`: 128 concurrent blocking exchanges, a 2,000 PPS global cap with a 4,000-query burst, and a 100 PPS per-source-IP cap with a 200-query burst. Drops are exported as `quicfuscate_dns_intercept_dropped_total`. The server budget is source-IP keyed rather than session keyed, its limits are compile-time rather than configuration-owned, and the shared forwarding primitives have no per-resolver budget. Accepted `spawn_blocking` workers still have no runtime owner or terminal-outcome barrier; TODO-650 owns that lifecycle gap. The active client listener and retained generic helper have no explicit caller-admission contract; TODO-668 owns that boundary, timeout/allocation/response-bound hardening remains TODO-669, DoH response semantics beyond transaction ID remains TODO-810, and shared wire admission remains TODO-770.
...
#### SIMD Gap Status
 ...
 - **FEC (large-window decode acceleration)**: Audit open - `simd::amx::matmul_gf256_amx` enters 16x64 tile load/store blocks but performs the GF(256) coefficient loop scalarly; planner, compiled/runtime eligibility, tile ownership, kernel semantics, and proof coverage remain open under TODO-676 and TODO-816-TODO-819.
...
- **Matrix multiplication**: AVX512F on x86 and dedicated SVE2 Gather/`svmla` on ARM; no current AMX arithmetic caller is proven (TODO-816, TODO-819)
...
- The public `TunDevice` read/write result contract remains an open boundary:
  generic read lengths are not yet checked against pooled blocks and generic
  short writes are not yet rejected or completed before callers report
  success. TODO-844 owns this contract.
...
The effective `[logging]` section is parsed and validated before Tokio and before the single global logger is installed. File, stderr, RFC 5424 UDP syslog, and the admin ring buffer are sink properties of that logger and are never replaced by a later level change. File and syslog I/O run on one bounded 8,192-record worker queue; saturation drops the newest record and increments `logging::stats().dropped_records`, while sink failures increment `sink_errors` without recursive logging or process termination. Clean shutdown uses `logging::FlushGuard` to enqueue a barrier and wait up to five seconds for all earlier records and owned file/stderr flushes. File rotation is size-only and retains `max_files` numbered generations. The production runtime calls `logging::init()` once; the isolated `qf-logging-probe` still contains a duplicate call tracked by TODO-674, and the failed global-logger-installation cleanup boundary is tracked by TODO-812.
...
Current validation requires all audit bounds to be non-zero and limits `max_segments` to 1,024. It does not yet impose explicit upper limits on `queue_capacity`, `max_segment_bytes`, or `flush_timeout_ms`, and direct `AuditOptions` construction bypasses the config-only retention ceiling; TODO-813 owns that contract. `queue_capacity` bounds accepted commands by count, `max_segment_bytes` controls deterministic rotation, `max_segments` includes the active segment, and `flush_timeout_ms` bounds only enqueue plus acknowledgement waits, not the worker's underlying filesystem calls.
...
The client module separately exports `Profile`, `ProfileError`, and `ProfileManager` as a profile-storage API. The current `ClientRuntime`, CLI, and desktop/admin surfaces do not own or call `ProfileManager`; profile ID and duplicate-record semantics remain tracked in TODO-658, and profile-file atomicity remains tracked in TODO-662.
...
Environment parsing is not one universal live-reload contract. `src/env_utils.rs` owns the shared helper paths, but set invalid values can currently be ignored, converted to `false`, or replaced by a caller default; TODO-670 tracks that helper contract, caller behavior, alias precedence, and snapshot boundary. Several active compression, memory-pool, FEC, zstd, Reality, and trusted-proxy paths still parse environment values directly with their own fallback rules; TODO-811 owns that separate parser-authority inventory. Dedicated server auth, DDoS, rate-limit, GeoIP, blacklist, and QKey policy loaders return typed errors or warnings and remain their own validated boundaries.
...
The explicit standalone CLI path can configure a dual-stack `interface::TunConfig`, but the
generic `EngineConfig`/`ClientRuntime` path is not equivalent: `InterfaceConfig` has no separate
IPv6 address/prefix pair and `ClientRuntime::start()` currently sets `TunConfig.ip6` and
`TunConfig.prefix6` to `None`. Server-side `client_ipv6` assignment is consumed by server
routing/session ownership and is not currently propagated to the client through a tunnel
configuration control-plane message. The public compatibility `ClientBackend` also carries only
one address family in `TunDeviceConfig`. These boundaries remain open under TODO-663; TODO-731
owns malformed standalone CLI address parsing.
...
- **SecretString UTF-8 invariant**: `src/secret.rs` uses `from_utf8_unchecked`, but the current `String` constructor, private tuple field, and call-site audit preserve valid UTF-8; no raw-byte or mutable `SecretString` path was found. The remaining gap is the unsafe, compiler-unchecked representation contract and missing focused checked-boundary tests. Tracked in TODO-651.
- **Privilege drop FFI boundary**: `src/privilege/drop.rs` checks libc status and a non-null result before `assume_init`, but does not prove result-pointer identity; the two account-name fields still reach `CStr::from_ptr` without null, in-buffer, or bounded-NUL validation. The completed TODO-684 audit additionally found a forgeable public identity boundary, a Windows `CurrentIds` compile failure, incomplete filesystem-ID and partial-transition proof, warning-only process-lock failure, embedded lock-policy divergence, and first-wins TLS identity lifecycle gaps. TODO-652, TODO-653, and TODO-849 through TODO-854 own the remediation; no production implementation was made in the audit phase.
- **TUN unsafe read**: `src/interface.rs` performs one unaligned `*const u32` load in the BMI2 IP-header parser. Linux and macOS `fcntl` setup paths check their return values; TODO-654 owns the alignment fix, while TODO-843 through TODO-848 own the separate BMI2 feature-dispatch and platform-FFI findings from the completed TODO-683 audit.
- **PKI ambient time source**: `src/pki/mod.rs` reads `OffsetDateTime::now_utc()` repeatedly for generated validity and `UnixTime::now()` separately when validating existing material. The pinned time conversion does not return a clock error; a checked, captured, injectable PKI time contract remains tracked in TODO-656.
- **Client profile persistence**: the publicly exported but currently unwired `ProfileManager` derives IDs from the low 32 bits of a nanosecond timestamp, `add()` silently replaces duplicate `HashMap` keys, and `load()` collapses duplicate JSON IDs with last-entry-wins behavior. `save()` serializes bearer-bearing profiles and then uses direct `std::fs::write`, which truncates the destination before the complete JSON document is written; no profile save/load or failed-replacement test exists. TODO-658 owns identity/duplicate semantics, TODO-662 owns atomic replacement, and TODO-671 owns the explicit sensitive-file mode/umask boundary.
...
- **DNS forwarding allocation surface**: the A/AAAA/NXDOMAIN response builders are test-only in the current caller graph; active forwarding still allocates UDP receive/copy buffers, DoH request/response buffers, and the response copy. Allocation frequency and latency impact are not measured. Tracked in TODO-669.

### Production-Readiness Gaps

- **Generic engine configuration reload and propagation**: `QuicFuscateEngine` has validated in-memory updates but no file-backed reload owner; after `start()`, generic client/server runtime configuration is duplicated and most setters update only the outer engine copy. The standalone SIGHUP/admin path is a separate, explicitly `NextConnectionOnly` reload with startup-owned fields and a non-transactional transport publication boundary. It reads `EngineConfig` only and does not rebuild `ServerConfig`, so server GeoIP, DDoS, auth, and blacklist policy remains construction-time state. Tracked in TODO-645 and TODO-724; TODO-660 owns blacklist worker lifecycle rather than reload.
- **uring_batch admission and executor ownership**: `src/optimize/uring_batch.rs` has no cross-call pending queue, but its public sender methods materialize arbitrary per-call input before SQ chunking, and synchronous `submit_and_wait`/CQE draining is called from async-owned client/server flows. The generic client outbound path also calls the backend `TunDevice::read()` directly, with nonblocking behavior guaranteed only by the native Linux descriptor setup. TODO-687 closed the unsafe/lifetime boundary, TODO-798 owns unordered partial-send disposition, and TODO-646 owns admission, executor, shutdown, and generic-reader contracts.
- **Admin HTTP admission and capacity**: `src/implementations/server/admin_http_parts/server_and_auth.rs` still hardcodes an active limit of 16, has no CLI/config/environment capacity owner, and acquires its semaphore only inside a task created after `accept()`. Saturated sockets and pending Tokio tasks therefore remain outside the nominal limit; standalone startup also does not retain their join handles for shutdown. The separate metrics servers acquire their 32-slot permits before spawning and are not evidence for admin-web. Tracked in TODO-647; request-body memory remains TODO-712 and the distinct synchronous-handler deadline/cancellation boundary remains TODO-661.
...
- **Linux DNS resolver path and symlink ownership**: `src/implementations/client/platform/linux.rs` fixes the legacy source at `/etc/resolv.conf` and the backup/state/lock paths below `/etc`, with no config owner or resolver target identity tracking. The fallback follows symlinks through ordinary filesystem calls; a broken pre-existing resolver symlink can be classified as absent and removed by failed activation rollback. Restore-copy deletion is correctly ordered. Tracked in TODO-649; absent-original and crash ownership remain TODO-623.
...
- **CLI probe parse defaults**: `qf-e2e-client` and `qf-e2e-desktop` silently retain defaults only for malformed `--timeout-ms` and `--hold-ms`; the privilege probe and the other E2E numeric flags already fail closed. Tracked in TODO-657.
- **qftls mlock without munlock**: `src/qftls.rs` does not `munlock` preloaded keys. The completed TODO-684 audit also found missing certificate-key correspondence proof and silent first-wins duplicate/conflicting preload behavior; TODO-643 and TODO-853 own the lifecycle and identity contract.
...
- **Blacklist sync lifecycle**: `LiveServerState::maybe_sync_blacklist()` is the sole current production dispatch from the server housekeeping loop, and the synchronizer has real HTTPS, redirect, request-timeout, body, UTF-8, entry, custom-CA, versioned-cache, expiry, and atomic last-known-good publication checks. The task itself is detached: its `JoinHandle` is dropped, `ServerRuntime` and its stop/drain paths own no worker state, and no sync completion, cancellation, freshness, or retry metrics exist. The due check and timestamp write are separate lock scopes, so the current single-owner path is safe only by call topology, not by an atomic in-flight protocol. The async worker also performs synchronous cache parsing and durable file publication. Tracked in TODO-660; broader DNS worker and engine-thread shutdown boundaries remain TODO-650 and TODO-699.
- **Admin HTTP operation deadline and cancellation**: `AdminHttpServer::run()` wraps the complete `http1::serve_connection()` future in a fixed 30-second timeout, so body collection, routing, handler execution, and response writing share a real outer boundary for pollable async work. `AdminHttpHandler` is entirely synchronous, however, and Tokio timeout cancellation cannot preempt a blocking filesystem, Argon2, registry, lock, serialization, or config operation running inline on the executor worker. The budget is not per-operation or configurable, no timeout/cancellation outcome is owned, and the connection permit/task remains occupied until the synchronous call returns. TODO-647 owns admission placement/capacity, TODO-700 direct runtime service-task shutdown, TODO-699 engine-thread lifetime, TODO-712 body memory, and TODO-787 credential durability; TODO-661 owns the missing operation-deadline/cancellation contract.
- **DNS admission ownership**: TODO-771 made `src/implementations/client/dns_runtime.rs::ClientDnsRuntime` an active production caller of `src/dns/mod.rs::process_dns_query()`. The client listener processes one query at a time per localhost address but has no explicit global/per-client rate policy, caller identity, or DNS-specific shedding metric. The server MASQUE path is bounded by TODO-611, but its budget is source-IP keyed rather than session keyed, is compile-time owned, and does not partition per upstream resolver. TODO-668 owns this remaining caller/admission contract; TODO-650 owns accepted server worker lifecycle, TODO-669 owns forwarding bounds, and TODO-770 owns wire admission.
- **DNS forwarding deadlines and response bounds**: plain UDP has a per-read 5-second socket timeout but sequential resolver fallback has no aggregate deadline; DoH has per-request/connect timeouts but sequential endpoint fallback has no aggregate deadline. The public plain branch can call synchronous forwarding directly from async code, DoH response bodies are collected without a streaming DNS-size cap, and the UDP receive buffer is not a proven oversize-datagram rejection. Tracked in TODO-669.
- **DoH DNS response semantics**: the active DoH helper checks HTTP status/content type, body length, and transaction ID, but does not validate QR/opcode or the QNAME/QTYPE/QCLASS question tuple. Tracked in TODO-810; UDP response matching remains TODO-721 and shared query admission/wire semantics remain TODO-770.
- **Environment helper contract**: `src/env_utils.rs` re-reads environment values at individual construction points; invalid numerics become `None` and invalid present booleans become `false`, so default-true FEC, Brain, TLS Cover, orchestrator, and Reality fallback flags can be disabled silently. `parse_bool()` and `env_flag()` also disagree on whitespace, and independent test locks do not form one process-wide guard. The external-process torn-read claim remains unproven. Tracked in TODO-670.
- **Direct environment parser authorities**: active compression, memory-pool, FEC decoder, optional zstd, Reality target, and trusted-proxy paths bypass the shared helper and silently use subsystem-specific fallback or entry filtering; lazy `OnceLock` and background auto-tuner reads do not share one snapshot contract. Dedicated server policy loaders are explicit exceptions with typed errors/warnings. Tracked in TODO-811.
- **File permission umask reliance**: Linux resolver source/lock/backup creation, rotating-log initial/reopen creation, and standalone bearer-bearing profile persistence do not share an explicit creation-mode contract. Audit, atomic, registry, resolver-state, routing-state, and PKI private-key writers already set restrictive modes; public certificates and generated systemd units are non-secret outputs. Tracked in TODO-671, with profile atomic publication owned by TODO-662.
- **Audit log blocking flush**: `src/audit/mod.rs` flushes synchronously with no timeout. Tracked in TODO-675.
- **AMX static mut and capability contract**: `src/simd/amx.rs` uses a `static mut` tile config; the concurrent caller, compiled/runtime mismatch, kernel semantics, detector process boundary, proof lane, and profile/documentation mapping are tracked in TODO-676 and TODO-816-TODO-819.
- **Time-source and clock-domain contract**: the exhaustive inventory covers direct Rust monotonic/wall-clock producers, Tokio clocks, browser `Date.now()`/`performance.now()`, event-loop timers, implicit `.elapsed()`/`duration_since()` paths, and timestamp boundaries across transport, H3, core, engine, stealth, qftls, server, client, runtime, telemetry, audit, PKI, Tauri, Svelte, tests, probes, and scripts. TODO-677 is the umbrella audit; TODO-820 owns transport/stealth/core, TODO-821 owns server/client state, TODO-822 owns Tokio/OS/runtime clock boundaries, TODO-823 owns wall-clock provenance, TODO-824 owns Rust injection/test isolation, and TODO-825 owns frontend/browser clocks. Existing TODO-640, TODO-656, TODO-658, TODO-662, TODO-671, TODO-675, TODO-768, TODO-584, and TODO-588 remain narrower owners.
...
- **io_uring/io_driver unsafe**: TODO-687 closed the SQE/CQE lifetime, raw socket buffer, receive-slot ownership, and eventfd ABI audit boundary. TODO-801 also closed the additional Linux zero-length-rearm and opt-in SendMsgZc evidence. TODO-646 owns executor ownership and TODO-798 owns unordered partial-send disposition.
- **Audit/limits unsafe**: TODO-688 completed the read-only audit of the complete audit implementation and tests, direct startup/runtime callers, audit probe, limits false positive, suites, guardrails, documentation, related owners, and history. The current inventory is three production FFI sites plus one Unix-only test guard. TODO-861 owns Windows interior-NUL rejection, local FFI safety contracts, security-hardening failure propagation, and platform-negative proof; TODO-671, TODO-675, TODO-726, TODO-728, TODO-813, TODO-814, and TODO-815 retain their separate mode, lifecycle, path-binding, bound, and shutdown owners. No production implementation or runtime verification was performed for TODO-688.
...
- **Client TUN dual-stack projection:** TODO-663 confirms that the canonical `interface::TunConfig` and native Linux/macOS/Wintun backends already support typed IPv4 plus IPv6 fields, but generic `ClientRuntime::start()` hardcodes `ip6`/`prefix6` to `None`. `EngineConfig::InterfaceConfig` cannot represent a simultaneous pair, no server-assigned IPv6 control-plane propagation into the client was found, and the public compatibility `ClientBackend` remains single-family. The standalone CLI supports explicit dual-stack values and its parse-failure boundary remains TODO-731. No implementation or test execution was performed in this audit pass.
- **Linux resolver restoration:** TODO-623 is locally implemented but blocked on native Linux verification. The legacy `/etc/resolv.conf` path now records an explicit absent-original state, persists create-only ownership with Linux boot/PID/start-time identity, coordinates sessions with an advisory lock, and writes a session marker into managed resolver content so stale recovery refuses missing or foreign ownership. The focused resolver suite passes 8/8 and the full local library reached 2,190/2,192; the two failures are unrelated baseline gates: the DoH cache test needs external `cloudflare-dns.com` resolution, and the qftls ClientHello assertion does not account for profile readiness jitter. The native Linux gate is unavailable because this macOS host lacks a Linux C sysroot and the configured Omega SSH path is unavailable. TODO-649 still owns fixed path configuration, resolver symlink/target identity, and broken-link rollback semantics.
...
- **Engine reload:** The generic `QuicFuscateEngine` exposes validated in-memory `update_config` but no file-backed reload owner. After client/server `start()`, its outer `EngineConfig` is duplicated into runtime-owned configuration, and only the FEC control path has explicit post-start propagation; the other generic setters do not update the runtime copy or active connection. The standalone server has a separate validated file reload path with explicit `NextConnectionOnly` scope, startup-owned fields, and shared-policy publication before best-effort transport mutation. That path reads `EngineConfig` only and leaves construction-time `ServerConfig`, including blacklist/GeoIP/DDoS/auth policy, unchanged. TODO-645 remains open for one supported reload and propagation contract; TODO-660 owns blacklist worker lifecycle; TODO-724 owns transactional publication.
- **io_uring:** `UringBatchSender` has no cross-call pending queue: scratch vectors are preallocated and submissions are chunked to SQ capacity. TODO-646 Finding 1 is stale. Payloads are staged in sender-owned slots; failed submissions quarantine the sender; SendMsgZc waits for every primary and every `CQE_F_MORE` notification; receive indices are checked and zero-length slots are re-armed. Synchronous submit, wait, and CQE draining still run from client/server packet paths and remain open under TODO-646. TODO-798 owns unordered partial-send disposition.
...
- **Linux DNS backup:** Restore copies the backup before removing it, so a failed copy preserves the backup. TODO-649 Finding 2 is closed as stale. The fixed source/backup/state/lock paths remain without a configuration owner, and the legacy fallback does not capture resolver symlink/target identity; a broken pre-existing link can be deleted by failed activation rollback. TODO-649 owns these path and target semantics; TODO-623 owns absent-original and crash ownership.
...
- **TUN fcntl:** Linux and macOS check `F_GETFL` and `F_SETFL`, close only on the error path, and return before device publication. TODO-655 is closed as stale. The independent BMI2 unaligned-read finding remains TODO-654; the broader interface/platform remediation is TODO-843 through TODO-848.
- **Probe parsing:** `qf-privilege-probe`, `--initial-count`, and `--initial-interval-us` already return parse errors. TODO-657 is narrowed to silent defaults for `--timeout-ms` and `--hold-ms` in `qf-e2e-client` and `qf-e2e-desktop`; no malformed-input parser test currently exists.
...
- **Blacklist sync:** The full current-source audit found one production housekeeping caller and confirmed the HTTPS, redirect, request-timeout, body, UTF-8, entry, custom-CA, cache-format, expiry, and last-known-good publication boundaries. The detached task has no `JoinHandle` owner or shutdown barrier; the timestamp check/write is not an atomic in-flight claim; configured body and entry limits have no absolute upper ceiling; synchronous cache publication runs in the async worker; and freshness, cancellation, failure, and retry state is log-only. TODO-660 owns these boundaries. No production implementation or test execution was performed in this audit phase.
...
- **File modes:** `fsutil`, audit persistence, QKey registry, resolver ownership state, routing ownership state, and PKI private-key writers set restrictive creation modes before opening files. TODO-671 retains the Linux resolver source/lock/backup contract, rotating-log initial/reopen creation, and credential-bearing standalone profile persistence; the profile atomic publication boundary remains TODO-662.
...
- **CLI control protocol:** `quicfuscate-ctl` still interpolates `kick`/`block`/`unblock` values into JSON without escaping, preflight bounds, or exact arity. The Unix reader is already bounded to 8 KiB with a five-second deadline by TODO-617, while Unix command-value validation still differs from the HTTP normalization path. TODO-673 owns request serialization, CLI bounds/arity, and Unix semantic consistency.
- **Audit persistence:** `AuditLog::flush` bounds the producer acknowledgement wait, but the writer's `flush` and `sync_data` remain synchronous and uninterruptible. Flush/checkpoint failure classification, sticky shutdown errors, terminal admission, payload bounds, configuration maxima, and close ordering remain TODO-675, TODO-726, TODO-814, TODO-813, and TODO-815 respectively.
...
- **Crypto corrections:** TODO-631's blanket round-key zeroization claim is stale because the AES-NI schedule exists only on x86_64 and is zeroized in its target-specific `Drop`; key and IV zeroization remains cross-target. TODO-642's zero-key fallback claim is stale because TLS cover entropy failure returns a typed crypto error before derivation. TODO-627 closes the constructor key/IV boundary; TODO-629, TODO-632, and TODO-633 retain the independent header-protection, nonce-lifecycle, and KDF input contracts.
- **AMX:** `src/optimize/parts/cpu_dispatch.rs` derives AMX planner flags from external `cpuid` output even when the binary is not compiled with `target_feature="amx-tile"`. `src/fec/parts/decoders.rs` can therefore select the AMX plan while the cfg-gated AMX call is absent and the next vector remains zero-initialized. The active AMX function performs scalar GF(256) multiplication after tile load/store, while the separate integer kernel is uncalled and dimension-blind. `src/simd/amx.rs` also accepts unchecked dimensions and releases tile state only on the normal return path. TODO-676 owns the core dispatch/race/dimension boundary; TODO-816-TODO-819 own kernel semantics, detector execution, proof coverage, and profile/documentation truth.
...
- **Scope boundary:** No UI or Omega state was changed. KDF input coercion remains TODO-633, fuzz-manifest dependency resolution remains TODO-758, and the broader project audit remains open under its existing task owners.
