...
- **SIMD unsafe surface**: TODO-679 completed the read-only audit of all 31 files in `src/simd/*` and `src/optimize/simd/*`, including 138 unsafe function declarations, 102 actual target-feature attributes, direct callers, tests, audit scripts, and history. TODO-834 completed the exact dispatch-owner audit and implementation of the SVE2 decode, ACK AVX512VL, SHA-VNNI, AES/VAES, GF16, AVX-512 compression/pattern/histogram, neural FMA, optimization string, stale BMI2 profile, scalar-claim, FEC, and packet transport boundaries. TODO-835 now closes the release-safe short-needle, matrix, BMI2, Berlekamp-Massey, private repeating-key, and private ChaCha XOR length boundaries with focused malformed-input tests; remaining vector tails were cross-checked. TODO-836 retains the proof-owner scope for the blanket safety-doc suppression, only four function-level `# Safety` sections, silent ISA-test returns, stale unsafe-surface matching, and missing native-ISA proof lane. Native x86/Linux, Windows, SVE2, sanitizer, and Miri evidence remain external.
...
- **SIMD:** The old AVX-512/GFNI Reed-Solomon delegation claim is stale for the active decoder. TODO-834's completed dispatch implementation now covers the exact SVE2 decode, ACK AVX512VL, SHA-VNNI, AES/VAES, GF16, AVX-512 compression/pattern/histogram, neural FMA, optimization string, stale BMI2 profile, scalar-claim, FEC, and packet transport boundaries. TODO-835 now closes the critical `find_pattern_sse42_short` short-needle load, release-safe matrix and BMI2 output checks, overlong Berlekamp-Massey rejection, and private key/XOR caller contracts; focused malformed-input tests are part of the owner. TODO-836's proof pass remains open for the blanket safety-doc suppression, only four function-level `# Safety` sections, silent ISA-test returns, stale unsafe-surface matching, and missing native-ISA proof lane. Native x86/Linux, Windows, SVE2, sanitizer, and Miri evidence remain external.
