...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline, so the generic I/O driver does not oversleep an outer-pacer release. The standalone client and server retain their established 5 ms housekeeping cadence. Three distinct exact ARM64 standalone direct-wakeup candidates, including an outer-release-only client path, failed closed with heartbeat timeouts, so that parallel runtime integration remains removed.
