...
- **Privilege/mlock/secret unsafe**: TODO-684 completed the current-source and owner reconciliation across privilege drop, memory locking, qftls, secret handling, callers, platform gates, tests, scripts, documentation, related TODOs, and history. TODO-651 closes the checked `SecretString` boundary, TODO-652 closes lookup result-pointer validation, TODO-653 closes bounded account-name conversion, and TODO-643 closes preloaded-key lock/publication ownership. TODO-849 through TODO-854 retain public identity, post-drop, embedded/process-lock policy, certificate-key, key-output, and native/negative-proof gaps; TODO-516 and TODO-678 retain pool lock ownership.
...
- **Optimize and UDP:** TODO-680's current reconciliation retains open reversed/clipped bitmap arithmetic, SSE2 short-pattern overwrite, overflow-prone pattern positions, SVE2 Base64 output-lane undercoverage, public QUIC packet-number and length checks, VNNI truncation beyond 64 samples, percentile input validation, and native/profile proof. TODO-834 closed the former P1f/P4a dispatch routes, BMI2 selection predicate, AVX2 packet-number byte order, and stale SVE2 search symbol; TODO-837 implements the bounded UDP result/address/partial-send and caller-fd contract, while its native proof remains open. TODO-682 split the remaining shared transport remediation into TODO-838-TODO-842. Valid vector tails and the bounded active connection caller were cross-checked.
...
- **Privilege and FFI:** TODO-684 completed the current-source and owner reconciliation across the full privilege, memory-lock, qftls, secret, caller, platform, cleanup, test, script, documentation, related-TODO, and history scope. TODO-651 closes the checked `SecretString` boundary, TODO-652 closes returned-pointer identity validation, TODO-653 closes bounded account-name conversion, and TODO-643 closes preloaded-key lock/publication ownership, all with the recorded local source evidence. Final-boundary identity validation, complete filesystem-ID verification, and a portable `CurrentIds` type remain open under TODO-849-TODO-850. Process-lock readiness/failure policy and embedded propagation remain TODO-851-TODO-852; certificate-key correspondence and public key-output ownership remain TODO-853; native and negative proof remains TODO-854. Pool lock ownership remains TODO-516/TODO-678. Windows audit-file path FFI, local safety comments, and security-hardening failure semantics are TODO-861; Unix audit pathname TOCTOU remains TODO-728. Native privilege proof remains environment-specific.
