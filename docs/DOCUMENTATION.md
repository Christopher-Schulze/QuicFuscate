...
- **AMX capability contract**: TODO-816 removed the former `static mut` tile config and unverified raw kernels from the active source. TODO-817 removes the external `cpuid` process and exposes separate in-process CPU, OS permission, compiler target, and product-backend evidence. The production FEC path reports only scalar Wiedemann operations until TODO-818 proves a real AMX arithmetic backend; final dispatch intersection and profile/documentation mapping remain tracked by TODO-676 and TODO-819.
...
- **Ownership:** TODO-817 closes detector execution bounds. TODO-676 retains the final compiled/runtime/OS dispatch intersection and tile ownership; TODO-818 retains native AMX arithmetic/build/runtime proof; TODO-819 retains profile/documentation mapping; TODO-760 remains the Cargo-feature contract owner.
