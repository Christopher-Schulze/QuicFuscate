...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline. The standalone client schedules that full deadline. The server schedules only `next_outbound_release_deadline()` and continues recovery through housekeeping, preventing an overdue PTO from continuously preempting a paced release. Due server connections flush at the release time, so the 5 ms housekeeping cadence cannot quantize a sub-millisecond pacing release.
