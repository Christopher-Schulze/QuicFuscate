...
- The three-client dual-stack harness now gives caller-selected evidence directories the same bounded admin-socket ownership and self-contained certificate behavior, then retains the exact binary SHA-256. It passed complete exact-ARM64 candidate runs, including source ownership, routing, PTB, 1280-to-1500 confirmation, and black-hole recovery. Its repeated 5-second PMTU medians were 16.34%, 14.77%, and 19.78% gain against a fixed 15% gate, so this is a truthful open TODO-559 performance-qualification issue, not a release-pass claim.
