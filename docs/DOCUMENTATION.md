...
- **Boundary ownership:** Tauri persistence owns checked Unix milliseconds for `createdAt` and log `tsMs`, while the browser currently repairs invalid desktop creation values with local `Date.now()`. Admin QKey metadata is Unix seconds and admin log values are Unix milliseconds, but the TypeScript types are unbranded. TODO-868 owns the unit, provenance, validation, and invalid-state contract.
- **Elapsed ownership:** Desktop throughput uses wall-clock deltas, admin throughput uses `performance.now()`, paste suppression uses wall-clock comparisons, and visibility/poll gaps have no shared policy. TODO-869 owns monotonic elapsed and visibility behavior.
...
- **Status:** TODO-825 is `AUDIT_COMPLETE` as a read-only owner. TODO-868 through TODO-872 are open remediation owners. Frontend checks and browser gates are not claimed because the requested implementation boundary was not authorized.
