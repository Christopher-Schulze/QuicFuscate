...
- `benches/fingerprint_normalizer.rs` asserts zero allocations after warmup and measures the common IPv4 UDP path. The privileged Omega matrix is owned by `scripts/tests/fingerprint-runtime-proof-netns.sh`: evidence run `evidence-fingerprint-20260731i` against binary SHA-256 `37c4ac6f7c79cd53e3e6f327dc9fcbff780b3d072eee73818110843b42d51dfa` passes disabled byte-exact passthrough, Linux/Windows/macOS/Android capture vectors, IPv4/TCP checksums, and p0f 3.09b classification. The exact proof-source bundle is `quicfuscate-todo543-v044-source-final-20260731.tar.gz` with SHA-256 `b00ebe5dd89efa0fb9a6ec33a804e239131c8b68d04ff3e1fdc53e114067101f`. The exact pushed commit `ec5090ffd5d620e716adeeb2fe8d4586980bc4a0` was benchmarked on Omega with two Cargo jobs and low process priority: the zero-allocation assertion passed and the IPv4 UDP hot path measured `37.798 ns`, `1.5769 GiB/s`, approximately `13.5 Gbit/s`, above the 900-Mbit/s gate. It records Nmap 7.94SVN output per profile without claiming exact active matching; macOS is not matched because active UDP/ICMP/closed-port probes and arbitrary client responses remain outside the server-side SYN-only uplink normalizer. TODO-765 owns bidirectional active-probe closure.
...
- `utils/verify-fingerprint-pcap.py` - Dependency-free pcap parser and checksum/vector verifier. Schema `quicfuscate.fingerprint-pcap.v2` distinguishes the normalized client SYN from the ordinary downlink SYN-ACK passthrough boundary.
