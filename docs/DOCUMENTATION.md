...
- **Tracker gate:** `bash scripts/tests/audits/verify-audit-completeness.sh` currently fails before corpus validation with `unexpected tracker section 'Blocked'`; TODO-799 owns alignment of the validator with the repository's canonical task lifecycle, TODO-800 owns the stale macOS runtime-reload fixtures exposed by the same CI run, and TODO-801 owns the missing opt-in io_uring kernel evidence.
