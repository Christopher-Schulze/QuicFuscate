...
- The three-client dual-stack harness gives caller-selected evidence directories bounded admin-socket ownership and self-contained certificate behavior, retains the exact binary SHA-256, and deletes every owned host veth after namespace teardown. Its paced IPv6 TCP probe replaces `iperf3`: the sender and receiver must report identical bytes and SHA-256, and receiver elapsed time is the throughput source. Against exact ARM64 binary `ee0243f6…61e95e88`, default trials were 9.496/9.517/2.273 Mbit/s and opt-in trials 9.065/9.953/10.089 Mbit/s. The unchanged 15% PMTU gate failed at 4.81%; the captured server snapshot reported zero TUN-downlink backpressure queue/retry/drop and MASQUE-response events. Cleanup left no process, namespace, host veth, or qdisc. This is an open TODO-559 performance-qualification blocker, not a release-pass claim.
