...
- **Machine inventory:** `scripts/audits/verify-time-source-inventory.sh` now reports 962 locations with zero unclassified entries. The active browser scope is 65 locations: 10 wall-clock reads, 2 monotonic reads, and 53 scheduler/animation locations. Frontend tests account for 390 classified locations. The earlier 957-location TODO-824 snapshot missed five `Date.now()` expressions inside template-literal interpolations; `mask_javascript()` now preserves `${...}` expressions and `scripts/tests/audits/test-time-source-inventory.sh` covers the regression.
