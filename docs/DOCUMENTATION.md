...
- Lazy receive polling is gated by `recovery_needed()` and `full_recovery_needed()`: clean blocks return systematic packets without polling heavy recovery, gap-only systematic arrivals drain partial output without running full recovery, flushed repairs or tail-loss repair availability trigger full recovery, and clean complete blocks prune their sequence tracker so long-running stable links do not retain unbounded FEC source IDs.
