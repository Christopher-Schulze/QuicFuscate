...
- Precommit seam evidence is `scripts/out/audits/workspace-seams-20260810T-varint-precommit/workspace-seams.json`: source revision `aa712329ea0efe6e8afe22c1740d6038de646adb`, `36` workspace packages, `326` Rust files, `206,558` source lines, `125` module edges, `109` Cargo workspace dependency edges, the unchanged 9-module product SCC (`brain`, `core`, `engine`, `fec`, `implementations`, `interface`, `qftls`, `stealth`, `transport`), and `protected_changes=[]`. The final post-push report will replace this provisional revision reference after publication.
