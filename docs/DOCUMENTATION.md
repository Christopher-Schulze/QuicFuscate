...
- `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh` has an opt-in external diagnostic, `QF_E2E_EXTERNAL_EGRESS_CAPTURE=1`. It captures only client 1's underlay UDP packets to the server on the host-side `qf523h1` veth, only during each three-trial throughput phase, and writes bounded packet-count and inter-packet-gap summaries. Exact ARM64 source `19ed8c8` rebuilt binary `c54d2d5e1c600790fcc0c2d437fdb5f3942e8337dadd2138896f0bf958ac6a2e` after `cargo clean` and passed the complete gate: default 8.048 Mbit/s, opt-in 10.300 Mbit/s, 27.97% gain, 2-second black-hole detection, and a 13,041,664-byte recovery transfer in 21.152 seconds. External summaries recorded 26,293/32,765 packets, 330,130/341,556-us maximum gaps, and 3/10 gaps at or above 100 ms; both queue families and cleanup remained zero. The product runtime, normal gate, packet payloads, and credentials remain untouched.
