...
- `MemoryPool::try_alloc_from_slice()` rejects oversized input with `MemoryPoolError::SliceTooLarge` instead of truncating it. The final ARM64 macOS evidence is green: the complete FEC library matrix passes `252/252`, the complete debug library passes `2,436/2,436`, the complete release library passes `2,436/2,436`, strict `unsafe_rust` library and all-target Clippy pass, formatting, diff hygiene, and locked metadata pass, and exact parser, generator, decoder-teardown, symbol-length, and wire-error counter assertions are included in the FEC matrix. TODO-833 remains the separate feature-gated DATAGRAM queue ownership boundary; TODO-682/TODO-683 remain the deeper raw UDP/interface FFI boundaries. The final `target/` directory is `2.6G`, below the 13-GiB task ceiling.
