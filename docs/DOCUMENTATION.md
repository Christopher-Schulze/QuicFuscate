...
- **Current public GitHub release: `v0.4.2`** - its manifest advertises `0.4.2`, but the published server and desktop artifacts still carry `0.4.0`; it is not accepted as a version-coherent production checkpoint. TODO-519 prepares the first fail-closed synchronized successor as `v0.4.3`.
...
- **Current native Windows evidence**: CI run `29838022208` job `88659311753` passes `cargo check --lib` on MSVC. Its 1,662-test process aborts with `0xc000001d` (`STATUS_ILLEGAL_INSTRUCTION`) before Clippy, so the Windows gate remains red and now runs tests serially with uncaptured output to isolate the exact CPU-dispatch defect without skipping coverage.
...
- The `windows-core-checks` job runs `cargo check --lib`, `cargo test --lib --features rust-tests`, and `cargo clippy --lib --features rust-tests -- -D warnings` on `windows-latest`; its first native green run remains TODO-519 acceptance evidence rather than an already-proven checkpoint. The TODO consistency job also runs `scripts/audits/verify-release-version.sh` on pushes and pull requests.
