...
- **Detection scope:** The current local run enumerates 724 files and 1,254,868 words: 671 code, 29 documents, 24 images, and 3 sensitive files accounted for as redacted. The detected code surface contains 332 Rust files, 138 shell/PowerShell scripts, 115 TypeScript files, and 50 Svelte files. Generated, dependency, agent-state, audit-output, and local-sensitive Git paths remain counted by the machine-readable Git scope without being treated as source inputs.
