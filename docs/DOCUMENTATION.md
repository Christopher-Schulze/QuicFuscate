...
- The `windows-core-checks` job caps Cargo at two jobs, checks and lints `tun-windows,rust-tests`, compiles its unit-test binary, provisions the integrity-checked upstream DLL beside that binary, executes ordinary tests plus the serial privileged Wintun adapter/I/O/close suite, rejects owned adapter or firewall-rule residue, and uploads provenance plus Windows-build evidence. Run `30504148233`, attempt 2, job `90752348969` is the successful pushed Wintun baseline; the WFP change requires its own successor native run. The `release-contract` job runs `scripts/audits/verify-release-version.sh` on pushes and pull requests.
