...
#### Windows
The WFP `Engine` and `Transaction` owners use `WfpOwnerState`. A failed `FwpmEngineClose0`, transaction commit, or abort retains native ownership and the exact status for an explicit retry; `Drop` makes one bounded final attempt and emits a durable pending-resource diagnostic. Deterministic engine-close and transaction-abort fault-injection tests run in the Windows core lane. Current Windows/BFE failure execution and new-failure residue proof remain unclaimed; historical native WFP evidence predates these tests.
...
- **Interface/platform unsafe**: TODO-683 completed the read-only audit of
  ...
  TODO-846/TODO-847 remain unclaimed under TODO-848.
