...
- **uring_batch executor ownership**: `src/optimize/uring_batch.rs` has no cross-call pending queue, but its synchronous `submit_and_wait` path is called from async-owned client/server flows. TODO-687 closed the separate unsafe/lifetime boundary; executor ownership remains tracked in TODO-646.
...
- **io_uring/io_driver unsafe**: TODO-687 closed the SQE/CQE lifetime, raw socket buffer, receive-slot ownership, and eventfd ABI audit boundary. TODO-646 owns executor ownership, TODO-798 owns unordered partial-send disposition, and TODO-801 owns the additional Linux zero-length-rearm and opt-in SendMsgZc evidence.
...
- **io_uring:** The current sender copies payloads into owned slots before publishing pointers and quarantines after submit/protocol errors; SendMsgZc tracks `CQE_F_MORE` and waits for every announced notification, including errored primaries. Receive completion metadata is range-checked, zero-length and error receives are re-armed, and ring destruction precedes pool-block return. Client eventfd reads require exactly eight bytes. TODO-687 closed this unsafe/lifetime boundary; synchronous submit/wait remains TODO-646, unordered partial-send retry disposition is TODO-798, and the additional kernel execution boundaries are TODO-801.
...
- **Evidence boundary:** macOS client io_driver focused tests passed 12/12; the complete host library gate passed 2,144/2,144; `cargo check --all-targets --features rust-tests`, targeted Clippy with warnings denied and documented baseline suppressions, `cargo fmt --all -- --check`, and `git diff --check` passed. Remote Clippy Matrix run `30791153445`, job `91614771683`, passed the `io_uring` feature lane. On the exact source commit `cec9c9c`, CI run `30791153467`, job `91614771645`, compiled the Linux release artifact and completed the TUN provisioning/rollback gate before reaching the native lifecycle and failing on durable routing cleanup owned by TODO-607. Its Linux fastpath job `91614771646` passed `rt-transport-uring` 13/13 and `rt-io-hotpath-kernel-integration` 1/1, providing the required standard Linux io_uring compilation and runtime evidence for TODO-687. The new revision adds bounded evidence commands for the Linux-gated zero-length unit regression and opt-in SendMsgZc execution; their remote kernel result is still pending under TODO-801. The overall CI run is not a green release gate because unrelated Windows, macOS, and native routing jobs failed. The local macOS host still cannot compile the Linux target because its GNU/Linux C sysroot is absent (`ring` cannot find `assert.h`).
- **Tracker gate:** `bash scripts/tests/audits/verify-audit-completeness.sh` currently fails before corpus validation with `unexpected tracker section 'Blocked'`; an independent filename/register reconciliation finds 709 tracker headings, 370 current detail files, and 340 archived filename-ID detail files with no filename-ID orphan or duplicate. The independent section/status check finds Active `1`/`IN_PROGRESS`, Blocked `1`/`BLOCKED`, Queue `158` (`144` `OPEN`, `14` `QUEUED`), and Completed `549`; 262 legacy Completed details lack frontmatter under the existing archive-schema boundary. TODO-799 owns alignment of the validator with the repository's canonical task lifecycle, TODO-800 owns the stale macOS runtime-reload fixtures exposed by the same CI run, and TODO-801 owns the missing opt-in io_uring kernel evidence.
