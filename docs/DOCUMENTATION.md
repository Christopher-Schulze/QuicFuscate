...
- `src/transport/frames.rs` - frame encoders/decoders and canonical ACK block logic. `wire_len()` returns `Result<usize, ConnectionError>` and validates ACK ranges, two-byte CRYPTO/STREAM lengths, New Connection ID invariants, and checked length arithmetic before serialization. `from_bytes()` returns `Frame<'a>` with `Cow::Borrowed` data fields for zero-copy parsing; construction sites use `Cow::Owned`.
...
- TODO-698 remains open in send commit ordering. Core FEC flush paths remove buffered packets before `write_to()` succeeds. DATAGRAM restoration covers frame serialization errors only; later padding, header-protection, AEAD, and short-header seal errors occur after queue removal. The pooled-buffer cleanup work in TODO-831, TODO-832, and TODO-833 does not close the high-level queue lease or commit boundary.
