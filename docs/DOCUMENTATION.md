...
- Native burst evidence is mixed: one first heavy-burst trial reached 12% and failed the per-run 10% gate, while three isolated repetitions passed with mild/heavy residual loss 0/5, 0/1, and 2/0%. The single-run threshold is therefore not yet a statistically qualified production contract; TODO-557 owns explicit repetitions and aggregation.
