...
- **Native probe gate:** `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh::prove_client_local_ptb()` sends both DF=1 and DF=0 (`ping -M dont`) IPv4 probes while the client TUN is 1500 and the server TUN remains 1280. The `.github/workflows/ci.yml` `linux-traffic-analysis-native` job executes the complete multi-client harness with an isolated CA and dedicated `server-ipv4-ptb-native` artifact path. First exact attempt `30817476201` reached the probe but did not observe the DF=0 visible response; the next run retains the ping output, captures the returned ICMP packet on the client TUN, checks the server metric delta, and normalizes root-owned artifacts before upload.
