...
- `qf-stealth` is the canonical owner of `TlsCoverCipherSuite`, including the stable cipher-name and TLS-ID mappings. The root stealth provider keeps preference parsing and runtime crypto installation, and the historical root type path remains available through a compatibility reexport.
