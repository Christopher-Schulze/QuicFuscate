...
- TUN reader lifecycle: `TunInterface::reader_loop_with_shutdown()` checks an atomic shutdown flag and waits for Unix descriptors with bounded `poll(2)` instead of a 1 ms sleep. Client and standalone server runtimes own the reader cancellation flag plus `JoinHandle`; they drop the bounded receiver before joining, use the native Wintun shutdown event where required, release the descriptor after the join, and use `Notify` to wake the event loop when a frame is queued.
