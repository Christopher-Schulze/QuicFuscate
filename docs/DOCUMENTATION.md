...
- **Architecture boundary:** The x86_64 build without compile-time `amx-tile` no longer allocates the previously unused scalar column vectors. AMX matrix/result/vector scratch remains local to the AMX branch, and runtime/compile-time dispatch conditions are unchanged.
- **Allocation accounting:** Telemetry now separates logical column-buffer, SpMV-accumulator, matrix/RHS, Krylov, per-iteration, candidate, and AMX scratch allocation events. The counters are exported through the existing plain-text telemetry surface so the remaining per-byte allocations are measurable instead of being conflated with column reuse.
