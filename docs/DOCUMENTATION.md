...
- **Native evidence boundary:** `prove_icmp_boundaries()` now retains the TTL ping output, a client-TUN pcap, a machine-readable verification result, and before/after server metrics. `scripts/tests/utils/verify-icmp-time-exceeded-pcap.py` fails closed unless the captured request and server response have the declared endpoints, TTLs, ICMP type/code, valid IPv4 and ICMP checksums, and an exact 28-byte quote matching the original request. The first corrected-source native rerun was `30826363327`, job `91728986360`; its existing TTL assertion reached the server and recorded `time_exceeded=1`, then the independent backpressure-quiescence gate failed on `quicfuscate_tun_downlink_backpressure_events_total{event="enqueued"}=6`. TODO-806 remains open until the enhanced pcap gate runs and the separate backpressure failure remains owned independently; the earlier PTB evidence remains independently closed under TODO-613.
