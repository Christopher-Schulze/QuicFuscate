...
- Lazy clean sequence tracking is heap-free on the common in-order path: `LazyDecoder` stores one contiguous normalized source range and only allocates sparse interval tracking after a real out-of-order/gap case. Duplicate sources do not inflate block accounting, and filled gaps can return to the lazy clean path.
