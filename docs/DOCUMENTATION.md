...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline, so the generic I/O driver does not oversleep an outer-pacer release. The standalone client and server retain their established 5 ms housekeeping cadence. Three distinct exact ARM64 standalone direct-wakeup candidates, including an outer-release-only client path, failed closed with heartbeat timeouts. The standalone direct runtime integration remains removed; PMTU performance qualification is open.
- Exact ARM64 evidence proves the real authenticated TUN/MASQUE carrier under sustained UDP/FEC and dual-stack TCP pressure: the current CUBIC control harness source `046a567` uses a short owned admin socket independent of a caller-selected evidence directory. Against binary SHA-256 `ee0243f6aae50ee66115ba9f11d596004c3f057e240654e9a4bf340461e95e88`, its three-clean/three-loss trials recorded Auto 2.984 Mbit/s and 99.45% retention versus FEC-off 2.857 Mbit/s and 95.20%, a measured delta of 0.128 Mbit/s and 4.25 percentage points; CUBIC/Reno fairness was 1.045/1.080 Mbit/s with Jain 0.999726. The specialized quantitative FEC-contract work remains separately tracked by TODO-557.
- The three-client dual-stack harness gives caller-selected evidence directories bounded admin-socket ownership and self-contained certificate behavior, retains the exact binary SHA-256, and deletes every owned host veth after namespace teardown. Its paced IPv6 TCP probe replaces `iperf3`: the sender and receiver must report identical bytes and SHA-256, and receiver elapsed time is the throughput source. Earlier PMTU trial figures, including the 4.81% result against exact ARM64 binary `ee0243f6…61e95e88`, are not payload-efficiency proof because both phases then forced the TUN ceiling to 1280. The harness now gives each peer its phase ceiling, preserves client-side confirmed-MTU synchronization, and keeps the 15% gate. The captured server snapshot reported zero TUN-downlink backpressure queue/retry/drop and MASQUE-response events. Fresh exact ARM64 evidence is required.
...
- `tun-e2e-fec-netns.sh` - Linux netns FEC smoke over the real tunnel with tc-netem loss. `UNIFORM_PING_SCENARIOS` owns every executed loss/bound pair and `UNIFORM_IPERF_SCENARIOS` owns every receiver-verified throughput case, so printed acceptance and execution cannot diverge. Current hard gates cover authenticated tunnel establishment, ping liveness/loss bounds, panic absence, cleanup, and bounded JSON-verified iperf receiver delivery. Sustained carrier delivery and quantitative FEC benefit remain pending TODO-559 and TODO-557. Truthful Off control and live telemetry are closed by TODO-558.
- Latest uniform-loss harness proof: exact source `b52add5edfac34e6407efee6f4116a1a9eb2c1ae` ran on Omega against packaged ARM64 binary SHA-256 `ee0243f6aae50ee66115ba9f11d596004c3f057e240654e9a4bf340461e95e88`. All six single-source cases passed: tunnel loss was 0/2/4/21% at 0/5/10/25% netem loss, and JSON-verified receiver throughput was 1.047764/1.047857 Mbit/s at 0/10%. No owned process, namespace, or veth remained.
- `tun-e2e-fec-burst-netns.sh` - Linux netns correlated burst-loss ping/liveness proof. `BURST_SCENARIOS` owns each loss/correlation profile plus median and worst-case bound consumed by the repetitions gate, endpoint handshake proof, and panic checks. TODO-558 supplies the truthful Off control; comparative FEC recovery benefit remains pending the TODO-557 acceptance contract.
- Native burst evidence now uses the explicit three-trial contract: exact source `b52add5edfac34e6407efee6f4116a1a9eb2c1ae` passed on Omega against ARM64 binary SHA-256 `ee0243f6aae50ee66115ba9f11d596004c3f057e240654e9a4bf340461e95e88`. Mild samples were 1/6/1% (median 1%, maximum 6%); heavy samples were 2/1/1% (median 1%, maximum 2%). Cleanup left no owned process, namespace, or veth.
- `tun-e2e-fec-transition-netns.sh` - Linux netns clean, impaired, and recovered policy proof whose `TRANSITION_SCENARIOS` is the single source for every profile's netem loss, phase ping counts, observed-loss bounds, recovery settle, recovery bound, maximum measured recovery duration, and Fountain policy. Each isolated client/server runtime exports the stable nine-mode mapping and six phase telemetry snapshots. Off must remain Zero with no repairs, switches, or wire overhead in every phase. Auto must remain Zero with no repairs, switches, or wire overhead while clean, commit a non-Zero client mode and positive wire overhead under loss, emit real repairs by the end of the run, and return to Zero within the declared monotonic-time recovery bound. Every phase fails closed if source/repair wire-byte, overhead, or recovery counters are absent. `QF_E2E_ARTIFACT_DIR` retains collision-safe manifests, both runtime logs, the declared scenario, binary SHA-256, panic/decryption counts, recovery duration, and telemetry snapshots. Comparative benefit and broader statistical transition-quality acceptance remain TODO-557.
- Latest complete transition proof: exact source `b52add5edfac34e6407efee6f4116a1a9eb2c1ae` ran all selected `TRANSITION_SCENARIOS` on Omega using the successful ARM64 Release Build bundle from run `30317521105`, SHA-256 `dc6e91c1c04d0afd059441b630c87257870cf19a010c064c207a507c371fe507`, with packaged binary SHA-256 `ee0243f6aae50ee66115ba9f11d596004c3f057e240654e9a4bf340461e95e88`. Auto/moderate was 0/22/0% with 35,379 ms recovery, Off/moderate was 0/16/0% with 35,427 ms recovery, and Auto/severe was 0/37/0% with 35,426 ms recovery. Every result stayed inside its 5/35/10% or 5/60/10% loss bounds and the scenario-owned 40,000 ms recovery bound, recorded one handshake at each endpoint, no panic or decryption failure, and passed all quantitative telemetry checks. No owned process, namespace, or veth remained.
- `tun-e2e-fec-netem-adversity.sh` - Linux netns ping/liveness matrix whose `LOSS_SCENARIOS`, `JITTER_SCENARIOS`, `BANDWIDTH_SCENARIOS`, `RTT_SCENARIOS`, `COMBINED_SCENARIO`, and `RECOVERY_SCENARIO` own every printed and executed netem input, phase timing, and loss bound. Every completed scenario captures both endpoints' active-FEC, observed/lost-packet, repair, and mode-switch counters, while its manifest records the measured loss, RTT, and bound. `QF_E2E_ARTIFACT_DIR` is optional, must name a new absolute path, and preserves only those telemetry snapshots plus a binary-hash and contract manifest on both success and a later harness failure. Quantitative throughput, overhead, mode-stability, carrier backpressure, and recovery-latency acceptance is pending TODO-559 and TODO-557.
- Latest exact-artifact liveness proof: exact source `b52add5edfac34e6407efee6f4116a1a9eb2c1ae` passed all 25 Omega cases against ARM64 binary SHA-256 `ee0243f6aae50ee66115ba9f11d596004c3f057e240654e9a4bf340461e95e88`. Loss was 0/0/2/6/26/62%, jitter and bandwidth were 0% throughout, RTT was 8/2/4/6/2/6%, combined was 6%, and recovery was clean/loss/recovered 0/28/0%. The retained manifest SHA-256 is `3cf3ac6534102580fcca32a4ec070d6f49464af972e85d055563d108a0b3ff9c`; no owned process, namespace, or veth remained. This is liveness evidence only, not a throughput, overhead, mode-stability, or comparative-FEC claim.
...
- `tun-e2e-fec-loss-stability.sh` runs the exact loss matrix three times with a new artifact root. It preserves each child manifest and telemetry snapshot, writes a TSV aggregate of trial, scenario, observed loss, RTT, and bound, and fails on a child failure, missing result, duplicate result, incomplete matrix, binary mismatch, or declared-bound violation.
- Harness-source `6ac3917` completed the three-trial Omega loss contract against ARM64 binary SHA-256 `322060acffd79abe30ed7d8e4238933b0106c2df1ab3e116793e62073da8d32b`. All 18 cases passed: 25% netem loss was 16/40/18% against 40%, and 50% was 46/52/56% against 65%. The raw three-manifest and 36-snapshot evidence is retained at `/tmp/qf-6ac3917-stability.OOaYHG`; Omega finished with no product process, namespace, veth, or source-tree residue.
...
- The same dual-stack contract accepts `QF_E2E_FEC_MODE=auto|off` as a controlled diagnosis boundary. The selected policy is written into the generated configuration consumed by both endpoints and retained in the stability manifest. Exact ARM64 source `88a12ae` produced a failed/passed/passed Auto contract: the failed clean opt-in trial lost 10 DATAGRAM and 2 control carriers despite exact host-veth delivery and zero UDP kernel drops, while a successful black-hole trial recovered after losing 11 fresh STREAM carriers and zero STREAM retransmissions. The explicit Off control determines whether the unresolved clean collapse belongs to active-FEC wire admission or the underlying QUIC receive/ACK path without changing runtime policy or acceptance thresholds.
- `scripts/tests/tun-e2e-multi-client-dual-stack-stability.sh` is the repeated exact-artifact acceptance wrapper for that diagnostic boundary. It forces the external capture for three isolated complete dual-stack runs, preserves every child artifact, verifies the child binary SHA-256 against one parent identity, independently revalidates all six receiver byte/hash/wall-clock results, the unchanged 15% PMTU gain, the black-hole bounds, and six complete per-trial egress summaries, then emits one bounded TSV row per run. A failed child or incomplete evidence fails the aggregate without discarding later raw evidence. Native ARM64 stability evidence follows.
...
- The next bounded client-processing diagnostic is enabled only by `QUICFUSCATE_CLIENT_RECV_DIAGNOSTICS=1`. At a heartbeat timeout it records socket datagrams and bytes, outer Core `recv` successes and errors, and the subset of successful Core receives that advanced the transport `last_activity` marker. The normal runtime is unchanged when the variable is absent. The dual-stack harness enables it only for test clients and preserves the single heartbeat line in a new non-overwriting failure artifact. Local focused Rust tests, Bash syntax, ShellCheck warning-level, helper self-tests, and runtime guardrails pass; native ARM64 evidence is pending.
