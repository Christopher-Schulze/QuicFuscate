...
- **Deterministic coverage:** Focused negative tests cover the shared conversion, blacklist cache, quota, retry token, qkey registry, revocation, admin log, admin-auth persistence, profile metadata, audit timestamp, and Tauri timestamp boundaries. Final command counts and remote evidence are recorded in `docs/todo/todo-823-wall-clock-provenance.md`.
