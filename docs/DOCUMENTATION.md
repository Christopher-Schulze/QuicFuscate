...
- Isolated qf-dns all-target/all-feature checking and `41/41` tests pass. Workspace default/all-target and `rust-tests` checks plus strict workspace Clippy pass; formatting and diff hygiene are clean. Full workspace tests and post-commit seam/release evidence are pending the child reconciliation.
