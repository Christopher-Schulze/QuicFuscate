...
- Local Windows-GNU compilation and product Clippy prove the complete feature-gated Rust surface. Native CI run `30504148233`, attempt 2, job `90752348969` proves the pushed MSVC check, 1,929 ordinary tests, privileged adapter/data-plane/close lifecycle, zero adapter residue, and strict Clippy. Successor run `30506076900`, job `90756064469` proved WFP object cleanup but rejected the original socket-error assertion because Windows may accept a UDP send before WFP silently discards the packet. The corrected Wintun-ring packet gate, authenticated QuicFuscate traffic, and exact signed MSI artifact evidence remain open under TODO-528.
...
- The tracked Windows MSI path provisions and hashes the DLL automatically. The pushed Wintun lifecycle baseline is native-green in CI run `30504148233`, attempt 2. Treat production kill-switch support as incomplete until TODO-528's successor native WFP packet, authenticated tunnel, process-exit, residue, and signed MSI gates pass.
