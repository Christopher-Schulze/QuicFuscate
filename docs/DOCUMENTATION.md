...
- Root compatibility now uses the single qf-common `EnvSnapshot` type in production and test builds. The deterministic `EnvSnapshot::from_pairs` constructor is feature-gated by `rust-tests` and supplied to child/root test targets through dev-dependency feature wiring, so test helpers do not become part of the default production API.
