...
- The three-client dual-stack harness now gives caller-selected evidence directories the same bounded admin-socket ownership and self-contained certificate behavior, retains the exact binary SHA-256, and deletes every owned host veth after namespace teardown. It passed complete exact-ARM64 candidate runs, including source ownership, routing, PTB, 1280-to-1500 confirmation, and black-hole recovery. Its repeated 5-second PMTU medians were 16.34%, 14.77%, 19.78%, and -2.18% against a fixed 15% gate. The -2.18% failure left no process, namespace, host veth, or qdisc, so this is a truthful open TODO-559 performance-qualification issue, not a release-pass claim.
