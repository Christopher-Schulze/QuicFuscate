...
- **Native probe preparation:** `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh::prove_client_local_ptb()` now sends both DF=1 and DF=0 (`ping -M dont`) IPv4 probes while the client TUN is 1500 and the server TUN remains 1280. The DF=0 packet therefore reaches the server uplink boundary without local fragmentation; privileged Linux execution remains pending.
