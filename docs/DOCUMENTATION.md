...
- `tun-e2e-fec-burst-netns.sh` - Linux netns correlated burst-loss ping/liveness proof with 10%/25%-correlation <=5% and 20%/50%-correlation <=10% tunnel-loss gates, endpoint handshake proof, and panic checks. TODO-558 supplies the truthful Off control; comparative FEC recovery benefit remains pending the TODO-557 acceptance contract.
