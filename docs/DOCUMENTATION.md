...
- The next bounded provenance revision extends that existing persistent-congestion event with triggering ACK newly-acked/lost counts, the terminal loss packet-versus-time threshold classification, smoothed RTT, RTT variance, and the computed loss delay. On a failed TCP probe the harness saves the last such client event in a separate non-overwriting artifact. This has no scheduling or recovery-policy change. The focused recovery regression, Bash syntax, ShellCheck warning-level, helper self-tests, and runtime guardrails pass locally; native ARM64 evidence is pending.
