...
- Admin authentication and session surface:
  ...
  - bounded state: session replay fingerprints use a `HashSet` for constant-time membership beside a `VecDeque` that evicts one oldest fingerprint per insertion after the 4,096-entry ceiling; login keys are pruned by lockout age and evicted least-recently-used at the hard cap.
...
#### SIMD Gap Status
 ...
 - **SIMD Reed-Solomon compatibility path (TODO-594)**: standalone x86 AVX2 and GFNI Reed-Solomon encode/decode now use the canonical GF(256) generator, full augmented-matrix inversion, dynamic per-coefficient nibble LUTs, release-safe shard metadata validation, and scalar tails for non-vector-aligned shards. Rosetta-executed x86 tests cover AVX2/GFNI roundtrips and the AVX2 matrix kernel. These helpers remain internal/test-only; production FEC stays on `src/fec/`. Broader FEC unsafe bounds and GF16 review remain owned by TODO-686, TODO-679, and TODO-715.
...
- `QUICFUSCATE_TLS_CACHE` - Per-thread cache size for pooled blocks (default: `0`). Explicit opt-in caches blocks by pool identity so distinct pools cannot exchange ownership.
...
Notes:
- Pool growth is bounded to 64 MiB per pool by default, or to an explicitly larger initial pool. `QUICFUSCATE_POOL_HARD_MAX_CAP` can override the block-count ceiling but cannot invalidate an already configured initial capacity. When the limit is reached, ephemeral blocks are allocated without mutating pool counters.
...
- **uring_batch executor ownership**: `src/optimize/uring_batch.rs` has no cross-call pending queue, but its synchronous `submit_and_wait` path is called from async-owned client/server flows. Tracked in TODO-646; TODO-687 owns the separate unsafe/lifetime boundary.
...
- **io_uring/io_driver unsafe**: SQE/CQE lifetimes, raw socket buffers, receive-slot ownership, and eventfd ABI boundaries require explicit proof. TODO-687 owns the lifetime audit; TODO-798 owns unordered partial-send disposition.
...
- **Retry admission:** `src/implementations/server/ddos.rs` rejects a valid bounded worst-case token whose encoded form reaches 169 bytes because `MAX_RETRY_TOKEN_LEN` is 160. The DDOS policy probe has no duplicate length constant. TODO-618 owns the capacity contract; TODO-659 separately owns rejecting oversized inputs before encoding and HMAC work.
- **Client runtime lifecycle:** `src/implementations/client/mod.rs::ClientRuntime::connect()` stores the connection before fallible runtime, UDP, TUN, and I/O setup. Later failure leaves `Running` with partial ownership, and a subsequent connect can replace the connection without the explicit close path. This is a lifecycle rollback gap, not a proven heap leak. Tracked in TODO-619.
- **Client backend configuration:** `src/implementations/client/backend.rs::connect_inner()` still hardcodes the legacy IPv4 TUN address, netmask, and gateway rather than applying the configured interface fields. TODO-620 is the canonical owner; TODO-604 is closed as its duplicate.
- **Linux resolver restoration:** `src/implementations/client/platform/linux.rs` has no ownership marker for an originally absent `/etc/resolv.conf`, so restore cannot remove the file created by the session. Backup ownership exists only in process memory, so a crash can leave a stale fixed backup that a later session overwrites. TODO-623 owns the absent-original and durable ownership contract; TODO-649 owns backup path and symlink/target behavior.
- **macOS pf activation:** `MacOSKillSwitch::ensure_pf_enabled()` already fails fast when the main ruleset lacks the required anchor reference. The remaining client gap is cleanup of a just-loaded anchor after that check fails and consistency between `enabled` and backend state. The server routing manager now rejects macOS before host mutation and retains only pure pf rule generation; TODO-607 closed the server-side teardown boundary. Tracked in TODO-624.
- **Client FEC surface:** `src/implementations/client/pipeline.rs` is not in the module graph. The retained `FecCodec` wrapper uses packet ID zero for every decode input and discards encode IDs, making later live wiring invalid without a framing contract. TODO-625 is canonical; TODO-602 is closed as its duplicate.
...
- **Receive slots:** `UringRecvBatch` validates every completion user-data value, tracks armed and pending state one-to-one, re-arms positive, negative, and zero-length receives, resets source-address storage on every consumed slot, and destroys the ring before returning pool blocks. A bounded Linux regression consumes a full receive depth of zero-length datagrams before proving a later marker datagram is delivered.
