...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline. The standalone client and server schedule a Tokio wake-up at that deadline and flush only due server connections, so the 5 ms housekeeping cadence cannot quantize a sub-millisecond pacing release.
