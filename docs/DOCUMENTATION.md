...
- Pacing deadline propagation: `QuicFuscateConnection::next_send_deadline()` returns the earliest outer-pacer release, stealth release, or recovery/PTO deadline, so the generic I/O driver does not oversleep an outer-pacer release. The standalone client keeps recovery/PTO ownership in its established 5 ms housekeeping path, but wakes directly for `next_outbound_release_deadline()` only. This excludes the overdue-PTO preemption that caused earlier direct-timer heartbeat failures while preserving sub-millisecond pacing releases. Local binary tests and all-target checking pass; exact ARM64 proof remains required.
