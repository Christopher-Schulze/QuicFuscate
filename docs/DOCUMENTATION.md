...
- Standalone-client egress diagnostics are opt-in with `--telemetry`: successful UDP socket sends are counted by flush trigger, packets, bytes, bounded burst maxima, and bounded inter-send gap buckets. The snapshot also exposes housekeeping ticks, heartbeat timeouts, and the latest connection-local loss/RTT sample. The tracker observes only completed sends and never schedules a wakeup or changes the flush limit.
- `scripts/tests/tun-e2e-multi-client-dual-stack-netns.sh` enables this client snapshot only with `QF_E2E_CLIENT_TELEMETRY=1`. It binds each client to `127.0.0.1:(QF_E2E_CLIENT_METRICS_PORT_BASE + client-index)`, retains phase snapshots, and fails if client egress counters are absent, no egress occurred, the burst limit exceeds 64, or a heartbeat timeout occurred.
- Exact ARM64 source `eeb7049` showed that this opt-in observation mode is not timing-neutral in the current standalone loop: two clients reached the 30-second heartbeat watchdog before the default phase could retain all snapshots. It is therefore diagnostic evidence only, not a production monitoring recommendation or a transport correction.
