...
impl UdpFastPath {
    ...
    pub fn send_batch(&mut self, packets: &[(&[u8], SocketAddr)]) -> io::Result<usize> {
        ...
        let sent_count = crate::send_batch(&self.socket, batch_packets)?;
        let total_bytes =
    ...
    pub fn recv_batch(&mut self, max_packets: usize) -> io::Result<Vec<(Vec<u8>, SocketAddr)>> {
        unsafe {
            ...
            let received_count = crate::checked_syscall_count(received, batch_size)?;
            let mut results = Vec::with_capacity(received_count);
            let mut total_bytes = 0usize;
            for i in 0..received_count {
                let len = crate::checked_received_len(
                    msgs[i].msg_len,
                ...
                let peer = crate::socket_addr_from_storage(&addrs[i], address_len)?;
