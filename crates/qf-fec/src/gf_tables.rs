...
#[inline(always)]
#[doc(hidden)]
pub fn prefetch_gf_log_lookup(idx: usize) {
    #[cfg(target_arch = "x86_64")]
    {
        unsafe {
            qf_cpu::prefetch(log_table().as_ptr().add(idx.min(255)), PrefetchHint::T0);
        }
    }
...
#[inline(always)]
#[doc(hidden)]
pub fn prefetch_fec_slice(ptr: *const u8) {
    #[cfg(target_arch = "x86_64")]
    {
        qf_cpu::prefetch(ptr, PrefetchHint::T0);
    }
    #[cfg(target_arch = "aarch64")]
    {
...
#[inline(always)]
#[doc(hidden)]
pub fn gf_mul_scalar_slice(coeff: u8, src: &[u8], out_xor: &mut [u8]) {
    ...
    if coeff == 1 {
        ...
        while i + 64 <= len {
            #[cfg(target_arch = "x86_64")]
            unsafe {
                qf_cpu::prefetch(src.as_ptr().add(i + 64), PrefetchHint::T0);
            }
    ...
    let policy = FeatureDetector::instance();
...
unsafe fn gf_mul_scalar_slice_avx2(coeff: u8, src: &[u8], out_xor: &mut [u8]) {
    ...
    qf_telemetry::FEC_AVX2_GF_OPS.inc();
}
...
pub fn gf16_mul(a: u16, b: u16) -> u16 {
    #[cfg(target_arch = "x86_64")]
    if FeatureDetector::instance().features_full().simd_dispatch_matrix().gf16_pclmul {
        // SAFETY: The dispatch matrix proves PCLMULQDQ support for this call.
        return unsafe { gf16_mul_pclmul(a, b) };
    }
    #[cfg(target_arch = "aarch64")]
    {
        let matrix = FeatureDetector::instance().features_full().simd_dispatch_matrix();
        if matrix.sve2 {
            // SAFETY: The dispatch matrix proves SVE2 support for this call.
...
#[inline(always)]
fn gf16_mul_scalar(a: u16, b: u16) -> u16 {
    let mut aa = a;
    let mut bb = b;
    let mut res = 0u16;
    while bb != 0 {
...
#[inline(always)]
#[doc(hidden)]
pub fn gf16_mul_add(a: u16, b: u16, acc: u16) -> u16 {
    // Scalar GF(2^16) multiply with XOR-accumulate
    #[cfg(all(target_arch = "aarch64", target_feature = "sve2"))]
    {
        if FeatureDetector::instance().features_full().simd_dispatch_matrix().sve2 {
