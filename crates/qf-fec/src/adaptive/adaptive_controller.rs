...
impl AdaptiveFec {
    ...
    fn from_runtime_plan(
        ...
        let wiedemann_threshold =
            environment.parse::<usize>("QUICFUSCATE_FEC_WIEDEMANN_K").unwrap_or(256);
        let fec = Self {
            config: config.clone(),
    ...
    fn commit_pending_target_if_ready_inner(&mut self, diagnostics_enabled: bool) {
        ...
        let active_window_blocks_transition =
        ...
        let (mode, k, n, depth) =
            Self::run_feedback_phase(diagnostics_enabled, "transition-parameters", || {
