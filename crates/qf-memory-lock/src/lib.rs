...
/// Ownership result for an individual sensitive memory allocation.
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum MemoryLockAllocationStatus {
    /// Individual allocation locking was disabled by policy.
    Disabled,
    /// The allocation owns a successful individual `mlock` call.
    Locked,
    /// A successful process-wide `mlockall(MCL_FUTURE)` covers the allocation.
    CoveredByProcess,
    /// The requested individual lock was unavailable or failed.
    Unavailable,
}

impl MemoryLockAllocationStatus {
    /// Return the stable diagnostic spelling for this allocation status.
    pub const fn as_str(self) -> &'static str {
        match self {
            Self::Disabled => "disabled",
            Self::Locked => "locked",
            Self::CoveredByProcess => "covered-by-process",
...
/// Enable or disable `mlock` for newly allocated memory-pool blocks.
pub fn set_lock_blocks(enabled: bool) {
    LOCK_BLOCKS.store(enabled, Ordering::Relaxed);
}

/// Return whether newly allocated memory-pool blocks should be locked.
pub fn lock_blocks_enabled() -> bool {
    LOCK_BLOCKS.load(Ordering::Relaxed)
}

/// Publish whether process-wide locking covers future allocations.
#[doc(hidden)]
...
/// Return whether process-wide locking covers future allocations.
#[doc(hidden)]
pub fn process_memory_lock_covers_future_allocations() -> bool {
    PROCESS_MEMORY_LOCK_COVERS_FUTURE.load(Ordering::Acquire)
}
...
impl MemoryLockPolicy {
    ...
    pub fn apply_before_tls_identity(
        ...
        set_process_memory_lock_covers_future_allocations(false);
        ...
        set_lock_blocks(self.lock_blocks);
        Ok(status)
    }
        ...
        #[cfg(unix)]
        {
            match lock_process_memory(self.failure_policy) {
                Ok(outcome) => {
                    ...
                    set_process_memory_lock_covers_future_allocations(
...
mod tests {
    use super::{
        lock_blocks_enabled, set_lock_blocks, MemoryLockAllocationStatus, MemoryLockFailureKind,
        MemoryLockFailurePolicy, MemoryLockLimit, MemoryLockPolicy, MemoryLockProcessMode,
        MemoryLockStartupError, MemoryLockState, LOCK_BLOCKS_TEST_MUTEX,
    };
    ...
    #[test]
    fn individual_allocation_lock_status_has_stable_diagnostics() {
        assert_eq!(MemoryLockAllocationStatus::Disabled.as_str(), "disabled");
        assert_eq!(MemoryLockAllocationStatus::Locked.as_str(), "locked");
        assert_eq!(MemoryLockAllocationStatus::CoveredByProcess.as_str(), "covered-by-process");
        assert_eq!(MemoryLockAllocationStatus::Unavailable.as_str(), "unavailable");
    }

    impl Drop for PoolLockSettingRestore {
        fn drop(&mut self) {
            set_lock_blocks(self.0);
        }
    }

    #[test]
    fn default_policy_preserves_secure_startup_defaults() {
        assert_eq!(
            MemoryLockPolicy::default(),
            MemoryLockPolicy {
                lock_memory: true,
                lock_blocks: true,
                failure_policy: MemoryLockFailurePolicy::BestEffort,
