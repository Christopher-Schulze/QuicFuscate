//! Browser-persona HTTP/3 request-header generation.

use crate::{BrowserProfile, FingerprintProfile, OsProfile};
use qf_cpu::ascii::AsciiSimdBackend;
...
fn make_header(backend: &AsciiSimdBackend, name: &[u8], value: &[u8]) -> Header {
    ...
    Header::from_parts(name_vec, value_vec)
}
...
impl Http3Masquerade {
    ...
    pub fn generate_headers(&self, host: &str, path: &str) -> Vec<Header> {
        let mut headers = vec![
    ...
    #[doc(hidden)]
    pub fn build_sec_ch_ua(&self) -> String {
    ...
    #[doc(hidden)]
    pub fn extract_major_version(&self, ua: &str, token: &str) -> Option<u32> {
    ...
    #[doc(hidden)]
    pub fn get_sec_fetch_site(&self, host: &str) -> &'static str {
    ...
    fn generate_realistic_cookies(&self) -> Option<String> {
        let timestamp = qf_common::time_source::now_system()
...
fn host_contains(haystack: &str, needle: &str) -> bool {
    ...
    qf_cpu::string::string_contains(haystack, needle)
}

#[cfg(test)]
mod tests {
    use super::Http3Masquerade;
    use crate::{BrowserProfile, FingerprintProfile, OsProfile};

    fn header_value<'a>(headers: &'a [qf_transport_types::h3::Header], name: &[u8]) -> &'a [u8] {
        headers
            .iter()
            .find(|header| header.name() == name)
            .map(qf_transport_types::h3::Header::value)
            .expect("expected persona header")
    }

    #[test]
    fn chromium_persona_preserves_pseudo_and_client_hint_order() {
        let masquerade = Http3Masquerade::new(FingerprintProfile::new(
            BrowserProfile::Chrome,
            OsProfile::Windows,
        ));
        let headers = masquerade.generate_headers("example.org", "/index.html");
        let names: Vec<&[u8]> = headers.iter().map(qf_transport_types::h3::Header::name).collect();
        ...
        assert_eq!(
            ...
            &[
                ...
                b"accept".as_slice(),
                b"accept-language".as_slice(),
                b"accept-encoding".as_slice(),
                b"sec-ch-ua".as_slice(),
            ]
        );
        assert_eq!(header_value(&headers, b":authority"), b"example.org");
        assert_eq!(header_value(&headers, b"sec-ch-ua-platform"), b"\"Windows\"");
    }

    #[test]
    fn firefox_persona_uses_title_case_without_chromium_hints() {
        let masquerade = Http3Masquerade::new(FingerprintProfile::new(
            BrowserProfile::Firefox,
            OsProfile::Linux,
        ));
        let headers = masquerade.generate_headers("example.org", "/");

        assert!(headers.iter().any(|header| header.name() == b"User-Agent"));
        assert!(!headers.iter().any(|header| header.name() == b"sec-ch-ua"));
        assert!(!headers.iter().any(|header| header.name() == b"cookie"));
    }

    #[test]
    fn deterministic_cookie_rendering_preserves_exact_ascii_contract() {
        let masquerade =
            Http3Masquerade::new(FingerprintProfile::new(BrowserProfile::Safari, OsProfile::MacOS));

        assert_eq!(
            masquerade.generate_realistic_cookies_at(1_700_000_000),
            "s_sess=1700000000%2014814810%20End"
        );
    }

    #[test]
    fn cross_site_referer_matches_browser_persona() {
        let masquerade =
            Http3Masquerade::new(FingerprintProfile::new(BrowserProfile::Safari, OsProfile::MacOS));
        let headers = masquerade.generate_headers("cdn.example.org", "/");

        assert_eq!(header_value(&headers, b"Referer"), b"https://www.apple.com/");
        assert!(!headers.iter().any(|header| header.name() == b"Sec-Fetch-Site"));
    }
}
