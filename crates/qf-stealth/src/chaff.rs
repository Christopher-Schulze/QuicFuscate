...
impl TrafficAnalysisScheduler {
    ...
    #[doc(hidden)]
    pub fn with_lifecycle_with_clock(
        rate_pps: u32,
        chaff_size_bytes: u32,
        ack_eliciting: bool,
        constant_rate: bool,
        ...
        let next_interval = if constant_rate { base } else { Self::jitter_interval(base) };
        let phase = if rate_pps == 0 {
            TrafficAnalysisPhase::Stopped
        } else {
            TrafficAnalysisPhase::Active
        };
        Self {
            rate_pps,
            chaff_size_bytes,
    ...
    fn cadence_for(
        &self,
        now: std::time::Instant,
    ) -> Option<(TrafficAnalysisPhase, std::time::Duration)> {
        ...
        Some((TrafficAnalysisPhase::RampDown, std::time::Duration::from_nanos(interval_ns.max(1))))
    }
...
mod traffic_analysis_scheduler_tests {
    ...
    fn idle_chaff_jitter_stays_bounded_across_ten_seconds() {
        ...
        while deadline < end {
            ...
            assert!((Duration::from_millis(90)..=Duration::from_millis(110)).contains(&interval));
            emitted += 1;
            deadline = next;
        }
