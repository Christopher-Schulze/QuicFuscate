//! Probe-count-driven stealth escalation state.

use qf_common::env_utils::EnvSnapshot;
use qf_transport_types::IntelligentLevelHints;
...
impl ProbeHistory {
    fn new() -> Self {
        Self { buckets: VecDeque::with_capacity(32), count_60s: 0, count_120s: 0 }
    }
...
impl EscalationState {
    #[doc(hidden)]
    pub fn new(level_hints: Arc<IntelligentLevelHints>, environment: &EnvSnapshot) -> Self {
    ...
    pub fn record_probe(&self) -> Option<u8> {
    ...
    pub fn check_de_escalation(&self) -> Option<u8> {
        let current = self.current_level.load(Ordering::Relaxed);
        if current == 0 {
            return None;
        }
