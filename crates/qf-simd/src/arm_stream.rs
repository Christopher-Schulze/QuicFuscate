...
fn decode_varint_at(input: &[u8], pos: usize) -> Option<(u64, usize)> {
    ...
    let (value, used) = crate::transport::decode_varint(tail)?;
...
mod tests {
    ...
    fn parse_header_roundtrip() {
        ...
        let mut buf = [0u8; 32];
        let mut off = 0usize;
        off += crate::transport::encode_varint(42, &mut buf[off..]); // stream_id
        off += crate::transport::encode_varint(0x1234, &mut buf[off..]); // offset
        off += crate::transport::encode_varint(144, &mut buf[off..]); // length
