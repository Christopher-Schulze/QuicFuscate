...
fn neon_validate_header_random_parity() {
    ...
    for _ in 0..512 {
        ...
        {
            if FeatureDetector::instance().has_feature(qf_cpu::CpuFeature::SVE2) {
...
fn neon_varint_random_parity() {
    ...
    for _ in 0..1024 {
        ...
        let len_neon = crate::arm_varint::encode_varint_neon(val, &mut buf_neon);
        ...
        let (dec_neon, used_neon) =
            crate::arm_varint::decode_varint_neon(&buf_neon[..len_neon]).expect("neon decode");
        ...
        #[cfg(target_feature = "sve2")]
        {
            if FeatureDetector::instance().has_feature(qf_cpu::CpuFeature::SVE2) {
                ...
                let len_sve = crate::arm_varint::encode_varint_sve2(val, &mut buf_sve);
                ...
                let (dec_sve, used_sve) =
                    crate::arm_varint::decode_varint_sve2(&buf_sve[..len_sve])
                        .expect("sve2 decode");
