...
type TelemetryExporter = fn() -> String;

async fn handle_connection(mut stream: TcpStream, export_telemetry_text: TelemetryExporter) {
    ...
    if is_telemetry_request(req) {
        let body = export_telemetry_text();
        let resp = format!(
...
pub fn spawn_telemetry_server(export_telemetry_text: TelemetryExporter) {
                ...
                loop {
                    match listener.accept().await {
                        Ok((stream, _peer)) => {
                            ...
                            tokio::spawn(async move {
                                handle_connection(stream, export_telemetry_text).await;
                                drop(permit);
                            });
                        }
                        Err(e) => {
...
mod tests {
    ...
    fn custom_metrics_addr() {
        let _env_lock = qf_common::env_utils::test_support::acquire_env_lock();
