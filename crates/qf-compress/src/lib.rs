...
use qf_cpu::compression::classify as classify_bytes;
use qf_cpu::{CpuProfile, FeatureDetector};
...
impl CompressionManager {
    ...
    pub fn should_compress(&self, len: usize, rtt_ms: f32, loss: f32, bw_bps: u64) -> bool {
        telemetry::COMPRESS_DECISIONS_TOTAL.inc();
        if len < self.cfg.min_len {
            telemetry::COMPRESS_DECISIONS_SKIP_LEN.inc();
            return false;
        }
        ...
        if !loss_gate {
            telemetry::COMPRESS_DECISIONS_SKIP_LOSS.inc();
            return false;
        }
        let allow = slow_link || high_rtt;
        if allow {
            telemetry::COMPRESS_DECISIONS_ALLOW.inc();
            true
        } else {
            telemetry::COMPRESS_DECISIONS_SKIP_PROFILE.inc();
            false
        }
    }
    ...
    pub fn looks_textual(data: &[u8]) -> bool {
        ...
        if analysis.is_textual() {
            telemetry::ENTROPY_TEXTUAL_SEEN.inc();
            true
        } else {
            telemetry::ENTROPY_SKIP.inc();
            false
        }
    }
    ...
    pub fn compress_to_pool(
        ...
        telemetry::COMPRESS_BYTES_IN.inc_by(orig_len as u64);
        ...
        telemetry::COMPRESS_SUCCESS.inc();
        telemetry::COMPRESS_BYTES_OUT.inc_by(compressed_len as u64);
...
#[doc(hidden)]
pub fn global_policy_with_snapshot(environment: &EnvSnapshot) -> CompressionPolicy {
    GLOBAL_POLICY
...
pub fn estimate_entropy_bits_per_byte(data: &[u8]) -> f64 {
    ...
    let histogram = qf_cpu::simd_compress::histogram(data);
...
pub fn compress_with_dict(
    ...
    telemetry::COMPRESS_SUCCESS.inc();
    telemetry::COMPRESS_DICT_USED.inc();
    telemetry::COMPRESS_BYTES_IN.inc_by(orig_len as u64);
    telemetry::COMPRESS_BYTES_OUT.inc_by(n as u64);
    Some((out, 9 + n))
}
...
pub fn body_pool() -> Arc<MemoryPool> {
    BODY_POOL
        .get_or_init(|| {
            ...
            telemetry::BODY_POOL_BLOCK_SIZE
                .store(pool.block_size() as u64, std::sync::atomic::Ordering::Relaxed);
            telemetry::BODY_POOL_CAPACITY.store(cap as u64, std::sync::atomic::Ordering::Relaxed);
            pool
        })
        .clone()
}
...
mod tests {
    ...
    fn body_pool_telemetry_reports_the_effective_block_size() {
        let pool = body_pool();
        let reported = telemetry::BODY_POOL_BLOCK_SIZE.load(std::sync::atomic::Ordering::Relaxed);
