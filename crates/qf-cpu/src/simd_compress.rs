//! SIMD histogram and pattern-search helpers for compression heuristics.
...
unsafe fn histogram_sve2(data: &[u8]) -> [u32; 256] {
    #[cfg(target_feature = "sve2")]
    {
        ...
        telemetry::PATTERN_SVE2_OPS.inc();
        return hist;
    }
...
unsafe fn histogram_neon(data: &[u8]) -> [u32; 256] {
    ...
    telemetry::PATTERN_NEON_OPS.inc();
    hist
}
...
fn find_pattern_scalar(haystack: &[u8], needle: &[u8]) -> Option<usize> {
    ...
    for i in 0..=(haystack_len.saturating_sub(needle_len)) {
        if haystack[i..i + needle_len] == *needle {
            telemetry::PATTERN_SCALAR_OPS.inc();
            return Some(i);
        }
    }
