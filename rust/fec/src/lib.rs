...
pub struct FECPacket {
    ...
    pub is_repair: bool,
    pub data: Vec<u8>,
    pool: Option<Arc<MemoryPool>>, // return buffer to pool on drop
}

impl FECPacket {
    fn new(seq: u32, is_repair: bool, data: Vec<u8>, pool: Arc<MemoryPool>) -> Self {
        Self {
            sequence_number: seq,
            is_repair,
            data,
...
impl Drop for FECPacket {
    fn drop(&mut self) {
        if let Some(pool) = self.pool.take() {
            let mut buf = Vec::new();
            std::mem::swap(&mut buf, &mut self.data);
            pool.deallocate(buf);
        }
    }
}
...
impl FECModule {
    ...
    fn calculate_current_redundancy(&self) -> f64 {
        ...
        base.clamp(
            self.config.min_redundancy_ratio,
            self.config.max_redundancy_ratio,
        )
    }
...
pub extern "C" fn fec_module_encode(
    ...
    if let Some(pkt) = packets.first() {
        unsafe {
            *out_len = pkt.as_slice().len();
        }
        let mut buf = pkt.as_slice().to_vec();
...
pub fn fec_module_create_stub() -> *mut FECModule {
    std::ptr::null_mut()
}
pub fn fec_module_destroy_stub(_h: *mut FECModule) {}
pub fn fec_module_encode_stub(_h: *mut FECModule, data: &[u8]) -> Vec<u8> {
    data.to_vec()
}
pub fn fec_module_decode_stub(_h: *mut FECModule, data: &[u8]) -> Vec<u8> {
    data.to_vec()
}
