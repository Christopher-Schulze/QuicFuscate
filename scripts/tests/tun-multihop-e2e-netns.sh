...
THROUGHPUT_RATE_BPS="${QF_MULTIHOP_E2E_THROUGHPUT_RATE_BPS:-}"
MIN_THROUGHPUT_RATIO="${QF_MULTIHOP_E2E_MIN_THROUGHPUT_RATIO:-}"
...
if [ "$IMPAIRED" = "1" ]; then
  # Nested QUIC DATAGRAM carriers never retransmit: compounded per-hop loss
  # collapses TCP goodput far below the configured stream rate, so the
  # impaired cell proves a verified transfer at an attainable rate instead
  # of demanding the unimpaired throughput floor.
  THROUGHPUT_RATE_BPS="${THROUGHPUT_RATE_BPS:-1000000}"
  MIN_THROUGHPUT_RATIO="${MIN_THROUGHPUT_RATIO:-0.30}"
  ...
  THROUGHPUT_RATE_BPS="${THROUGHPUT_RATE_BPS:-10000000}"
  MIN_THROUGHPUT_RATIO="${MIN_THROUGHPUT_RATIO:-0.70}"
