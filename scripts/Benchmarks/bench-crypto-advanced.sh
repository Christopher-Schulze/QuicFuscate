#!/usr/bin/env bash
# Description: crypto-bench rolling mode, JSON/time if available
...
CMD="cargo run --features benches --release --quiet -- crypto-bench --iterations 200000 --payload 1200 --mode rolling --json"
