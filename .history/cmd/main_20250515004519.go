...
// getTLSConfig returns TLS configuration with fake TLS settings
...
func main() {
	...
	// Setup TUN interface with detected MTU
	iface, err := SetupTUN(udpListener)
	if err != nil {
		log.Fatalf("Failed to setup TUN interface: %v", err)
	}

	// Start QUIC listener with fake TLS config
	listener, err := quic.Listen(udpListener, getTLSConfig(config.Server.CACert), generateQUICConfig())
	if err != nil {
	...
	go startMetricsServer(config.Server.MetricsAddress)
...
func handleConnection(conn quic.Connection, wg *sync.WaitGroup, iface *water.Interface) {
	...
	// Create FEC encoder
	fecEncoder, err := newFECEncoder()
	if err != nil {
		log.Printf("Failed to create FEC encoder: %v", err)
		return
	}
	...
	// Start TUN traffic handler with both FEC and Tetrys
	go HandleTUNTraffic(context.Background(), iface, conn, fecEncoder, tetrysEncoder)

	// Start keepalive ticker from config
	config, err := loadConfig("config.yaml")
	if err != nil {
		log.Printf("Failed to load config for keepalive: %v", err)
		return
	}
