...
pub struct LiveServerState {
    ...
    pub(super) clients: std::collections::HashMap<SocketAddr, QuicFuscateConnection>,
...
pub(crate) fn build_live_server_client_init(
    ...
    if let Some(state) = initial_ctx.pending_qkey_auth.as_mut() {
        ...
        state.traffic_analysis_policy =
            state.traffic_analysis_policy.map(|requested| requested.bounded_by(ceiling));
    }
...
pub(super) fn accept_session_in_domain(
...
pub(super) fn remove_session_from_domain(
    sessions: &mut SessionManager,
    ip_pool: &mut IpPool,
...
pub(super) fn reap_expired_sessions_from_domain(
    sessions: &mut SessionManager,
    ip_pool: &mut IpPool,
    mut ipv6_pool: Option<&mut Ipv6Pool>,
    connection_limiter: &mut ConnectionLimiter,
...
impl LiveServerState {
    ...
    pub(crate) fn admit_incoming_datagram(
        ...
        self.domain.admit_incoming_datagram(from, packet, established, retry_eligible, metrics)
    }
    ...
    #[cfg(feature = "rate_limiter")]
    pub(super) fn close_blacklist_sync(&self) {
