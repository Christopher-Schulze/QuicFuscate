...
pub(super) struct ServerTunIps {
    pub(super) ipv4: Ipv4Addr,
    pub(super) ipv6: Option<Ipv6Addr>,
}

#[derive(Clone, Debug)]
pub(super) struct ServerAssignmentSettings {
    pub(super) ipv4_prefix: u8,
    pub(super) ipv6_prefix: u8,
    pub(super) mtu: u16,
    pub(super) dns_servers: Vec<IpAddr>,
}

#[cfg(unix)]
pub(super) fn record_systemd_notification(state: &str, result: std::io::Result<()>) {
...
impl Default for BlacklistConfig {
    fn default() -> Self {
        Self {
            ...
            cache_path: Some(std::path::PathBuf::from("config/local/blacklist-cache.json")),
            custom_ca_path: None,
        }
    }
}
...
impl ServerConfig {
    pub(super) fn assignment_settings(&self, mtu: u16) -> Result<ServerAssignmentSettings, String> {
        let raw_mask = u32::from(self.server_netmask);
        let ipv4_prefix = raw_mask.leading_ones() as u8;
        let canonical_mask = if ipv4_prefix == 0 { 0 } else { u32::MAX << (32 - ipv4_prefix) };
        ...
        if self.ipv6_server_ip.is_some() && mtu < 1280 {
            return Err(format!("server TUN MTU must be at least 1280 for IPv6, got {mtu}"));
        }
        let mut dns_servers =
            Vec::with_capacity(self.dns_servers.len().saturating_add(self.ipv6_dns_servers.len()));
        ...
        Ok(ServerAssignmentSettings {
            ipv4_prefix,
            ipv6_prefix: self.ipv6_prefix_len,
            mtu,
            dns_servers,
        })
    }
    ...
    fn validate_address_pools(&self, ipv4_prefix: u8) -> Result<(), String> {
        ...
        let ipv4_first_client = ipv4_network
            .checked_add(1)
            .ok_or_else(|| "server IPv4 network has no usable client address range".to_string())?;
        let ipv4_last_client = ipv4_broadcast
            .checked_sub(1)
            .ok_or_else(|| "server IPv4 network has no usable client address range".to_string())?;
        ...
        match (self.ipv6_server_ip, self.ipv6_pool_start, self.ipv6_pool_end) {
            ...
            (None, _, _) => {
                return Err(
                    "IPv6 server address and IPv6 client pool must be enabled together".to_string()
                );
            }
            (Some(server_ip), Some(pool_start), Some(pool_end)) => {
                if self.ipv6_prefix_len == 0 {
                    return Err(
                        "ipv6_prefix_len must be at least 1 when IPv6 is enabled".to_string()
                    );
                }
    ...
    pub(super) fn server_tun_config(
        &self,
        name: Option<String>,
        mtu: u16,
        zero_copy: bool,
    ...
    pub(super) fn validate_engine_interface_alignment(
        &self,
    ...
    pub(super) fn reconcile_standalone_tun_config(
        &self,
...
impl ServerConfig {
    pub(super) fn validate_revocation_retention(&self) -> Result<(), String> {
    ...
    pub(super) fn validate_downlink_scheduler(&self) -> Result<(), String> {
...
fn load_downlink_scheduler_from_env() -> Result<(u64, u64), String> {
    let rate = parse_auth_policy_env_u64("QUICFUSCATE_SERVER_DOWNLINK_RATE_BYTES_PER_SECOND", 0)?;
    let burst = parse_auth_policy_env_u64("QUICFUSCATE_SERVER_DOWNLINK_BURST_BYTES", 0)?;
...
fn load_bandwidth_policy_from_env() -> Result<BandwidthPolicy, String> {
    ...
    let weight = parse("QUICFUSCATE_CLIENT_BANDWIDTH_WEIGHT", u64::from(defaults.weight))?;
    let policy = BandwidthPolicy {
        ...
        burst_bytes: parse("QUICFUSCATE_CLIENT_BURST_BYTES", defaults.burst_bytes)?,
...
fn parse_auth_policy_env_u64(name: &str, default: u64) -> Result<u64, String> {
    match std::env::var(name) {
        Ok(raw) => {
            raw.trim().parse::<u64>().map_err(|error| format!("invalid {name}='{raw}': {error}"))
        }
...
fn parse_policy_env_f64(name: &str, default: f64) -> Result<f64, String> {
    match std::env::var(name) {
        Ok(raw) => {
            raw.trim().parse::<f64>().map_err(|error| format!("invalid {name}='{raw}': {error}"))
        }
        Err(std::env::VarError::NotPresent) => Ok(default),
        Err(error) => Err(format!("could not read {name}: {error}")),
    }
...
fn load_ddos_policy_config_from_env() -> Result<limits::DdosPolicyConfig, String> {
    ...
    let config = limits::DdosPolicyConfig {
        ...
        clear_window: milliseconds("QUICFUSCATE_DDOS_CLEAR_WINDOW_MS", defaults.clear_window)?,
        ewma_alpha: parse_policy_env_f64("QUICFUSCATE_DDOS_EWMA_ALPHA", defaults.ewma_alpha)?,
        spike_multiplier: parse_policy_env_f64(
            "QUICFUSCATE_DDOS_SPIKE_MULTIPLIER",
            defaults.spike_multiplier,
        )?,
        clear_factor: parse_policy_env_f64("QUICFUSCATE_DDOS_CLEAR_FACTOR", defaults.clear_factor)?,
...
fn load_auth_policy_config_from_env() -> Result<AuthPolicyConfig, String> {
    ...
    let config = AuthPolicyConfig {
        ...
        backoff_base: milliseconds("QUICFUSCATE_AUTH_BACKOFF_BASE_MS", defaults.backoff_base)?,
        backoff_max: milliseconds("QUICFUSCATE_AUTH_BACKOFF_MAX_MS", defaults.backoff_max)?,
        block_after_failures: as_u32(
            "QUICFUSCATE_AUTH_BLOCK_AFTER_FAILURES",
            defaults.block_after_failures,
        )?,
        block_duration: seconds("QUICFUSCATE_AUTH_BLOCK_DURATION_SECS", defaults.block_duration)?,
        idle_timeout: seconds("QUICFUSCATE_AUTH_IDLE_TIMEOUT_SECS", defaults.idle_timeout)?,
        prune_interval: seconds("QUICFUSCATE_AUTH_PRUNE_INTERVAL_SECS", defaults.prune_interval)?,
        max_tracked_ips: as_usize("QUICFUSCATE_AUTH_MAX_TRACKED_IPS", defaults.max_tracked_ips)?,
...
pub(super) fn stateless_version_negotiation_response(
    packet: &[u8],
...
fn load_blacklist_config_from_env() -> Result<BlacklistConfig, String> {
    ...
    let custom_ca_path = env_string("QUICFUSCATE_BLACKLIST_CA_PATH").map(std::path::PathBuf::from);
    ...
    let cache_path = match std::env::var("QUICFUSCATE_BLACKLIST_CACHE_PATH") {
        ...
        Ok(raw) if raw.trim().is_empty() => {
            return Err("QUICFUSCATE_BLACKLIST_CACHE_PATH must be a path or 'disabled'".to_string())
        }
...
pub(crate) fn parse_logging_mode(mode: &str) -> Result<qf_logging::LoggingMode, String> {
    match mode {
        ...
        _ => Err(format!("Invalid logging mode '{}'. Valid: {:?}", mode, SUPPORTED_LOGGING_MODES)),
    }
}
