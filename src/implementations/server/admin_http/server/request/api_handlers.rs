...
pub(in crate::implementations::server::admin_http::server) fn get_cookie(
    req: &HttpRequest,
    name: &str,
) -> Option<String> {
...
pub(in crate::implementations::server::admin_http::server) fn build_session_cookie(
    session_id: &str,
    req: &HttpRequest,
    environment: &AdminHttpEnvironment,
) -> String {
...
pub(in crate::implementations::server::admin_http::server) fn build_expired_cookie(
    req: &HttpRequest,
    environment: &AdminHttpEnvironment,
) -> String {
    let mut cookie = format!(
...
pub(super) fn admin_response_status(resp: &AdminResponse) -> u16 {
    if resp.success {
        return 200;
    }
...
pub(in crate::implementations::server::admin_http::server) fn constant_time_token_eq(
    left: &str,
    right: &str,
) -> bool {
...
pub(super) fn validate_csrf_request(
    req: &HttpRequest,
...
pub(in crate::implementations::server::admin_http::server) fn hash_password(
    password: &str,
...
pub(in crate::implementations::server::admin_http::server) fn verify_password(
    password_phc: &str,
    password: &str,
) -> bool {
