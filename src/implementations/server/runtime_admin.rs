...
pub struct ServerRuntime {
    ...
    /// Memory pool
    pub(super) pool: Arc<MemoryPool>,
    /// Embedded host resources
    pub(super) host_resources: Option<ServerHostResources>,
    /// Shared server domain owner
    pub(super) domain: SharedServerDomain,
    /// Shutdown signal
    ...
    pub(super) live: Option<ServerLiveRuntime>,
    /// Owner for accepted standalone DNS interception blocking operations.
    pub(super) dns_intercept_workers: Option<Arc<DnsInterceptWorkerOwner>>,
    /// Shared owner for all stealth background workers of this generation.
    pub(super) stealth_runtime: Arc<StealthRuntimeOwner>,
}
...
impl GracefulShutdown {
    ...
    pub(super) fn lifecycle(&self) -> ShutdownLifecycle {
    ...
    pub(super) fn set_grace_ms(&self, grace_ms: u64) {
    ...
    pub(super) fn status_json(&self, active_connections: u64) -> serde_json::Value {
        serde_json::json!({
...
#[derive(Clone)]
pub(super) struct SharedServerDomain {
    pub(super) clock: ProtocolClock,
    pub(super) sessions: Arc<RwLock<SessionManager>>,
    pub(super) forwarding_policy: Arc<ClientIsolationManager>,
    ip_pool: Arc<parking_lot::Mutex<IpPool>>,
    /// IPv6 address pool (None = IPv6 disabled). Allocated lazily from ServerConfig.
    pub(super) ipv6_pool: Option<Arc<parking_lot::Mutex<Ipv6Pool>>>,
    pub(super) connection_limiter: Arc<parking_lot::Mutex<ConnectionLimiter>>,
    #[cfg(feature = "rate_limiter")]
    pub(super) packet_rate_limiter: Arc<parking_lot::Mutex<PacketRateLimiterDomain>>,
    ...
    #[cfg(feature = "rate_limiter")]
    pub(super) ddos_detector: Arc<crate::implementations::server::limits::EwmaAnomalyDetector>,
    #[cfg(feature = "rate_limiter")]
    ...
    #[cfg(feature = "rate_limiter")]
    pub(super) blacklist: Arc<crate::implementations::server::limits::BlacklistSync>,
    max_clients: usize,
    client_timeout_secs: u64,
}

#[cfg(feature = "rate_limiter")]
pub(super) struct PacketRateLimiterDomain {
    pub(super) limiter: RateLimiter,
    pub(super) last_prune: Instant,
    pub(super) last_sample: Instant,
}

pub(super) struct ServerHostResources {
    pub(super) tun: TunInterface,
...
impl ServerHostResources {
    ...
    pub(super) fn teardown(self) -> Result<(), EngineError> {
...
impl SharedServerDomain {
    ...
    pub(super) fn remove(&self, session_id: SessionId) -> Option<Session> {
        let mut sessions = self.sessions.write();
        let mut pool = self.ip_pool.lock();
        let mut v6_pool = self.ipv6_pool.as_ref().map(|p| p.lock());
        let mut limiter = self.connection_limiter.lock();
    ...
    pub(super) fn reap_expired(&self) -> Vec<Session> {
        let mut sessions = self.sessions.write();
        let mut pool = self.ip_pool.lock();
        let mut v6_pool = self.ipv6_pool.as_ref().map(|p| p.lock());
        let mut limiter = self.connection_limiter.lock();
    ...
    pub(super) fn admit_incoming_datagram(
        ...
        let packet_cost = if established { 1 } else { self.ddos_detector.enhanced_packet_cost() };
    ...
    #[cfg(feature = "rate_limiter")]
    pub(super) fn remove_rate_limited_ip(&self, ip: IpAddr) {
        self.packet_rate_limiter.lock().limiter.remove_ip(ip);
    }

    pub(super) fn traffic_snapshot(&self) -> ServerTrafficSnapshot {
    ...
    pub(super) fn all_session_ids(&self) -> Vec<SessionId> {
        self.sessions.read().all_session_ids()
    }

    pub(super) fn session_stats(&self, session_id: SessionId) -> Option<Arc<SessionStats>> {
...
impl ServerAdminCore {
    ...
    pub fn base_status_json(&self) -> serde_json::Value {
        ...
        #[cfg(feature = "rate_limiter")]
        {
            ...
            if let Ok(health) =
                serde_json::from_str::<serde_json::Value>(&self.metrics.export_health())
            {
    ...
    pub fn health_json(&self) -> serde_json::Value {
        ...
        {
            data["geoip_status"] =
                serde_json::Value::String(self.geoip_status.as_str().to_string());
            if let Ok(health) =
                serde_json::from_str::<serde_json::Value>(&self.metrics.export_health())
            {
                data["blacklist_sync"] = health["blacklist_sync"].clone();
