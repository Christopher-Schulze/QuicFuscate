...
impl QKeyAuthState {
    ...
    pub fn is_expired_at(&self, now: Instant) -> bool {
        !self.authed
            && self.post_handshake_started_at.is_some_and(|started_at| {
...
pub(super) fn is_valid_sni_host(value: &str) -> bool {
    let s = value.trim();
    if s.is_empty() {
        return false;
    }
...
pub(super) fn extract_host_from_endpoint(endpoint: &str) -> Option<String> {
...
fn issue_qkey(
    ...
    let QKeyEntry { created_at, expires_at, .. } = registry
...
pub(super) fn normalize_qkey_name(name: Option<&str>) -> Result<Option<String>, String> {
...
pub(super) fn normalize_qkey_fec(fec: Option<&str>) -> Result<&'static str, String> {
...
pub(super) fn resolve_qkey_remote(listen_addr: &str, port: Option<u16>) -> Result<String, String> {
...
mod qkey_secret_tests {
    ...
    fn generated_qkey_token_bytes_and_hex_owner_erase_before_deallocation() {
        ...
        for (label, expected_len) in [("qkey_generated_token_bytes", 32), ("qkey_token", 64)] {
            let bytes = events
                .iter()
...
struct TransportOverrides {
    ...
    intelligent_traffic_analysis_ceiling: Option<crate::transport::config::TrafficAnalysisPolicy>,
}
...
fn parse_transport_overrides_from_toml(contents: &str) -> Result<TransportOverrides, String> {
    ...
    out.initial_max_stream_data_bidi_local =
        transport_varint_override(tbl, "initial_max_stream_data_bidi_local")?;
    out.initial_max_stream_data_bidi_remote =
        transport_varint_override(tbl, "initial_max_stream_data_bidi_remote")?;
    out.initial_max_stream_data_uni =
        transport_varint_override(tbl, "initial_max_stream_data_uni")?;
...
pub(super) const MAX_TRANSPORT_VARINT: u64 = (1u64 << 62) - 1;
...
fn parse_traffic_analysis_policy(
    ...
    policy.validate().map(Some).map_err(|error| format!("transport.{key} is invalid: {error}"))
}
