...
pub(crate) struct StandaloneServiceConfig {
    ...
    pub(super) admin_web: Option<std::net::SocketAddr>,
    pub(super) admin_web_max_connections: usize,
    pub(super) admin_web_operation_timeout_ms: u64,
    pub(super) admin_web_root: std::path::PathBuf,
    pub(super) admin_web_user: Option<String>,
    pub(super) admin_web_password: Option<String>,
}
...
impl OwnedRuntimeStealthPolicy {
    pub(super) fn from_runtime_policy(policy: RuntimeStealthPolicy<'_>) -> Self {
        Self {
...
pub(crate) struct PreparedStandaloneRuntimeConfig {
    ...
    pub(super) profiles: Vec<FingerprintProfile>,
    pub(super) profile_interval_secs: u64,
    pub(super) standalone_runtime_metadata: StandaloneRuntimeMetadata,
    pub(super) tun_enable: bool,
    /// Shared 0-RTT anti-replay strike register (server only).
    pub(super) strike_register: Option<Arc<crate::transport::anti_replay::StrikeRegister>>,
    /// Anti-replay configuration loaded from [anti_replay] TOML section.
    pub(super) anti_replay_section: qf_transport_anti_replay::AntiReplaySection,
}

pub struct PreparedStandaloneLaunch {
    pub(super) services: Option<StandaloneServiceConfig>,
    pub(super) runtime: PreparedStandaloneRuntimeConfig,
}
...
impl PreparedStandaloneLaunch {
    ...
    pub fn set_anti_replay_section(
        &mut self,
        section: qf_transport_anti_replay::AntiReplaySection,
    ) {
...
impl StandaloneServerBootstrapState {
    pub(super) fn into_runtime_parts(self) -> StandaloneRuntimeBootstrapParts {
        (
            self.blocked_ips,
            self.qkey_registry,
            StandaloneAdminWebBootstrap {
...
pub fn initialize_standalone_server_bootstrap_with_clock(
    ...
    let admin_log_buffer = admin_log_buffer_override.unwrap_or_else(|| {
        Arc::new(self::admin_logs::AdminLogBuffer::new_with_clock(4096, &clock))
    });
