...
pub struct StealthRuntimeOwner {
    ...
    next_session_stealth_config: parking_lot::Mutex<Option<Arc<std::sync::Mutex<StealthConfig>>>>,
}
...
impl StealthRuntimeOwner {
    ...
    pub fn new(reality_config: RealityConfig) -> Result<Self, String> {
        ...
        let reality_cache =
            reality_config.enabled.then(|| Arc::new(CoverHandshakeCache::new(reality_config)));
        Ok(Self {
    ...
    fn spawn_owned<F>(&self, name: &'static str, future: F) -> Result<(), String>
    where
...
mod runtime_owner_tests {
    ...
    fn one_owner_exposes_one_shared_cover_cache() {
        let owner =
            StealthRuntimeOwner::new(RealityConfig { enabled: true, ..RealityConfig::default() })
                .unwrap();
