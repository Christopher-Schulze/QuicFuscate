    ...
    let bench_once = |parallel: bool| -> std::io::Result<(f64, usize)> {
        ...
        let result = (|| -> std::io::Result<(f64, usize)> {
            ...
            let make_pkt = |id: u64| -> std::io::Result<FecPacket> {
                ...
                FecPacket::try_new(id, Some(block), len, true, None, 0, mem_pool.clone()).map_err(
                    |error| std::io::Error::other(format!("FEC benchmark packet: {error}")),
                )
            };
...
mod runtime;
#[cfg(test)]
#[path = "main/runtime_reload_tests.rs"]
