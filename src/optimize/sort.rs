...
/// Sort `u32` values with Rust's canonical unstable sort.
///
...
mod tests {
    ...
    fn test_sort_u32_large_input() {
        // Exercise the canonical large-slice path with a nontrivial permutation.
    ...
    #[test]
    fn test_sort_u32_adversarial_lengths_preserve_exact_multiset() {
        for len in (0usize..=128).chain([255, 256, 257, 1023, 1024, 1025]) {
            let mut data: Vec<u32> = (0..len)
                .map(|index| match index % 7 {
                    0 => 0,
                    1 => u32::MAX,
                    2 => 42,
                    _ => (index as u32).wrapping_mul(2_654_435_761),
                })
                .rev()
                .collect();
            let mut expected = data.clone();
            expected.sort_unstable();
            sort_u32(&mut data);
            assert_eq!(data, expected, "u32 sort mismatch at length {len}");
        }
    }
