...
impl std::fmt::Display for ZeroCopyError {
    fn fmt(&self, formatter: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Self::TotalLengthOverflow => formatter.write_str("zero-copy buffer lengths overflow usize"),
            ...
            Self::InvalidSocketAddress => formatter.write_str("zero-copy syscall returned a non-IP socket address"),
            Self::InvalidSocketAddressLength { length, max } => write!(
                formatter,
                "socket address length {length} exceeds platform maximum {max}"
            ),
...
fn checked_windows_buffer_count(count: usize) -> ZeroCopyResult<u32> {
    ...
    u32::try_from(count).map_err(|_| ZeroCopyError::InvalidBufferCount {
        count,
...
impl<'a> ZeroCopyBuffer<'a> {
    ...
    pub fn new(buffers: &[&'a [u8]]) -> ZeroCopyResult<Self> {
        ...
        for buffer in buffers {
            iovecs.push(iovec { iov_base: buffer.as_ptr() as *mut libc::c_void, iov_len: buffer.len() });
        }
...
impl<'a> ZeroCopyRecvBuffer<'a> {
    ...
    pub fn new_mut(buffers: &'a mut [&'a mut [u8]]) -> ZeroCopyResult<Self> {
        ...
        for buffer in buffers.iter_mut() {
            iovecs.push(iovec { iov_base: buffer.as_mut_ptr() as *mut libc::c_void, iov_len: buffer.len() });
        }
        Ok(Self { iovecs, iov_count, total_len, _marker: std::marker::PhantomData })
    }
...
#[cfg(target_os = "linux")]
pub mod zc_batch {
    //! These wrappers borrow caller buffers only for the synchronous syscall. The underlying
    //! UDP batch implementation owns its temporary iovec/message storage and retains the
    //! deeper transport FFI and completion contracts under TODO-682/TODO-683.
...
impl<'a> ZeroCopyBuffer<'a> {
    ...
    pub fn new(buffers: &[&'a [u8]]) -> ZeroCopyResult<Self> {
        ...
        for (index, buffer) in buffers.iter().enumerate() {
            ...
            total_len = total_len.checked_add(buffer.len()).ok_or(ZeroCopyError::TotalLengthOverflow)?;
    ...
    pub fn send_to(
        ...
        let address_length = i32::try_from(sockaddr.len()).map_err(|_| {
            ZeroCopyError::InvalidSocketAddressLength { length: sockaddr.len() as usize, max: i32::MAX as usize }
        })?;
        let mut sent = 0u32;
        let result = unsafe {
            WSASendTo(
