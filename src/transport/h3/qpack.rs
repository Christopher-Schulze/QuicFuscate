...
impl DynamicTable {
    fn new(maximum_capacity: u64) -> Self {
        ...
        Self { entries: VecDeque::new(), maximum_capacity, capacity: 0, size: 0, insert_count: 0 }
    }
    ...
    fn can_insert(
        ...
        for entry in &self.entries {
            let acknowledged =
                acknowledged_insert_count.is_none_or(|count| entry.absolute_index < count);
    ...
    fn insert(
        ...
        self.entries.push_back(DynamicEntry { absolute_index, name, value, size, references: 0 });
        Ok(absolute_index)
    }
    ...
    fn evict_until_fits(
        ...
        while self.size.checked_add(additional).ok_or(())? > capacity {
            ...
            let acknowledged =
                acknowledged_insert_count.is_none_or(|count| front.absolute_index < count);
    ...
    fn find_name(&self, name: &[u8]) -> Option<u64> {
        self.entries.iter().rev().find(|entry| entry.name == name).map(|entry| entry.absolute_index)
    }
...
impl Encoder {
    ...
    pub(super) fn prepare(&self, stream_id: u64, headers: &[Header]) -> Result<EncodePlan, Error> {
        let field_section_size = headers.iter().try_fold(0usize, |total, header| {
            total.checked_add(DynamicTable::entry_size(header.name(), header.value())?)
        });
        ...
        let blocked_slot_available =
            stream_already_blocked || next.blocked_stream_count() < next.peer_blocked_streams;
        ...
        for header in ordered {
                    ...
                    .is_some_and(|size| size <= next.peer_maximum_capacity as usize)
            {
                ...
                if next.table.can_insert(name, value, Some(next.known_received_count)) {
                    next.encode_insert(name, value, &mut encoder_instructions)?;
                    dynamic_index = Some(
                        next.table
                            .insert(name.to_vec(), value.to_vec(), Some(next.known_received_count))
                            .map_err(|_| Error::InternalError)?,
                    );
                }
            }
        ...
        let required_insert_count =
            references.iter().copied().try_fold(0u64, |maximum, index| {
                index.checked_add(1).map(|count| maximum.max(count)).ok_or(Error::InternalError)
            })?;
        ...
        if !references.is_empty() {
            ...
            next.outstanding
                .entry(stream_id)
                .or_default()
                .push_back(OutstandingSection { required_insert_count, references });
            owns_section = true;
        }

        Ok(EncodePlan { field_section, encoder_instructions, next, stream_id, owns_section })
    }

    fn encode_insert(&self, name: &[u8], value: &[u8], out: &mut Vec<u8>) -> Result<(), Error> {
        ...
        if let Some(absolute_index) = self.table.find_name(name) {
            let relative_index =
                self.table.relative_index(absolute_index).ok_or(Error::InternalError)?;
    ...
    fn stream_is_blocked(&self, stream_id: u64) -> bool {
        self.outstanding.get(&stream_id).is_some_and(|sections| {
            sections.iter().any(|section| section.required_insert_count > self.known_received_count)
        })
    }
    ...
    pub(super) fn process_decoder_stream(&mut self, data: &[u8]) -> Result<(), Error> {
        ...
        while offset < self.decoder_stream_buffer.len() {
            ...
            offset = offset.checked_add(instruction.1).ok_or(Error::QpackDecoderStreamError)?;
        }
        if offset > 0 {
    ...
    fn apply_decoder_instruction(&mut self, instruction: DecoderInstruction) -> Result<(), Error> {
        match instruction {
            ...
            DecoderInstruction::StreamCancellation(stream_id) => {
                let sections =
                    self.outstanding.remove(&stream_id).ok_or(Error::QpackDecoderStreamError)?;
...
impl Decoder {
    ...
    pub(super) fn process_encoder_stream(&mut self, data: &[u8]) -> Result<(), Error> {
        ...
        while offset < self.encoder_stream_buffer.len() {
            let instruction =
                match self.parse_encoder_instruction(&self.encoder_stream_buffer[offset..]) {
                    Ok(instruction) => instruction,
                    Err(ParseError::Incomplete) => break,
            ...
            inserted = inserted
                ...
                .ok_or(Error::QpackEncoderStreamError)?;
            offset = offset.checked_add(instruction.1).ok_or(Error::QpackEncoderStreamError)?;
        }
        if offset > 0 {
    ...
    fn parse_encoder_instruction(
        ...
        if first & 0x80 != 0 {
            ...
            let (value, value_used) =
                parse_string(&data[index_used..], 7, 0x80, self.table.maximum_capacity)?;
        ...
        if first & 0x40 != 0 {
            let (name, name_used) = parse_string(data, 5, 0x20, self.table.maximum_capacity)?;
            let (value, value_used) =
                parse_string(&data[name_used..], 7, 0x80, self.table.maximum_capacity)?;
    ...
    fn apply_encoder_instruction(&mut self, instruction: EncoderInstruction) -> Result<u64, Error> {
        match instruction {
            ...
            EncoderInstruction::InsertNameReference { static_table, name_index, value } => {
                ...
                self.table.insert(name, value, None).map_err(|_| Error::QpackEncoderStreamError)?;
                Ok(1)
            }
    ...
    pub(super) fn decode(&mut self, stream_id: u64, data: &[u8]) -> Result<DecodeOutcome, Error> {
        ...
        let sign = prefix_tail.first().is_some_and(|first| first & 0x80 != 0);
        let (delta_base, base_used) =
            decode_prefixed_integer(prefix_tail, 7).map_err(|_| Error::QpackDecompressionFailed)?;
        let required_insert_count =
            self.decode_required_insert_count(encoded_required_insert_count)?;
        let base = if sign {
            required_insert_count
        ...
        let body_offset = ric_used.checked_add(base_used).ok_or(Error::QpackDecompressionFailed)?;
        let body = data.get(body_offset..).ok_or(Error::QpackDecompressionFailed)?;
        let (headers, referenced) = self.decode_field_lines(body, required_insert_count, base)?;
        if required_insert_count == 0 {
            if !referenced.is_empty() {
                return Err(Error::QpackDecompressionFailed);
            }
        } else if referenced.iter().copied().max().and_then(|index| index.checked_add(1))
    ...
    fn decode_required_insert_count(&self, encoded: u64) -> Result<u64, Error> {
        ...
        if required > max_value {
            required = required.checked_sub(full_range).ok_or(Error::QpackDecompressionFailed)?;
        }
        if required == 0 {
            return Err(Error::QpackDecompressionFailed);
        }
    ...
    fn decode_field_lines(
        ...
        while offset < data.len() {
            ...
            if first & 0x20 != 0 {
                let (name, name_used) =
                    parse_string(&data[offset..], 3, 0x08, MAX_FIELD_SECTION_BYTES)
                        .map_err(|_| Error::QpackDecompressionFailed)?;
                let (value, value_used) = parse_string(
                    data.get(offset + name_used..).ok_or(Error::QpackDecompressionFailed)?,
                    7,
            ...
            if first & 0x10 != 0 {
                let (post_base_index, used) = decode_prefixed_integer(&data[offset..], 4)
                    .map_err(|_| Error::QpackDecompressionFailed)?;
                let absolute_index =
                    base.checked_add(post_base_index).ok_or(Error::QpackDecompressionFailed)?;
                let entry = self.dynamic_reference(absolute_index, required_insert_count)?;
                referenced.insert(absolute_index);
                headers.push(Header::new(&entry.name, &entry.value));
                offset = offset.checked_add(used).ok_or(Error::QpackDecompressionFailed)?;
            ...
            let absolute_index =
                base.checked_add(post_base_index).ok_or(Error::QpackDecompressionFailed)?;
            let name = self.dynamic_reference(absolute_index, required_insert_count)?.name.clone();
            referenced.insert(absolute_index);
            let (value, value_used) = parse_string(
                data.get(offset + name_used..).ok_or(Error::QpackDecompressionFailed)?,
                7,
...
enum EncoderInstruction {
    SetCapacity(u64),
    InsertNameReference { static_table: bool, name_index: u64, value: Vec<u8> },
    InsertLiteralName { name: Vec<u8>, value: Vec<u8> },
    Duplicate(u64),
}

fn encode_dynamic_index(out: &mut Vec<u8>, absolute_index: u64, base: u64) -> Result<(), Error> {
...
fn encode_field_section_prefix(
    ...
    let encoded = if required_insert_count == 0 {
        ...
        let full_range =
            max_entries.checked_mul(2).filter(|range| *range > 0).ok_or(Error::InternalError)?;
...
fn is_sensitive(name: &[u8]) -> bool {
    ...
    SENSITIVE_NAMES.iter().any(|sensitive| name.eq_ignore_ascii_case(sensitive))
}
...
fn encode_prefixed_integer(
    ...
    let prefix_max = if prefix_bits == 8 { u8::MAX as u64 } else { (1u64 << prefix_bits) - 1 };
    if value < prefix_max {
...
fn decode_prefixed_integer(data: &[u8], prefix_bits: u8) -> Result<(u64, usize), ParseError> {
    ...
    loop {
        ...
        let contribution = u64::from(byte & 0x7f).checked_shl(shift).ok_or(ParseError::Invalid)?;
...
fn parse_string(
    ...
    match huff_decode_into(encoded, &mut decoded) {
        ...
        Err(Error::BufferTooShort) if initial_len < maximum_decoded_len => {
            ...
            let written =
                huff_decode_into(encoded, &mut decoded).map_err(|_| ParseError::Invalid)?;
...
mod tests {
    ...
    fn appendix_b_dynamic_section_blocks_then_decodes_and_acknowledges() {
        ...
        assert!(matches!(must_succeed(decoder.decode(4, &field_section)), DecodeOutcome::Blocked));
    ...
    fn encoder_stream_parser_retains_every_fragment_boundary() {
        ...
        assert_eq!(decoder.insert_count(), 2);
        assert_eq!(total_insert_count_increment(decoder.pending_decoder_instructions()), 2);
    }
    ...
    fn prepared_dynamic_section_roundtrips_and_releases_references() {
        ...
        must_succeed(encoder.process_decoder_stream(decoder.pending_decoder_instructions()));
    ...
    fn blocked_stream_cancellation_releases_encoder_ownership() {
        ...
        assert!(matches!(must_succeed(decoder.decode(8, &field_section)), DecodeOutcome::Blocked));
    ...
    fn blocked_stream_limit_and_impossible_references_fail_decompression() {
        ...
        assert!(matches!(decoder.decode(0, &[0x05, 0x00]), Err(Error::QpackDecompressionFailed)));
    }
}
